...
function patchPortal(path) {
  ...
  if (inlineTag.test(source)) {
    source = source.replace(inlineTag, `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-followups2"></script>`);
  } else {
    source = source.replace(
      operationsTag,
      `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-followups2"></script>\n  $&`
    );
  }
