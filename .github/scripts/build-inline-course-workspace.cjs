...
function patchPortal(path) {
  ...
  if (inlineTag.test(source)) {
    source = source.replace(inlineTag, `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-calendar-fit5"></script>`);
  } else {
    source = source.replace(
      operationsTag,
      `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-calendar-fit5"></script>\n  $&`
    );
  }
