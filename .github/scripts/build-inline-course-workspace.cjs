...
function patchPortal(path) {
  ...
  if (inlineTag.test(source)) {
    source = source.replace(inlineTag, `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-payment1"></script>`);
  } else {
    source = source.replace(
      operationsTag,
      `<script src="operations-course-inline.js?v=${VERSION}&period=20260920-payment1"></script>\n  $&`
    );
  }
