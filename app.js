...
function isAdmin(user){ const role=String(user?.role||'').trim().toLowerCase(); return ['admin','superadmin','管理員','超級管理員'].includes(role); }
function getLandingPage(user){ return isAdmin(user) ? 'task.html' : 'dashboard.html'; }
function fillHeader(){ const user=requireLogin(); if(!user) return; qsa('[data-user-name]').forEach(el=>el.textContent=user.name||'員工'); qsa('[data-if-parttime]').forEach(el=>el.style.display=user.isPartTime?'':'none'); qsa('[data-if-admin]').forEach(el=>el.style.display=isAdmin(user)?'':'none'); }
