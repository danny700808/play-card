...
async function withGlobalLoading(label, runner, failText='處理失敗'){
  return await runner(beginGlobalLoading(label||'讀取中'));
}
...
function startActionButtonProgress(btn, options={}){
  ...
  if(existing&&existing.timer) clearInterval(existing.timer);
  const state={
    pct:Math.max(0,Math.min(100,Number(options.startPct!=null?options.startPct:3)||0)),
    maxPct:Math.max(0,Math.min(95,Number(options.maxPct!=null?options.maxPct:82)||82)),
  ...
  if(options.auto!==false){
    timer=setInterval(()=>{
      ...
      let step=0;
      if(state.pct<12) step=0.9;
      else if(state.pct<28) step=1.05;
      else if(state.pct<48) step=0.95;
      else if(state.pct<66) step=0.7;
      else if(state.pct<76) step=0.38;
      else step=0.16;
      state.pct=Math.min(state.maxPct, state.pct+Math.min(remain, step));
      render();
    }, Number(options.interval||210));
  }
