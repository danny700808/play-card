(function(global){
  ...
  function normalizeTeacherPayroll(payload){
    return array(payload.teacherPayroll).map(function(row,index){
      return {
        ...
        baseTeacherAmount:numberOf(row.baseTeacherAmount),teacherPayAdjustment:numberOf(row.teacherPayAdjustment),teacherPayAdjustmentReason:clean(row.teacherPayAdjustmentReason),
  ...
  function stoppedCourseReceivables(stops,periods,balanceOf){
    var byId=new Map(array(periods).map(function(period){return [period.id,period];})),seen=new Set(),result=[];
  ...
  function studentCourseFollowups(modes,stops,day,teacherId){
    var byKey=new Map();
    activeIrregularCourses(modes,day,teacherId).forEach(function(row){array(row.studentIds).forEach(function(id,index){var source=row.source||{};byKey.set(id+'|'+row.subjectId+'|'+row.teacherId,{studentId:id,studentName:array(source.studentNames)[index]||'',subjectId:row.subjectId,teacherId:row.teacherId,kind:'irregular',effectiveDate:row.effectiveDate||source.effectiveDate||'',resumedFrom:row.resumedFrom,subjectName:source.subjectName||''});});});
    array(stops).filter(function(row){return row.status==='active'&&(!row.effectiveDate||row.effectiveDate<=day)&&(!teacherId||row.teacherId===teacherId);}).forEach(function(row){var key=row.studentId+'|'+row.subjectId+'|'+row.teacherId;byKey.set(key,Object.assign({},row,{kind:'stopped',unpaidBalance:numberOf(row.currentUnpaidAmount),balanceVerified:row.balanceVerified===true}));});
    return Array.from(byKey.values()).sort(function(a,b){var rank=function(r){return r.kind==='irregular'?2:r.unpaidBalance>0?0:1;};return rank(a)-rank(b)||clean(a.studentName).localeCompare(clean(b.studentName),'zh-Hant');});
  }
  ...
  function buildState(payload,anchorDate){
    ...
    periods.forEach(function(period){
      ...
      period.attendanceUsageOffset=payload.dataMode==='mirror'?numberOf(period.usedCount)-linkedUsage:0;
      period.usedCount=Math.max(0,linkedUsage+period.attendanceUsageOffset);
    });
  ...
  async function payrollTransfer(data){await ensureTeacherPayrollManagerAuth();var result=await firebaseFunctions('asia-east1').httpsCallable('coursePayrollTransferTaiwan',{timeout:180000})(data);return result.data;}

  async function loadScheduleCatalog(){await ensureTeacherPayrollManagerAuth();var payload=await call(AUTO_LOAD_FUNCTION_NAME,{scope:'schedule-catalog'});if(!payload||!payload.ok)throw new Error('收費方案讀取未完成');return buildState(payload,todayKey()).feePlans;}
  ...
  function createAttendanceUpdater(options){
    var pending=new Map(),dirty=new Map(),epoch=0,reading=false,timer=null;
    function schedule(){if(pending.size||reading||!dirty.size)return;clearTimeout(timer);timer=setTimeout(flush,120);}
    async function flush(){
      if(pending.size||reading||!dirty.size)return;
      reading=true;var batch=Array.from(dirty.values()).slice(0,10),version=epoch,failed=false;
      try{
        var snapshot=await options.read(batch);
        if(version!==epoch||pending.size)return;
        options.apply(snapshot);
        if(options.onSynced)options.onSynced(batch,Math.max(0,dirty.size-batch.length));
        batch.forEach(function(job){if(dirty.get(job.key)===job)dirty.delete(job.key);});
      }catch(error){failed=true;if(options.onRefreshError)options.onRefreshError(error,batch);}
      finally{reading=false;if(!failed)schedule();}
    }
    async function submit(job){
      if(pending.has(job.key))return false;
      pending.set(job.key,job);epoch++;options.onPending(job,true);
      var committed=false;
      try{
        var result=await options.write(job);committed=true;dirty.set(job.key,job);options.onSaved(job,result);
      }catch(error){
        if(committed){dirty.set(job.key,job);if(options.onRefreshError)options.onRefreshError(error,[job]);}
        else options.onWriteError(error,job);
      }finally{pending.delete(job.key);options.onPending(job,false);schedule();}
      return committed;
    }
    return {submit:submit,retry:schedule,invalidate:function(){epoch++;schedule();},pendingJobs:function(){return Array.from(pending.values());},isPending:function(key){return pending.has(key);},flush:flush};
  }
  async function refreshAttendance(options){
    var result=await courseAdminMutation('coursePortalAdminSetAttendance',Object.assign({action:'refresh'},options),options&&options.manualSyncPin);
    if(!result||result.ok!==true)throw new Error('簽到資料尚未更新。');
    return result;
  }
  function applyAttendanceSnapshot(state,payload){
    var ids=new Set(array(payload.studentIds)),scopes=array(payload.payrollScopes);
    var periods=state.tuitionPeriods.filter(function(row){return !ids.has(row.studentId);}).concat(normalizePeriods(payload,state.feePlans));
  ...
  async function cancelPortalRental(options){
    ...
    var result=await courseAdminMutation('coursePortalAdminCancelRoomBooking',{
      bookingId:bookingId,
