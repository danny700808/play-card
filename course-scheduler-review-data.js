(function () {
  ...
  function build(rows) {
    ...
    function ensureStudent(name) { var key = slug(name); if (!studentMap[key]) { studentMap[key] = { id: key, name: clean(name), phone: '', active: true, notes: '7/12～7/23 核對版：僅保留課表必要資料。' }; students.push(studentMap[key]); } return studentMap[key]; }
    ...
    return { readOnly: true, dataMode: 'review', currentDate: '2026-07-12', settings: { startHour: 10, endHour: 21, interval: 30, defaultLessons: 4 }, rooms: rooms, subjects: subjects, teachers: teachers, students: students, feePlans: feePlans, tuitionPeriods: Object.keys(periodMap).map(function (key) { return periodMap[key]; }), leaveReasons: [], events: events, dataMeta: { reviewRange: '2026-07-12 ～ 2026-07-23', selectionRule: '透明請假排除；同教室同時段採最後來源，並套用已人工確認兩格。', dataQuality: { visibleEventCount: events.length, excludedLeaveCount: 0, unresolvedTimeRecords: 0 } } };
  }
  window.YouziCourseReviewData = { load: function () { if (cache) return Promise.resolve(clone(cache)); return fetch('course-scheduler-review-0712-0723.csv?v=20260723').then(function (response) { if (!response.ok) throw new Error('找不到核對 CSV（HTTP ' + response.status + '）'); return response.text(); }).then(function (text) { cache = build(parseCSV(text.replace(/^\uFEFF/, ''))); return clone(cache); }); } };
}());
