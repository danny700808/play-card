(function(){
  ...
  function openEntity(type,id){if(!writable('編輯設定'))return;entityContext={type:type,id:id||''};var row={};if(type==='student')row=studentById(id);if(type==='teacher')row=teacherById(id);if(type==='room')row=roomById(id);if(type==='subject')row=subjectById(id);if(type==='fee')row=feeById(id);if(type==='leave')row=state.leaveReasons.find(function(item){return item.id===id;})||{};var title={student:'學生',teacher:'老師',room:'教室',subject:'科目',fee:'收費方案',leave:'請假原因'}[type];$('entityModal').classList.toggle('teacher-entity-modal',type==='teacher');$('entityModalTitle').textContent=(id?'編輯':'新增')+title;$('entityModalSubtitle').textContent=type==='teacher'?'勾選可授課科目後，排課只會顯示符合老師。':type==='fee'?'本設定會在建立學生學費期別時保存快照。':type==='room'?'已被歷史課程引用的教室建議停用，不直接刪除。':'';var html='';
