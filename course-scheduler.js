(function(){
  ...
  var entityContext={type:'',id:''},policyRoomId='',loadingMigration=false,roomDragId='',roomDropSide='before';
  ...
  function reorderRoomColumns(sourceId,targetId,after){
    ...
    renderCalendar();
    toast('教室排列已更新',source.name+' 已移到新的位置；此變更只保留在測試版。');
    return true;
  }
  ...
  function renderCalendar(){
    ...
    rooms.forEach(function(room,index){var drag=isSandbox()?' draggable="true" data-room-drag="'+esc(room.id)+'" title="按住後左右拖曳可調整教室順序"':'';html+='<div class="room-head"'+drag+' style="grid-column:'+(index+2)+';grid-row:1"><span class="room-drag-grip" aria-hidden="true">⋮⋮</span><div>'+esc(room.name)+'<small>'+esc(room.note||'')+'</small></div></div>';});
  ...
  function eventDetails(event){
    ...
    if(event.type!=='rental')html='<div class="status-actions event-command-row">'+(!isReadOnly()?'<button type="button" data-event-new-period="'+esc(student.id)+'">增加期數</button><button type="button" data-event-action="copy">增加課程</button><button type="button" data-event-action="cut">調課</button>':'')+'<button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>✓ 簽到</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>取消簽到</button><button data-attendance="leave" '+(isReadOnly()?'disabled':'')+'>請假</button><button data-attendance="absent" '+(isReadOnly()?'disabled':'')+'>曠課</button></div>'+html;
  ...
  function openTuition(studentId,id){
    if(!writable('新增或編輯學費'))return;refreshFormOptions();$('tuitionForm').reset();$('tuitionId').value=id||'';$('tuitionPurchasePeriods').value='1';$('tuitionStudent').value=studentId||currentStudentId||'';$('tuitionPaymentDate').value=todayKey();$('tuitionCollectNow').checked=!id;$('tuitionPaymentMethod').value='現金';var row=periodById(id);
  ...
  function renderTuitionSnapshot(){var plan=feeById($('tuitionPlan').value);if(!plan.id){$('tuitionSnapshot').innerHTML='<b>尚未選擇方案</b>選擇後會帶入金額、堂數、請假規則與老師拆帳。';return;}$('tuitionLessonCount').value=plan.lessonCount;$('tuitionAmount').value=plan.amount;if(!$('tuitionId').value)$('tuitionPaymentAmount').value=plan.amount;$('tuitionSnapshot').innerHTML='<b>本期將保存方案快照</b>'+esc(plan.name)+'・'+money(plan.amount)+'／'+plan.lessonCount+' 堂・老師拆帳 '+splitLabel(plan)+'・請假'+(plan.leaveNoDeduct?'不扣堂':'照常扣堂');}
  ...
  function submitTuition(event){event.preventDefault();if(!writable('儲存學費期別'))return;var id=$('tuitionId').value,plan=feeById($('tuitionPlan').value),studentId=$('tuitionStudent').value,existing=id?periodById(id):{},nextPeriodNo=state.tuitionPeriods.filter(function(period){return period.studentId===studentId;}).reduce(function(max,period){return Math.max(max,numberOf(period.periodNo));},0)+1,collectNow=!id&&$('tuitionCollectNow').checked,paymentAmount=numberOf($('tuitionPaymentAmount').value),transactions=id?(existing.transactions||[]):[];if(collectNow&&paymentAmount<=0){toast('收費金額未完成','請輸入本次實際收費金額，或取消勾選「本次同時收費」。','error');return;}if(collectNow)transactions.push({id:uid('tx'),type:'payment',date:$('tuitionPaymentDate').value||todayKey(),amount:paymentAmount,method:$('tuitionPaymentMethod').value,note:'新增期數時同時收費'});var row={id:id||uid('period'),studentId:studentId,subjectId:$('tuitionSubject').value,teacherId:$('tuitionTeacher').value,planId:plan.id,periodNo:id?numberOf(existing.periodNo):nextPeriodNo,startDate:id?existing.startDate:todayKey(),expiryDate:id?existing.expiryDate:'',lessonCount:numberOf($('tuitionLessonCount').value)||plan.lessonCount,expectedAmount:numberOf($('tuitionAmount').value),discount:numberOf($('tuitionDiscount').value),usedCount:id?existing.usedCount:0,status:'active',note:$('tuitionNote').value.trim(),transactions:transactions,lessonAdjustments:id?(existing.lessonAdjustments||[]):[],planSnapshot:clone(plan)};if(!row.studentId||!row.subjectId||!row.planId){toast('資料未完成','請選擇學生、科目與收費方案。','error');return;}if(id)Object.assign(existing,row);else state.tuitionPeriods.push(row);save(studentById(row.studentId).name+'・'+(id?'編輯':'建立')+'第 '+row.periodNo+' 期 '+plan.name+(collectNow?'・收費 '+money(paymentAmount):''));closeModal('tuitionModal');if(currentStudentId){studentTab='tuition';renderStudentModal();}renderStudents();toast(id?'學費期別已更新':'學費期別已建立',id?'本期資料已更新；原有收退款紀錄保持不變。':'本次新增 1 期'+(collectNow?'，並已登記收費 '+money(paymentAmount):'；目前顯示未繳費')+'。');}
  ...
  function bindEvents(){
    ...
    $('tuitionForm').addEventListener('submit',submitTuition);$('tuitionSubject').addEventListener('change',function(){updateTuitionForm();});$('tuitionPlan').addEventListener('change',renderTuitionSnapshot);$('tuitionCollectNow').addEventListener('change',updateTuitionCollectionFields);$('transactionForm').addEventListener('submit',submitTransaction);$('transactionType').addEventListener('change',renderRefundLessonOptions);$('transactionRefundCount').addEventListener('change',function(){syncRefundSelection(true);});$('refundSlotChoices').addEventListener('change',function(){syncRefundSelection(false);});
