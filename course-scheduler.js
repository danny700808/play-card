(function(){
  ...
  function renderWeekSchedule(){
    if(!weekMode)return;var start=weekStartKey(weekAnchor||state.currentDate),end=shiftDate(start,6),selected=$('weekTeacher').value,teachers=state.teachers.filter(function(row){return row.active!==false;}).sort(bySort);
  ...
  function selectScheduleStudent(studentId,autofill){
    ...
    if(autofill&&period.id){$('eventSubject').value=period.subjectId;updateTeacherOptions(period.teacherId);updateTuitionOptions(period.id);}else updateTuitionOptions();
  ...
  function setScheduleKind(type,roomChanged){
    ...
    $$('.rental-only').forEach(function(node){node.classList.toggle('hidden',!rental);});$$('.trial-only').forEach(function(node){node.classList.toggle('hidden',!trial);});$$('.student-course-only').forEach(function(node){node.classList.toggle('hidden',!studentCourse);});$$('.learning-only').forEach(function(node){node.classList.toggle('hidden',rental);});
  ...
  function openSchedule(options){
    ...
    var selectedTeacher=source&&source.teacherId,selectedPeriod=source&&source.tuitionPeriodId;updateTeacherOptions(selectedTeacher);updateTuitionOptions(selectedPeriod);$('eventFrequency').disabled=!!options.event;setScheduleKind($('eventType').value);if($('eventStudent').value)selectScheduleStudent($('eventStudent').value,false);updateScheduleConflict();openModal('scheduleModal');
  }
  function formEvent(){var type=$('eventType').value,rental=type==='rental',trial=type==='trial',studentCourse=type==='fixed'||type==='single';return {id:$('eventId').value,seriesId:$('eventSeriesId').value,date:$('eventDate').value,roomId:$('eventRoom').value,start:$('eventStart').value,duration:numberOf($('eventDuration').value),type:type,frequency:type==='fixed'?$('eventFrequency').value:'once',studentIds:studentCourse?[$('eventStudent').value].filter(Boolean):[],teacherId:rental?'':$('eventTeacher').value,subjectId:rental?'':$('eventSubject').value,tuitionPeriodId:studentCourse?$('eventTuitionPeriod').value:'',clientName:rental?$('eventClient').value.trim():'',clientPhone:rental?$('eventClientPhone').value.trim():'',rentalFee:rental?numberOf($('eventRentalFee').value):0,rentalPaymentStatus:rental?$('eventRentalPaymentStatus').value:'',trialName:trial?$('eventTrialName').value.trim():'',trialPhone:trial?$('eventTrialPhone').value.trim():'',trialFee:trial?numberOf($('eventTrialFee').value):0,note:$('eventNote').value.trim(),status:'scheduled'};}
  ...
  function eventDetails(event){
    ...
    var html='<div class="event-compact-summary">'+(event.type==='rental'?'<div><small>租用客戶</small><b>'+esc(event.clientName||'未填')+'</b></div><div><small>聯絡電話</small><b>'+esc(event.clientPhone||'未填')+'</b></div><div><small>教室</small><b>'+esc(room.name||'未設定')+'</b></div><div><small>租用金額</small><b>'+money(event.rentalFee)+'</b></div><div><small>收款</small><b>'+(event.rentalPaymentStatus==='paid'?'已收款':'未收款')+'</b></div><div><small>狀態</small><b>'+esc(statusName(event.status))+'</b></div>':event.type==='trial'?'<div><small>體驗者</small><b>'+esc(event.trialName||'未填')+'</b></div><div><small>聯絡電話</small><b>'+esc(event.trialPhone||'未填')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>體驗費</small><b>'+money(event.trialFee)+'</b></div><div><small>狀態</small><b>'+esc(statusName(event.status))+'</b></div>':'<div><small>學生</small><b>'+esc(student.name||'未指定')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>固定老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>本次狀態</small><b>'+esc(statusName(event.status))+'</b></div><div><small>目前期別</small><b>'+(period.id?'第 '+esc(period.periodNo)+' 期':'未指定')+'</b></div><div><small>繳費</small><b>'+(period.id?(periodBalance(period)?'<span class="tuition-due">未繳 '+money(periodBalance(period))+'</span>':'<span class="tuition-paid">已繳清</span>'):'—')+'</b></div>')+'</div>';
  ...
  function updateTuitionPurchaseSummary(resetPayment){var count=tuitionPurchaseCount(),perAmount=numberOf($('tuitionAmount').value),total=perAmount*count;$('tuitionPurchasePeriods').value=String(count);$('tuitionSubmitBtn').textContent=$('tuitionId').value?'儲存本期':'儲存 '+count+' 期';if(!$('tuitionId').value&&resetPayment!==false)$('tuitionPaymentAmount').value=total;}
  ...
  function submitTuition(event){
    event.preventDefault();if(!writable('儲存學費期別'))return;var id=$('tuitionId').value,plan=feeById($('tuitionPlan').value),studentId=$('tuitionStudent').value,existing=id?periodById(id):{},count=tuitionPurchaseCount(),nextPeriodNo=state.tuitionPeriods.filter(function(period){return period.studentId===studentId;}).reduce(function(max,period){return Math.max(max,numberOf(period.periodNo));},0)+1,collectNow=!id&&$('tuitionCollectNow').checked,paymentTotal=numberOf($('tuitionPaymentAmount').value),perExpected=numberOf($('tuitionAmount').value),perDiscount=numberOf($('tuitionDiscount').value),perNet=Math.max(0,perExpected-perDiscount);
    if(!studentId||!$('tuitionSubject').value||!plan.id){toast('資料未完成','請選擇學生、科目與收費方案。','error');return;}if(collectNow&&paymentTotal<=0){toast('收費金額未完成','請輸入本次實際收費總額，或取消勾選「本次同時收費」。','error');return;}
