(function(){
  ...
  function renderStudentModal(){
    ...
    html+='<section class="student-section"><div class="student-record-heading"><div><h3>學費紀錄</h3><p>一期原則為四堂；未繳金額以紅色標示，點「查看」展開付款、退款與簽到。</p></div></div><div class="tuition-table-wrap"><table class="tuition-record-table"><thead><tr><th>期別</th><th>收費方案</th><th>應收</th><th>實收</th><th>未繳</th><th>堂數</th><th>收費</th><th>明細</th></tr></thead><tbody>';
    html+=periods.map(function(period,index){
      ...
      var balance=periodBalance(period),paid=periodPaid(period),expected=numberOf(period.expectedAmount)-numberOf(period.discount),lessonText=numberOf(period.usedCount)+'／'+numberOf(period.lessonCount)+(numberOf(period.voidedLessonCount)?'（註銷 '+numberOf(period.voidedLessonCount)+'）':'');
      return '<tr class="tuition-period-row'+(period.id===latest.id?' latest':'')+'"><td><b>第 '+esc(period.periodNo)+' 期</b></td><td><b>'+esc(planName)+'</b><small>'+esc(subjectById(period.subjectId).name||'未設定科目')+'</small></td><td>'+money(expected)+'</td><td>'+money(paid)+'</td><td>'+(balance>0?'<b class="tuition-due">'+money(balance)+'</b>':'<span class="tuition-paid">已繳清</span>')+'</td><td>'+lessonText+'</td><td>'+(isSandbox()?'<button type="button" class="btn small secondary" data-period-pay="'+esc(period.id)+'">收費／退費</button>':'<span class="tuition-readonly">正式唯讀</span>')+'</td><td><button type="button" class="btn small outline" data-period-toggle="'+esc(period.id)+'">查看</button></td></tr><tr class="tuition-detail-row hidden" data-period-detail-row="'+esc(period.id)+'"><td colspan="8"><div class="period-details"><div class="period-detail-title"><span>'+esc(subjectById(period.subjectId).name||'未設定科目')+'・固定老師：'+esc(teacherById(period.teacherId).name||'未指定')+'</span><small>'+numberOf(period.lessonCount)+' 堂／期</small></div><div class="period-columns"><section><h5>簽到紀錄</h5><ul class="period-list attendance-list">'+attendanceHtml+'</ul></section><section><h5>付款與退款</h5><ul class="period-list">'+transactionHtml+'</ul></section></div></div></td></tr>';
    }).join('')||'<tr><td colspan="8" class="student-empty">尚未建立學費期別。</td></tr>';
