(function(){
  ...
  function teacherPayBase(event){
    var period=periodById(event.tuitionPeriodId),plan=(period.planSnapshot&&Object.keys(period.planSnapshot).length?period.planSnapshot:feeById(period.planId))||{},lessonPrice=period.id&&numberOf(period.lessonCount)>0?(numberOf(period.expectedAmount)-numberOf(period.discount))/numberOf(period.lessonCount):0,amount=0;
  ...
  function openTuitionReceipt(imageUrl){
    imageUrl=clean(imageUrl);if(!/^https:\/\//i.test(imageUrl)){toast('無法開啟收據','收據圖片網址不正確，請重新整理資料。','error');return;}
    var viewer=window.open('','_blank');if(!viewer){toast('無法開啟收據','瀏覽器阻擋彈出式視窗，請允許後再試。','error');return;}
    viewer.opener=null;
  ...
  async function ensureAndOpenTuitionReceipt(button){
    ...
    if(!period.id||!transaction){toast('找不到收費紀錄','請重新整理資料後再試。','error');return;}
    ...
    try{
      ...
      button.dataset.tuitionReceiptUrl=transaction.receiptImageUrl;button.textContent='查看／補印收據';
      openTuitionReceipt(transaction.receiptImageUrl);
    }catch(error){
  ...
  function renderTeacherPayroll(){
    ...
    $('teacherPayrollTitle').textContent=teacher.name+'・上課拆帳與薪資';$('teacherPayrollSubtitle').textContent=month+'｜課堂拆帳與獎勵／扣薪完整歷史';
