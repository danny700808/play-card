(function(){
  ...
  function applyBusinessRoomDefaults(room){
    if(room.businessPolicyVersion===1)return room;
    ...
    ['sun','mon','tue','wed','thu','fri','sat'].forEach(function(day){
      ...
      for(var minute=600;minute<1260;minute+=30){
        var time=minToTime(minute),closed=day==='mon'||(['tue','wed','thu','fri'].indexOf(day)>=0&&minute<750),existing=Object.prototype.hasOwnProperty.call(policies[day],time)?policies[day][time]:null;
  ...
  function updateModeUI(){
    ...
    $('sideModeBadge').textContent=empty?'資料尚未載入':'本機工作區';
  ...
  async function persistScheduleChange(row,options){
    ...
    await refreshSavedWorkspace({calendarScopes:scopes});
    renderCalendar();
  }
  ...
  function eventDetails(event){
    ...
    if(event.type!=='rental')html='<div class="status-actions event-command-row">'+(!isReadOnly()&&student.id?'<button type="button" data-event-new-period="'+esc(student.id)+'">增加期數</button>':'')+(!isReadOnly()?'<button type="button" data-event-action="copy">增加課程</button><button type="button" data-event-action="cut">單次調課</button>'+(event.type==='fixed'?'<button type="button" data-event-action="permanent">固定調課</button>':'')+'':'')+(student.id&&!isReadOnly()?'<button type="button" data-event-action="irregular">不定時</button><button type="button" data-event-action="stopped">停課</button>':'')+'<button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>✓ 簽到</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>取消簽到</button><button data-attendance="leave" '+(isReadOnly()?'disabled':'')+'>請假</button><button data-attendance="absent" '+(isReadOnly()?'disabled':'')+'>曠課</button></div>'+html;
  ...
  function renderStudents(){
    ...
    $('studentRows').innerHTML=rows.map(function(student){var period=latestPeriod(student.id),event=nextEvent(student.id),subject=subjectById(period.subjectId),teacher=teacherById(period.teacherId);return '<tr><td><b>'+esc(student.name)+'</b><small>'+(studentCourseStatusLabel(student))+'</small></td><td>'+esc(student.phone||'未填')+'<small>LINE：'+(student.line===true?'已綁定':student.line===false?'未綁定':'未確認')+'</small></td><td>'+esc(subject.name||'尚無學費期別')+'<small>'+esc(teacher.name||'未指定老師')+'</small></td><td>'+(period.id?'<b>'+period.usedCount+' / '+period.lessonCount+'</b><small>剩 '+periodRemaining(period)+' 堂</small>':'—')+'</td><td>'+(period.id?'<b>'+money(periodPaid(period))+' / '+money(periodNetExpectedAmount(period))+'</b><small>'+(periodBalance(period)?'尚欠 '+money(periodBalance(period)):'已繳清')+'</small>':'—')+'</td><td>'+(event.id?esc(event.date+' '+event.start):'尚未排課')+'</td><td><button class="btn small secondary" data-student-id="'+esc(student.id)+'">查看學費紀錄</button></td></tr>';}).join('')||'<tr><td colspan="7">沒有符合條件的學生。</td></tr>';
  }
  ...
  function populateTimeSettings(){var options='';for(var hour=6;hour<=24;hour++)options+='<option value="'+hour+'">'+pad(hour%24)+':00</option>';$('startHour').innerHTML=options;$('endHour').innerHTML=options;$('startHour').value=state.settings.startHour;$('endHour').value=state.settings.endHour;$('defaultLessons').value=state.settings.defaultLessons;fillSelect($('feeSubjectFilter'),configurableSubjects(),function(row){return row.name;},function(row){return row.id;});$('feeSubjectFilter').insertAdjacentHTML('afterbegin','<option value="all">全部科目</option>');}
