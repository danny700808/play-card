(function(){
  ...
  function statusName(status){return {scheduled:'未簽到',attended:'已簽到',leave:'請假',absent:'缺席',cancelled:'取消'}[status]||'未簽到';}
  ...
  function periodRemaining(period){return Math.max(0,numberOf(period.lessonCount)-numberOf(period.usedCount));}
  ...
  function loadDemo(){
    try{var saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');if(saved&&saved.version===3)return normalizeState(saved);}catch(_){ }
    return defaultState();
  }
  ...
  function eventConflictReasons(candidate,ignoreIds){
    ...
    state.events.forEach(function(other){if(ignored.has(other.id)||other.date!==candidate.date||isHiddenEvent(other)||other.status==='leave'||other.status==='absent')return;var a=timeToMin(other.start),b=a+numberOf(other.duration);if(start>=b||end<=a)return;if(other.roomId===candidate.roomId)reasons.push('教室與「'+eventDisplayName(other)+'」重疊');if(candidate.teacherId&&other.teacherId===candidate.teacherId)reasons.push('老師與「'+eventDisplayName(other)+'」重疊');if((candidate.studentIds||[]).some(function(id){return (other.studentIds||[]).indexOf(id)>=0;}))reasons.push('學生與「'+eventDisplayName(other)+'」重疊');});
  ...
  function renderCalendar(){
    var date=state.currentDate,rooms=activeRooms(),events=state.events.filter(function(row){return row.date===date&&!isHiddenEvent(row);}).sort(function(a,b){return timeToMin(a.start)-timeToMin(b.start);}),conflicts=dayConflictIds(events),used=new Set(events.map(function(row){return row.roomId;}));
    $('calendarDate').value=date;$('dateTitle').textContent=zhDate(date);$('dateSubtitle').textContent=weekdayName(date)+(date===todayKey()?'・今天':'');$('kpiLessons').textContent=events.filter(function(row){return row.type!=='rental';}).length;$('kpiAttended').textContent=events.filter(function(row){return row.status==='attended';}).length;$('kpiRooms').textContent=used.size+' / '+rooms.length;$('kpiWarnings').textContent=Object.keys(conflicts).length+events.filter(function(row){return row.status==='leave'||row.status==='absent';}).length;$('calendarHint').textContent='30 分鐘／格・'+rooms.length+' 間啟用教室'+(isReadOnly()?'・唯讀':'');
    ...
    $('clipboardBar').classList.toggle('hidden',!state.clipboard);if(state.clipboard){var source=state.events.find(function(row){return row.id===state.clipboard.eventId;});$('clipboardText').textContent=(state.clipboard.mode==='cut'?'剪下':'複製')+'：'+eventDisplayName(source||{})+'，請點新的空白格。';}
  }
  ...
  function submitSchedule(event){
    ...
    if(row.id){var index=state.events.findIndex(function(item){return item.id===row.id;}),old=state.events[index];row.seriesId=old.seriesId;row.status=old.status;state.events[index]=Object.assign({},old,row);}
    else{var step=$('eventFrequency').value==='biweekly'?14:7,repeat=$('eventFrequency').value!=='once'&&row.type!=='rental',until=repeat?$('eventRepeatUntil').value:row.date,seriesId=repeat?uid('series'):'';for(var cursor=row.date,guard=0;cursor<=until&&guard<104;cursor=shiftDate(cursor,repeat?step:9999),guard++){var copy=Object.assign({},row,{id:uid('event'),seriesId:seriesId,date:cursor,frequency:repeat?$('eventFrequency').value:'once'});var conflicts=eventConflictReasons(copy,[]);if(!conflicts.length)state.events.push(copy);if(!repeat)break;}}
    save('排課已儲存');closeModal('scheduleModal');renderCalendar();toast('排課完成','建立或移動課程不會扣堂；簽到後才扣抵。');
  }

  function setAttendance(eventId,status,reasonId){
    if(!writable('更新簽到'))return;var event=state.events.find(function(row){return row.id===eventId;});if(!event)return;event.status=status;var studentId=(event.studentIds||[])[0],period=periodById(event.tuitionPeriodId),record=state.attendance.find(function(row){return row.eventId===eventId&&row.studentId===studentId;});var deducted=status==='attended'||status==='absent'||(status==='leave'&&period.id&&!(period.planSnapshot||{}).leaveNoDeduct);if(status==='scheduled'){state.attendance=state.attendance.filter(function(row){return !(row.eventId===eventId&&row.studentId===studentId);});}else if(record){Object.assign(record,{status:status,date:event.date,periodId:event.tuitionPeriodId,teacherId:event.teacherId,deducted:deducted,reasonId:reasonId||record.reasonId||''});}else if(studentId){state.attendance.push({id:uid('attendance'),eventId:eventId,studentId:studentId,periodId:event.tuitionPeriodId,status:status,date:event.date,lessonNo:0,teacherId:event.teacherId,deducted:deducted,reasonId:reasonId||''});}
    recalcPeriods();save('簽到狀態已更新');closeModal('eventModal');renderCalendar();toast('已更新為「'+statusName(status)+'」',deducted?'本堂已計入堂數扣抵。':'本堂未扣抵堂數。');
  }
  function recalcPeriods(){state.tuitionPeriods.forEach(function(period){period.usedCount=state.attendance.filter(function(row){return row.periodId===period.id&&row.deducted===true;}).length;if(period.usedCount>=period.lessonCount)period.status='completed';else if(period.status==='completed')period.status='active';});}
  ...
  function eventDetails(event){
    ...
    var html='<div class="detail-hero"><section class="detail-block"><h3>'+(event.type==='rental'?'租用資料':'學生與課程')+'</h3>'+(event.type==='rental'?detailLine('客戶',event.clientName||'未填')+detailLine('租用金額',money(event.rentalFee)):detailLine('學生',student.name||'未指定')+detailLine('電話',student.phone||'未填')+detailLine('科目',subject.name||'未設定')+detailLine('老師',teacher.name||'未設定'))+detailLine('狀態',statusName(event.status))+detailLine('備註',event.note||'無')+'</section><section class="detail-block"><h3>'+(event.type==='rental'?'教室使用':'本期學費勾稽')+'</h3>'+(event.type==='rental'?detailLine('教室',room.name||'未設定')+detailLine('是否簽退',event.status==='attended'?'已簽退':'尚未簽退'):(period.id?detailLine('期別','第 '+period.periodNo+' 期')+detailLine('堂數',period.usedCount+' / '+period.lessonCount+'（剩 '+periodRemaining(period)+'）')+detailLine('應收',money(period.expectedAmount-period.discount))+detailLine('已收',money(periodPaid(period)))+detailLine('未收',money(periodBalance(period))):'<p>尚未指定學費期別；此堂簽到不會自動扣抵。</p>'))+'</section></div>';
    if(reasons.length)html+='<div class="validation-box has-conflict"><b>排課衝突</b><span>'+reasons.map(esc).join('<br>')+'</span></div>';
    if(event.type!=='rental')html+='<div class="status-actions"><button data-attendance="attended">✓ 簽到</button><button data-attendance="leave">請假</button><button data-attendance="absent">缺席</button><button data-attendance="scheduled">取消簽到</button></div>';
    ...
    var left='<div><button class="btn outline" type="button" data-event-action="copy">複製</button> <button class="btn outline" type="button" data-event-action="cut">剪下調課</button></div>',right='<div>'+(student.id?'<button class="btn secondary" type="button" data-event-action="student">學生完整資料</button> ':'')+'<button class="btn outline" type="button" data-event-action="edit">編輯</button> <button class="btn danger" type="button" data-event-action="delete">取消本次</button></div>';$('eventModalFoot').innerHTML=left+right;$('eventModal').dataset.eventId=event.id;openModal('eventModal');
  }
  function detailLine(label,value){return '<div class="detail-line"><span>'+esc(label)+'</span><b>'+esc(value)+'</b></div>';}
  function eventAction(action){var event=state.events.find(function(row){return row.id===$('eventModal').dataset.eventId;});if(!event)return;if(action==='student'){closeModal('eventModal');openStudent((event.studentIds||[])[0]);return;}if(!writable('修改課程'))return;if(action==='edit'){closeModal('eventModal');openSchedule({event:event});}if(action==='copy'||action==='cut'){if(action==='cut'&&state.attendance.some(function(row){return row.eventId===event.id;})){toast('無法剪下','已有簽到紀錄的課程請先取消簽到。','error');return;}state.clipboard={mode:action,eventId:event.id};closeModal('eventModal');renderCalendar();toast(action==='cut'?'已剪下課程':'已複製課程','請點新的教室與時間格。');}if(action==='delete'){if(state.attendance.some(function(row){return row.eventId===event.id;})){toast('無法取消本次課程','已有簽到、請假或缺席紀錄，請先取消該紀錄，避免堂數帳被破壞。','error');return;}if(window.confirm('確定取消這一次課程嗎？')){event.status='cancelled';save('課程已取消');closeModal('eventModal');renderCalendar();}}}
  function pasteToSlot(roomId,start){
    var clip=state.clipboard,source=clip&&state.events.find(function(row){return row.id===clip.eventId;});if(!clip||!source)return false;var target=Object.assign({},source,{id:clip.mode==='copy'?uid('event'):source.id,date:state.currentDate,roomId:roomId,start:start,status:'scheduled',type:source.type==='fixed'?'single':source.type,frequency:'once',movedFrom:source.date+' '+source.start});var reasons=eventConflictReasons(target,clip.mode==='cut'?[source.id]:[]);if(reasons.length){toast('無法放到這個格子',reasons.join('；'),'error');return true;}if(clip.mode==='copy')state.events.push(target);else Object.assign(source,target);state.clipboard=null;save('課程已移動');renderCalendar();toast(clip.mode==='copy'?'已複製排課':'調課完成','排課異動本身不會重複扣堂。');return true;
  }
  ...
  function nextEvent(studentId){return state.events.filter(function(row){return (row.studentIds||[]).indexOf(studentId)>=0&&row.date>=todayKey()&&!isHiddenEvent(row);}).sort(function(a,b){return (a.date+a.start).localeCompare(b.date+b.start);})[0]||{};}
  ...
  function renderStudentModal(){
    ...
    var events=state.events.filter(function(row){return (row.studentIds||[]).indexOf(student.id)>=0;}).sort(function(a,b){return (clean(a.date)+clean(a.start)).localeCompare(clean(b.date)+clean(b.start));});
    var latest=periods.length?periods[periods.length-1]:{},html='<section class="student-overview"><div><span>狀態</span><b>'+(student.active===false?'已停課':'上課中')+'</b></div><div><span>電話</span><b>'+esc(student.phone||'未填')+'</b></div><div><span>LINE</span><b>'+(student.line===true?'已綁定':student.line===false?'未綁定':'未確認')+'</b></div><div><span>目前科目</span><b>'+esc(subjectById(latest.subjectId).name||'尚未建立期別')+'</b></div></section>';
    if(student.note)html+='<section class="student-note"><b>備註</b><p>'+esc(student.note)+'</p></section>';
    html+='<section class="student-section"><h3>學費與上課紀錄</h3><div class="student-periods">';
    html+=periods.map(function(period,index){
      var periodAttendance=attendance.filter(function(row){return row.periodId===period.id;});
      var periodEvents=events.filter(function(row){return row.tuitionPeriodId===period.id;});
      var hasCourseRecord=periodAttendance.length>0||periodEvents.some(function(row){return !isHiddenEvent(row);});
      var unpaid=periodBalance(period)>0&&hasCourseRecord;
      var transactions=(period.transactions||[]).slice().sort(function(a,b){return clean(a.date).localeCompare(clean(b.date));});
      var attendanceHtml=periodAttendance.map(function(row){var ev=state.events.find(function(item){return item.id===row.eventId;})||{},reason=state.leaveReasons.find(function(item){return item.id===row.reasonId;})||{};return '<li><time>'+esc(row.date||'未填日期')+'</time><b>'+esc(statusName(row.status))+'</b><span>'+esc(teacherById(row.teacherId||ev.teacherId).name||'未指定老師')+(row.status==='leave'&&reason.name?'・'+esc(reason.name):'')+'</span></li>';}).join('')||'<li class="empty">尚無簽到紀錄</li>';
      var transactionHtml=transactions.map(function(tx){return '<li><time>'+esc(tx.date||'未填日期')+'</time><b>'+esc(tx.type==='refund'?'退款':'付款')+'</b><span>'+money(tx.amount)+(tx.method?'・'+esc(tx.method):'')+'</span></li>';}).join('')||'<li class="empty">尚無付款或退款紀錄</li>';
      return '<article class="period-card'+(period.id===latest.id?' latest':'')+'"><header class="period-head"><div><span>第 '+esc(period.periodNo)+' 期</span><h4>'+esc(subjectById(period.subjectId).name||'未設定科目')+'・'+esc(teacherById(period.teacherId).name||'未指定老師')+'</h4><small>'+esc(period.startDate||'未填開始日')+'</small></div><div class="period-count"><b>'+esc(period.lessonCount)+' 堂</b>'+(period.id===latest.id?'<span>剩 '+periodRemaining(period)+' 堂</span>':'')+'</div></header>'+(unpaid?'<div class="period-warning">未繳費</div>':'')+'<div class="period-columns"><section><h5>本期簽到</h5><ul class="period-list">'+attendanceHtml+'</ul></section><section><h5>付款與退款</h5><ul class="period-list">'+transactionHtml+'</ul>'+(isReadOnly()?'':'<button class="btn small secondary" data-period-pay="'+esc(period.id)+'">新增付款／退款</button>')+'</section></div></article>';
    }).join('')||'<p class="student-empty">尚未建立學費期別。</p>';
    html+='</div></section><section class="student-section"><h3>完整排課紀錄</h3><div class="table-card student-schedule"><table class="data-table"><thead><tr><th>日期</th><th>時間</th><th>類型</th><th>教室</th><th>科目／老師</th><th>狀態</th></tr></thead><tbody>'+(events.map(function(ev){return '<tr><td>'+esc(ev.date||'')+'</td><td>'+esc(ev.start||'')+'–'+esc(minToTime(timeToMin(ev.start)+numberOf(ev.duration)))+'</td><td><span class="tag '+(ev.type==='fixed'?'blue':ev.type==='trial'?'yellow':ev.type==='rental'?'red':'green')+'">'+esc(typeName(ev.type))+'</span></td><td>'+esc(roomById(ev.roomId).name||'未設定')+'</td><td>'+esc(subjectById(ev.subjectId).name||'')+'<small>'+esc(teacherById(ev.teacherId).name||'')+'</small></td><td>'+esc(statusName(ev.status))+'</td></tr>';}).join('')||'<tr><td colspan="6">尚無排課紀錄。</td></tr>')+'</tbody></table></div></section>';
    $('studentModalBody').innerHTML=html;$('studentModalFoot').innerHTML=isReadOnly()?'':'<div><button class="btn outline" type="button" data-student-action="edit">編輯基本資料</button></div><div><button class="btn primary" type="button" data-student-action="tuition">＋ 新增學費期別</button></div>';
  }
  ...
  function openTuition(studentId,id){
    ...
    else{$('tuitionPeriodNo').value=state.tuitionPeriods.filter(function(p){return p.studentId===$('tuitionStudent').value;}).length+1;$('tuitionStartDate').value=todayKey();$('tuitionDiscount').value=0;}
  ...
  async function loadMigrationFromMirror(pin){
    ...
    state=normalizeState(loaded);updateModeUI();refreshFormOptions();switchView('calendar');
    return loaded;
  }
