(function(){
  ...
  function setAttendance(eventId,status,reasonId){
    if(!writable('更新簽到'))return;var event=materializeEvent(findEvent(eventId));if(!event)return;var studentId=(event.studentIds||[])[0],record=state.attendance.find(function(row){return row.eventId===event.id&&row.studentId===studentId;}),rollover={period:periodById(event.tuitionPeriodId),created:false};if(status==='attended')rollover=ensureAttendancePeriod(event,studentId,record);event.status=status;var deducted=status==='attended',period=rollover.period.id?rollover.period:periodById(event.tuitionPeriodId);if(status==='scheduled'){state.attendance=state.attendance.filter(function(row){return !(row.eventId===event.id&&row.studentId===studentId);});}else if(record){Object.assign(record,{status:status,date:event.date,periodId:period.id||event.tuitionPeriodId,teacherId:event.teacherId,deducted:deducted,lessonNo:record.lessonNo||numberOf(period.usedCount)+1,reasonId:reasonId||record.reasonId||''});}else if(studentId){state.attendance.push({id:uid('attendance'),eventId:event.id,studentId:studentId,periodId:period.id||event.tuitionPeriodId,status:status,date:event.date,lessonNo:deducted?numberOf(period.usedCount)+1:0,teacherId:event.teacherId,deducted:deducted,reasonId:reasonId||''});}
    recalcPeriods();syncSandboxPayroll(event,status);save(eventDisplayName(event)+'・'+statusName(status)+(rollover.created?'・自動延續第 '+rollover.period.periodNo+' 期':''));closeModal('eventModal');renderCalendar();toast(rollover.created?'已自動延續第 '+rollover.period.periodNo+' 期':'已更新為「'+statusName(status)+'」',rollover.created?'上一期已滿；本堂已歸入新一期，新一期目前顯示未繳費。':deducted?'本堂已計入堂數扣抵。':'本堂未扣抵堂數。');
  }
  ...
  function eventDetails(event){
    ...
    var html='<div class="event-compact-summary">'+(event.type==='rental'?'<div><small>租用客戶</small><b>'+esc(event.clientName||'未填')+'</b></div><div><small>教室</small><b>'+esc(room.name||'未設定')+'</b></div><div><small>租用金額</small><b>'+money(event.rentalFee)+'</b></div><div><small>狀態</small><b>'+esc(statusName(event.status))+'</b></div>':'<div><small>學生</small><b>'+esc(student.name||'未指定')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>固定老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>本次狀態</small><b>'+esc(statusName(event.status))+'</b></div><div><small>目前期別</small><b>'+(period.id?'第 '+esc(period.periodNo)+' 期':'未指定')+'</b></div><div><small>繳費</small><b>'+(period.id?(periodBalance(period)?'<span class="tuition-due">未繳 '+money(periodBalance(period))+'</span>':'<span class="tuition-paid">已繳清</span>'):'—')+'</b></div>')+'</div>';
    ...
    else html+='<div class="status-actions"><button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>✓ 簽退完成</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>恢復未簽退</button></div>';
    if(student.id)html+='<section class="event-tuition-section"><div class="student-record-heading"><div><h3>學生學費紀錄</h3><p>不必再進第二層；每一期與四堂簽到直接顯示在這裡。</p></div></div>'+tuitionTableHtml(student.id)+'</section>';
    $('eventModalBody').innerHTML=html;
    var left=isReadOnly()?'':('<div><button class="btn outline" type="button" data-event-action="copy">增加課程</button> <button class="btn outline" type="button" data-event-action="cut">調課</button></div>'),right=isReadOnly()?'':'<div><button class="btn outline" type="button" data-event-action="edit">編輯</button> <button class="btn danger" type="button" data-event-action="delete">刪除這一次課程</button></div>';$('eventModalFoot').innerHTML=left+right;$('eventModal').dataset.eventId=event.id;openModal('eventModal');
  }
  ...
  function periodLessonSlots(period){
    var attended=state.attendance.filter(function(row){return row.periodId===period.id&&normalizedStatus(row.status)==='attended';}).sort(function(a,b){return clean(a.date).localeCompare(clean(b.date))||numberOf(a.lessonNo)-numberOf(b.lessonNo);}),count=Math.max(1,numberOf(period.lessonCount)||4),html='<div class="lesson-slots">';
    for(var index=0;index<count;index++){var row=attended[index],date=row&&clean(row.date).replace(/-/g,'/');html+='<span class="lesson-slot'+(row?' completed':' pending')+'"><small>第 '+(index+1)+' 堂</small><b>'+(row?esc(date):'—')+'</b><em>'+(row?'已上課':'尚未上課')+'</em></span>';}
    return html+'</div>';
  }
  function tuitionTableHtml(studentId){
    var periods=state.tuitionPeriods.filter(function(row){return row.studentId===studentId;}).sort(function(a,b){return numberOf(a.periodNo)-numberOf(b.periodNo);}),latest=periods.length?periods[periods.length-1]:{},rows=periods.map(function(period){
      var transactions=(period.transactions||[]).slice().sort(function(a,b){return clean(a.date).localeCompare(clean(b.date));}),transactionHtml=transactions.map(function(tx){var method=displayTransactionMethod(tx.method);return '<li><time>'+esc(tx.date||'未填日期')+'</time><b>'+esc(tx.type==='refund'?'退款':'付款')+'</b><span>'+money(tx.amount)+(method?'・'+esc(method):'')+(tx.operatorName?'・'+esc(tx.operatorName):'')+'</span></li>';}).join('')||'<li class="empty">尚無付款或退款紀錄</li>',planName=clean((period.planSnapshot||{}).name)||clean(feeById(period.planId).name)||'既有收費方案',balance=periodBalance(period),payment=balance>0?'<b class="tuition-due">未繳 '+money(balance)+'</b>':'<span class="tuition-paid">已繳清</span>';
      return '<tr class="tuition-period-row'+(period.id===latest.id?' latest':'')+'"><td><b>第 '+esc(period.periodNo)+' 期</b></td><td><b>'+esc(subjectById(period.subjectId).name||'未設定科目')+'</b><small>'+esc(planName)+'・固定老師 '+esc(teacherById(period.teacherId).name||'未指定')+'</small></td><td>'+payment+'</td><td>'+periodLessonSlots(period)+'</td><td>'+(isSandbox()?'<button type="button" class="btn small secondary" data-period-pay="'+esc(period.id)+'">收費／退費</button>':'<span class="tuition-readonly">正式唯讀</span>')+'</td><td><button type="button" class="btn small outline" data-period-toggle="'+esc(period.id)+'">款項明細</button></td></tr><tr class="tuition-detail-row hidden" data-period-detail-row="'+esc(period.id)+'"><td colspan="6"><div class="period-payment-detail"><div><b>第 '+esc(period.periodNo)+' 期付款與退款</b><small>應收 '+money(numberOf(period.expectedAmount)-numberOf(period.discount))+'・目前實收 '+money(periodPaid(period))+'</small></div><ul class="period-list">'+transactionHtml+'</ul></div></td></tr>';
    }).join('')||'<tr><td colspan="6" class="student-empty">尚未建立學費期別。</td></tr>';
    return '<div class="tuition-table-wrap"><table class="tuition-record-table"><thead><tr><th>期別</th><th>課程／方案</th><th>繳費狀態</th><th>上課紀錄</th><th>收費</th><th>查看</th></tr></thead><tbody>'+rows+'</tbody></table></div>';
  }
  function togglePeriodDetail(toggle,root){var detail=qs('[data-period-detail-row="'+toggle.dataset.periodToggle+'"]',root);if(!detail)return;var opening=detail.classList.contains('hidden');detail.classList.toggle('hidden',!opening);toggle.textContent=opening?'收合款項':'款項明細';}
  ...
  function openTuition(studentId,id){
    if(!writable('新增或編輯學費'))return;refreshFormOptions();$('tuitionForm').reset();$('tuitionId').value=id||'';$('tuitionPurchasePeriods').value='1';$('tuitionStudent').value=studentId||currentStudentId||'';var row=periodById(id);if(row.id){$('tuitionStudent').value=row.studentId;$('tuitionSubject').value=row.subjectId;$('tuitionPeriodNo').value=row.periodNo;$('tuitionLessonCount').value=row.lessonCount;$('tuitionAmount').value=row.expectedAmount;$('tuitionDiscount').value=row.discount;$('tuitionStartDate').value=row.startDate;$('tuitionExpiryDate').value=row.expiryDate;$('tuitionNote').value=row.note||'';}
  ...
  function submitTuition(event){event.preventDefault();if(!writable('儲存學費期別'))return;var id=$('tuitionId').value,plan=feeById($('tuitionPlan').value),studentId=$('tuitionStudent').value,existing=id?periodById(id):{},nextPeriodNo=state.tuitionPeriods.filter(function(period){return period.studentId===studentId;}).reduce(function(max,period){return Math.max(max,numberOf(period.periodNo));},0)+1,row={id:id||uid('period'),studentId:studentId,subjectId:$('tuitionSubject').value,teacherId:$('tuitionTeacher').value,planId:plan.id,periodNo:id?numberOf(existing.periodNo):nextPeriodNo,startDate:id?existing.startDate:todayKey(),expiryDate:id?existing.expiryDate:'',lessonCount:numberOf($('tuitionLessonCount').value)||plan.lessonCount,expectedAmount:numberOf($('tuitionAmount').value),discount:numberOf($('tuitionDiscount').value),usedCount:id?existing.usedCount:0,status:'active',note:$('tuitionNote').value.trim(),transactions:id?(existing.transactions||[]):[],planSnapshot:clone(plan)};if(!row.studentId||!row.subjectId||!row.planId){toast('資料未完成','請選擇學生、科目與收費方案。','error');return;}if(id)Object.assign(existing,row);else state.tuitionPeriods.push(row);save(studentById(row.studentId).name+'・建立第 '+row.periodNo+' 期 '+plan.name);closeModal('tuitionModal');if(currentStudentId){studentTab='tuition';renderStudentModal();}renderStudents();toast('學費期別已建立','本次新增 1 期；固定老師、金額、堂數、拆帳與請假規則已保存。');}
  ...
  function renderRefundLessonOptions(){
    var refund=$('transactionType').value==='refund',period=periodById($('transactionPeriodId').value),rows=state.attendance.filter(function(row){return row.periodId===period.id&&normalizedStatus(row.status)!=='cancelled';}).sort(function(a,b){return clean(a.date).localeCompare(clean(b.date));});
