(function(){
  ...
  function eventStudentNames(event){return Array.from(new Set((event.studentIds||[]).map(function(id){return studentById(id).name;}).concat(event.studentNames||[]).map(clean).filter(Boolean)));}
  ...
  async function submitEntity(event){
    ...
    if(type==='student'){row={id:id||uid('student'),name:$('entityName').value.trim(),phone:$('entityPhone').value.trim(),line:$('entityLine').value==='null'?null:$('entityLine').value==='true',active:$('entityActive').value==='true',note:$('entityNote').value.trim()};upsert(state.students,row);}
    if(type==='teacher'){
      old=teacherById(id);row={id:id||uid('teacher'),name:$('entityName').value.trim(),phone:$('entityPhone').value.trim(),reward:numberOf(old.reward),deduction:numberOf(old.deduction),active:$('entityActive').value==='true',note:$('entityNote').value.trim(),subjectIds:$$('input[name="teacherSubject"]',$('entityModalBody')).filter(function(box){return box.checked;}).map(function(box){return box.value;})};
