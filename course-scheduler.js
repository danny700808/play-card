(function(){
  ...
  var entityContext={type:'',id:''},policyRoomId='',loadingMigration=false,scheduleDragMoved=false;
  ...
  function clearScheduleForm(){
    $('scheduleForm').reset();$('eventId').value='';$('eventSeriesId').value='';$('eventDate').disabled=false;$('eventStart').disabled=false;$('eventRoom').disabled=false;$('eventDate').value=state.currentDate;$('eventDuration').value='60';$('eventType').value='fixed';$('eventFrequency').value='weekly';$('eventRepeatUntil').value='';$('repeatUntilField').classList.remove('hidden');$('eventNote').value='';$('eventClient').value='';$('eventRentalFee').value='';
  }
  function openSchedule(options){
    if(!writable('新增或編輯排課'))return;options=options||{};refreshFormOptions();clearScheduleForm();var source=options.event||options.copy||null;$('scheduleModalTitle').textContent=options.event?'編輯這一次課程（日期、時間與教室請使用調課）':options.copy?'增加課程':'新增排課';if(source){$('eventId').value=options.event?source.id:'';$('eventSeriesId').value=source.seriesId||'';$('eventDate').value=source.date;$('eventStart').value=source.start;$('eventDuration').value=String(source.duration||60);$('eventRoom').value=source.roomId;$('eventType').value=source.type;$('eventFrequency').value=options.event?'once':source.frequency||'once';$('eventStudent').value=(source.studentIds||[])[0]||'';$('eventSubject').value=source.subjectId||'';$('eventClient').value=source.clientName||'';$('eventRentalFee').value=source.rentalFee||'';$('eventNote').value=source.note||'';if(options.event){$('eventDate').disabled=true;$('eventStart').disabled=true;$('eventRoom').disabled=true;}}
  ...
  function eventDetails(event){
    ...
    if(!isReadOnly()&&event.type!=='rental')html='<div class="event-quick-actions"><button class="btn primary" type="button" data-event-action="copy">增加課程</button><button class="btn secondary" type="button" data-event-action="cut">調課</button></div>'+html;
    ...
    $('eventModalFoot').innerHTML='<div class="formal-view-note">'+(isReadOnly()?'正式資料目前只供查看。':'點「第幾期／課程方案」可開啟編輯與刪除本次課程。')+'</div>';$('eventModal').dataset.eventId=event.id;openModal('eventModal');
  }
  ...
  function tuitionTableHtml(studentId,options){
    ...
    var periods=state.tuitionPeriods.filter(function(row){return row.studentId===studentId;}).sort(function(a,b){return numberOf(a.periodNo)-numberOf(b.periodNo);}),latest=periods.length?periods[periods.length-1]:{},rows=periods.map(function(period){
      var transactions=(period.transactions||[]).slice().sort(function(a,b){return clean(a.date).localeCompare(clean(b.date));}),transactionHtml=transactions.map(function(tx){var method=displayTransactionMethod(tx.method);return '<span class="payment-line"><time>'+esc(tx.date||'未填日期')+'</time><b>'+esc(tx.type==='refund'?'退款':'收費')+' '+money(tx.amount)+'</b>'+(method?'<small>'+esc(method)+'</small>':'')+'</span>';}).join('')||'<small class="no-payment">尚無收退款紀錄</small>',planName=clean((period.planSnapshot||{}).name)||clean(feeById(period.planId).name)||'既有收費方案',balance=periodBalance(period),payment=balance>0?'<b class="tuition-due">未繳 '+money(balance)+'</b>':'<span class="tuition-paid">已繳清</span>',current=options.eventId&&period.id===options.currentPeriodId,actions='<div class="period-course-actions hidden" data-period-course-actions="'+esc(period.id)+'">'+(isSandbox()?'<button type="button" class="btn small outline" data-period-edit="'+esc(period.id)+'">編輯本期期別</button>':'')+(current&&isSandbox()?'<button type="button" class="btn small outline" data-event-action="edit">編輯這一次課程</button><button type="button" class="btn small danger" data-event-action="delete">刪除這一次課程</button>':'')+'</div>';
      return '<tr class="tuition-period-row'+(period.id===latest.id?' latest':'')+'"><td><button type="button" class="period-course-button" data-period-course-toggle="'+esc(period.id)+'"><b>第 '+esc(period.periodNo)+' 期・'+esc(subjectById(period.subjectId).name||'未設定科目')+'</b><small>'+esc(planName)+'</small></button>'+actions+'</td><td><div class="payment-status-cell">'+payment+transactionHtml+'</div></td><td>'+(isSandbox()?'<button type="button" class="btn small secondary" data-period-pay="'+esc(period.id)+'">收費／退款</button>':'<span class="tuition-readonly">正式唯讀</span>')+'</td><td>'+periodLessonSlots(period)+'</td></tr>';
  ...
  function togglePeriodCourse(toggle,root){var actions=qs('[data-period-course-actions="'+toggle.dataset.periodCourseToggle+'"]',root);if(actions)actions.classList.toggle('hidden');}
  ...
  function submitTransaction(event){
    ...
    closeModal('transactionModal');studentTab='tuition';if(currentStudentId)renderStudentModal();renderStudents();renderTeachers();toast(type==='refund'?'退款紀錄完成':'收費紀錄完成',selectedSlots.length?'已標示 '+selectedSlots.length+' 堂退款；退款不會被重新算成欠費。':'未覆寫原金額，已新增一筆獨立異動。');
  }
  ...
  function bindEvents(){
    ...
    $('tuitionForm').addEventListener('submit',submitTuition);$('tuitionSubject').addEventListener('change',function(){updateTuitionForm();});$('tuitionPlan').addEventListener('change',renderTuitionSnapshot);$('transactionForm').addEventListener('submit',submitTransaction);$('transactionType').addEventListener('change',renderRefundLessonOptions);$('transactionRefundCount').addEventListener('change',function(){syncRefundSelection(true);});$('refundSlotChoices').addEventListener('change',function(){syncRefundSelection(false);});
