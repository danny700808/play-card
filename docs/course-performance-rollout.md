...
前端改用 `youzi-c1b74-taiwan`，96 個設定引用更新為 `20260919-taiwan-storage-v3`，須重新整理／重新開啟頁面才採用。前端 release manifest 指定後端 run `35444663273` 與版本 `a124a57be81bee4b4302f889dd00d3b4813fd8dd`；部署前核對該 run 成功且 functions 程式完全相同，僅發布 hosting，不重部署函式或 LINE Webhook。合併與正式完成狀態以這兩階段部署紀錄為準。

租賃管理頁刪除附件會一併處理已知兩桶的同一路徑；權限失敗不回報成功。舊影片及未重新整理的頁面仍可在原桶完成，新功能不會將尚未完成的上傳誤認成副本。前端設定、跨桶刪除與相關搜尋／助手快取檢查共 29 項通過。順便修正先前台灣入口更新後仍檢查 `operations-phase1` 舊快取字串的兩個測試及 CI markup gate，沒有修改搜尋或助手功能。
