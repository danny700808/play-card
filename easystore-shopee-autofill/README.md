...
3. 在供應商頁按右鍵選「柚子掌櫃：框選截圖」，第一次同意 Chrome 的快速截圖權限；之後可按「＋ 快速點圖」直接點綠框，也可隨時按 `Ctrl+Shift+Y` 框選畫面。截圖完成後，右上角的「已加入／結束收圖」小框會立即恢復，圖片則在背景送入同一個 SKU／EasyStore 商品 ID 的圖片區。
4. 綠框原圖若被供應商網站阻擋，助手會顯示繁中提示；改用右鍵選單或 `Ctrl+Shift+Y` 框選即可，不需要開放所有網站的永久截圖權限。
5. 若使用 Windows 的 `Win+Shift+S`，截完後回供應商頁按 `Ctrl+V`，截圖也會直接送入目前商品。
6. 每件商品最多保留 20 張來源圖片；正式處理時仍最多勾選 12 張。截錯可回「準備上架」按圖片下方的「刪除」。「＋ 快速點圖」按一次即持續開啟到結束收圖或重新整理頁面。
7. 按 `Esc` 或「結束收圖」即可停止。這一步只收圖，不做簡繁轉換。
