...
7. 0.3.29 會要求「所有網站」與腳本執行權限；這兩項用於頁內框選截圖，以及替更新前已開啟的普通網頁補載入收圖面板。
8. 同一台電腦不要同時保留 0.3.28 與 0.3.29；更新後重新整理營運中心，再重新開始搜圖。
...
2. 保留商品頁，另外開啟任何一般商品網頁；右上角會自動出現「柚子掌櫃收圖中」。
   - 開始搜圖時，0.3.29 也會替更新前已開著的普通網頁自動補載入；新開或重新整理的商品頁會照常自動出現。
3. 要抓單張圖片時，把滑鼠移到圖片上；出現綠框後點一下，圖片會自動加入同一個 SKU／EasyStore 商品 ID。助手會優先讀取乾淨原圖，網站阻擋時才自動改用可見畫面截圖。
4. 要截取畫面的一部分時，直接按右上角的「框選截圖」，按住滑鼠拉出範圍，放開後按「確認截圖」。不需要右鍵，也不需要移動或縮放四角。
