...
3. 在供應商頁按右鍵選「柚子掌櫃：框選截圖」，或直接按 `Ctrl+Shift+Y`；再用滑鼠拉出需要的畫面。截圖完成後，右上角的「已加入／結束收圖」小框會立即恢復，圖片則在背景送入同一個 SKU／EasyStore 商品 ID 的圖片區。
...
6. 每件商品最多保留 12 張來源圖片；截錯可回「準備上架」按圖片下方的「刪除」。原圖點選預設關閉，需要時可自行開啟。
