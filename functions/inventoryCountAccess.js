...
function adjustLayers(raw,target){return averageCostModel.snapshot(raw,target);}
...
function createInventoryCountAccess({db,FieldValue,requireManager,now=Date.now}){
  ...
  async function save(data,req){
    ...
    return db.runTransaction(async tx=>{
      ...
      tx.set(inventoryRef,{type:'adjustment',productId:id,productName:clean(raw.internalName||raw.originalName||raw.onlineName),sku,qtyChange:target-before,beforeStock:before,afterStock:target,unitCost:averageCostModel.unit(raw),costMethod:'MANUAL_AVERAGE',referenceType:'stocktake',referenceId:'COUNT-'+operationId,stocktakeNo:'COUNT-'+operationId,counterName:operator,source:'mobile',note:clean(data.note)||'手機庫存盤點',occurredAt:new Date(),createdAt:FieldValue.serverTimestamp(),createdBy:operator,version:'mobile-count-server-v1'});
      tx.create(operationRef,{fingerprint,result,createdAt:FieldValue.serverTimestamp()});return result;
    });
  }
  return {login,products,save,session};
}
