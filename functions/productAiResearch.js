...
function buildLocalizedImagePrompt(context, listingCase, position, total) {
  ...
  return [
    ...
    '一般來源僅尺寸不足但商品與文字仍可辨識時，已授權依平台規格等比例調整尺寸，並以真實繁體字型重新排字；不必再次詢問。不得裁切、改構圖、重畫商品或把插值宣稱為新增細節。模糊、不完整或無法正常辨識的來源仍不得放大假清晰或憑印象重畫；改找同商品更清楚完整來源，找不到就跳過該圖。相容圖必須檢查實際尺寸、比例、格式、檔案大小與視覺可讀性，並保留來源與輸出雜湊。',
...
function buildBrandTemplateImagePrompt(context, listingCase, role) {
  ...
  return [
    ...
    '材質保真：風格紋理只能用在海報背景，不得加到商品布面或改變口袋、五金。若先前生成圖已新增花紋或錯誤材質，不能沿用錯圖反覆修補；改用原始來源或已核對 cleanMain 作商品基準，保留同一 styleId 重建。相同材質錯誤連續兩次仍失敗就記錄 image-qa-failed，停止盲目循環，未核對完成不可標 ready 或 approved。',
