...
function buildMainTemplateImagePrompt(context, listingCase) {
  ...
  return [
    ...
    '固定品牌骨架：保留頂端薄荷綠紋理品牌區、紅色標語與右上柚子樂器標誌；移除左下角可愛寶寶與右下角 PIC COLLAGE，不得加入價格、地址、電話、QR Code、浮水印或聯絡資訊。',
    '綠色品牌區下方必須重新設計成有層次的淺色商業展示底板，不限定純白。依商品配色選用奶油白、米色、淺灰、淺藍、淺粉或其他低彩度淺色，搭配柔和弧形、展示台、棚拍光影或乾淨景深；底板要明亮、協調、可讀，不能搶過商品。',
    ...
    '整體要像專業商品型錄封面：主商品最醒目、資訊層級清楚、留白充足，不做滿版文字、雜亂拼貼或過多裝飾。不同商品可以更換淺色底板、點綴色與局部排列，但固定維持相同的品牌頁首與「左側資訊、右側／中央商品主視覺」骨架。',
