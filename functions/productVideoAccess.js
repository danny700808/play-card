...
function createProductVideoAccess({db,FieldValue,bucket,authorize,resolveBucket=()=>bucket,legacyBucketName=bucket.name,now=Date.now}){
  ...
  async function start(data){
    ...
    await db.collection('inventoryMediaUploads').doc(uploadId).set({productId,operator,size,contentType,path,fileName:clean(data.fileName).slice(0,180),durationSeconds:Math.max(0,Math.min(86400,Number(data.durationSeconds)||0)),storageBucket:bucket.name,sessionHash:hash(clean(data.token)),expiresAtMs:now()+8*3600000,status:'uploading',createdAt:FieldValue.serverTimestamp()});
  ...
  async function finish(data){
    ...
    const uploadBucket=resolveBucket(upload.storageBucket||legacyBucketName);
    const file=uploadBucket.file(upload.path),[metadata]=await file.getMetadata();
    ...
    const url='https://firebasestorage.googleapis.com/v0/b/'+uploadBucket.name+'/o/'+encodeURIComponent(upload.path)+'?alt=media&token='+downloadToken;
