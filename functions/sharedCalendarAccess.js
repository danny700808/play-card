...
function createSharedAccess({db,ownerSession,passwordless,verifier=webauthn}){
 ...
 async function freshMember(request){const who=await identity(request);if(who.role!=='member'||who.createdAt<Date.now()-300000)fail('請重新用 LINE 或 Email 登入，再啟用 Face ID。');return member(who.id);}
 async function api(request){
  ...
  if(['activate','password'].includes(action))fail('請改用 LINE 或 Email 登入。','failed-precondition');
  if(['lineLoginStart','emailLoginStart','loginRedeem'].includes(action)){if(!passwordless)fail('登入服務尚未啟用。');return passwordless().api(request);}
  ...
  if(action==='authenticationOptions'){
   const m=await member(d.memberId),keys=(await credentials.where('memberId','==',m.id).get()).docs.map(d=>d.data()).filter(k=>k.version===m.version);if(!keys.length)fail('請先用 LINE 或 Email 登入，再啟用 Face ID。');return challenge(await verifier.generateAuthenticationOptions({rpID:RP,userVerification:'required',allowCredentials:keys.map(k=>({id:k.id,transports:k.transports}))}),'login',m);
  }
