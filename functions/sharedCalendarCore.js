...
function createSharedCore({db,auth,bucket,ownerLine,sendLine}){
 ...
 async function tick(){
  ...
  for(const doc of docs){const r=doc.data();if(r.draft||!r.remind||['done','cancelled'].includes(r.status))continue;const offsets=reminderOffsets(r);for(const minutes of offsets){const at=Date.parse(r.start)-minutes*60000;if(at>now||at<=now-86400000)continue;const suffix=minutes===(r.reminderMinutes??10)?'':'|'+minutes,ref=outbox.doc(hash(doc.id+'|remind|'+r.scheduleVersion+suffix));await db.runTransaction(async tx=>{if((await tx.get(ref)).exists)return;tx.create(ref,{ownerUid:uid,taskId:doc.id,to:r.assignedTo,actorId:'system',actorName:'定時提醒',kind:'reminder',title:r.title,start:r.start,reminderMinutes:minutes,taskVersion:r.scheduleVersion,key:crypto.randomUUID(),state:'pending',createdAt:now,attempts:0});});}}
