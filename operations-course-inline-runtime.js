(function(){
  ...
  function submitSchedule(event){
    ...
    runUiOperation('正在保存雲端排課…',$('scheduleSubmitBtn'),async function(){
  ...
  function autoSaveRentalPayment(){
    var original=findEvent($('eventId').value);if(!original||original.type!=='rental')return;
    var input=$('eventRentalPaymentStatus'),previous=original.rentalPaymentStatus||'unpaid',row=formEvent();
    if(operationRunning){input.value=previous;return;}
    input.disabled=true;runUiOperation('正在儲存租用收款…',null,async function(){try{await persistScheduleChange(row,{source:original,mode:'save'});toast(row.rentalPaymentStatus==='paid'?'已收款・租用簽到完成':'已改為未收款・取消簽到','已自動儲存。');}catch(error){if(!error.saved)input.value=previous;throw error;}finally{input.disabled=false;}});
  }
