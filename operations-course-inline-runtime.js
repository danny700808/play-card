(function(){
  ...
  function renderFollowupCounts(){
    $('dayIrregularCount').textContent=followupStopsCache===null?'…':followupRows().length;
    $('weekIrregularCount').textContent=$('weekTeacher').value?(followupStopsCache===null?'…':followupRows($('weekTeacher').value).length):0;
  }
  ...
  function openFollowupList(kind,selectedMonth){
    ...
    var teacher=kind==='week'?$('weekTeacher').value:'',allRows=followupRows(teacher),months=Array.from(new Set(allRows.map(followupMonthKey))).sort().reverse();
    var rows=allRows.filter(function(row){return followupMonthFilter==='all'||followupMonthKey(row)===followupMonthFilter;}).sort(function(a,b){return followupMonthKey(b).localeCompare(followupMonthKey(a));});
    var monthLabel=function(key){return key==='unknown'?'未記錄生效日期':Number(key.slice(0,4))+' 年 '+Number(key.slice(5,7))+' 月';};
    var filterHtml='<div class="filters"><label>發生月份 <select class="control" data-followup-month><option value="all">全部月份</option>'+months.map(function(month){return '<option value="'+esc(month)+'" '+(month===followupMonthFilter?'selected':'')+'>'+monthLabel(month)+'</option>';}).join('')+'</select></label><span>共 '+rows.length+' 筆學生科目</span></div>',lastMonth='';
    $('calendarIssuesTitle').textContent=(teacher?teacherById(teacher).name+'・':'')+'不定時／停課學生';
    $('calendarIssuesSubtitle').textContent='依學生、科目與老師列示目前狀態；未收款以最新帳務為準。點擊可查看學費紀錄。';
    $('calendarIssuesBody').innerHTML=filterHtml+(rows.length?'<div class="calendar-issue-list">'+rows.map(function(row){var key=followupMonthKey(row),heading=key!==lastMonth?'<h3>'+monthLabel(key)+'</h3>':'';lastMonth=key;var stopped=row.kind==='stopped',label=!stopped?'不定時':!row.balanceVerified?'已停課・帳務待核對':row.unpaidBalance>0?'已停課・尚有未收款':'已停課・已結清',details=stopped?(row.effectiveDate?'停課日期 '+esc(row.effectiveDate)+'｜':'')+(!row.balanceVerified?'請開啟學費紀錄核對':row.unpaidBalance>0?'未收款 '+money(row.unpaidBalance)+'・前往收款':'目前無未收款'):(row.resumedFrom?esc(row.resumedFrom)+' 起恢復固定課':'需另約上課時間');return heading+'<button type="button" class="calendar-issue-row" data-followup-student="'+esc(row.studentId)+'" data-followup-subject="'+esc(row.subjectId||'')+'"><span class="tag '+(stopped&&row.unpaidBalance>0?'red':stopped?'gray':'green')+'">'+label+'</span><b>'+esc(studentById(row.studentId).name||row.studentName||'學生')+'</b><small>'+esc(subjectById(row.subjectId).name||row.subjectName||'')+'｜'+esc(teacherById(row.teacherId).name||row.teacherName||'')+'</small><em>'+details+'</em></button>';}).join('')+'</div>':'<div class="calendar-issue-empty">這個月份沒有不定時或已停課學生。</div>');openModal('calendarIssuesModal');
  }
  ...
  function renderCalendar(){
    ...
    renderFollowupCounts();var date=state.currentDate,rooms=activeRooms(),events=effectiveEventsForDate(date).filter(function(event){return !desktopCalendar()||!window.YouziCoursePreviewData.isIrregularPlaceholder(event,state.irregularCourses);}),conflicts=dayConflictIds(events),issues=calendarIssuesForDate(date),hours=scheduleHoursForDate(date),used=new Set(events.map(function(row){return row.roomId;}));
    $('calendarDate').value=date;$('dateTitle').textContent=zhDate(date);$('dateSubtitle').textContent=weekdayName(date)+(date===todayKey()?'・今天':'');$('kpiLessons').textContent=events.filter(function(row){return row.type!=='rental';}).length;$('kpiAttended').textContent=events.filter(function(row){return normalizedStatus(row.status)==='attended';}).length;$('kpiRooms').textContent=used.size+' / '+rooms.length;$('kpiWarnings').textContent=issues.length;$('calendarHint').textContent='30 分鐘／格・'+rooms.length+' 間啟用教室'+(calendarPartial()?'・點課程查看明細':isReadOnly()?'・資料尚未載入':isSandbox()?'・操作自動儲存':'');
  ...
  function openStudent(id,tab){if(afterWorkspaceReady(function(){openStudent(id,tab);}))return;currentStudentId=id;currentStudentSubject='';studentTab=tab||'profile';var student=studentById(id);if(!student.id)return;$('studentModalTitle').textContent=student.name;$('studentModalSubtitle').textContent=(student.phone||'未填手機')+'・'+(studentCourseStatusLabel(student));renderStudentModal();openModal('studentModal');}
