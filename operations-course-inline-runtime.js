(function(){
  ...
  function completedUnpaidPeriods(){return state.tuitionPeriods.filter(function(period){return periodRemaining(period)===0&&numberOf(period.usedCount)>0&&periodBalance(period)>0;});}
  ...
  function openFollowupList(kind){
    ...
    $('calendarIssuesSubtitle').textContent=unpaid?'已完成本期堂數、仍有學費未收清的期別。':'依老師目前設定顯示；恢復固定課後，於生效日從這裡移除。';
    $('calendarIssuesBody').innerHTML=rows.length?'<div class="calendar-issue-list">'+rows.map(function(row){var ids=unpaid?[row.studentId]:(row.studentIds||[]),source=row.source||{},names=ids.map(function(id){return studentById(id).name;}).filter(Boolean).join('／')||(source.studentNames||[]).join('／');return '<button type="button" class="calendar-issue-row" data-followup-student="'+esc(ids[0]||'')+'"><b>'+esc(names||'學生')+'</b><small>'+esc(subjectById(row.subjectId).name||source.subjectName||'')+'｜'+esc(teacherById(row.teacherId).name||'')+'</small><em>'+(unpaid?'第 '+row.periodNo+' 期・未收款 '+money(periodBalance(row)):row.resumedFrom?esc(row.resumedFrom)+' 起恢復固定課':'不定時')+'</em></button>';}).join('')+'</div>':'<div class="calendar-issue-empty">目前沒有'+(unpaid?'課程結束未收款':'不定時學生')+'。</div>';openModal('calendarIssuesModal');
  }
  ...
  async function refreshIrregularList(){
    ...
    irregularLoading=true;try{var rows=await window.YouziCoursePreviewData.loadIrregularCourses(),changed=JSON.stringify(rows)!==JSON.stringify(state.irregularCourses||[]);irregularLoadedAt=Date.now();if(changed){await loadPublishedWorkspace();}else{renderFollowupCounts();}}catch(error){if(error.reauth)$('calendarReauthPanel').classList.remove('hidden');}finally{irregularLoading=false;}
  }
