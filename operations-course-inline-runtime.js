(function(){
  ...
  function renderCalendar(){
    ...
    events.forEach(function(event){
      ...
      var span=Math.min(slots.length-si,Math.max(1,Math.ceil(numberOf(event.duration)/30))),normalized=normalizedStatus(event.status),status=normalized==='scheduled'?'':normalized,badge=normalized==='attended'?'✓':normalized==='leave'?'假':normalized==='absent'?'曠':normalized==='pending_conflict'?'待':'',studentCount=(event.studentIds||[]).length,groupLabel=studentCount>2?'團體課':studentCount===2?'雙人課':'';
