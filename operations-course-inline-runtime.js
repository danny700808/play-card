(function(){
  ...
  function completedUnpaidPeriods(){return window.YouziCoursePreviewData.stoppedCourseReceivables(state.stoppedCourseReceivables,state.tuitionPeriods,periodBalance);}
  function renderFollowupCounts(){
    $('completedUnpaidCount').textContent=calendarPartial()?'…':completedUnpaidPeriods().length;
    $('dayIrregularCount').textContent=irregularRows().length;
    $('weekIrregularCount').textContent=$('weekTeacher').value?irregularRows($('weekTeacher').value).length:0;
  }
  function openFollowupList(kind){
    if(kind==='unpaid'&&afterWorkspaceReady(function(){openFollowupList(kind);}))return;
    var unpaid=kind==='unpaid',teacher=kind==='week'?$('weekTeacher').value:'',rows=unpaid?completedUnpaidPeriods():irregularRows(teacher);
    $('calendarIssuesTitle').textContent=unpaid?'課程結束未收款':(teacher?teacherById(teacher).name+'・':'')+'不定時學生';
    $('calendarIssuesSubtitle').textContent=unpaid?'僅列出本功能啟用後，老師新辦理停課且仍有未收款的科目。收款、退款或轉課完成後，依原科目帳務餘額更新。':'依老師目前設定顯示；恢復固定課後，於生效日從這裡移除。';
    $('calendarIssuesBody').innerHTML=rows.length?'<div class="calendar-issue-list">'+rows.map(function(row){var ids=unpaid?[row.studentId]:(row.studentIds||[]),source=row.source||{},names=ids.map(function(id){return studentById(id).name;}).filter(Boolean).join('／')||(source.studentNames||[]).join('／');return '<button type="button" class="calendar-issue-row" data-followup-student="'+esc(ids[0]||'')+'" data-followup-subject="'+esc(unpaid?row.subjectId:'')+'"><b>'+esc(names||'學生')+'</b><small>'+esc(subjectById(row.subjectId).name||source.subjectName||'')+'｜'+esc(teacherById(row.teacherId).name||'')+'</small><em>'+(unpaid?'第 '+row.periodNo+' 期・未收款 '+money(row.unpaidBalance)+'・前往收款':row.resumedFrom?esc(row.resumedFrom)+' 起恢復固定課':'不定時')+'</em></button>';}).join('')+'</div>':'<div class="calendar-issue-empty">目前沒有'+(unpaid?'課程結束未收款':'不定時學生')+'。</div>';openModal('calendarIssuesModal');
  }
  var irregularLoadPromise=null,irregularLoadedAt=0;
  function refreshIrregularList(){
    if(irregularLoadPromise)return irregularLoadPromise;
    if(calendarBootstrapLoading||!desktopCalendar()||window.location.hash!=='#course-calendar'||window.document.hidden||Date.now()-irregularLoadedAt<30000)return Promise.resolve();
    irregularLoadPromise=(async function(){
      try{
        ...
        if(changed)renderCalendar();else renderFollowupCounts();
      }catch(error){if(error.reauth)$('calendarReauthPanel').classList.remove('hidden');}
  ...
  function init(){
    ...
    $('completedUnpaidBtn').addEventListener('click',async function(){await refreshIrregularList();openFollowupList('unpaid');});$('dayIrregularBtn').addEventListener('click',async function(){await refreshIrregularList();openFollowupList('day');});$('weekIrregularBtn').addEventListener('click',async function(){await refreshIrregularList();openFollowupList('week');});window.addEventListener('focus',refreshIrregularList);window.addEventListener('hashchange',refreshIrregularList);window.setInterval(refreshIrregularList,60000);
