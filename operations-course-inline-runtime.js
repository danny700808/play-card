(function(){
  ...
  function updateTuitionOptions(selected){
    var studentId=$('eventStudent').value,subjectId=$('eventSubject').value,current=selected||$('eventTuitionPeriod').value,rows=state.tuitionPeriods.filter(function(row){return row.studentId===studentId&&row.subjectId===subjectId&&row.status!=='cancelled';}).sort(function(a,b){return numberOf(b.periodNo)-numberOf(a.periodNo);});
    fillSelect($('eventTuitionPeriod'),rows,function(row){return '第 '+row.periodNo+' 期・剩 '+periodRemaining(row)+' / '+row.lessonCount+' 堂・'+(periodBalance(row)?'未繳 '+money(periodBalance(row)):'已繳清');},function(row){return row.id;},!studentId?'請先選擇學生':!subjectId?'請先選擇科目':rows.length?'請選擇期別':'尚未建立此科目的學費期別');
    if(current&&rows.some(function(row){return row.id===current;}))$('eventTuitionPeriod').value=current;else if(rows.length===1)$('eventTuitionPeriod').value=rows[0].id;
    $('scheduleNewTuition').disabled=!studentId;
    $('scheduleTuitionHint').textContent=!studentId?'先搜尋既有學生，或按「新增學生」。':!subjectId?'請選擇科目；新學生可直接建立第一期。':!rows.length?'請按「建立學費期別」，完成後會回到這裡繼續排課。':'排課本身不扣堂，簽到後才從選定期別扣抵。';
  }
  function scheduleDraft(){var draft=formEvent();draft.endDate=$('eventRepeatUntil').value;return draft;}
  function createScheduleTuition(){
    if(!$('eventStudent').value)return;
    scheduleDraftAfterTuition=scheduleDraft();closeModal('scheduleModal');openTuition(scheduleDraftAfterTuition.studentIds[0],'',scheduleDraftAfterTuition.subjectId);
    if(scheduleDraftAfterTuition.subjectId){$('tuitionSubject').value=scheduleDraftAfterTuition.subjectId;updateTuitionForm({teacherId:scheduleDraftAfterTuition.teacherId});}
    $('tuitionModalSubtitle').textContent='建立完成後會回到原本的排課時段。建立期別不代表已收款。';
  }
  function selectScheduleStudent(studentId,autofill){
    ...
    if(autofill){var subjects=unique(state.tuitionPeriods.filter(function(p){return p.studentId===student.id&&p.status!=='cancelled';}).map(function(p){return p.subjectId;}));$('eventSubject').value=subjects.length===1?subjects[0]:'';$('eventTuitionPeriod').value='';updateTeacherOptions(subjects.length===1?period.teacherId:'');updateRoomOptions();}updateTuitionOptions();
  ...
  function submitSchedule(event){
    ...
    runUiOperation('正在保存雲端排課…',$('scheduleSubmitBtn'),async function(){
      await persistScheduleChange(row,{operationId:operationId,source:original,mode:permanentScheduleSource?'permanent_move':'save',repeatUntil:$('eventRepeatUntil').value});
  ...
  async function submitEntity(event){
    ...
    if(type==='student'){row={id:id||uid('student'),name:$('entityName').value.trim(),phone:$('entityPhone').value.trim(),line:$('entityLine').value==='null'?null:$('entityLine').value==='true',active:$('entityActive').value==='true',note:$('entityNote').value.trim()};entityContext.id=row.id;try{var studentSaved=await window.YouziCoursePreviewData.saveStudent(Object.assign({manualSyncPin:storedMigrationPin()},row));row=Object.assign(row,studentSaved.student);upsert(state.students,row);}catch(error){toast('學生資料尚未保存',clean(error&&error.message||error),'error');return;}}
    if(type==='teacher'){
