(function(){
  ...
  function submitSchedule(event){
    ...
    runUiOperation('正在保存雲端排課…',$('scheduleSubmitBtn'),async function(){
      await persistScheduleChange(row,{operationId:operationId,source:original,repeatUntil:$('eventRepeatUntil').value});
  ...
  function eventDetails(event){
    ...
    if(event.type!=='rental')html='<div class="status-actions event-command-row">'+(!isReadOnly()&&student.id?'<button type="button" data-event-new-period="'+esc(student.id)+'" data-event-subject="'+esc(event.subjectId)+'">增加期數</button>':'')+(!isReadOnly()?'<button type="button" data-event-action="copy">增加課程</button><button type="button" data-event-action="cut">調課</button>':'')+'<button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>✓ 簽到</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>取消簽到</button><button data-attendance="leave" '+(isReadOnly()?'disabled':'')+'>請假</button><button data-attendance="absent" '+(isReadOnly()?'disabled':'')+'>曠課</button></div>'+html;
