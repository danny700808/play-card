(function(){
  ...
  var state=null,formalState=null,currentView='calendar',currentStudentId='',currentStudentSubject='',currentTeacherId='',studentTab='profile';
  ...
  var workspaceSaveTimer=0,weekMode=false,weekAnchor='',scheduleDraftAfterStudent=null,scheduleDraftAfterTuition=null,permanentScheduleSource=null,returnToScheduleAfterStudent=false,operationRunning=false,embeddedMode=false;
  var teacherPayrollMonthRefreshes={};
  var workspaceLoadPromise=null,calendarBootstrapLoading=false;
  ...
  function money(value){return '$'+numberOf(value).toLocaleString('zh-TW',{maximumFractionDigits:2});}
  ...
  function eventStudentNames(event){var names=(event.studentIds||[]).map(function(id){return studentById(id).name;}).concat(event.studentNames||[]);return Array.from(new Set(names.reduce(function(all,name){return all.concat(clean(name).split(/[、，,－]/));},[]).map(clean).filter(Boolean)));}
  ...
  function rentalPaymentValue(status){return ['paid','received','已收款','已付款'].indexOf(clean(status).toLowerCase())>=0?'paid':'unpaid';}
  ...
  function subjectNeedsPricing(row){var status=clean(row&&row.pricingStatus).toLowerCase();return row&&['configured','complete'].indexOf(status)<0&&!state.feePlans.some(function(plan){return plan.subjectId===row.id&&plan.active!==false&&plan.listed!==false;});}
  function configurableSubjects(){return state.subjects.filter(function(row){return row.active!==false||subjectNeedsPricing(row);}).sort(bySort);}
  ...
  function scheduleWorkspaceSave(){
    ...
    workspaceSaveTimer=setTimeout(async function(){
      if(!isSandbox()||calendarPartial())return;
      var saved=await storeWorkspaceDatabase(state);
      if($('saveState'))$('saveState').textContent=saved?'本機暫存已完成':'本機暫存失敗・請勿關閉此頁';
    },650);
  }
  function save(message){
    ...
    $('saveState').textContent=calendarPartial()?'課表已更新，點選課程查看明細':isReadOnly()?'正在開啟資料':'正在保存本機暫存';
  }
  ...
  function closeModal(id){$(id).classList.remove('open');if(!document.querySelector('.modal-backdrop.open'))document.body.style.overflow='';if(id==='tuitionModal'&&scheduleDraftAfterTuition){var draft=scheduleDraftAfterTuition;scheduleDraftAfterTuition=null;openSchedule({draft:draft});}}
  ...
  function updateModeUI(){
    ...
    if($('syncInjiaoyunBtn')){
      ...
      $('syncInjiaoyunBtn').disabled=true;
    }
  ...
  function followupRows(teacher){return window.YouziCoursePreviewData.studentCourseFollowups(state.irregularCourses,followupStopsCache||[],todayKey(),teacher);}
  function renderFollowupCounts(){
    ['day','week'].forEach(function(view){var teacher=view==='week'?$('weekTeacher').value:'',rows=followupRows(teacher);['Irregular','Stopped'].forEach(function(type){$(view+type+'Count').textContent=view==='week'&&!teacher?0:followupStopsCache===null?'…':rows.filter(function(row){return (row.kind==='stopped')===(type==='Stopped');}).length;});});
  }
  var followupMonthFilter='all',currentFollowupKind='day';
  function followupMonthKey(row){return clean(row.effectiveDate||row.stopDate||row.requestedAtText).slice(0,7)||'unknown';}
  function openFollowupList(kind,selectedMonth){
    currentFollowupKind=kind;if(selectedMonth)followupMonthFilter=selectedMonth;
    var stoppedList=kind.indexOf('stopped')>=0,teacher=kind.indexOf('week')===0?$('weekTeacher').value:'',allRows=followupRows(teacher).filter(function(row){return (row.kind==='stopped')===stoppedList;}),months=Array.from(new Set(allRows.map(followupMonthKey))).sort().reverse();
    var rows=allRows.filter(function(row){return !stoppedList||followupMonthFilter==='all'||followupMonthKey(row)===followupMonthFilter;}).sort(function(a,b){return followupMonthKey(b).localeCompare(followupMonthKey(a));});
    var monthLabel=function(key){return key==='unknown'?'未記錄生效日期':Number(key.slice(0,4))+' 年 '+Number(key.slice(5,7))+' 月';};
    var filterHtml=stoppedList?'<div class="filters"><label>停課月份 <select class="control" data-followup-month><option value="all">全部月份</option>'+months.map(function(month){return '<option value="'+esc(month)+'" '+(month===followupMonthFilter?'selected':'')+'>'+monthLabel(month)+'</option>';}).join('')+'</select></label><span>共 '+rows.length+' 筆學生科目</span></div>':'<div class="filters">共 '+rows.length+' 筆學生科目</div>',lastMonth='';
    $('calendarIssuesTitle').textContent=(teacher?teacherById(teacher).name+'・':'')+(stoppedList?'停課學生':'不定時學生');
    $('calendarIssuesSubtitle').textContent=stoppedList?'依停課月份列示，未收款以最新帳務為準。點擊可查看學費紀錄。':'列出目前所有不定時學生，不依月份限制。點擊可查看學費紀錄。';
    $('calendarIssuesBody').innerHTML=filterHtml+(rows.length?'<div class="calendar-issue-list">'+rows.map(function(row){var key=followupMonthKey(row),heading=stoppedList&&key!==lastMonth?'<h3>'+monthLabel(key)+'</h3>':'';lastMonth=key;var stopped=row.kind==='stopped',label=!stopped?'不定時':!row.balanceVerified?'已停課・帳務待核對':row.unpaidBalance>0?'已停課・尚有未收款':'已停課・已結清',details=stopped?(row.effectiveDate?'停課日期 '+esc(row.effectiveDate)+'｜':'')+(!row.balanceVerified?'請開啟學費紀錄核對':row.unpaidBalance>0?'未收款 '+money(row.unpaidBalance)+'・前往收款':'目前無未收款'):(row.resumedFrom?esc(row.resumedFrom)+' 起恢復固定課':'需另約上課時間');return heading+'<button type="button" class="calendar-issue-row" data-followup-student="'+esc(row.studentId)+'" data-followup-subject="'+esc(row.subjectId||'')+'"><span class="tag '+(stopped&&row.unpaidBalance>0?'red':stopped?'gray':'green')+'">'+label+'</span><b>'+esc(studentById(row.studentId).name||row.studentName||'學生')+'</b><small>'+esc(subjectById(row.subjectId).name||row.subjectName||'')+'｜'+esc(teacherById(row.teacherId).name||row.teacherName||'')+'</small><em>'+details+'</em></button>';}).join('')+'</div>':'<div class="calendar-issue-empty">'+(stoppedList?'這個月份沒有停課學生。':'目前沒有不定時學生。')+'</div>');openModal('calendarIssuesModal');
  }
  var irregularLoadPromise=null,irregularLoadedAt=0;
  function refreshIrregularList(force){
    if(irregularLoadPromise)return irregularLoadPromise;
    if(calendarBootstrapLoading||!desktopCalendar()||window.location.hash!=='#course-calendar'||window.document.hidden||(force!==true&&Date.now()-irregularLoadedAt<30000))return Promise.resolve();
    irregularLoadPromise=(async function(){
      try{
        var snapshot=await window.YouziCoursePreviewData.loadCalendarFollowupState();
        var changed=JSON.stringify(snapshot.irregularCourses||[])!==JSON.stringify(state.irregularCourses||[]);
        state.irregularCourses=snapshot.irregularCourses||[];
        state.stoppedCourseReceivables=snapshot.stoppedCourseReceivables||[];
        followupStopsCache=snapshot.followupStops||[];
        irregularLoadedAt=Date.now();
        if(changed)renderCalendar();else renderFollowupCounts();return true;
      }catch(error){if(error.reauth)$('calendarReauthPanel').classList.remove('hidden');if(force===true)toast('名單讀取失敗','請稍後再試。','error');return false;}
      finally{irregularLoadPromise=null;}
    })();
    return irregularLoadPromise;
  }
  ...
  function renderWeekSchedule(){
    ...
    if(!calendarRangeReady(start,end,'weekScheduleDays',renderWeekSchedule))return;
    ...
    dates.forEach(function(date,dateIndex){
      weekOverlapGroups(eventsByDate[date]).forEach(function(group){
        var visibleStart=Math.max(group.startMin,scheduleStart),visibleEnd=Math.min(group.endMin,overallRange.end);if(visibleEnd<=visibleStart)return;
  ...
  function refreshFormOptions(){
    var overallRange=overallScheduleRange(state.settings.dailyHours),times=[];for(var min=overallRange.start;min<overallRange.end;min+=30)times.push({value:minToTime(min)});fillSelect($('eventStart'),times,function(row){return row.value;},function(row){return row.value;});fillSelect($('eventStudent'),state.students.slice().sort(bySort),function(row){return row.name+'・'+(row.phone||'無電話');},function(row){return row.id;},'請搜尋或選擇學生');fillSelect($('eventSubject'),activeSubjects(),function(row){return row.name;},function(row){return row.id;},'請選擇科目');updateRoomOptions();
  ...
  function roomAllowsSubject(room,subjectId){
    if(!subjectId)return true;
    if(Array.isArray(room.allowedSubjectIds)&&room.allowedSubjectIds.length&&room.allowedSubjectIds.indexOf(subjectId)<0)return false;
    if(roomKindOf(room)!=='normal')return true;
    var subject=clean(subjectById(subjectId).name).toLowerCase(),name=clean(room.name).toLowerCase(),equipment=Array.isArray(room.rentalEquipment)?room.rentalEquipment:[],explicit=room.roomRulesVersion===1&&Array.isArray(room.rentalEquipment);
    if(/爵士鼓|電子鼓|傳統鼓|鼓組/.test(subject))return equipment.some(function(item){return ['acoustic_drums','electronic_drums'].indexOf(item)>=0;})||(!explicit&&/鼓|展演|團練/.test(name));
    if(/鋼琴|電子琴|keyboard|piano/.test(subject))return roomPianoType(room)!=='none';
    if(/古箏/.test(subject))return equipment.indexOf('guzheng')>=0||/展演|kawai|卡哇伊/.test(name);
    return true;
  }
  ...
  function scheduleIsNewStudent(){return $('scheduleForm').dataset.studentMode==='new'&&$('eventType').value==='fixed';}
  function scheduleNeedsPlan(){return !permanentScheduleSource&&!$('eventId').value&&['fixed','single'].indexOf($('eventType').value)>=0&&!($('eventType').value==='single'&&$('eventSingleKind').value==='special')&&(scheduleIsNewStudent()||$('scheduleSetup').value==='new');}
  function usableSchedulePeriods(){var studentId=$('eventStudent').value,subjectId=$('eventSubject').value,date=$('eventDate').value;return state.tuitionPeriods.filter(function(p){return p.studentId===studentId&&p.subjectId===subjectId&&p.status!=='cancelled'&&(!!$('eventId').value||(p.status!=='completed'&&periodRemaining(p)>0&&(!p.expiryDate||p.expiryDate>=date)));}).sort(function(a,b){return numberOf(b.periodNo)-numberOf(a.periodNo);});}
  function updateScheduleEnrollment(){
    var type=$('eventType').value,fresh=scheduleIsNewStudent(),course=['fixed','single'].indexOf(type)>=0,special=type==='single'&&$('eventSingleKind').value==='special',rows=usableSchedulePeriods(),subjectId=$('eventSubject').value;
    $('scheduleExistingStudent').classList.toggle('hidden',fresh);$('scheduleNewStudentFields').classList.toggle('hidden',!fresh);$('quickAddStudent').classList.toggle('hidden',type!=='fixed'||fresh||!!permanentScheduleSource||!!$('eventId').value);
    if(fresh||!rows.length)$('scheduleSetup').value='new';
    $('scheduleSetupField').classList.toggle('hidden',!course||fresh||special||!rows.length||!!permanentScheduleSource||!!$('eventId').value);
    var newPlan=scheduleNeedsPlan(),selected=$('schedulePlan').value,plans=state.feePlans.filter(function(p){return p.subjectId===subjectId&&p.active!==false&&p.listed!==false;});
    fillSelect($('schedulePlan'),plans,function(p){return p.name+'・'+money(p.amount)+'／'+p.lessonCount+' 堂';},function(p){return p.id;},subjectId?'請選擇收費方案':'請先選擇課程科目');if(plans.some(function(p){return p.id===selected;}))$('schedulePlan').value=selected;
    $('schedulePlanField').classList.toggle('hidden',!newPlan);$('tuitionPeriodField').classList.toggle('hidden',!course||special||newPlan||!!permanentScheduleSource);$('scheduleNewTuition').classList.add('hidden');
    var plan=feeById($('schedulePlan').value);$('schedulePlanHint').textContent=plan.id?'共 '+plan.lessonCount+' 堂，學費 '+money(plan.amount)+'。確認排課時一併建立課程；收款另外登記。':subjectId&&!plans.length?'此科目尚未設定收費方案，請先到設定新增方案。':'選擇方案後，系統會建立學費資料，不會自動登記收款。';
    $('scheduleStudentLabel').textContent=type==='single'?'選擇要加課的既有學生':'選擇學生';
  }
  function setScheduleStudentMode(mode){
    $('scheduleForm').dataset.studentMode=mode;selectScheduleStudent('',false);$('eventStudentSearch').value='';$('scheduleSetup').value='new';updateScheduleEnrollment();if(mode==='new'){ensureScheduleCatalog().then(updateScheduleEnrollment).catch(function(error){toast('收費方案讀取失敗',clean(error.message),'error');});$('scheduleStudentName').focus();}else $('eventStudentSearch').focus();
  }
  function updateTuitionOptions(selected){
    var current=selected||$('eventTuitionPeriod').value,rows=usableSchedulePeriods();
    fillSelect($('eventTuitionPeriod'),rows,function(p){return subjectById(p.subjectId).name+'・第 '+p.periodNo+' 期・剩 '+periodRemaining(p)+' 堂';},function(p){return p.id;},'請選擇要使用的原課程');
    if(rows.some(function(p){return p.id===current;}))$('eventTuitionPeriod').value=current;else if(rows.length===1)$('eventTuitionPeriod').value=rows[0].id;
    $('scheduleTuitionHint').textContent='本次排課不扣堂；簽到時才從選定課程扣堂。';updateScheduleEnrollment();
  }
  function scheduleDraft(){var draft=formEvent();draft.endDate=$('eventRepeatUntil').value;return draft;}
  function createScheduleTuition(){$('scheduleSetup').value='new';updateScheduleEnrollment();}
  var scheduleCatalogPromise=null,scheduleCatalogReady=false,scheduleStudentLoads=new Map(),scheduleSelectionVersion=0,scheduleDataLoading=false;
  function scheduleWritable(){return calendarPartial()&&state.dataMode==='migration'||writable('排課');}
  function ensureScheduleCatalog(){
    if(!calendarPartial()||scheduleCatalogReady)return Promise.resolve();
    if(!scheduleCatalogPromise)scheduleCatalogPromise=window.YouziCoursePreviewData.loadScheduleCatalog().then(function(rows){state.feePlans=rows;scheduleCatalogReady=true;}).finally(function(){scheduleCatalogPromise=null;});
    return scheduleCatalogPromise;
  }
  async function loadSelectedScheduleStudent(studentId,autofill){
    var version=++scheduleSelectionVersion;scheduleDataLoading=true;$('scheduleTuitionHint').textContent='正在讀取這位學生的課程…';$('scheduleSubmitBtn').disabled=true;
    try{
      if(!scheduleStudentLoads.has(studentId))scheduleStudentLoads.set(studentId,Promise.all([ensureScheduleCatalog(),window.YouziCoursePreviewData.refreshWorkspaceSlice({studentIds:[studentId]})]).then(function(results){window.YouziCoursePreviewData.applyWorkspaceSlice(state,results[1]);}).catch(function(error){scheduleStudentLoads.delete(studentId);throw error;}));
      await scheduleStudentLoads.get(studentId);
      if(version!==scheduleSelectionVersion)return;
      selectScheduleStudent(studentId,autofill,true);
    }catch(error){if(version===scheduleSelectionVersion){$('eventStudent').value='';$('scheduleTuitionHint').textContent='這位學生的課程讀取失敗，請重新選擇。';toast('課程資料讀取失敗',clean(error.message),'error');}}
    finally{if(version===scheduleSelectionVersion){scheduleDataLoading=false;$('scheduleSubmitBtn').disabled=false;updateScheduleConflict();}}
  }
  function selectScheduleStudent(studentId,autofill,ready){
    if(calendarPartial()&&studentId&&!ready){loadSelectedScheduleStudent(studentId,autofill);return;}
    if(!studentId){scheduleSelectionVersion++;scheduleDataLoading=false;$('scheduleSubmitBtn').disabled=false;}
    var student=studentById(studentId);if(student.id){$('scheduleForm').dataset.studentMode='existing';$('scheduleSetup').value='existing';}$('eventStudent').value=student.id||'';$('eventStudentMatches').classList.add('hidden');
    if(!student.id){$('eventStudentSelected').classList.add('hidden');$('eventStudentSelected').innerHTML='';$('eventSubject').value='';updateTeacherOptions();updateTuitionOptions();updateScheduleConflict();return;}
    ...
    if(autofill){$('scheduleSetup').value='existing';var subjects=unique(state.tuitionPeriods.filter(function(p){return p.studentId===student.id&&p.status!=='cancelled';}).map(function(p){return p.subjectId;}));$('eventSubject').value=subjects.length===1?subjects[0]:'';$('eventTuitionPeriod').value='';updateTeacherOptions(subjects.length===1?period.teacherId:'');updateRoomOptions();}updateTuitionOptions();
  ...
  function updateScheduleSaveLabels(type){
  ...
  function updateSpecialLessonFields(resetValues){
    ...
    updateScheduleEnrollment();if(!special)return;var period=periodById($('eventTuitionPeriod').value),studentId=$('eventStudent').value,subjectId=$('eventSubject').value;if(!period.id)period=state.tuitionPeriods.filter(function(row){return row.studentId===studentId&&(!subjectId||row.subjectId===subjectId);}).sort(function(a,b){return numberOf(b.periodNo)-numberOf(a.periodNo);})[0]||{};var plan=(period.planSnapshot&&Object.keys(period.planSnapshot).length?period.planSnapshot:feeById(period.planId))||{},price=period.id?Math.round(periodNetExpectedAmount(period)/Math.max(1,numberOf(period.lessonCount))):0,pay=plan.splitType==='fixed'?numberOf(plan.splitValue):Math.round(price*teacherShareRatio(plan.splitValue));if(resetValues||!$('eventSpecialLessonPrice').value)$('eventSpecialLessonPrice').value=price;if(resetValues||!$('eventSpecialTeacherPay').value)$('eventSpecialTeacherPay').value=pay;
  }
  function updateRentalFields(roomChanged){if(permanentScheduleSource){updateRoomOptions();return;}setScheduleKind($('eventType').value,roomChanged);}
  function updateScheduleConflict(){var event=formEvent(),ignore=event.id?[event.id]:[];if(permanentScheduleSource){var source=permanentScheduleSource;ignore=effectiveEventsForDate(event.date).filter(function(e){return e.id===source.id||(source.sourceCourseId&&e.sourceCourseId===source.sourceCourseId&&normalizedStatus(e.status)==='scheduled');}).map(function(e){return e.id;});}var reasons=eventConflictReasons(event,ignore),box=$('conflictBox');box.classList.toggle('has-conflict',reasons.length>0);box.innerHTML=reasons.length?'<b>發現 '+reasons.length+' 項衝突</b><span>'+reasons.map(esc).join('<br>')+'</span>':'<b>目前未發現衝突</b><span>系統會依已填寫的資料檢查衝突。</span>';return reasons;}
  function clearScheduleForm(){
    delete $('scheduleForm').dataset.sourceSubjectId;delete $('scheduleForm').dataset.halfHourAcknowledged;delete $('scheduleForm').dataset.studentMode;
    permanentScheduleSource=null;$('eventFrequency').querySelector('[value=once]').disabled=false;['eventSubject','eventTeacher','eventDuration','eventStudentSearch'].forEach(function(id){$(id).disabled=false;});$('scheduleTypeTabs').classList.remove('hidden');$('studentField').classList.remove('hidden');$('scheduleNewTuition').classList.remove('hidden');$('schedulePurposeHint').textContent='先選用途。新學生填姓名、手機後選科目、老師與方案；舊學生可搜尋後沿用原課程或選新方案。';
    $('scheduleForm').reset();delete $('scheduleForm').dataset.operationId;$('eventId').value='';$('eventSeriesId').value='';$('eventDate').disabled=false;$('eventStart').disabled=false;$('eventRoom').disabled=false;$('eventDate').value=state.currentDate;$('eventDuration').value='60';$('eventType').value='fixed';$('eventFrequency').value='weekly';$('eventRepeatUntil').value='';$('eventSingleKind').value='normal';$('eventSpecialLessonPrice').value='';$('eventSpecialTeacherPay').value='';$('repeatUntilField').classList.remove('hidden');$('eventNote').value='';$('eventRenterType').value='walkin';$('rentalStudentPicker').classList.add('hidden');$('rentalStudentSearch').value='';$('rentalStudentId').value='';$('rentalStudentSelected').textContent='';$('rentalStudentMatches').classList.add('hidden');$('eventClient').value='';$('eventClientPhone').value='';$('eventRentalFee').value='';$('eventRentalPaymentStatus').value='unpaid';$('eventTrialName').value='';$('eventTrialPhone').value='';$('eventTrialFee').value='';$('eventStudentSearch').value='';$('eventStudentSelected').innerHTML='';$('eventStudentSelected').classList.add('hidden');$('eventStudentMatches').classList.add('hidden');
  }
  function openSchedule(options){
    scheduleSelectionVersion++;scheduleDataLoading=false;$('scheduleSubmitBtn').disabled=false;scheduleStudentLoads.clear();
    if(!scheduleWritable())return;options=options||{};refreshFormOptions();clearScheduleForm();var source=options.event||options.copy||options.draft||null;$('scheduleModalTitle').textContent=options.event?'編輯這一次課程（日期、時間與教室請使用調課）':options.copy?'增加課程':'快速排課';if(source){$('eventId').value=options.event?source.id:'';$('eventSeriesId').value=source.seriesId||'';$('eventDate').value=source.date;$('eventStart').value=source.start;$('eventDuration').value=String(source.duration||60);$('eventRoom').value=source.roomId;$('eventType').value=source.type;$('eventFrequency').value=options.event?'once':source.frequency||'once';$('eventRepeatUntil').value=source.endDate||'';$('eventSingleKind').value=source.specialLesson?'special':'normal';$('eventSpecialLessonPrice').value=Object.prototype.hasOwnProperty.call(source,'specialLessonPrice')?source.specialLessonPrice:'';$('eventSpecialTeacherPay').value=Object.prototype.hasOwnProperty.call(source,'specialTeacherPay')?source.specialTeacherPay:'';$('eventStudent').value=options.newStudentId||(source.studentIds||[])[0]||'';$('eventSubject').value=source.subjectId||'';$('eventRenterType').value=source.renterStudentId?'student':'walkin';$('rentalStudentId').value=source.renterStudentId||'';$('rentalStudentSearch').value=source.clientName||'';$('rentalStudentSelected').textContent=source.renterStudentId?'已選擇：'+source.clientName:'';$('rentalStudentPicker').classList.toggle('hidden',!source.renterStudentId);$('eventClient').value=source.clientName||'';$('eventClientPhone').value=source.clientPhone||'';$('eventRentalFee').value=Object.prototype.hasOwnProperty.call(source,'rentalFee')?source.rentalFee:'';$('eventRentalPaymentStatus').value=rentalPaymentValue(source.rentalPaymentStatus);$('eventTrialName').value=source.trialName||'';$('eventTrialPhone').value=source.trialPhone||'';$('eventTrialFee').value=source.trialFee||'';$('eventNote').value=source.note||'';if(options.event){$('eventDate').disabled=true;$('eventStart').disabled=true;$('eventRoom').disabled=true;}}
    ...
    updateEventStartOptionsForDate($('eventDate').value,$('eventStart').value);var selectedTeacher=source&&source.teacherId,selectedPeriod=source&&source.tuitionPeriodId;updateTeacherOptions(selectedTeacher);updateTuitionOptions(selectedPeriod);updateRoomOptions(source&&source.roomId);$('eventFrequency').disabled=!!options.event;setScheduleKind($('eventType').value);if($('eventStudent').value)selectScheduleStudent($('eventStudent').value,false);if(source&&source.halfHourAcknowledged)$('scheduleForm').dataset.halfHourAcknowledged='true';if(source&&source.type!=='rental'&&source.subjectId){$('scheduleForm').dataset.sourceSubjectId=source.subjectId;$('eventSubject').value=source.subjectId;$('eventSubject').disabled=true;updateRoomOptions(source.roomId);}updateSpecialLessonFields(!source);updateScheduleConflict();openModal('scheduleModal');
  }
  function openPermanentSchedule(source){
    openSchedule({draft:source});permanentScheduleSource=clone(source);
    $('scheduleModalTitle').textContent='固定調課：'+eventDisplayName(source);$('schedulePurposeHint').textContent='日期是新固定課的第一堂，也是生效日。從這天起改用新時段，原本之後的固定時段會釋出；之前的紀錄保留。';
    $('eventFrequency').value=source.frequency==='biweekly'?'biweekly':'weekly';$('eventFrequency').querySelector('[value=once]').disabled=true;
    $('scheduleTypeTabs').classList.add('hidden');$('studentField').classList.add('hidden');$('repeatUntilField').classList.add('hidden');$('tuitionPeriodField').classList.add('hidden');
    ['eventSubject','eventTeacher','eventDuration','eventStudentSearch'].forEach(function(id){$(id).disabled=true;});
    updateScheduleEnrollment();updateScheduleConflict();
  }
  function updateRentalStudentPicker(){var student=$('eventRenterType').value==='student';$('rentalStudentPicker').classList.toggle('hidden',!student);if(!student){$('rentalStudentId').value='';$('rentalStudentSelected').textContent='';}updateScheduleConflict();}
  function searchRentalStudents(){var q=clean($('rentalStudentSearch').value).toLowerCase(),box=$('rentalStudentMatches');$('rentalStudentId').value='';$('rentalStudentSelected').textContent='';box.classList.toggle('hidden',!q);box.innerHTML=q?state.students.filter(function(s){return (s.name+' '+(s.phone||'')).toLowerCase().indexOf(q)>=0;}).slice(0,15).map(function(s){return '<button type="button" data-rental-student="'+esc(s.id)+'"><b>'+esc(s.name)+'</b><span>'+esc(s.phone||'未填手機')+'</span></button>';}).join('')||'<p>找不到學生，請確認姓名或手機。</p>':'';}
  function selectRentalStudent(id){var student=studentById(id);if(!student.id)return;$('rentalStudentId').value=student.id;$('rentalStudentSearch').value=student.name;$('rentalStudentSelected').textContent='已選擇：'+student.name;$('eventClient').value=student.name;$('eventClientPhone').value=student.phone||'';$('rentalStudentMatches').classList.add('hidden');updateScheduleConflict();}
  function formEvent(){var type=$('eventType').value,rental=type==='rental',trial=type==='trial',studentCourse=type==='fixed'||type==='single',special=type==='single'&&$('eventSingleKind').value==='special';return {id:$('eventId').value,seriesId:$('eventSeriesId').value,date:$('eventDate').value,roomId:$('eventRoom').value,start:$('eventStart').value,duration:numberOf($('eventDuration').value),halfHourAcknowledged:$('scheduleForm').dataset.halfHourAcknowledged==='true',type:type,frequency:type==='fixed'?$('eventFrequency').value:'once',studentIds:studentCourse?(scheduleIsNewStudent()?['new-student-draft']:[$('eventStudent').value].filter(Boolean)):[],teacherId:rental?'':$('eventTeacher').value,subjectId:rental?'':$('eventSubject').value,tuitionPeriodId:studentCourse&&!special?$('eventTuitionPeriod').value:'',specialLesson:special,specialLessonPrice:special?numberOf($('eventSpecialLessonPrice').value):0,specialTeacherPay:special?numberOf($('eventSpecialTeacherPay').value):0,renterStudentId:rental&&$('eventRenterType').value==='student'?$('rentalStudentId').value:'',clientName:rental?$('eventClient').value.trim():'',clientPhone:rental?$('eventClientPhone').value.trim():'',rentalFee:rental?numberOf($('eventRentalFee').value):0,rentalPaymentStatus:rental?$('eventRentalPaymentStatus').value:'',trialName:trial?$('eventTrialName').value.trim():'',trialPhone:trial?$('eventTrialPhone').value.trim():'',trialFee:trial?numberOf($('eventTrialFee').value):0,note:$('eventNote').value.trim(),status:'scheduled'};}
  function submitSchedule(event){
    event.preventDefault();if(!scheduleWritable())return;if(scheduleDataLoading){toast('正在讀取所選課程，請稍候','','error');return;}var lockedSubject=$('scheduleForm').dataset.sourceSubjectId;if(lockedSubject)$('eventSubject').value=lockedSubject;var row=formEvent();if(!permanentScheduleSource&&row.duration===30&&['fixed','single'].indexOf(row.type)>=0&&!row.halfHourAcknowledged){if(!window.confirm('30分鐘增課提醒\n\n新增本次30分鐘課程後，請於接下來四堂課內，再安排一次30分鐘，合計為一堂。兩次分別新增、分別簽到，系統不會自動安排。\n\n確認表示已了解並會安排完成。'))return;$('scheduleForm').dataset.halfHourAcknowledged='true';row.halfHourAcknowledged=true;}var kind=roomKindOf(roomById(row.roomId)),warning=kind==='video'?'視訊教室不是實體教室，老師需自行安排實際上課地點。確定要使用嗎？':kind==='holding'?'不定時教室只供暫時放置，之後仍需重新安排正式教室。確定要使用嗎？':'';if(warning&&!window.confirm(warning))return;var reasons=updateScheduleConflict();if(reasons.length){toast('無法儲存','請先排除教室、老師、學生或時段規則衝突。','error');return;}if((row.type==='fixed'||row.type==='single')&&(!row.studentIds.length||!row.subjectId||!row.teacherId)){toast('資料未完成','請搜尋並確認學生、科目與可授課老師。','error');return;}if(row.type==='trial'&&(!row.trialName||!row.subjectId||!row.teacherId)){toast('資料未完成','請填寫體驗者姓名、科目與老師。','error');return;}if(row.type==='rental'&&$('eventRenterType').value==='student'&&!row.renterStudentId){toast('請先搜尋並選擇學生','','error');return;}if(row.type==='rental'&&!row.clientName){toast('資料未完成','請填寫租用客戶或團體名稱。','error');return;}
    var enrollment=null;if(!permanentScheduleSource&&!row.id&&['fixed','single'].indexOf(row.type)>=0&&!row.specialLesson){if(scheduleIsNewStudent()&&!clean($('scheduleStudentName').value)){toast('請填寫學生姓名','','error');return;}if(scheduleNeedsPlan()){var plan=feeById($('schedulePlan').value);if(!plan.id||plan.subjectId!==row.subjectId){toast('請選擇收費方案','選好方案後即可建立課程。','error');return;}enrollment={planId:plan.id};if(scheduleIsNewStudent())enrollment.newStudent={name:clean($('scheduleStudentName').value),phone:clean($('scheduleStudentPhone').value)};row.tuitionPeriodId='';}else if(!row.tuitionPeriodId){toast('請選擇原課程或新方案','','error');return;}}
    var original=permanentScheduleSource|| (row.id?findEvent(row.id):null);if(permanentScheduleSource)row.studentIds=clone(permanentScheduleSource.studentIds);
    if(original&&!permanentScheduleSource){row.date=original.date;row.start=original.start;row.roomId=original.roomId;}
    var operationId=$('scheduleForm').dataset.operationId||($('scheduleForm').dataset.operationId=uid('schedule'));
    runUiOperation(row.type==='rental'?'正在儲存租用資料…':'正在保存雲端排課…',$('scheduleSubmitBtn'),async function(){
      await persistScheduleChange(row,{operationId:operationId,source:original,enrollment:enrollment,mode:permanentScheduleSource?'permanent_move':'save',repeatUntil:$('eventRepeatUntil').value});
      closeModal('scheduleModal');if(row.type==='rental')toast('租用資料已儲存',row.rentalPaymentStatus==='paid'?'已收款，租用簽到完成。':'未收款，維持未簽到。');else toast('排課已保存至雲端','老師手機與管理者會讀取同一份課表。');
    });
  }
  ...
  async function persistScheduleChange(row,options){
    ...
    if(options.source&&options.source.type==='rental'&&options.mode!=='delete'&&row.date===options.source.date&&row.start===options.source.start&&row.roomId===options.source.roomId&&row.duration===options.source.duration){
      var saved=await window.YouziCoursePreviewData.saveLessonSettings({manualSyncPin:storedMigrationPin(),kind:'rentalDetails',date:options.source.date,sourceEventId:options.source.portalBookingId||options.source.sourceId||options.source.id,sourceCourseId:options.source.sourceCourseId||'',bookingId:options.source.portalBookingId||'',event:row});
      if(!saved||saved.ok!==true)throw new Error('尚未確認儲存，請重試。');
      Object.assign(materializeEvent(findEvent(options.source.id)||options.source),saved.fields||{});renderCalendar();scheduleWorkspaceSave();return;
    }
    else var result=await window.YouziCoursePreviewData.saveSchedule({manualSyncPin:storedMigrationPin(),operationId:options.operationId||uid('schedule'),mode:options.mode||'save',event:row,enrollment:options.enrollment||null,sourceEventId:options.source&&(options.source.portalBookingId||options.source.sourceId||options.source.id)||'',sourceCourseId:options.source&&options.source.sourceCourseId||'',sourceDate:options.source&&options.source.date||'',repeatUntil:options.repeatUntil||''});
    if(!options.source&&(!options.mode||options.mode==='save')&&result&&result.course){
      var students=state.students.slice();if(result.student){var student=Object.assign({},result.student,{active:result.student.studentActive!==false});upsert(students,student);upsert(state.students,student);}
  ...
  function ensureAttendancePeriod(event,studentId,record){
    ...
    if(record&&record.deducted===true)return {period:current,created:false};
    var original=current,created=false;
    while(numberOf(current.usedCount)>=Math.max(0,numberOf(current.lessonCount)-numberOf(current.voidedLessonCount))){
    ...
    event.tuitionPeriodId=current.id;
    if(current.id!==original.id){
      state.recurringRules.forEach(function(rule){if(rule.id===event.ruleId||rule.id===event.seriesId)rule.tuitionPeriodId=current.id;});
      state.events.forEach(function(item){if(item.id!==event.id&&item.tuitionPeriodId===original.id&&item.date>=event.date&&normalizedStatus(item.status)==='scheduled'&&(item.ruleId===event.ruleId||item.seriesId===event.seriesId))item.tuitionPeriodId=current.id;});
    }
  ...
  function syncSandboxPayroll(event,status){
    ...
    state.teacherPayroll.push({id:'sandbox_pay_'+event.id,eventId:event.id,source:'sandbox',teacherId:event.teacherId,studentId:(event.studentIds||[])[0]||'',date:event.date,occurredAt:event.date+'T'+event.start+':00',studentName:eventDisplayName(event),subject:subjectById(event.subjectId).name||'',lessonPrice:Math.round(lessonPrice),collectedAmount:0,baseTeacherAmount:baseTeacherAmount,teacherPayAdjustment:payAdjustment,teacherPayAdjustmentReason:clean(event.teacherPayAdjustmentReason),teacherAmount:teacherAmount,schoolShare:Math.max(0,Math.round(lessonPrice)-teacherAmount),allotRate:event.specialLesson&&lessonPrice?baseTeacherAmount/lessonPrice:plan.splitType==='ratio'?teacherShareRatio(plan.splitValue):0,hourlyFee:event.specialLesson?baseTeacherAmount:plan.splitType==='fixed'?numberOf(plan.splitValue):0,specialLesson:event.specialLesson===true});
  }

  function eventDetails(event){
    if(afterStudentDetailsReady(event.studentIds||[],function(){eventDetails(findEvent(event.id)||event);},'eventModal',eventDisplayName(event)))return;
    var room=roomById(event.roomId),subject=subjectById(event.subjectId),teacher=teacherById(event.teacherId),student=studentById((event.studentIds||[])[0]),studentLabel=eventStudentNames(event).join('－'),period=periodById(event.tuitionPeriodId),reasons=eventConflictReasons(event,[event.id]);$('eventTypeBadge').textContent=event.specialLesson?'贈送／特殊加課':typeName(event.type);$('eventModalTitle').textContent=eventDisplayName(event);$('eventModalSubtitle').textContent=zhDate(event.date)+' '+weekdayName(event.date)+'・'+event.start+'–'+minToTime(timeToMin(event.start)+event.duration)+'・'+(room.name||'未設定教室');
    var html='<div class="event-compact-summary">'+(event.type==='rental'?'<div><small>租用人</small><b>'+esc(event.clientName||'未提供')+'</b></div><div><small>預約身分</small><b>'+esc(rentalRoleName(event.rentalRole))+'</b></div><div><small>聯絡電話</small><b>'+esc(event.clientPhone||'未提供')+'</b></div><div><small>租用用途</small><b>'+esc(event.useName||typeName(event.type))+'</b></div><div><small>教室</small><b>'+esc(room.name||'未設定')+'</b></div><div><small>租用金額</small><b>'+money(event.rentalFee)+'</b></div><div><small>收款</small><b>'+esc(rentalPaymentName(event.rentalPaymentStatus))+'</b></div><div><small>預約狀態</small><b>'+esc(rentalStatusName(event))+'</b></div>':event.type==='trial'?'<div><small>體驗者</small><b>'+esc(event.trialName||'未填')+'</b></div><div><small>聯絡電話</small><b>'+esc(event.trialPhone||'未填')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>體驗費</small><b>'+money(event.trialFee)+'</b></div><div><small>狀態</small><b>'+esc(statusName(event.status))+'</b></div>':event.specialLesson?'<div><small>學生</small><b>'+esc(studentLabel||'未指定')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>本堂原價</small><b>'+money(event.specialLessonPrice)+'</b></div><div><small>學生收費</small><b>$0・不扣堂</b></div><div><small>老師實領</small><b>'+money(event.specialTeacherPay)+'</b></div>':'<div><small>學生</small><b>'+esc(studentLabel||'未指定')+'</b></div><div><small>科目</small><b>'+esc(subject.name||'未設定')+'</b></div><div><small>固定老師</small><b>'+esc(teacher.name||'未設定')+'</b></div><div><small>本次狀態</small><b>'+esc(statusName(event.status))+'</b></div><div><small>目前期別</small><b>'+(period.id?'第 '+esc(period.periodNo)+' 期':'未指定')+'</b></div><div><small>繳費</small><b>'+(period.id?(periodBalance(period)?'<span class="tuition-due">未繳 '+money(periodBalance(period))+'</span>':'<span class="tuition-paid">已繳清</span>'):'—')+'</b></div>')+'</div>';
    if(event.type!=='rental')html='<div class="status-actions event-command-row">'+(!isReadOnly()&&student.id?'<button type="button" data-event-new-period="'+esc(student.id)+'" data-event-subject="'+esc(event.subjectId)+'">增加期數</button>':'')+(!isReadOnly()?'<button type="button" data-event-action="copy">增加課程</button><button type="button" data-event-action="cut">單次調課</button>'+(event.type==='fixed'?'<button type="button" data-event-action="permanent">固定調課</button>':'')+'':'')+(student.id&&!isReadOnly()?'<button type="button" data-event-action="irregular">不定時</button><button type="button" data-event-action="stopped">停課</button>':'')+'<button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>✓ 簽到</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>取消簽到</button><button data-attendance="leave" '+(isReadOnly()?'disabled':'')+'>請假</button><button data-attendance="absent" '+(isReadOnly()?'disabled':'')+'>曠課</button></div>'+html;
    ...
    if(event.type!=='rental'&&isReadOnly())html+='<p class="formal-action-hint">請先載入新系統資料，再進行課程操作。</p>';
    else if(event.type==='rental')html+='<div class="status-actions rental-actions"><button data-attendance="attended" '+(isReadOnly()?'disabled':'')+'>租用簽到完成</button><button data-attendance="scheduled" '+(isReadOnly()?'disabled':'')+'>取消簽到</button>'+(!isReadOnly()?'<button data-event-action="edit">修改租用金額</button>':'')+(event.source==='course-portal'&&event.portalBookingId&&event.canAdminCancel!==false&&event.active!==false?'<button class="danger" data-portal-rental-cancel="'+esc(event.portalBookingId)+'">強制取消租用</button>':!isReadOnly()&&event.active!==false?'<button class="danger" data-event-action="cancel-rental">強制取消租用</button>':'')+'</div>';
    if(student.id)html+='<section class="event-tuition-section"><div class="student-record-heading"><div><h3>學生學費紀錄</h3><p>不必再進第二層；每一期與四堂簽到直接顯示在這裡。</p></div></div>'+tuitionTableHtml(student.id,{eventId:event.id,currentPeriodId:event.tuitionPeriodId,fallbackTeacherId:event.teacherId,subjectId:event.subjectId})+'</section>';
    $('eventModalBody').innerHTML=html;
    $('eventModalFoot').innerHTML=(isSandbox()?'<div><b>本堂 '+esc(event.date)+' '+esc(event.start)+'</b> <button type="button" class="btn small outline" data-event-action="edit">編輯本堂</button> <button type="button" class="btn small danger" data-event-action="delete">刪除本堂</button></div>':'')+'<div class="formal-view-note">'+(isReadOnly()?'資料尚未載入。':'點「第幾期／課程方案」會直接編輯本期期別。')+'</div>';$('eventModal').dataset.eventId=event.id;openModal('eventModal');if(attendanceUpdater&&attendanceUpdater.isPending(attendanceKey(event)))attendancePendingUI({event:event},true);
  }
  ...
  function studentCourseStatusLabel(student){
    var period=latestPeriod(student.id),day=todayKey();
    if(student.active===false||(state.stoppedCourseReceivables||[]).some(function(row){return row.status==='active'&&row.studentId===student.id&&row.teacherId===period.teacherId&&(!row.subjectId||row.subjectId===period.subjectId)&&row.effectiveDate<=day;}))return '已停課';
    if(window.YouziCoursePreviewData.activeIrregularCourses(state.irregularCourses,day,period.teacherId).some(function(row){return row.subjectId===period.subjectId&&(row.studentIds||[]).indexOf(student.id)>=0;}))return '不定時';
    return '上課中';
  }
  function setStudentCourseMode(event,mode){
    var ids=event.studentIds||[],studentId=ids[0];if(!studentId||!event.teacherId||!event.subjectId)return;
    if(ids.length>1){var choice=window.prompt('請選擇要變更的學生：\n'+ids.map(function(id,index){return (index+1)+'．'+studentById(id).name;}).join('\n'),'1');if(choice===null)return;var index=Number(choice)-1;if(!Number.isInteger(index)||index<0||index>=ids.length){toast('請選擇有效的學生編號','','error');return;}studentId=ids[index];}
    var label=mode==='irregular'?'不定時':'停課',name=studentById(studentId).name;
    if(!window.confirm(name+'・'+subjectById(event.subjectId).name+'・'+teacherById(event.teacherId).name+'\n確定從今天起設為「'+label+'」？'+(mode==='irregular'?'未來固定時段會釋出，已約定的單堂課保留。':'未來這位老師的此科目課程會停止。')+'既有學費、簽到與薪資紀錄保留，老師端會同步。'))return;
    runUiOperation('正在設為'+label+'…',null,async function(){
      await window.YouziCoursePreviewData.saveLessonSettings({kind:'studentMode',mode:mode,studentId:studentId,teacherId:event.teacherId,subjectId:event.subjectId,date:event.date,sourceEventId:event.sourceId||event.id,manualSyncPin:storedMigrationPin()});
      closeModal('eventModal');
      await refreshSavedWorkspace({followup:true,calendarScopes:[{startDate:state.dataMeta&&state.dataMeta.rangeStart||shiftDate(state.currentDate||todayKey(),-240),endDate:state.dataMeta&&state.dataMeta.rangeEnd||shiftDate(state.currentDate||todayKey(),420),studentIds:[studentId]}]});
      renderCalendar();renderStudents();toast('已設為'+label,'老師端已同步。');
    });
  }
  function persistLessonSettings(row,fields){
    runUiOperation('正在保存雲端課程設定…',null,async function(){
      ...
      if(!saved||saved.ok!==true)throw new Error('尚未確認儲存，請重試。');
      if(fields.kind==='rentalStatus'){var current=materializeEvent(findEvent(row.id)||row);Object.assign(current,saved.fields||{status:fields.status,rentalPaymentStatus:fields.status==='attended'?'paid':'unpaid'});closeModal('eventModal');renderCalendar();scheduleWorkspaceSave();toast(current.status==='attended'?'租用簽到完成':'已取消簽到','');return;}
  ...
  function submitTeacherPayOverride(event){
    event.preventDefault();if(!writable('儲存本堂特殊老師費用'))return;
    var row=findEvent($('teacherPayOverrideEventId').value),type=$('teacherPayOverrideType').value,amount=numberOf($('teacherPayOverrideAmount').value),reason=$('teacherPayOverrideReason').value.trim(),signed=type==='increase'?amount:type==='decrease'?-amount:0;
    if(type!=='none'&&(!amount||!reason)){toast('資料未完成','設定增加或減少時，必須填寫金額與原因。','error');return;}
    persistLessonSettings(row,{kind:'teacherPay',amount:signed,reason:signed?reason:''});
  }

  function eventAction(action){var event=findEvent($('eventModal').dataset.eventId);if(!event)return;if(!writable('修改課程'))return;if(action==='irregular'||action==='stopped'){setStudentCourseMode(event,action);return;}if(action==='cancel-rental'){if(event.status==='attended'){toast('請先取消簽到','再取消這筆租用。','error');return;}if(window.confirm('確定強制取消這筆租用並釋放時段嗎？'))runUiOperation('正在取消租用…',null,async function(){await persistScheduleChange(event,{source:event,mode:'delete'});closeModal('eventModal');toast('租用已取消','教室時段已釋放。');});return;}if(action==='permanent'){closeModal('eventModal');openPermanentSchedule(event);return;}if(action==='edit'){closeModal('eventModal');openSchedule({event:event});}if(action==='copy'||action==='cut'){if(action==='cut'&&state.attendance.some(function(row){return row.eventId===event.id&&normalizedStatus(row.status)==='attended';})){toast('目前不能調課','這堂已經簽到，請先取消簽到再調課。','error');return;}state.clipboard={mode:action,eventId:event.id,event:clone(event)};closeModal('eventModal');renderCalendar();toast(action==='cut'?'已選擇單次調課':'已選擇增加課程','請點新的教室與時間格；取消前不會改變原課程。');}if(action==='delete'){event=materializeEvent(event);if(state.attendance.some(function(row){return row.eventId===event.id&&normalizedStatus(row.status)==='attended';})){toast('無法刪除這一次課程','已有簽到紀錄，請先取消簽到再刪除。','error');return;}if(window.confirm('確定只刪除這一次課程嗎？固定課規則與其他日期不受影響。')){runUiOperation('正在刪除雲端課程…',null,async function(){await persistScheduleChange(event,{source:event,mode:'delete'});closeModal('eventModal');toast('已刪除這一次課程','固定課的其他日期仍會照常保留。');});}}}
  function pasteToSlot(roomId,start){
    var clip=state.clipboard,source=clip&&(findEvent(clip.eventId)||clone(clip.event));if(!clip||!source)return false;if(clip.mode==='copy'&&source.type!=='rental'){state.clipboard=null;openSchedule({copy:Object.assign({},source,{date:state.currentDate,start:start,roomId:roomId,type:'single',frequency:'once',duration:60})});return true;}var target=Object.assign({},source,{id:uid('event'),dynamic:false,recurrenceKey:'',seriesId:source.seriesId||'',date:state.currentDate,roomId:roomId,start:start,status:'scheduled',type:source.type==='rental'?'rental':'single',frequency:'once',movedFrom:source.date+' '+source.start}),reasons=eventConflictReasons(target,clip.mode==='cut'?[source.id]:[]);if(reasons.length){toast('無法放到這個格子',reasons.join('；'),'error');return true;}
    clip.operationId=clip.operationId||uid('schedule');
    runUiOperation(clip.mode==='cut'?'正在保存雲端調課…':'正在保存雲端加課…',null,async function(){
      await persistScheduleChange(target,{operationId:clip.operationId,source:clip.mode==='cut'?source:null});
      state.clipboard=null;renderCalendar();toast('課表已保存至雲端','尚未簽到的課程不會扣堂或計薪。');
    });return true;
  }

  function latestPeriod(studentId,subjectId){return state.tuitionPeriods.filter(function(row){return row.studentId===studentId&&(!subjectId||row.subjectId===subjectId);}).sort(function(a,b){return clean(b.startDate).localeCompare(clean(a.startDate))||numberOf(b.periodNo)-numberOf(a.periodNo);})[0]||{};}
  ...
  var recentStudentsData=null,recentStudentsPromise=null,studentHistoryMode=false;
  function renderRecentStudents(){
    if(!recentStudentsData){
      $('studentMetrics').innerHTML='';$('studentRows').innerHTML='<tr><td colspan="7">正在讀取學生資料…</td></tr>';
      if(!recentStudentsPromise){recentStudentsPromise=window.YouziCoursePreviewData.loadRecentStudents().then(function(loaded){recentStudentsData=loaded;if(currentView==='students'&&!studentHistoryMode)renderRecentStudents();}).catch(function(error){$('studentRows').innerHTML='<tr><td colspan="7">學生資料讀取失敗，請重新開啟此頁重試。</td></tr>';toast('學生資料讀取失敗',clean(error.message),'error');}).finally(function(){recentStudentsPromise=null;});}
      return;
    }
    var data=recentStudentsData,search=clean($('studentSearch').value).toLowerCase(),filter=$('studentPaymentFilter').value,subjects=new Map(data.subjects.map(function(row){return [row.id,row];})),teachers=new Map(data.teachers.map(function(row){return [row.id,row];})),periodsByStudent=new Map();
    data.tuitionPeriods.forEach(function(period){var current=!calendarPartial()?state.tuitionPeriods.find(function(row){return row.id===period.id;}):null;period=current||period;var rows=periodsByStudent.get(period.studentId)||[];rows.push(period);periodsByStudent.set(period.studentId,rows);});
    var items=data.students.map(function(student){var periods=(periodsByStudent.get(student.id)||[]).sort(function(a,b){return numberOf(b.periodNo)-numberOf(a.periodNo)||clean(b.startDate).localeCompare(clean(a.startDate));}),period=periods[0]||{},subject=subjects.get(period.subjectId)||{},teacher=teachers.get(period.teacherId)||{};return {student:student,period:period,subject:subject,teacher:teacher,due:periods.some(function(row){return periodBalance(row)>0;}),low:period.id&&periodRemaining(period)<=1,hay:[student.name,student.phone].concat(periods.map(function(row){return (subjects.get(row.subjectId)||{}).name;})).join(' ').toLowerCase()};});
    $('studentMetrics').innerHTML=metric('學生',items.length,'近期上課學生')+metric('尚有未繳',items.filter(function(row){return row.due;}).length,'近期學生的未繳期別')+metric('剩 1 堂以下',items.filter(function(row){return row.low;}).length,'近期學生的目前期別');
    items=items.filter(function(item){return (!search||item.hay.indexOf(search)>=0)&&(filter!=='due'||item.due)&&(filter!=='low'||item.low)&&(filter!=='active'||item.student.active!==false);}).sort(function(a,b){return bySort(a.student,b.student);});
    $('studentRows').innerHTML=items.map(function(item){var student=item.student,period=item.period,event=nextEvent(student.id);return '<tr><td><b>'+esc(student.name)+'</b><small>'+(student.active===false?'已停課':'上課中')+'</small></td><td>'+esc(student.phone||'未填')+'</td><td>'+esc(item.subject.name||'未記錄科目')+'<small>'+esc(item.teacher.name||'未指定老師')+'</small></td><td>'+(period.id?period.usedCount+' / '+period.lessonCount+'<small>剩 '+periodRemaining(period)+' 堂</small>':'—')+'</td><td>'+(period.id?money(periodPaid(period))+' / '+money(periodNetExpectedAmount(period))+'<small>'+(periodBalance(period)?'尚欠 '+money(periodBalance(period)):'已繳清')+'</small>':'—')+'</td><td>'+(event.id?esc(event.date+' '+event.start):calendarPartial()?'點開紀錄查看':'尚未排課')+'</td><td><button class="btn small secondary" data-student-id="'+esc(student.id)+'">查看學費紀錄</button></td></tr>';}).join('')||'<tr><td colspan="7">目前沒有符合的學生。</td></tr>';
  }
  function renderStudents(){
    if(!studentHistoryMode&&window.YouziCoursePreviewData&&window.YouziCoursePreviewData.loadRecentStudents){renderRecentStudents();return;}
    ...
    $('studentRows').innerHTML=rows.map(function(item){var student=item.student,period=item.latest,event=item.event,subject=item.subject,teacher=item.teacher;return '<tr><td><b>'+esc(student.name)+'</b><small>'+(studentCourseStatusLabel(student))+'</small></td><td>'+esc(student.phone||'未填')+'<small>LINE：'+(student.line===true?'已綁定':student.line===false?'未綁定':'未確認')+'</small></td><td>'+esc(subject.name||'尚無學費期別')+'<small>'+esc(teacher.name||'未指定老師')+'</small></td><td>'+(period.id?'<b>'+period.usedCount+' / '+period.lessonCount+'</b><small>剩 '+periodRemaining(period)+' 堂</small>':'—')+'</td><td>'+(period.id?'<b>'+money(periodPaid(period))+' / '+money(periodNetExpectedAmount(period))+'</b><small>'+(periodBalance(period)?'尚欠 '+money(periodBalance(period)):'已繳清')+'</small>':'—')+'</td><td>'+(event.id?esc(event.date+' '+event.start):'尚未排課')+'</td><td><button class="btn small secondary" data-student-id="'+esc(student.id)+'">查看學費紀錄</button></td></tr>';}).join('')||'<tr><td colspan="7">沒有符合條件的學生。</td></tr>';
  }
  ...
  function openStudent(id,tab,subjectId){if(afterStudentDetailsReady([id],function(){openStudent(id,tab,subjectId);},'studentModal',studentById(id).name))return;currentStudentId=id;currentStudentSubject=clean(subjectId);studentTab=tab||'profile';var student=studentById(id);if(!student.id)return;$('studentModalTitle').textContent=student.name;$('studentModalSubtitle').textContent=(student.phone||'未填手機')+'・'+(studentCourseStatusLabel(student));renderStudentModal();openModal('studentModal');}
  function periodLessonSlots(period){
    if(window.YouziLessonUnits){
      ...
      if(records.some(function(row){return ledger.units(row)!==1;})){
        var grid=ledger.slots(period,records);return '<div class="lesson-slots">'+grid.map(function(parts,index){var used=parts.reduce(function(sum,row){return sum+row.slotUnits;},0);return '<div class="lesson-slot'+(used?' completed':' pending')+'"><small>第 '+(index+1)+' 格・60分鐘</small>'+parts.map(function(row){return '<span class="half-hour-entry">'+esc(row.date)+' '+esc(row.startTime||'')+'<br>'+row.slotUnits*60+'分鐘・'+(normalizedStatus(row.status)==='absent'?'曠課扣課':'已簽到')+'</span>';}).join('')+(used<1?'<em>剩餘'+(1-used)*60+'分鐘</em>':'')+'</div>';}).join('')+'</div>';
      }
    }
  ...
  var tuitionReceiptViewer=null,tuitionReceiptBusy=false;
  function openTuitionReceiptPlaceholder(){
    if(tuitionReceiptViewer)tuitionReceiptViewer.remove();
    var panel=document.createElement('div');panel.className='modal-backdrop open';panel.style.zIndex='10000';panel.setAttribute('role','dialog');panel.setAttribute('aria-label','學費收據');
    panel.innerHTML='<div style="background:white;padding:20px;border-radius:12px;max-height:90vh;overflow:auto;width:min(680px,95vw)"><button type="button" class="btn outline" data-receipt-close>關閉收據</button><div data-receipt-body role="status">讀取收據中…</div></div>';
    panel.querySelector('[data-receipt-close]').onclick=function(){panel.remove();if(tuitionReceiptViewer===panel)tuitionReceiptViewer=null;};
    document.body.appendChild(panel);tuitionReceiptViewer=panel;return panel;
  }
  function openTuitionReceipt(imageUrl,viewer){
    imageUrl=clean(imageUrl);viewer=viewer||openTuitionReceiptPlaceholder();if(!viewer.isConnected)return;
    var body=viewer.querySelector('[data-receipt-body]');
    if(!/^https:\/\//i.test(imageUrl)){body.textContent='收據圖片網址不正確，請重新載入後再試。';return;}
    body.innerHTML='<p>學費收據</p><img style="display:block;width:100%;height:auto" alt="學費收據"><button class="btn primary" type="button">列印收據</button>';
    var img=body.querySelector('img');img.onerror=function(){body.textContent='收據圖片暫時無法載入，請關閉後再試。';};img.src=imageUrl;
    body.querySelector('button').onclick=function(){var printWindow=window.open('','_blank');if(!printWindow){toast('無法列印','請允許列印視窗。','error');return;}printWindow.opener=null;printWindow.document.write('<!doctype html><title>學費收據</title><style>@page{size:15cm 10cm;margin:0}body{margin:0}img{width:15cm;height:10cm}</style><img alt="學費收據" src="'+esc(imageUrl)+'">');printWindow.document.close();var printImage=printWindow.document.querySelector('img');var printed=false;function printOnce(){if(!printed){printed=true;printWindow.print();}}printImage.onload=printOnce;if(printImage.complete)printOnce();};
  }
  async function ensureAndOpenTuitionReceipt(button){
    if(tuitionReceiptBusy)return;
    ...
    if(!period.id||!transaction)return;
    var viewer=openTuitionReceiptPlaceholder();tuitionReceiptBusy=true;button.disabled=true;
    try{
  ...
  function correctTuitionTransaction(id,index){
    ...
    var date=window.prompt('第 '+period.periodNo+' 期：修正繳費日期（YYYY-MM-DD）',row.date||todayKey());if(date===null)return;
  ...
  function renderStudentModal(){
    ...
    var subjects=Array.from(new Set(state.tuitionPeriods.filter(function(row){return row.studentId===student.id;}).map(function(row){return row.subjectId;}).filter(Boolean)));
    if(subjects.length===1)currentStudentSubject=subjects[0];
    if(currentStudentSubject&&subjects.indexOf(currentStudentSubject)<0)currentStudentSubject='';
    ...
    html+='<section class="student-section"><div class="student-record-heading"><div><h3>學費紀錄</h3><p>每一期維持同一排；四堂實際簽到日期直接顯示，未繳金額以紅色標示。</p></div></div>'+'<label>收費科目<select data-student-tuition-subject><option value="">請選擇科目</option>'+subjects.map(function(id){return '<option value="'+esc(id)+'" '+(id===currentStudentSubject?'selected':'')+'>'+esc(subjectById(id).name||id)+'</option>';}).join('')+'</select></label>'+(currentStudentSubject?tuitionTableHtml(student.id,{subjectId:currentStudentSubject}):'<p>先選擇科目，再查看期別或收費。</p>')+'</section>';
  ...
  function openTuition(studentId,id,subjectId){
    if(!writable('新增或編輯學費'))return;refreshFormOptions();$('tuitionForm').reset();delete $('tuitionForm').dataset.operationId;$('tuitionId').value=id||'';$('tuitionPurchasePeriods').value='1';$('tuitionPurchasePeriods').disabled=!!id;$('tuitionStudent').value=studentId||currentStudentId||'';$('tuitionPaymentDate').value=todayKey();$('tuitionCollectNow').checked=false;$('tuitionPaymentMethod').value='現金';var row=periodById(id);
    ...
    else{var previous=latestPeriod($('tuitionStudent').value,subjectId),studentPeriods=state.tuitionPeriods.filter(function(p){return p.studentId===$('tuitionStudent').value;});$('tuitionModalTitle').textContent=previous.id?'增加下一期':'新增第 1 期';$('tuitionModalSubtitle').textContent=previous.id?'沿用上一期方案與固定老師；可在建立時同時登記本次收費。':'建立第一期，並可同時登記本次收費。';if(previous.id){$('tuitionSubject').value=previous.subjectId;row=clone(previous);var oldPlan=feeById(previous.planId);if(!oldPlan.id){var snapshot=clone(previous.planSnapshot||{}),matching=state.feePlans.find(function(p){return p.subjectId===previous.subjectId&&numberOf(p.amount)===numberOf(previous.expectedAmount)&&numberOf(p.lessonCount)===numberOf(previous.lessonCount)&&p.splitType===snapshot.splitType&&numberOf(p.splitValue)===numberOf(snapshot.splitValue);});if(!matching){matching=Object.assign({},snapshot,{id:previous.planId||'continued_'+previous.id,subjectId:previous.subjectId,name:snapshot.name||'沿用上一期',amount:previous.expectedAmount,lessonCount:previous.lessonCount,active:true,listed:true});state.feePlans.push(matching);}row.planId=matching.id;}}$('tuitionPeriodNo').value=studentPeriods.reduce(function(max,p){return Math.max(max,numberOf(p.periodNo));},0)+1;$('tuitionStartDate').value=todayKey();$('tuitionDiscount').value=previous.id?numberOf(previous.discount):0;}
    if(row&&row.id&&!feeById(row.planId).id){state.feePlans.push(Object.assign({},row.planSnapshot||{},{id:row.planId,subjectId:row.subjectId,name:(row.planSnapshot||{}).name||'本期原方案',amount:row.expectedAmount,lessonCount:row.lessonCount,active:false}));}updateTuitionCollectionFields();updateTuitionForm(row);if(row&&row.id){$('tuitionLessonCount').value=row.lessonCount;$('tuitionAmount').value=row.expectedAmount;}$('tuitionStudent').disabled=!!id||calendarPartial();updateTuitionPurchaseSummary(true);openModal('tuitionModal');
  }
  function updateTuitionForm(existing){
    var subjectId=$('tuitionSubject').value,teacherSelected=existing&&existing.teacherId||$('tuitionTeacher').value,planSelected=existing&&existing.planId||$('tuitionPlan').value,teachers=state.teachers.filter(function(row){return row.id===teacherSelected||row.active!==false&&(!subjectId||(row.subjectIds||[]).indexOf(subjectId)>=0);}).sort(bySort),plans=state.feePlans.filter(function(row){return (row.id===planSelected||row.active!==false&&row.listed!==false)&&(!subjectId||row.subjectId===subjectId);}).sort(bySort);fillSelect($('tuitionTeacher'),teachers,function(row){return row.name;},function(row){return row.id;},'不指定固定老師');fillSelect($('tuitionPlan'),plans,function(row){return row.name+'・'+money(row.amount)+'／'+row.lessonCount+' 堂';},function(row){return row.id;},'請選擇收費方案');if(teacherSelected&&teachers.some(function(row){return row.id===teacherSelected;}))$('tuitionTeacher').value=teacherSelected;if(planSelected)$('tuitionPlan').value=planSelected;renderTuitionSnapshot();
  }
  ...
  function splitLabel(plan){if(plan.splitType==='ratio')return Math.round(teacherShareRatio(plan.splitValue)*100)+'%';if(plan.splitType==='fixed')return money(plan.splitValue)+'／堂';if(plan.splitType==='none')return '$0／堂';return '未設定';}
  function submitTuition(event){
    ...
    if(!$('tuitionForm').dataset.operationId)$('tuitionForm').dataset.operationId=uid('tuition');var operationId=$('tuitionForm').dataset.operationId;
    runUiOperation(id?'正在儲存本期期別…':'正在建立 '+count+' 個期別…',$('tuitionSubmitBtn'),async function(){var created=[],remaining=paymentTotal;
      for(var index=0;index<count;index++){var editing=id&&index===0,base=editing?existing:{},allocation=0;if(collectNow&&remaining>0){allocation=index===count-1?remaining:Math.min(perNet,remaining);remaining=Math.max(0,remaining-allocation);}var transactions=editing?(base.transactions||[]):[];if(allocation>0)transactions.push({id:operationId+'_pay_'+index,type:'payment',date:$('tuitionPaymentDate').value||todayKey(),amount:allocation,method:$('tuitionPaymentMethod').value,note:'新增 '+count+' 期時同時收費'});var row={id:editing?base.id:operationId+'_period_'+index,studentId:studentId,subjectId:$('tuitionSubject').value,teacherId:$('tuitionTeacher').value,planId:plan.id,periodNo:editing?numberOf(base.periodNo):nextPeriodNo+index,startDate:$('tuitionStartDate').value||todayKey(),expiryDate:$('tuitionExpiryDate').value||'',lessonCount:numberOf($('tuitionLessonCount').value)||plan.lessonCount,expectedAmount:perExpected,discount:perDiscount,usedCount:editing?base.usedCount:0,status:'active',note:$('tuitionNote').value.trim(),transactions:transactions,lessonAdjustments:editing?(base.lessonAdjustments||[]):[],planSnapshot:clone(plan)};created.push(row);}
      if(!window.YouziCoursePreviewData||!window.YouziCoursePreviewData.saveTuitionPeriods)throw new Error('學費保存元件尚未載入，請重新整理。');
      await window.YouziCoursePreviewData.saveTuitionPeriods({operationId:operationId,edit:!!id,periods:created,manualSyncPin:storedMigrationPin()});
      created.forEach(function(row){var prior=periodById(row.id);if(prior.id)Object.assign(prior,row);else state.tuitionPeriods.push(row);});
      if(id){var payrollMonth=teacherListMonthKey();if(teacherPayrollMonthRefreshes[payrollMonth])await teacherPayrollMonthRefreshes[payrollMonth];await refreshTeacherPayrollMonth(payrollMonth);}
      save(studentById(studentId).name+'・'+(id?'編輯第 '+existing.periodNo+' 期':'建立第 '+nextPeriodNo+'～'+(nextPeriodNo+count-1)+' 期')+(collectNow?'・收費 '+money(paymentTotal):''));var returnDraft=scheduleDraftAfterTuition;scheduleDraftAfterTuition=null;closeModal('tuitionModal');if(returnDraft){var first=created[0];returnDraft.subjectId=first.subjectId;returnDraft.teacherId=first.teacherId||returnDraft.teacherId;returnDraft.tuitionPeriodId=first.id;openSchedule({draft:returnDraft});}if($('studentModal').classList.contains('open')&&currentStudentId===studentId){studentTab='tuition';renderStudentModal();}if(currentView==='students')renderStudents();toast(id?'學費期別已更新':'學費期別已建立',id?'本期資料已更新；原有收退款紀錄保持不變。':'已新增 '+count+' 期'+(collectNow?'，並分配收費總額 '+money(paymentTotal):'；目前均顯示未繳費')+'。');
    });
  }
  ...
  function openTransaction(periodId,preferredSlotNo){if(!writable('新增收費或退款'))return;$('transactionForm').reset();delete $('transactionForm').dataset.operationId;$('transactionPeriodId').value=periodId;$('transactionPreferredSlot').value=preferredSlotNo||'';$('transactionDate').value=todayKey();$('transactionType').value=preferredSlotNo?'refund':'payment';renderRefundLessonOptions();openModal('transactionModal');}
  ...
  async function submitTransaction(event){
    event.preventDefault();if(!writable('儲存付款或退款')||operationRunning)return;
    var period=periodById($('transactionPeriodId').value),amount=numberOf($('transactionAmount').value),type=$('transactionType').value;
    if(!period.id||amount<=0){toast('資料不完整','請輸入正確金額。','error');return;}
    if(type==='refund'&&amount>Math.max(0,periodPaid(period))){toast('退款金額超過可退餘額','最多可退 '+money(Math.max(0,periodPaid(period)))+'。','error');return;}
    ...
    try{
      ...
      if(!(period.transactions||[]).some(function(row){return row.id===request.id;})){if(!period.transactions)period.transactions=[];period.transactions.push(result.transaction);}
  ...
  async function refreshTeacherPayrollMonth(monthKey){
    ...
    teacherPayrollMonthRefreshes[monthKey]=task;task.then(function(){if(teacherPayrollMonthRefreshes[monthKey]===task)delete teacherPayrollMonthRefreshes[monthKey];});return task;
  }
  ...
  function payrollPlanText(row){
    if(row.specialLesson)return '贈送／特殊加課';
    var snapshot=row.planSnapshot||{},name=clean(snapshot.name||row.planName),parts=[];
    if(!name){
      var students=(row.payrollCalculation||{}).students||[];
      students.forEach(function(item){var label=clean(((item.inputs||{}).planSnapshot||{}).name);if(label&&parts.indexOf(label)<0)parts.push(label);});
      name=parts.join('、');
    }
    if(!name&&row.periodId){
      var periods=(state.tuitionPeriods||[]).filter(function(period){return (period.id===row.periodId||period.sourcePaymentId===row.periodId)&&(!row.studentId||period.studentId===row.studentId);});
      if(periods.length===1)name=clean((periods[0].planSnapshot||{}).name);
    }
    return name||'未記錄方案';
  }
  function splitText(row){if(numberOf(row.hourlyFee))return '每堂固定 '+money(row.hourlyFee);if(numberOf(row.allotRate))return '比例 '+Math.round(numberOf(row.allotRate)*10000)/100+'%';return ['none','fixed','ratio'].indexOf(row.splitType)>=0?'$0':'未設定';}
  ...
  function renderTeacherPayroll(){
    ...
    var previousDate='',table='<div class="table-wrap"><table class="payroll-detail-table"><thead><tr><th>日期</th><th>學生</th><th>授課／收費方案</th><th>本堂學費</th><th>拆帳方式</th><th>老師實領</th></tr></thead><tbody>'+rows.map(function(row){var dayStart=previousDate&&previousDate!==row.date,studentId=row.studentId||(state.students.find(function(student){return student.name===row.studentName;})||{}).id||'',student=studentId?'<button type="button" class="student-link" data-payroll-student="'+esc(studentId)+'">'+esc(row.studentName||studentById(studentId).name||'未命名學生')+'</button>':esc(row.studentName||'未命名學生'),collected=Object.prototype.hasOwnProperty.call(row,'collectedAmount')?numberOf(row.collectedAmount):numberOf(row.lessonPrice),payAdjustment=numberOf(row.teacherPayAdjustment),payDetail=payAdjustment?'<small class="payroll-special-adjustment">原拆帳 '+money(numberOf(row.baseTeacherAmount))+(payAdjustment>0?' ＋ ':' － ')+money(Math.abs(payAdjustment))+'<br>'+esc(row.teacherPayAdjustmentReason||'本堂特殊調整')+'</small>':'';previousDate=row.date;return '<tr class="'+(dayStart?'payroll-day-start':'')+'"><td>'+esc(row.date)+'</td><td>'+student+'</td><td>'+esc(payrollPlanText(row))+(row.payrollPeriodError?'<small>方案重算待確認：'+esc(row.payrollPeriodError)+'</small>':'')+'</td><td>'+money(row.lessonPrice)+'</td><td>'+esc(splitText(row))+'</td><td><b>'+money(row.teacherAmount)+'</b>'+payDetail+'</td></tr>';}).join('')+'</tbody></table></div>';
  ...
  function renderSubjectRows(){$('subjectRows').innerHTML=state.subjects.slice().sort(bySort).map(function(row){var teachers=state.teachers.filter(function(t){return t.subjectIds.indexOf(row.id)>=0;}).map(function(t){return t.name;}),plans=state.feePlans.filter(function(p){return p.subjectId===row.id;}),pending=subjectNeedsPricing(row),status=row.active===false?'停用':pending?'使用中・未設定收費':'使用中',actions=(pending?'<button class="btn small primary" data-subject-price="'+row.id+'">設定收費</button> <button class="btn small outline" data-subject-map="'+row.id+'">對應既有</button> ':'')+'<button class="btn small outline" data-subject-edit="'+row.id+'">編輯</button>';return '<tr class="'+(pending?'subject-pending-row':'')+'"><td>'+row.sort+'</td><td><b>'+esc(row.name)+'</b>'+(pending?'<small>共用科目已可選；有學生需要時再設定收費即可</small>':'')+'</td><td>'+esc(teachers.join('、')||'尚無')+'</td><td>'+plans.length+' 套</td><td><span class="tag '+(row.active===false?'gray':pending?'yellow':'green')+'">'+status+'</span></td><td>'+actions+'</td></tr>';}).join('');}
  ...
  function openEntity(type,id,defaults){if(afterWorkspaceReady(function(){openEntity(type,id,defaults);}))return;if(!writable('編輯設定'))return;defaults=defaults||{};entityContext={type:type,id:id||''};var row={};if(type==='student')row=studentById(id);if(type==='teacher')row=teacherById(id);if(type==='room')row=roomById(id);if(type==='subject'||type==='subjectMap')row=subjectById(id);if(type==='fee'){var feeSource=feeById(id),feeSubjectId=feeSource.subjectId||clean(defaults.subjectId);row=Object.assign({},feeSource,{subjectId:feeSubjectId});if(!id&&!clean(row.name)&&feeSubjectId)row.name=clean(subjectById(feeSubjectId).name)+'四堂';}if(type==='leave')row=state.leaveReasons.find(function(item){return item.id===id;})||{};var title={student:'學生',teacher:'老師',room:'教室',subject:'科目',subjectMap:'科目對應',fee:'收費方案',leave:'請假原因'}[type];$('entityModal').classList.toggle('teacher-entity-modal',type==='teacher');$('entityModalTitle').textContent=type==='subjectMap'?'對應既有科目':(id?'編輯':'新增')+title;$('entityModalSubtitle').textContent=type==='teacher'?'勾選可授課科目後，排課只會顯示符合老師。':type==='fee'?'學生收費與老師拆帳由管理者設定；沒有學生使用時可以先不建立。':type==='subjectMap'?'把老師填寫的項目併入既有共用科目。':type==='room'?'已被歷史課程引用的教室建議停用，不直接刪除。':'';var html='';
    if(type==='student')html=formFields([{id:'entityName',label:'學生名稱',value:row.name,required:true},{id:'entityPhone',label:'手機號碼',value:row.phone},{id:'entityLine',label:'LINE 狀態',type:'select',value:String(row.line),options:[['null','未確認'],['true','已綁定'],['false','未綁定']]},{id:'entityActive',label:'狀態',type:'select',value:String(row.active!==false),options:[['true','上課中'],['false','停課']]},{id:'entityNote',label:'備註',type:'textarea',value:row.note,full:true}].filter(function(field){return !!id||['entityLine','entityActive'].indexOf(field.id)<0;}));
    if(type==='teacher')html=formFields([{id:'entityName',label:'老師名稱',value:row.name,required:true},{id:'entityPhone',label:'手機號碼',value:row.phone},{id:'entityActive',label:'狀態',type:'select',value:String(row.active!==false),options:[['true','啟用'],['false','停用']]},{id:'entityNote',label:'備註',type:'textarea',value:row.note,full:true}])+'<div class="field full compact-subject-field"><label>可教授科目</label><small>這裡只記錄老師會教的項目，與收費方案分開管理；排課時會依這裡篩選老師。</small><div class="compact-check-grid">'+activeSubjects().map(function(subject){return '<label class="check-label"><input type="checkbox" name="teacherSubject" value="'+subject.id+'" '+((row.subjectIds||[]).indexOf(subject.id)>=0?'checked':'')+'>'+esc(subject.name)+'</label>';}).join('')+'</div></div>';
    ...
    if(type==='subjectMap')html=formFields([{id:'entitySourceSubject',label:'老師填寫的項目',value:row.name,required:true},{id:'entityTargetSubject',label:'對應到既有科目',type:'select',required:true,options:activeSubjects().filter(function(subject){return subject.id!==row.id;}).map(function(subject){return [subject.id,subject.name];})}]);
    if(type==='fee')html=formFields([{id:'entitySubject',label:'科目',type:'select',value:row.subjectId,required:true,options:configurableSubjects().map(function(s){return [s.id,s.name];})},{id:'entityName',label:'方案名稱',value:row.name,required:true},{id:'entityAmount',label:'學生收費金額',type:'number',value:row.amount||0},{id:'entityLessons',label:'每期堂數',type:'number',value:row.lessonCount||4},{id:'entitySplitType',label:'老師薪資方式',type:'select',value:row.splitType||'ratio',options:[['ratio','比例'],['fixed','每堂固定金額'],['none','老師薪資 $0']]},{id:'entitySplitValue',label:'比例或每堂金額（可填 0）',type:'number',value:row.splitValue||0},{id:'entityLeave',label:'請假規則',type:'select',value:String(row.leaveNoDeduct!==false),options:[['true','請假不扣堂'],['false','請假照常扣堂']]},{id:'entityExpiry',label:'有效天數（0 為不限）',type:'number',value:row.expiryDays||0},{id:'entityActive',label:'狀態',type:'select',value:String(row.active!==false),options:[['true','上架中'],['false','停用']]}]);
    $('entityModalBody').innerHTML='<div class="form-grid three'+(type==='teacher'?' teacher-entity-form':'')+'">'+html+'</div>';openModal('entityModal');
  }
  ...
  async function submitEntity(event){
    ...
    var type=entityContext.type,id=entityContext.id,row={},old={},nextSort=function(collection){return collection.reduce(function(max,item){return Math.max(max,numberOf(item.sort));},0)+10;};
    if(type==='student'){row={id:id||uid('student'),name:$('entityName').value.trim(),phone:$('entityPhone').value.trim(),line:$('entityLine')?($('entityLine').value==='null'?null:$('entityLine').value==='true'):null,active:$('entityActive')?$('entityActive').value==='true':true,note:$('entityNote').value.trim()};entityContext.id=row.id;try{var studentSaved=await window.YouziCoursePreviewData.saveStudent(Object.assign({manualSyncPin:storedMigrationPin()},row));row=Object.assign(row,studentSaved.student);upsert(state.students,row);}catch(error){toast('學生資料尚未保存',clean(error&&error.message||error),'error');return;}}
    if(type==='teacher'){
      old=teacherById(id);if(!id){id=uid('teacher');entityContext.id=id;}row={id:id,name:$('entityName').value.trim(),phone:$('entityPhone').value.trim(),reward:numberOf(old.reward),deduction:numberOf(old.deduction),active:$('entityActive').value==='true',note:$('entityNote').value.trim(),subjectIds:$$('input[name="teacherSubject"]',$('entityModalBody')).filter(function(box){return box.checked;}).map(function(box){return box.value;})};
      if(!window.YouziCoursePreviewData||typeof window.YouziCoursePreviewData.saveTeacherSubjects!=='function'){toast('授課科目元件尚未載入','請重新整理後再試。','error');return;}
      try{var teacherSubjectResult=await window.YouziCoursePreviewData.saveTeacherSubjects({manualSyncPin:storedMigrationPin(),teacherId:row.id,subjectIds:row.subjectIds,profile:{name:row.name,phone:row.phone,active:row.active,note:row.note}});row.subjectIds=teacherSubjectResult.subjectIds||row.subjectIds;}catch(error){toast('老師授課科目尚未同步',clean(error&&error.message||error).slice(0,220),'error');return;}
      upsert(state.teachers,row);
    }
    if(type==='room'){old=roomById(id);row={id:id||uid('room'),name:$('entityName').value.trim(),sort:numberOf(old.sort)||nextSort(state.rooms),rentalFee:numberOf($('entityRentalFee').value),active:$('entityActive').value==='true',note:$('entityNote').value.trim(),allowedSubjectIds:Array.isArray(old.allowedSubjectIds)?old.allowedSubjectIds:[],policies:old.policies||{}};entityContext.id=row.id;try{await window.YouziCoursePreviewData.saveRoomSettings(Object.assign({manualSyncPin:storedMigrationPin(),roomId:row.id,publicName:row.name},row));}catch(error){toast('教室尚未保存',clean(error.message),'error');return;}upsert(state.rooms,row);}
    if(type==='subject'){
      row={id:id||uid('subject'),name:$('entityName').value.trim(),sort:numberOf($('entitySort').value),active:$('entityActive').value==='true'};
      if(!window.YouziCoursePreviewData||typeof window.YouziCoursePreviewData.saveSubjectCatalog!=='function'){toast('共用科目元件尚未載入','請重新整理後再試。','error');return;}
      try{var subjectResult=await window.YouziCoursePreviewData.saveSubjectCatalog({manualSyncPin:storedMigrationPin(),id:id,name:row.name,sort:row.sort,active:row.active});row=Object.assign({},row,subjectResult.subject||{});}catch(error){toast('共用科目尚未同步',clean(error&&error.message||error).slice(0,220),'error');return;}
      upsert(state.subjects,row);
    }
    if(type==='subjectMap'){
      var targetId=$('entityTargetSubject').value;
      if(!targetId){toast('請選擇既有科目','對應目標不能空白。','error');return;}
      if(!window.YouziCoursePreviewData||typeof window.YouziCoursePreviewData.mapSubjectSuggestion!=='function'){toast('科目對應元件尚未載入','請重新整理後再試。','error');return;}
      try{var mappingResult=await window.YouziCoursePreviewData.mapSubjectSuggestion({manualSyncPin:storedMigrationPin(),suggestionId:id,targetSubjectId:targetId});row=subjectById(targetId);state.subjects=state.subjects.filter(function(subject){return subject.id!==id;});state.teachers.forEach(function(teacher){if((mappingResult.teacherIds||[]).indexOf(teacher.id)<0)return;teacher.subjectIds=unique((teacher.subjectIds||[]).filter(function(subjectId){return subjectId!==id;}).concat([targetId]));});}catch(error){toast('科目對應失敗',clean(error&&error.message||error).slice(0,220),'error');return;}
    }
    if(type==='leave'){row={id:id||uid('leave'),name:$('entityName').value.trim(),sort:numberOf($('entitySort').value),active:$('entityActive').value==='true'};entityContext.id=row.id;try{await window.YouziCoursePreviewData.saveLeaveReason(Object.assign({manualSyncPin:storedMigrationPin()},row));}catch(error){toast('請假原因尚未保存',clean(error.message),'error');return;}upsert(state.leaveReasons,row);}
    if(type==='fee'){
      old=feeById(id);row={id:id||'',subjectId:$('entitySubject').value,sort:numberOf(old.sort)||nextSort(state.feePlans),name:$('entityName').value.trim(),amount:numberOf($('entityAmount').value),lessonCount:numberOf($('entityLessons').value),splitType:$('entitySplitType').value,splitValue:numberOf($('entitySplitValue').value),leaveNoDeduct:$('entityLeave').value==='true',expiryDays:numberOf($('entityExpiry').value),active:$('entityActive').value==='true',listed:true};
      if(!window.YouziCoursePreviewData||typeof window.YouziCoursePreviewData.saveFeePlan!=='function'){toast('正式收費元件尚未載入','請重新整理後再試。','error');return;}
      try{var feeResult=await window.YouziCoursePreviewData.saveFeePlan(Object.assign({manualSyncPin:storedMigrationPin()},row));row=feeResult.feePlan;upsert(state.feePlans,row);if(feeResult.subject)upsert(state.subjects,feeResult.subject);}catch(error){toast('正式收費方案尚未同步',clean(error&&error.message||error).slice(0,220),'error');return;}
    }
    if(!row.name){toast('請填寫名稱','名稱不能空白。','error');return;}
    save('設定已儲存');closeModal('entityModal');refreshFormOptions();
    if(type==='student'&&returnToScheduleAfterStudent){var draft=scheduleDraftAfterStudent;returnToScheduleAfterStudent=false;scheduleDraftAfterStudent=null;openSchedule({draft:draft,newStudentId:row.id});toast('學生已新增','已返回排課並自動選擇 '+row.name+'。');return;}
    if(currentView==='settings')renderSettings();if(currentView==='teachers')renderTeachers();if(currentView==='students')renderStudents();
  }
  ...
  async function savePolicy(){
    ...
    try{
      ...
      upsert(state.rooms,room);save('教室設定已保存至雲端');closeModal('policyModal');refreshFormOptions();renderSettings();renderCalendar();
      ...
      toast('教室設定尚未保存',message.slice(0,220),'error');
    }
  }
  ...
  async function forceCancelPortalRental(button){
    ...
    try{
      ...
      closeModal('eventModal');
      materializeEvent(event).status='cancelled';materializeEvent(event).active=false;renderCalendar();scheduleWorkspaceSave();
  ...
  function preserveWorkspaceConfiguration(target,previous){
    ...
    target.feePlans=(target.feePlans||[]).map(function(row){
      var prior=previousFees[row.id];if(prior){row.sort=prior.sort;}return row;
    });
  ...
  function calendarPartial(){return Boolean(state&&state.dataMeta&&state.dataMeta.partial);}
  // Cache only ledgers explicitly opened in this calendar snapshot. A new
  // calendar state gets a new cache, so an old request cannot overwrite it.
  var studentDetailCaches=new WeakMap(),studentDetailRequest=0;
  function studentDetailCache(){
    var cache=studentDetailCaches.get(state);
    if(!cache){cache={ready:new Set(),pending:new Map(),catalog:null};studentDetailCaches.set(state,cache);}
    return cache;
  }
  async function ensureStudentDetails(ids){
    var target=state,cache=studentDetailCache(),api=window.YouziCoursePreviewData;
    if(!cache.catalog)cache.catalog=api.loadScheduleCatalog().then(function(rows){target.feePlans=rows;}).catch(function(error){cache.catalog=null;throw error;});
    var requests=ids.map(function(id){
      if(cache.ready.has(id))return Promise.resolve();
      if(!cache.pending.has(id))cache.pending.set(id,Promise.all([cache.catalog,api.refreshWorkspaceSlice({studentIds:[id]})]).then(function(results){
        var payload=results[1];
        if(state!==target)return;
        api.applyWorkspaceSlice(target,payload);cache.ready.add(id);
      }).finally(function(){cache.pending.delete(id);}));
      return cache.pending.get(id);
    });
    await Promise.all([cache.catalog].concat(requests));
    if(state!==target)return false;
    // Mutations use authenticated, scoped server APIs; keep partial=true so
    // this bounded snapshot is never saved as a complete workspace.
    state.readOnly=false;state.dataMode='sandbox';return true;
  }
  function afterStudentDetailsReady(ids,action,modalId,title){
    var request=++studentDetailRequest;
    if(!calendarPartial())return false;
    ids=Array.from(new Set(ids.map(clean).filter(Boolean)));
    var cache=studentDetailCache();
    if(!isReadOnly()&&cache.catalog&&ids.every(function(id){return cache.ready.has(id);}))return false;
    $(modalId+'Title').textContent=title||'課程明細';$(modalId+'Subtitle').textContent='正在讀取這位學生的資料…';
    $(modalId+'Body').textContent='正在讀取學費與簽到紀錄…';$(modalId+'Foot').innerHTML='';
    if(modalId==='eventModal'){$('eventTypeBadge').textContent='';delete $(modalId).dataset.eventId;}
    openModal(modalId);
    ensureStudentDetails(ids).then(function(ready){
      if(ready&&request===studentDetailRequest&&$(modalId).classList.contains('open'))action();
    }).catch(function(error){
      if(request!==studentDetailRequest||!$(modalId).classList.contains('open'))return;
      $(modalId+'Subtitle').textContent='資料讀取未完成';$(modalId+'Body').textContent='請關閉視窗後重新點選這位學生。';toast('學生資料讀取失敗',clean(error.message),'error');
    });
    return true;
  }
  async function ensureWorkspaceDetails(){
    if(workspaceLoadPromise)await workspaceLoadPromise;
    if(calendarPartial())return loadPublishedWorkspace();
    return !isReadOnly();
  }
  function afterWorkspaceReady(action){
    if(!calendarPartial()&&!calendarBootstrapLoading)return false;
    ensureWorkspaceDetails().then(function(ready){if(ready)action();});
    return true;
  }
  function calendarRangeReady(start,end,nodeId,render){
  ...
  function loadPublishedWorkspace(options){
    ...
    var calendarOnly=options&&options.calendarOnly===true;
    ...
    workspaceLoadPromise=(async function(){
      try{
        ...
        $('loadPublishedBtn').disabled=false;$('operationProgress').classList.add('hidden');updateModeUI();if(calendarOnly)refreshIrregularList();
  ...
  function bindEvents(){
    ...
    $('studentRows').addEventListener('click',function(event){var button=event.target.closest('[data-student-id]');if(button)openStudent(button.dataset.studentId);});$('studentTabs').addEventListener('click',function(event){var button=event.target.closest('[data-student-tab]');if(button){studentTab=button.dataset.studentTab;renderStudentModal();}});$('studentModalBody').addEventListener('change',function(event){if(event.target.matches('[data-student-tuition-subject]')){currentStudentSubject=event.target.value;renderStudentModal();}});$('studentModalBody').addEventListener('click',function(event){var deletion=event.target.closest('[data-period-delete]');if(deletion){deleteTuitionPeriod(deletion.dataset.periodDelete);return;}var correction=event.target.closest('[data-transaction-correct]');if(correction){correctTuitionTransaction(correction.dataset.transactionCorrect,Number(correction.dataset.transactionIndex));return;}var receipt=event.target.closest('[data-tuition-receipt-period]');if(receipt){ensureAndOpenTuitionReceipt(receipt);return;}var editPeriod=event.target.closest('[data-period-edit]');if(editPeriod){closeModal('studentModal');openTuition('',editPeriod.dataset.periodEdit);return;}var lesson=event.target.closest('[data-lesson-period]');if(lesson){openLessonAction(lesson.dataset.lessonPeriod,lesson.dataset.lessonSlot);return;}var button=event.target.closest('[data-period-pay]');if(button)openTransaction(button.dataset.periodPay);});$('studentModalFoot').addEventListener('click',function(event){var button=event.target.closest('[data-student-action]');if(!button)return;if(button.dataset.studentAction==='edit'){closeModal('studentModal');openEntity('student',currentStudentId);}else {var subjects=Array.from(new Set(state.tuitionPeriods.filter(function(row){return row.studentId===currentStudentId;}).map(function(row){return row.subjectId;})));if(subjects.length>1&&!currentStudentSubject){toast('請先選擇科目','選擇要延續的科目後，再新增學費期別。','error');return;}openTuition(currentStudentId,'',currentStudentSubject);}});$('addStudentBtn').addEventListener('click',function(){openEntity('student','');});$('studentSearch').addEventListener('input',debounce(function(){studentHistoryMode=!!clean($('studentSearch').value);if(studentHistoryMode&&typeof afterWorkspaceReady==='function'&&afterWorkspaceReady(function(){if(currentView==='students')renderStudents();}))return;renderStudents();},400));$('studentPaymentFilter').addEventListener('change',renderStudents);
  ...
  function init(){
    ...
    if($('refreshCloudBtn'))$('refreshCloudBtn').addEventListener('click',loadPublishedWorkspace);
    $('dayIrregularBtn').addEventListener('click',async function(){if(await refreshIrregularList(true)===false)return;openFollowupList('day');});$('weekIrregularBtn').addEventListener('click',async function(){if(await refreshIrregularList(true)===false)return;openFollowupList('week');});$('dayStoppedBtn').addEventListener('click',async function(){if(await refreshIrregularList(true)===false)return;openFollowupList('day-stopped');});$('weekStoppedBtn').addEventListener('click',async function(){if(await refreshIrregularList(true)===false)return;openFollowupList('week-stopped');});window.addEventListener('focus',refreshIrregularList);window.addEventListener('hashchange',refreshIrregularList);window.setInterval(refreshIrregularList,60000);
