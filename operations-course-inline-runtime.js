(function(){
  ...
  function eventConflictReasons(candidate,ignoreIds){
    ...
    if(candidate.type!=='rental'&&candidate.subjectId&&Array.isArray(room.allowedSubjectIds)&&room.allowedSubjectIds.length&&room.allowedSubjectIds.indexOf(candidate.subjectId)<0)reasons.push('這間教室不開放「'+(subjectById(candidate.subjectId).name||'此科目')+'」');
  ...
  function clearScheduleForm(){
    delete $('scheduleForm').dataset.halfHourAcknowledged;delete $('scheduleForm').dataset.studentMode;
  ...
  function persistLessonSettings(row,fields){
    runUiOperation('正在保存雲端課程設定…',null,async function(){
      var saved=await window.YouziCoursePreviewData.saveLessonSettings(Object.assign({manualSyncPin:storedMigrationPin(),date:row.date,teacherId:row.teacherId,sourceEventId:row.portalBookingId||row.sourceId||row.id,sourceCourseId:row.sourceCourseId||''},fields));
