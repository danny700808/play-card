(function(){
  ...
  function renderTeacherPayroll(){
    ...
    var previousDate='',table='<div class="table-wrap"><table class="payroll-detail-table"><thead><tr><th>日期</th><th>學生</th><th>課程</th><th>本堂學費</th><th>拆帳方式</th><th>老師實領</th></tr></thead><tbody>'+rows.map(function(row){var dayStart=previousDate&&previousDate!==row.date,studentId=row.studentId||(state.students.find(function(student){return student.name===row.studentName;})||{}).id||'',student=studentId?'<button type="button" class="student-link" data-payroll-student="'+esc(studentId)+'">'+esc(row.studentName||studentById(studentId).name||'未命名學生')+'</button>'+ (row.durationMinutes?'<small>'+esc(row.startTime||'')+'–'+esc(row.endTime||'')+'・'+numberOf(row.durationMinutes)+'分鐘</small>':''):esc(row.studentName||'未命名學生'),collected=Object.prototype.hasOwnProperty.call(row,'collectedAmount')?numberOf(row.collectedAmount):numberOf(row.lessonPrice),payAdjustment=numberOf(row.teacherPayAdjustment),payDetail=payAdjustment?'<small class="payroll-special-adjustment">原拆帳 '+money(numberOf(row.baseTeacherAmount))+(payAdjustment>0?' ＋ ':' － ')+money(Math.abs(payAdjustment))+'<br>'+esc(row.teacherPayAdjustmentReason||'本堂特殊調整')+'</small>':'';previousDate=row.date;return '<tr class="'+(dayStart?'payroll-day-start':'')+'"><td>'+esc(row.date)+'</td><td>'+student+'</td><td>'+esc(row.specialLesson?'贈送／特殊加課・'+(row.subject||'未標示課程'):row.subject||'未標示課程')+'</td><td>'+money(row.lessonPrice)+'</td><td>'+esc(splitText(row))+'</td><td><b>'+money(row.teacherAmount)+'</b>'+payDetail+'</td></tr>';}).join('')+'</tbody></table></div>';
