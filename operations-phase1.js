...
function ensureSalesClock(){
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const codexAction='<section class="ops-listing-codex-action" data-workflow-version="'+attr(PRODUCT_LISTING_WORKFLOW_VERSION)+'"><div class="ops-listing-codex-heading"><span>✦</span><div><b>帶入這個 Codex 對話</b><small>固定流程 v2：圖片完成 → MOMO → 酷澎 → EasyStore → 蝦皮。所有固定欄位進站前一次準備，頁面未改版就直接填寫。</small></div></div><div class="ops-listing-codex-flow" aria-label="固定五階段"><span>1 圖片完成</span><span>2 MOMO</span><span>3 酷澎</span><span>4 EasyStore</span><span>5 蝦皮</span></div><button class="ops-button primary" type="button" data-action="product-listing-codex-complete">帶入這個 Codex 對話</button><label class="ops-button soft ops-listing-upload-button" title="進階用途：檔名須為 01-cleanMain.jpg、01-brandedHero.jpg 等來源順序與角色"><input id="productCompletedImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><span>進階回填角色完成圖</span></label><div id="productCompletedImageUploadStatus" aria-live="polite"></div><div id="productListingCodexStatus"></div></section>';
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      '[固定流程]',
      '請接手全通路營運中心的待上架商品。這是唯一標準流程，workflowVersion 必須等於 '+PRODUCT_LISTING_WORKFLOW_VERSION+'；v1 或任何其他舊快照一律停止並要求從營運中心建立新的 v2 快照，不得沿用、混合或降級。商品資料可以不同，但圖片完成 → MOMO → 酷澎 → EasyStore → 蝦皮 的流程、角色與核對規則不可漂移。',
      ...
      '[固定發布與核對]',
      '瀏覽器只使用這個 Codex 對話旁邊的內建瀏覽器，沿用其中已登入的 EasyStore、酷澎與 MOMO 頁籤；不得操作使用者主要 Chrome，也不得在同一案件中途切換瀏覽器工作區。若登入狀態失效，可先用該內建瀏覽器已保存的帳號密碼重試登入；只有 OTP、驗證碼或登入失效且保存資料無法恢復時才停下交給使用者。',
      '按下「帶入這個 Codex 對話」已同時授權四通路的建立、修改與最後正式發布。分類、物流、保固、細項及平台送出都應依 preparedSnapshot 自動完成；不得再詢問「確認上架／確認提交／套用細項」等例行確認。每站核對成功後直接進入下一站。',
      '速度規則：進入第一個平台前先產生四站完整欄位表，之後沿用已知網址、頁面區段與欄位位置，不得每站重新掃描整頁或重新思考固定欄位。半年保固、立即上架、MOMO 第三方 000001、平台順序與最後送出直接套用；只有商品分類、分類產生的必填屬性、依材積決定的超商選項及平台實際錯誤需要動態判斷。若頁面版型簽章改變，只重新辨識目前區段，不回頭重做前站。',
      ...
      '平台售價空白時沿用共同網路售價、網路售價或門市售價；分類限定在樂器／樂器配件。固定順序為 MOMO、酷澎、EasyStore、蝦皮，沿用同一 jobId、最終 preparedSnapshot、preparedPlatformFieldPlan 與 currentStage；重試不得重新讀取案件或重建快照。只有 job schema、目前 automationPolicy、固定 platformOrder、最終完成圖快照、穩定 fingerprint 與案件 handoff input snapshot 完全一致的未完成 v2 工作可復用；任何早期 v2 或舊資料不符都要拒絕。每站送出後只核對一次正式清單與正式商品資料，確認相同 SKU、價格、庫存、圖片角色及狀態；成功提示本身不算完成，前站未核對成功，後站保持 blocked-by-previous-stage。',
  ...
  async function handoffProductListingToCodex(form){
    ...
    try{
      ...
      const prompt=productListingCodexHandoffPrompt(product,snapshot),threadUrl=productListingCodexThreadUrl(prompt),serverTime=serverTimestamp(),platforms={momo:{status:'pending'},coupang:{status:'blocked-by-previous-stage'},easyStore:{status:'blocked-by-previous-stage'},shopee:{status:'blocked-by-previous-stage'}};
      await state.db.collection(COLLECTIONS.listingCases).doc(id).set({
        ...
        codexExecution:{workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,snapshotId:snapshot.snapshotId,currentStage:'codex-preflight',platformOrder:PRODUCT_LISTING_PLATFORM_ORDER.slice(),platforms:platforms,retrySameSkuDraftAndStage:true,neverRebuildPreflightDuringRetry:true,updatedAt:serverTime},
      ...
      setProductListingCodexUi(form,'completed','已授權固定 v2 自動上架','完成圖資料齊全後會由後端依 MOMO、酷澎、EasyStore、蝦皮固定順序自動續跑；固定欄位會在進站前一次準備，不再要求第二次確認。'+(copied?' 工作指令也已複製。':''));
