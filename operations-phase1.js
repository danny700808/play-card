(function(global){
  ...
  const VERSION = '2026.07.16-product-cost-price-stock-v1';
  const DASHBOARD_CACHE_KEY = 'youzi_ops_dashboard_overview_v7_price_sync';
...
const DEFAULT_PLATFORM_FEE_SETTINGS = {
  ...
  function normalizeInternal(obj,docId){
    ...
    return {
      ...
      easyStorePrice:numberOrNull(firstValue(obj,['easyStorePrice','onlinePrice'])),momoPrice:numberOrNull(obj.momoPrice),coupangPrice:numberOrNull(obj.coupangPrice),
  ...
  function buildCatalogValues(online,internal){
    ...
    const easyStorePrice=internal&&internal.easyStorePrice!=null?internal.easyStorePrice:onlinePrice,momoPrice=internal&&internal.momoPrice!=null?internal.momoPrice:null,coupangPrice=internal&&internal.coupangPrice!=null?internal.coupangPrice:null;
...
async function loadPlatformLocalAgent(){
  ...
  function priceSyncAnomalies(){
    ...
    (state.catalog||[]).forEach(function(product){
      ...
      ['EasyStore','MOMO','Coupang'].forEach(function(platform){
        ...
        if(!['error','unmapped','unsupported'].includes(status))return;
  ...
  function platformSyncAnomalies(){
    ...
    return inventory.concat(priceSyncAnomalies()).sort(function(a,b){return (dateFrom(b.lastAttemptAt)||0)-(dateFrom(a.lastAttemptAt)||0)||a.platform.localeCompare(b.platform,'zh-Hant')||a.sku.localeCompare(b.sku,'zh-Hant');});
  }
...
function renderOverviewV7(){
  ...
  const syncAnomalies=platformSyncAnomalies(),syncAnomalyCount=syncAnomalies.length;
  ...
  const alerts=[];
  if(syncAnomalyCount){const counts={};syncAnomalies.forEach(function(row){counts[row.platform]=Number(counts[row.platform]||0)+1;});const detail=Object.keys(counts).sort().map(function(name){return name+' '+counts[name]+' 項';}).join('、');alerts.push('<button type="button" class="ops-v8-attention-row" data-action="overview-sync-errors"><span class="ops-v8-attention-icon danger">同</span><span><b>平台同步異常</b><small>'+formatNumber(syncAnomalyCount)+' 項'+(detail?'：'+escapeHtml(detail):'')+'</small></span><em>查看明細</em></button>');}
...
function posCustomerBarV5(){
  ...
  function renderInlineCheckout(){
    ...
    const preorder=state.checkoutOrderType==='preorder',subtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),c=selectedCustomer(),walkin=!c||state.posCustomerMode==='walkin',settings=membershipRuleForDate(new Date()),manualDiscount=Math.max(0,Number(state.checkoutDiscount||0)),maxPoints=(preorder||walkin)?0:maxRedeemablePoints(c,Math.max(0,subtotal-manualDiscount));
    ...
    const points=Math.min(maxPoints,Math.max(0,Math.floor(Number(state.checkoutPoints||0)))),pointValue=pointDiscount(points),total=Math.max(0,subtotal-manualDiscount-pointValue),status=preorder?'partial':walkin?'paid':state.checkoutPaymentStatus,member=!!(c&&c.customerType==='member'),earnEnabled=member&&state.checkoutEarnPoints!==false;
...
function renderSalesV5(){
  ...
  function renderSalesHistoryV7(){
    ...
    const visible=rows.slice(0,expanded?160:8),pendingCount=state.sales.filter(function(sale){return sale.saleType==='preorder'&&sale.fulfillmentStatus!=='delivered';}).length;
    ...
    const rowHtml=visible.length?visible.map(function(entry){
      ...
      const sale=entry.row,internalUse=sale.saleType==='internalUse',waiting=sale.saleType==='preorder'&&sale.fulfillmentStatus!=='delivered',deliveredPreorder=sale.saleType==='preorder'&&!waiting,items=(sale.items||[]),names=items.slice(0,2).map(function(item){return clean(item.name)||clean(item.sku)||'商品';}),itemText=names.join('、')+(items.length>2?' 等 '+formatNumber(items.length)+' 項':''),paymentStatus=clean(sale.paymentStatus)||'paid',status=waiting?'等待到貨':internalUse?'已扣庫存':paymentStatus==='paid'?'已收清':paymentStatus==='partial'?'部分收款':'未收款',ownerText=internalUse?(sale.usageReason||'內部耗用／報廢'):(sale.customerName||'門市散客'),type=waiting?'預購／訂金':deliveredPreorder?'預購交貨':internalUse?'內部耗用／報廢':'商品銷售',orderTotal=Number(sale.orderTotal||sale.total||0),outstanding=Math.max(0,orderTotal-Number(sale.receivedAmount||0)),amountHtml=waiting?'<b>總額 '+money(orderTotal)+'</b><br><small>已收 '+money(sale.receivedAmount)+'／尾款 '+money(outstanding)+'</small>':'<b>'+money(sale.total)+'</b>'+(internalUse?'<br><small>成本 '+money(sale.costTotal)+'</small>':'');
      return '<tr><td>'+escapeHtml(dateTimeText(entry.date))+'</td><td><b>'+escapeHtml(type)+'</b><br><small>'+escapeHtml(sale.saleNo||sale.id)+'</small></td><td>'+escapeHtml(ownerText)+'</td><td>'+escapeHtml(itemText||'—')+(sale.returnStatus?'<br><small>已退回／處理中</small>':'')+'</td><td>'+statusTag(status,waiting?'yellow':internalUse?'yellow':paymentStatus==='paid'?'green':'yellow')+'</td><td class="num">'+amountHtml+'</td><td>'+(waiting?'<button class="ops-button small primary" data-action="preorder-fulfill" data-id="'+attr(sale.id)+'">到貨交貨</button>':'<button class="ops-button small ghost" data-action="sale-edit" data-id="'+attr(sale.id)+'">修改</button>')+'</td></tr>';
    }).join(''):'';
    const table=rowHtml?'<div class="ops-table-wrap"><table class="ops-table ops-v8-sales-history-table"><thead><tr><th>時間</th><th>類型／單號</th><th>客戶／用途</th><th>內容</th><th>收款／狀態</th><th class="num">金額／成本</th><th></th></tr></thead><tbody>'+rowHtml+'</tbody></table></div>':emptyHtml('找不到符合條件的紀錄',expanded?'請調整搜尋文字或日期區間。':'完成商品銷售、預購、維修、其他收入或內部耗用後會顯示最近紀錄。');
    return '<section class="ops-card ops-v8-sales-history"><div class="ops-v8-section-head"><div><h2>最近銷售、預購、收入與耗用紀錄</h2><p>'+(pendingCount?'目前有 '+formatNumber(pendingCount)+' 筆預購等待到貨；到貨後直接按「到貨交貨」。':expanded?'可使用日期與關鍵字查詢完整紀錄':'預設顯示最近 8 筆；每一筆都可處理')+'</p></div><button class="ops-button small ghost" data-action="sales-history-toggle">'+(expanded?'收合紀錄':'展開全部紀錄')+'</button></div>'+toolbar+table+'</section>';
  }
...
function renderSync(){
  ...
  const runSummary=latestRun&&latestRun.summary||{},platformFetch=runSummary.platformFetch||{},syncErrors=platformSyncAnomalies();
  ...
  const errorList=syncErrors.length?'<div class="ops-sync-anomaly-list">'+syncErrors.map(function(row){const platformColor=row.platform==='EasyStore'?'green':row.platform==='MOMO'?'blue':row.platform==='Coupang'?'yellow':'red',isPrice=row.kind==='price',targetText=isPrice?'目標售價 '+money(row.targetPrice):'目標庫存 '+formatNumber(row.targetStock),actionHtml=isPrice?'<button class="ops-button small primary" data-action="price-anomaly-product" data-id="'+attr(row.productId)+'" data-sku="'+attr(row.sku)+'">前往商品價格</button>':'<button class="ops-button small primary" data-action="inventory-anomaly-product" data-sku="'+attr(row.sku)+'">前往商品庫存</button>';return '<article class="ops-sync-anomaly-row"><div class="ops-sync-anomaly-platform">'+statusTag(row.platform,platformColor)+'</div><div class="ops-sync-anomaly-main"><b>'+escapeHtml(row.productName||'未命名商品')+'</b><small>SKU '+escapeHtml(row.sku||'未設定')+'・'+escapeHtml(targetText)+'</small><strong>'+escapeHtml((isPrice?'價格：':'庫存：')+row.reason)+'</strong>'+(row.message?'<p>'+escapeHtml(row.message)+'</p>':'')+'<small>最後嘗試 '+escapeHtml(dateTimeText(row.lastAttemptAt))+(row.runId?'・批次 '+escapeHtml(row.runId):'')+'</small></div><div class="ops-sync-anomaly-actions">'+actionHtml+'</div></article>';}).join('')+'</div>':emptyHtml('目前沒有平台同步異常','下一次同步若成功，原本的庫存或價格異常會自動從這裡消失。');
...
function ensureSalesClock(){
  ...
  async function saveProduct(form){
    ...
    const currentPriceSync=p&&p.internal&&p.internal.platformPriceSync&&typeof p.internal.platformPriceSync==='object'?p.internal.platformPriceSync:{},nextPriceSync=Object.assign({},currentPriceSync);let priceChanged=false;
  ...
  async function saveCheckoutV4(form){
    ...
    await writeAudit(preorder?'建立預購／訂金單':'完成現場銷售',preorder?'preorder':'storeSale',saleRef.id,saleNo);state.cart=[];state.posSearch='';state.selectedCustomerId='';state.posCustomerMode='walkin';state.posMemberSearch='';state.posMemberPickerOpen=false;state.checkoutPaymentMethod='現金';state.checkoutPaymentStatus='paid';state.checkoutOrderType='sale';state.checkoutDiscount=0;state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;state.checkoutReceived='';toast(preorder?'預購單已建立':'現場銷售完成',preorder?saleNo+'｜等待到貨交付':saleNo,'success');await loadAll(true);
  }
