...
function ensureSalesClock(){
  ...
  function productVariantRepresentativeCardHtml(product,options){
    ...
    return '<article class="ops-listing-variant-item '+(options.className||'')+'" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-field="'+attr(options.fieldName||'')+'" data-variant-image-dropzone="1"'+(options.primary?' data-variant-primary="1"':'')+'><div class="ops-listing-variant-item-main"><span class="ops-listing-variant-kind">'+escapeHtml(options.kind||'細項商品')+'</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b>'+(options.valueName?'<label class="ops-listing-variant-name-field">細項名稱<input class="ops-input" name="'+attr(options.valueName)+'" value="'+attr(options.value||'')+'" placeholder="例如：藍色" '+(options.valueDisabled?'disabled':'')+'></label>':'')+'<input type="hidden" name="'+attr(options.fieldName||'variantImageUrl')+'" value="'+attr(selected)+'">'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status>'+(missing?'尚未選代表圖':'已選代表圖')+'</span>'+productVariantCollectorHtml(product.docId,sourceImages.length)+'<div class="ops-listing-variant-quick-actions"><button class="ops-button soft danger '+(selected?'':'is-hidden')+'" type="button" data-action="product-variant-image-clear" data-id="'+attr(product.docId)+'" data-role="'+attr(role)+'">清除代表圖</button></div>'+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div></article>';
  }
  ...
  function refreshProductVariantImagePickers(form){
    ...
    queryAll('.ops-listing-variant-item[data-product-id]',form).forEach(function(card){
      ...
      if(preview)preview.innerHTML=productVariantImageThumbsHtml(product,completedImages.length?completedImages.slice(0,1):selected?[selected]:[]);
      ...
      const clearButton=query('[data-action="product-variant-image-clear"]',card);if(clearButton)clearButton.classList.toggle('is-hidden',!selected);
