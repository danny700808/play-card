...
function ensureSalesClock(){
  ...
  function productVariantRepresentativeCardHtml(product,options){
    options=options||{};const selected=safeUrl(options.selectedUrl),role=clean(options.role),sourceImages=normalizeProductResearchSourceUrls(options.sourceImages),completed=clean(product&&product.imageSource)==='localized-clean-main'?productVariantImages(product).slice(0,1):[],missing=!selected&&!completed.length;
    return '<article class="ops-listing-variant-item '+(options.className||'')+'" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-field="'+attr(options.fieldName||'')+'" data-variant-image-dropzone="1"'+(options.primary?' data-variant-primary="1"':'')+'><div class="ops-listing-variant-item-main"><div class="ops-listing-variant-key-fields">'+productVariantRepresentativePreviewHtml(product,selected,completed,role)+'<div class="ops-listing-variant-key-copy"><span class="ops-listing-variant-kind">'+escapeHtml(options.kind||'細項商品')+'</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b>'+(options.valueName?'<label class="ops-listing-variant-name-field">細項名稱<input class="ops-input" name="'+attr(options.valueName)+'" value="'+attr(options.value||'')+'" placeholder="例如：藍色" '+(options.valueDisabled?'disabled':'')+'></label>':'')+'</div></div><input type="hidden" name="'+attr(options.fieldName||'variantImageUrl')+'" value="'+attr(selected)+'">'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status hidden>'+(missing?'尚未選代表圖':'已選代表圖')+'</span>'+productVariantCollectorHtml(product.docId,sourceImages.length)+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div></article>';
  }
  ...
  function refreshProductVariantImagePickers(form){
    ...
    queryAll('.ops-listing-variant-item[data-product-id]',form).forEach(function(card){
      ...
      if(selected&&sourceImages.length&&!sourceImages.includes(selected)){selected='';if(hidden)hidden.value='';}
      ...
      if(status){status.className=selected||completedImages.length?'ok':'missing';status.textContent=completedImages.length?'繁體代表圖已完成':selected?'代表圖已選擇':'尚未選擇代表圖';}
  ...
  async function clearProductVariantRepresentativeImage(form,productId,role){
    ...
    const hidden=productVariantImageHidden(card);if(hidden)hidden.value='';form.dataset.dirty='1';refreshProductVariantImagePickers(form);refreshProductVariantImageProcessingQueue(form);
...
async function syncPlatformOrdersNow(){const yes=await confirmAction('要求店內電腦立即同步','系統會建立同步請求；店內電腦開機且背景程式正常時，最慢約 20 秒內接收。若同步正在進行，不會重複執行。','建立請求');if(!yes)return;const ref=state.db.collection(COLLECTIONS.platformSyncRequests).doc();await ref.set({requestId:ref.id,status:'pending',requestedAt:serverTimestamp(),requestedBy:userLabel(),source:'operations-hub',version:VERSION});toast('同步請求已建立','店內電腦會在約 20 秒內接收，完成後重新讀取即可查看結果。','success');}
  ...
  function handleAction(action,el){
    ...
    if(action==='product-variant-group-add'){
      ...
      return loadProductListingVariantMedia(product.docId).then(function(media){items.push({productId:product.docId,sku:product.sku,name:product.originalName||product.name,attributeValue:productVariantSuggestion(product),imageUrls:[],sourceImageUrls:media.sourceImageUrls,completedImageUrls:media.completedImageUrls,gallerySourceImageUrls:[],imageMatchStatus:'missing'});renderProductVariantGroupItems(form,items);const input=query('[name="variantGroupSearch"]',form);if(input){input.value='';input.focus();}form.dataset.dirty='1';}).catch(function(error){toast('商品圖片尚未載入',errorMessage(error),'error');});
    }
