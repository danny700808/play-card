...
function ensureSalesClock(){
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const codexAction='<section class="ops-listing-codex-action"><div class="ops-listing-codex-heading"><span>✦</span><div><b>帶入這個 Codex 對話</b><small>按下後會儲存目前資料，並把完整工作帶入這個 Codex 對話的輸入框；切換後請按 Enter 才會正式開始。網頁本身不會先啟動 OpenAI。</small></div></div><button class="ops-button primary" type="button" data-action="product-listing-codex-complete">帶入這個 Codex 對話</button><input id="productCompletedImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple aria-label="Codex 完成圖上傳" style="position:fixed;left:0;top:0;width:1px;height:1px;opacity:.01;z-index:1"><div id="productCompletedImageUploadStatus" aria-live="polite"></div><div id="productListingCodexStatus"></div></section>';
