(function(global){
  ...
  const VERSION = '2026.07.17-pos-clean-layout-v1';
...
function renderSalesV5(){
  ...
  function renderSalesHistoryV7(){
    ...
    const table=rowHtml?'<div class="ops-table-wrap"><table class="ops-table ops-v8-sales-history-table"><thead><tr><th>時間</th><th>類型／單號</th><th>客戶／用途</th><th>內容</th><th>收款／狀態</th><th class="num">金額／成本</th><th></th></tr></thead><tbody>'+rowHtml+'</tbody></table></div>':emptyHtml('找不到符合條件的紀錄','請調整搜尋文字或日期區間。');
  ...
  function renderSalesV7(){
    ...
    const customerBlock='<div class="ops-v10-sales-customer-block"><span class="ops-v10-sales-label">客戶</span>'+posCustomerBarV5()+(usageMode?'<small class="ops-v10-usage-note">內部耗用／報廢不會連結客戶，也不會累積會員點數。</small>':'')+'</div>';
    const modes='<div class="ops-v10-sales-type-block"><span class="ops-v10-sales-label">交易類型</span><div class="ops-sales-modes ops-v8-sales-modes"><button data-action="sales-mode" data-mode="product" class="'+(state.salesMode==='product'?'active':'')+'">商品銷售</button><button data-action="sales-mode" data-mode="repair" class="'+(state.salesMode==='repair'?'active':'')+'">維修收入</button><button data-action="sales-mode" data-mode="other" class="'+(state.salesMode==='other'?'active':'')+'">其他收入</button><button data-action="sales-mode" data-mode="usage" class="'+(usageMode?'active':'')+'">內部耗用／報廢</button></div></div>';
    const salesTop='<div class="ops-v10-sales-top"><div class="ops-v10-sales-top-grid">'+customerBlock+modes+'<div class="ops-v8-sales-clock ops-v10-sales-clock"><span>現在時間</span><b id="opsSalesClock">'+formatSalesClock(new Date())+'</b></div></div></div>';
