...
function ensureSalesClock(){
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const codexAction='<section class="ops-listing-codex-action" data-workflow-version="'+attr(PRODUCT_LISTING_WORKFLOW_VERSION)+'"><div class="ops-listing-codex-heading"><span>✦</span><div><b>帶入這個 Codex 對話</b><small>固定流程 v2：圖片完成 → MOMO／酷澎／EasyStore 交錯處理；EasyStore 完成後接蝦皮。所有固定欄位進站前一次準備，頁面未改版就直接填寫。</small></div></div><div class="ops-listing-codex-flow" aria-label="固定交錯發布流程"><span>1 圖片完成</span><span>2A MOMO</span><span>2B 酷澎</span><span>2C EasyStore</span><span>3 蝦皮</span></div><button class="ops-button primary" type="button" data-action="product-listing-codex-complete">帶入這個 Codex 對話</button><label class="ops-button soft ops-listing-upload-button" title="進階用途：檔名須為 01-cleanMain.jpg、01-brandedHero.jpg 等來源順序與角色"><input id="productCompletedImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><span>進階回填角色完成圖</span></label><div id="productCompletedImageUploadStatus" aria-live="polite"></div><div id="productListingCodexStatus"></div></section>';
