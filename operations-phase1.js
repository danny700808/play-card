(function(global){
  ...
  const VERSION = '2026.08.29-verified-covers-easystore-sync-v2';
...
function ensureSalesClock(){
  ...
  function readyProductListingImageRows(listingCase){
    ...
    return generated.filter(function(row){return row.status==='ready'&&(row.localizationStatus==='completed'||row.qaStatus==='approved')&&!!row.sourceImageUrl&&!allSourceUrls.has(row.url);});
  }
  ...
  async function uploadProductCompletedListingImages(form,files){
    ...
    const generated=uploaded.map(function(url,index){const assignment=assignments[index],richHero=assignment.role==='brandedHero'||assignment.role==='storefrontPortrait',cleanMain=assignment.role==='cleanMain';return {id:'codex-chat-'+Date.now()+'-'+index,url:url,status:'ready',mode:'codex-chat-single-pass',model:'Codex 對話圖片處理',qaStatus:'approved',localizationStatus:'completed',qaModel:'人工確認後上傳',qaAttempts:0,sourceImageUrl:sourceImages[assignment.sourceIndex],sourceOrder:assignment.sourceIndex+1,roles:[assignment.role],assetFlags:{containsLogo:false,containsText:richHero,containsContactInfo:false,containsQrCode:false,greenBrandTemplate:false,momoPromotionEligible:cleanMain},instructions:instructions,createdAt:createdAt};});
  ...
  function requestProductImageCollection(action,payload){
    ...
    return new Promise(function(resolve,reject){
      const sessionId=clean(payload&&payload.sessionId),timer=global.setTimeout(function(){if(!productImageCollectionPending||productImageCollectionPending.sessionId!==sessionId)return;productImageCollectionPending=null;reject(new Error('找不到收圖助手，請先更新到柚子掌櫃助手 0.3.30 並重新整理頁面。'));},3500);
    ...
    if(message.type===PRODUCT_IMAGE_COLLECTION.sessionAck){
      ...
      if(payload.ok===true){
        if(pending.action==='start'&&!productImageCollectionVersionAtLeast(payload.extensionVersion,PRODUCT_IMAGE_COLLECTION.minimumVersion)){
          ...
          pending.reject(new Error('目前收圖助手版本過舊，請更新到 0.3.30，重新載入後再開始搜圖。'));
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const cards=orderedKeys.map(function(key){
      ...
      const helper=key==='shopee'&&row.autofillPayload?'<div class="ops-card-actions"><button class="ops-button primary" type="button" data-action="product-shopee-autofill-open">安全開啟 EasyStore／蝦皮</button><a class="ops-button soft" href="youzi-easystore-shopee-autofill-v0.3.30.zip" download>下載／更新助手 0.3.30</a></div><small>'+(shopeeHomeLogisticsNeedsManualConfirmation?'會先套用 EasyStore 進階圖文介紹，再帶入分類、品牌與商品屬性；一般宅配的實際物流仍須確認。商品身分依本案已凍結的處理方式與中央平台 ID 執行，不會自行改成新增或修改。':'會先套用 EasyStore 進階圖文介紹；商品身分依本案已凍結的處理方式與中央平台 ID 執行，不先搜尋平台目錄，也不會自行改成新增或修改。')+'</small>':'';
  ...
  async function openShopeeAutofillHelper(){
    ...
    else toast('助手沒有收到商品資料',clean(result&&result.error)||'請安裝或更新到助手 0.3.30，重新整理營運中心後，再按一次「安全開啟 EasyStore／蝦皮」。','warning');
    return result;
  }
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '同一輪圖片處理只產生本次目標通路需要的首圖版型，不得替未選通路多做圖片，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 為 750×1000 px、3:4；包含蝦皮時 brandedHero 為 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是無品牌框、無 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得含任何文字。storefrontPortrait 為 3:4 中性淺色商業展示版：商品主體清楚，用層級呈現 3～5 個已查證賣點，最多 2 個有來源依據的輔助視覺。brandedHero 為 1:1 中性商業首圖：商品約占 55～65%，只放 1～3 個已查證短賣點。storefrontPortrait 與 brandedHero 都不得加入店名、店鋪 Logo、店鋪標語、地址、電話、QR Code、聯絡資訊或導外訊息；只有官方來源可驗證的製造商品牌標誌，且平台規則允許時才可使用。三者都不得加入價格、浮水印或虛構功能／配件；不得含殘缺文字。',
