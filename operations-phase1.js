...
function ensureSalesClock(){
  ...
  function productListingAutomaticDescription(p){
    ...
    const intro=/(?:吉他)?腳踏板|足凳|腳凳/i.test(name)?'木製吉他腳踏板，適合演奏時支撐腳部使用；可依坐姿調整高度，使用後可摺疊收納。':'本商品為柚子樂器販售的樂器或樂器配件，商品內容與規格以頁面圖片及實際出貨品為準。';
    const reminders=/(木製|櫸木|烏金木|實木)/.test(name)?'天然木材的木紋與色澤可能略有差異，屬正常現象。':'螢幕顯示可能造成些微色差，實際顏色以收到商品為準。';
    return [name,'',intro,'','商品資訊','• 商品編號：'+(sku||'依商品標示'),'','購買提醒','• '+reminders,'• 如需確認尺寸、相容性或包裝內容，歡迎於購買前聯絡客服。'].join('\n');
  }
  ...
  function productListingCodexPreparedCase(raw,product){
    ...
    return {
      ...
      brand:normalized.brand,model:normalized.model,barcode:normalized.barcode,material:normalized.material,color:normalized.color,countryOfOrigin:normalized.countryOfOrigin,productDescription:normalized.productDescription,specificationText:normalized.specificationText,includedItems:normalized.includedItems,
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '[商品內容]',
      '商品標題依「品牌＋型號＋商品種類＋重要規格或材質」組成；可靠的品牌與型號優先，共同標題只寫共同事實，差異寫細項名稱；有空間才在尾端加「柚子樂器」。商品介紹只用案件文字、圖片與可驗證規格：先列最多 10 點不重複具體特色，再列資料足夠的使用方式／情境，最後獨立列尺寸、材質、段數、重量、內容物與細項差異。資料不足就少寫，不得猜測或湊數；保固只填平台保固欄，不寫進介紹。每平台介紹最後固定加入「商品圖片與規格僅供參考，實際內容以收到的實體商品為準。」',
      '[固定發布與核對]',
      ...
      '酷澎固定走已驗證的「以圖片建立」同一草稿：先上傳 cleanMain 與繁體完成副圖，再填樂器葉分類及可驗證品牌，按一次「產生商品資訊」，接著依 preparedPlatformFieldPlan 一次填完細項、售價、庫存、賣家 SKU、配送、人工繁體介紹與 TW_General 合規資料，最後按「建立產品」並以完全相同 SKU 在正式清單核對一次。產生後的細項欄位若呈唯讀或值無效，只能在尚未填寫細項商務、配送或介紹以前回同一草稿圖片步驟重新產生一次；後續區段已填後禁止返回或重新產生，晚期錯誤只修正目前區段並保留已完成資料。不得整頁重跑、名稱廣搜或另建商品。審核中只記為已送審，不得誤報已上架。',
