...
function ensureSalesClock(){
  ...
  function productVariantGroupItemsFromForm(form){
    ...
    return queryAll('.ops-listing-variant-item[data-variant-role^="group-child-"]',form).map(function(card){
      ...
      return {productId:product.docId,sku:clean(product.sku),name:clean(product.originalName||product.name),attributeValue:clean(query('[data-variant-group-value]',card)&&query('[data-variant-group-value]',card).value),imageUrls:imageUrls.slice(0,1),sourceImageUrls:sourceImageUrls,completedImageUrls:completedImageUrls,imageMatchStatus:imageUrls.length?'manual':'missing'};
    }).filter(Boolean);
  }
