(function(global){
  ...
  const VERSION = '2026.07.11-v5.2-product-layout';
  const DASHBOARD_CACHE_KEY = 'youzi_ops_dashboard_member_full_v2';
  ...
  function normalizeSale(obj){
    return {id:clean(obj.__id),saleNo:clean(obj.saleNo)||clean(obj.__id),soldAt:obj.soldAt||obj.createdAt||'',items:Array.isArray(obj.items)?obj.items:[],subtotal:firstNumber(obj,['subtotal']).value,manualDiscount:firstNumber(obj,['manualDiscount']).found?firstNumber(obj,['manualDiscount']).value:firstNumber(obj,['discount']).value,pointDiscount:firstNumber(obj,['pointDiscount']).value,discount:firstNumber(obj,['discount']).value,total:firstNumber(obj,['total']).value,costTotal:firstNumber(obj,['costTotal']).value,grossProfit:firstNumber(obj,['grossProfit']).value,paymentMethod:clean(obj.paymentMethod),customerId:clean(obj.customerId),customerName:clean(obj.customerName),customerType:clean(obj.customerType),memberNo:clean(obj.memberNo),pricingTier:clean(obj.pricingTier),paymentStatus:clean(obj.paymentStatus)||'paid',receivedAmount:firstNumber(obj,['receivedAmount']).value,pointsEarned:firstNumber(obj,['pointsEarned']).value,pendingPointsEarned:firstNumber(obj,['pendingPointsEarned']).value,pointsRedeemed:firstNumber(obj,['pointsRedeemed']).value,note:clean(obj.note),status:clean(obj.status)||'completed',createdAt:obj.createdAt||''};
  }
  ...
  function render(){
    ...
    const renderers={overview:renderOverviewV6,products:renderProducts,sales:renderSalesV6,customers:renderCustomersV6,receivables:renderReceivablesV5,purchases:renderPurchases,rentals:renderRentals,cases:renderCases,finance:renderFinance,sync:renderSync,connection:renderConnection};
  ...
  function renderOverviewV6(){
    ...
    const term=lower(state.overviewSearch).trim();
    if(term){
      productRows=productRows.filter(function(row){return lower([row.name,row.sku,row.no].join(' ')).includes(term);});
      incomeRows=incomeRows.filter(function(row){return lower([row.name,row.no].join(' ')).includes(term);});
      rentalRows=rentalRows.filter(function(row){return lower([row.name,row.no].join(' ')).includes(term);});
    }
    ...
    const cost=sum(productRows,function(row){return row.cost;});
    const profit=revenue-cost;
    const qty=sum(productRows,function(row){return row.qty;});
    const allRows=productRows.concat(incomeRows,rentalRows).sort(function(a,b){return (dateFrom(b.date)||0)-(dateFrom(a.date)||0);});
    const tabs='<div class="ops-range-tabs"><button class="'+(state.overviewRange==='today'?'active':'')+'" data-action="overview-range" data-range="today">今天</button><button class="'+(state.overviewRange==='month'?'active':'')+'" data-action="overview-range" data-range="month">本月</button><button class="'+(state.overviewRange==='year'?'active':'')+'" data-action="overview-range" data-range="year">今年</button><button class="'+(state.overviewRange==='custom'?'active':'')+'" data-action="overview-range" data-range="custom">自訂區間</button></div>';
    const custom=state.overviewRange==='custom'?'<div class="ops-range-custom"><label>開始日期<input class="ops-input" id="overviewFrom" type="date" value="'+attr(state.overviewFrom)+'"></label><label>結束日期<input class="ops-input" id="overviewTo" type="date" value="'+attr(state.overviewTo)+'"></label></div>':'';
    const list=allRows.length?'<div class="ops-today-list">'+allRows.map(function(row){const label=row.type==='rental'?'租賃':row.type==='repair'?'維修':row.type==='other'?'其他':'商品';return '<div class="ops-today-row"><div class="no-image">'+label+'</div><div><b>'+escapeHtml(row.name)+'</b><small>'+escapeHtml((row.sku?'編號 '+row.sku+'・':'')+row.no+'・'+dateTimeText(row.date))+'</small></div><strong>'+money(row.amount)+'</strong></div>';}).join('')+'</div>':emptyHtml('這段期間沒有收入','');
    return tabs+custom+'<div class="ops-kpi-grid ops-today-kpis">'+kpi('商品銷售',money(productRevenue),'','＄')+kpi('維修收入',money(repairRevenue),'','修')+kpi('其他收入',money(otherRevenue),'','＋')+kpi('租賃收益',money(rentalRevenue),'','租')+kpi('總收入',money(revenue),'','合')+kpi('商品成本',money(cost),'','成本')+kpi(bounds.label+'賺多少',money(profit),'','↗')+kpi('賣出數量',formatNumber(qty),'','件')+'</div><section class="ops-card"><div class="ops-card-head"><h2>'+bounds.label+'收入明細</h2><button class="ops-button small primary" data-nav="sales">前往銷售</button></div><input class="ops-input ops-overview-search" id="overviewSearch" placeholder="商品、編號、單號、客戶或租賃合約" value="'+attr(state.overviewSearch)+'">'+list+'</section>';
  }
  ...
  function productCard(p){
    const urls=Array.from(new Set((p.imageUrls||[]).concat(p.imageUrl?[p.imageUrl]:[]).filter(Boolean))).slice(0,2),reward=p.saleRewardPercent==null?membershipRuleForDate(new Date()).rewardPercent:p.saleRewardPercent;
    const imageHtml=urls.length?urls.map(function(url){return '<img loading="lazy" src="'+attr(url)+'" alt="'+attr(p.originalName||p.name)+'" onerror="this.style.display=\'none\'">';}).join(''):'<div class="placeholder">無圖</div>';
    return '<button type="button" class="ops-product-card ops-product-card-full" data-action="product-edit" data-id="'+attr(p.docId)+'"><div class="ops-product-image-grid '+(urls.length<2?'single':'')+'">'+imageHtml+'</div><div class="ops-product-body"><div class="ops-product-key"><div><span>商品編號</span><b>'+escapeHtml(p.sku||'未設定')+'</b></div><div><span>庫存</span><b>'+escapeHtml(formatNumber(p.currentStock))+'</b></div></div><div class="ops-product-name-rows"><b>'+escapeHtml(p.originalName||p.name||'未命名')+'</b><b>'+escapeHtml(p.onlineName||'未配對')+'</b></div><div class="ops-product-detail-grid"><div><span>門市定價</span><b>'+money(p.storePrice)+'</b></div><div><span>網路售價</span><b>'+money(p.onlinePrice)+'</b></div><div><span>進貨成本</span><b>'+money(p.latestPurchaseCost)+'</b></div><div><span>平均成本</span><b>'+money(p.averageCost)+'</b></div><div><span>會員回饋</span><b>'+formatNumber(reward)+'%</b></div></div></div></button>';
  }
  ...
  function posCustomerBarV5(){
    const c=selectedCustomer(),term=lower(state.posMemberSearch),members=state.customers.filter(function(x){return x.enabled&&x.customerType==='member'&&(!term||lower([x.name,x.phone,x.memberNo].join(' ')).includes(term));}).slice(0,12),results=state.posCustomerMode==='member'?'<div class="ops-member-finder"><input class="ops-input" id="posMemberSearch" inputmode="tel" placeholder="輸入電話或姓名" value="'+attr(state.posMemberSearch)+'"><div class="ops-member-results">'+(term?members.map(function(x){return '<button type="button" class="'+(x.id===state.selectedCustomerId?'active':'')+'" data-action="pos-member-select" data-id="'+attr(x.id)+'"><b>'+escapeHtml(x.name)+'</b><span>'+escapeHtml(x.phone||x.memberNo||'')+'</span><strong>'+formatNumber(x.pointBalance)+' 點</strong></button>';}).join(''):'')+'</div></div>':'';
    return '<section class="ops-card ops-pos-customer-v5"><div class="ops-customer-mode-grid"><button type="button" class="'+(state.posCustomerMode==='walkin'?'active':'')+'" data-action="pos-customer-mode" data-mode="walkin">門市散客</button><button type="button" class="'+(state.posCustomerMode==='member'?'active':'')+'" data-action="pos-customer-mode" data-mode="member">'+(c?escapeHtml(c.name):'選擇會員')+'</button><button type="button" data-action="pos-customer-new">新增會員</button></div>'+results+(c&&state.posCustomerMode==='member'?'<div class="ops-selected-member"><b>'+escapeHtml(c.name)+'</b><span>'+escapeHtml(c.phone||c.memberNo||'')+'</span><strong>'+formatNumber(c.pointBalance)+' 點</strong><button type="button" class="ops-button small ghost" data-action="customer-history" data-id="'+attr(c.id)+'">消費紀錄</button></div>':'')+'</section>';
  }
  function renderInlineCheckout(){
    if(!state.cart.length)return '';const subtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),c=selectedCustomer(),walkin=!c||state.posCustomerMode==='walkin',settings=membershipRuleForDate(new Date()),maxPoints=walkin?0:maxRedeemablePoints(c,Math.max(0,subtotal-Number(state.checkoutDiscount||0)));if(!walkin&&settings.redemptionMode==='auto'&&!state.checkoutPointsTouched)state.checkoutPoints=maxPoints;const points=Math.min(maxPoints,Math.max(0,Math.floor(Number(state.checkoutPoints||0)))),pointValue=pointDiscount(points),total=Math.max(0,subtotal-Math.max(0,Number(state.checkoutDiscount||0))-pointValue),status=walkin?'paid':state.checkoutPaymentStatus;
    state.checkoutPoints=points;if(walkin){state.checkoutPaymentStatus='paid';state.checkoutReceived='';state.checkoutPointsTouched=false;}
    return '<form id="checkoutFormInline" class="ops-inline-checkout"><h3>結帳</h3><input type="hidden" name="customerId" value="'+attr(walkin?'':c.id)+'"><input type="hidden" name="paymentMethod" value="'+attr(state.checkoutPaymentMethod)+'"><input type="hidden" name="paymentStatus" value="'+attr(status)+'"><div class="ops-checkout-block"><label>付款方式</label>'+posChoiceButtons('paymentMethod',state.checkoutPaymentMethod,[{value:'現金',label:'現金'},{value:'信用卡',label:'信用卡'},{value:'轉帳',label:'轉帳'}])+'</div><div class="ops-checkout-block"><label>收款狀態</label>'+posChoiceButtons('paymentStatus',status,walkin?[{value:'paid',label:'已收清'}]:[{value:'paid',label:'已收清'},{value:'partial',label:'部分收款'},{value:'unpaid',label:'未收款'}])+'</div>'+(status==='partial'?'<div class="ops-field"><label>本次已收</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="receivedAmount" value="'+attr(state.checkoutReceived)+'"></div>':'')+'<div class="ops-checkout-fields"><div class="ops-field"><label>額外折扣</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="discount" value="'+attr(state.checkoutDiscount)+'"></div>'+(c&&c.customerType==='member'&&settings.redemptionMode!=='earn-only'?'<div class="ops-field"><label>點數折抵</label><input class="ops-input ops-number-clean" type="number" inputmode="numeric" min="0" max="'+maxPoints+'" step="'+Math.max(1,Math.floor(Number(settings.redeemPoints||1)))+'" name="pointsToRedeem" value="'+points+'"></div>':'<input type="hidden" name="pointsToRedeem" value="0">')+'</div><div class="ops-summary-list"><div class="ops-summary-line"><span>商品金額</span><b>'+money(subtotal)+'</b></div><div class="ops-summary-line"><span>折扣</span><b id="inlineDiscountTotal">'+money(Math.max(0,Number(state.checkoutDiscount||0))+pointValue)+'</b></div><div class="ops-summary-line total"><span>應收金額</span><b id="inlineCheckoutTotal">'+money(total)+'</b></div></div><button class="ops-button primary wide" type="submit">確認結帳</button></form>';
  }
  ...
  function renderTodayInvoices(){
    const term=lower(state.saleInvoiceSearch),rows=todayRows(state.sales,function(x){return x.soldAt;}).filter(function(x){return !term||lower([x.saleNo,x.customerName,x.paymentMethod].concat(x.items.map(function(i){return i.name+' '+i.sku;})).join(' ')).includes(term);}).sort(function(a,b){return (dateFrom(b.soldAt)||0)-(dateFrom(a.soldAt)||0);});
    return '<section class="ops-card ops-invoice-list"><div class="ops-card-head"><h2>今日單據</h2></div><input class="ops-input" id="saleInvoiceSearch" placeholder="搜尋單號、會員或商品" value="'+attr(state.saleInvoiceSearch)+'"><div class="ops-invoice-rows">'+(rows.length?rows.map(function(x){return '<button type="button" data-action="sale-edit" data-id="'+attr(x.id)+'"><span>'+escapeHtml(dateTimeText(x.soldAt).slice(11))+'</span><b>'+escapeHtml(x.saleNo)+'</b><em>'+escapeHtml(x.customerName||'門市散客')+'</em><strong>'+money(x.total)+'</strong><i>修改</i></button>';}).join(''):'<div class="ops-empty"><strong>尚無單據</strong></div>')+'</div></section>';
  }
  function renderSalesV5(){
    const products=state.catalog.filter(function(p){return p.initialized&&p.status!=='inactive';}),term=lower(state.posSearch).trim(),choices=term?products.filter(function(p){return lower([p.originalName,p.onlineName,p.sku,p.barcode,p.brand,p.category].join(' ')).includes(term);}).slice(0,30):[],cartSubtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),todaySales=todayRows(state.sales,function(x){return x.soldAt;}),todayIncome=todayRows(state.incomes,function(x){return x.occurredAt;}),repairIncome=todayIncome.filter(function(x){return x.category==='維修收入';}),otherIncome=todayIncome.filter(function(x){return x.category!=='維修收入';});let main='';
    if(state.salesMode==='product'){let productHtml='';if(choices.length)productHtml=choices.map(function(p){const image=p.imageUrl||'';return '<button class="ops-pos-item" data-action="cart-add" data-id="'+attr(p.docId)+'">'+(image?'<img loading="lazy" src="'+attr(image)+'" alt="" onerror="this.style.display=\'none\'">':'<div class="ops-pos-no-image">無圖</div>')+'<div><b>'+escapeHtml(p.originalName||p.name)+'</b><small>編號 '+escapeHtml(p.sku||'未設定')+'・庫存 '+formatNumber(p.currentStock)+'・'+money(p.storePrice)+'</small></div></button>';}).join('');else if(term)productHtml='<div class="ops-no-result">找不到商品</div>';const cartHtml=state.cart.length?state.cart.map(function(item,index){const p=catalogById(item.productId);return '<div class="ops-cart-row"><div><b>'+escapeHtml(item.name)+'</b><small>編號 '+escapeHtml(item.sku||'')+'・庫存 '+formatNumber(p?p.currentStock:item.currentStock)+'</small></div><input class="ops-number-clean" aria-label="數量" type="number" min="1" step="1" value="'+item.qty+'" data-cart-qty="'+index+'"><input class="ops-number-clean" aria-label="售價" type="number" min="0" step="1" value="'+item.unitPrice+'" data-cart-price="'+index+'"><button class="ops-icon-button" data-action="cart-remove" data-index="'+index+'">×</button></div>';}).join(''):'<div class="ops-empty"><strong>尚未選商品</strong></div>';main='<div class="ops-pos-layout"><section class="ops-card"><div class="ops-toolbar"><input class="ops-input grow ops-pos-search" id="posSearch" placeholder="商品編號／名稱" value="'+attr(state.posSearch)+'"><button class="ops-button ghost" data-action="pos-clear-search">清除</button></div><div class="ops-pos-products">'+productHtml+'</div></section><section class="ops-card"><div class="ops-card-head"><h2>要賣的商品</h2><button class="ops-button small ghost" data-action="cart-clear">清空</button></div><div class="ops-cart">'+cartHtml+'</div>'+(state.cart.length?'<div class="ops-summary-line total"><span>商品金額</span><b id="cartSubtotal">'+money(cartSubtotal)+'</b></div>':'')+renderInlineCheckout()+'</section></div>';}else main=renderDirectIncomeV5(state.salesMode);
    const productRevenue=sum(todaySales,function(x){return x.total;}),repairRevenue=sum(repairIncome,function(x){return x.amount;}),otherRevenue=sum(otherIncome,function(x){return x.amount;});return posCustomerBarV5()+'<section class="ops-sale-workspace"><div class="ops-sales-modes"><button data-action="sales-mode" data-mode="product" class="'+(state.salesMode==='product'?'active':'')+'">商品銷售</button><button data-action="sales-mode" data-mode="repair" class="'+(state.salesMode==='repair'?'active':'')+'">維修收入</button><button data-action="sales-mode" data-mode="other" class="'+(state.salesMode==='other'?'active':'')+'">其他收入</button></div>'+main+'</section><section class="ops-card ops-sales-total"><div class="ops-kpi-grid">'+kpi('商品銷售',money(productRevenue),'','＄')+kpi('維修收入',money(repairRevenue),'','修')+kpi('其他收入',money(otherRevenue),'','＋')+kpi('今日總計',money(productRevenue+repairRevenue+otherRevenue),'','合')+'</div></section>'+renderTodayInvoices();
  }
  ...
  async function saveCheckoutV4(form){
    if(!state.cart.length)throw new Error('銷售清單是空的');const data=new FormData(form),discount=Math.max(0,numberOrNull(data.get('discount'))||0),soldAt=new Date(),customerId=clean(data.get('customerId')),paymentChoice=clean(data.get('paymentStatus'))||'paid',requestedPoints=Math.max(0,Math.floor(numberOrNull(data.get('pointsToRedeem'))||0));if(paymentChoice!=='paid'&&!customerId)throw new Error('未收款必須選擇會員');if(paymentChoice!=='paid'&&requestedPoints>0)throw new Error('未收款不能使用點數');
    ...
    await writeAudit('完成現場銷售','storeSale',saleRef.id,saleNo);state.cart=[];state.posSearch='';state.selectedCustomerId='';state.posCustomerMode='walkin';state.posMemberSearch='';state.checkoutPaymentMethod='現金';state.checkoutPaymentStatus='paid';state.checkoutDiscount=0;state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutReceived='';toast('現場銷售完成',saleNo,'success');await loadAll(true);
  }
  ...
  async function saveSaleEdit(form){
    ...
    await state.db.runTransaction(async function(tx){
      ...
      const pointValue=Math.max(0,Number(rawSale.pointDiscount||0)),total=Math.max(0,subtotal-manualDiscount-pointValue),loggedPayments=sum(state.receivablePayments.filter(function(x){return x.saleId===id;}),function(x){return x.amount;}),enteredReceived=numberOrNull(data.get('receivedAmount')),receivedAmount=paymentStatus==='paid'?total:paymentStatus==='unpaid'?0:Math.min(total,Math.max(loggedPayments,enteredReceived||0));if(paymentStatus==='partial'&&(receivedAmount<=0||receivedAmount>=total))throw new Error('部分收款金額必須大於 0 且小於應收金額');const customerRaw=customerSnap&&customerSnap.exists?(customerSnap.data()||{}):null,isMember=customerRaw&&(clean(rawSale.customerType)==='member'||clean(customerRaw.customerType)==='member'),rewardPoints=isMember?calculatePreparedRewardPoints(prepared,total,subtotal,rawSale.soldAt||new Date()):0,newEarned=paymentStatus==='paid'?rewardPoints:0,oldEarned=Math.max(0,Number(rawSale.pointsEarned||0)),pointDelta=newEarned-oldEarned,newPending=paymentStatus!=='paid'?rewardPoints:0;
      if(pointDelta){if(!customerRaw)throw new Error('找不到會員資料');const oldBalance=Math.max(0,Number(customerRaw.pointBalance||0)),newBalance=oldBalance+pointDelta;if(newBalance<0)throw new Error('會員點數已使用，無法回收原單點數');tx.update(customerRef,{pointBalance:newBalance,updatedAt:serverTimestamp()});const pointRef=state.db.collection(COLLECTIONS.points).doc();tx.set(pointRef,{customerId:clean(rawSale.customerId),saleId:id,type:'saleEdit',points:pointDelta,balanceAfter:newBalance,note:clean(rawSale.saleNo),createdAt:serverTimestamp(),createdBy:userLabel(),version:VERSION});}
      tx.set(saleRef,{items:prepared.map(function(x){return {productId:x.requested.productId,name:clean(x.oldItem.name),sku:clean(x.oldItem.sku),imageUrl:clean(x.oldItem.imageUrl),qty:x.requested.qty,unitPrice:x.requested.unitPrice,lineTotal:x.lineTotal,lineCost:x.fifo.costTotal,fifoBreakdown:x.fifo.breakdown,unknownCostQty:x.fifo.unknownCostQty,rewardPercent:productRewardPercent(x.raw,rawSale.soldAt||new Date())};}),subtotal:subtotal,manualDiscount:manualDiscount,pointDiscount:pointValue,discount:manualDiscount+pointValue,total:total,costTotal:costTotal,grossProfit:total-costTotal,unknownCostQty:unknownCostQty,paymentMethod:paymentMethod,paymentStatus:paymentStatus,receivedAmount:receivedAmount,pointsEarned:newEarned,pendingPointsEarned:newPending,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
  ...
  function exportBackup(){
    const payload={exportedAt:new Date().toISOString(),version:VERSION,projectId:(global.APP_CONFIG&&APP_CONFIG.FIREBASE_CONFIG&&APP_CONFIG.FIREBASE_CONFIG.projectId)||'',onlineSource:state.onlineSource,data:{internalProducts:state.internalProducts,sales:state.sales,incomes:state.incomes,customers:state.customers,pointTransactions:state.pointTransactions,receivables:state.receivables,receivablePayments:state.receivablePayments,membershipSettings:state.membershipSettings,purchases:state.purchases,inventoryTransactions:state.inventory,rentalLedgers:state.rentalLedgers,cases:state.cases,expenses:state.expenses,syncJobs:state.syncJobs,auditLogs:state.audit}};
  ...
  function handleAction(action,el){
    ...
    if(action==='pos-customer-mode'){state.posCustomerMode=el.dataset.mode||'walkin';if(state.posCustomerMode==='walkin'){state.selectedCustomerId='';state.posMemberSearch='';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;state.checkoutPointsTouched=false;}return render();}
    if(action==='pos-member-select'){state.selectedCustomerId=el.dataset.id||'';state.posCustomerMode='member';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;state.checkoutPointsTouched=false;return render();}
    if(action==='pos-choice'){const name=el.dataset.name,value=el.dataset.value;if(name==='paymentMethod')state.checkoutPaymentMethod=value;if(name==='paymentStatus')state.checkoutPaymentStatus=value;if(value!=='partial')state.checkoutReceived='';return render();}
  ...
  function updateInlineCheckoutTotals(){
    const form=byId('checkoutFormInline');if(!form)return;const subtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),discount=Math.max(0,Number(state.checkoutDiscount||0)),c=selectedCustomer(),maxPoints=maxRedeemablePoints(c,Math.max(0,subtotal-discount)),pointInput=query('[name="pointsToRedeem"]',form),rule=membershipRuleForDate(new Date());if(pointInput){pointInput.max=String(maxPoints);if(c&&rule.redemptionMode==='auto'&&!state.checkoutPointsTouched){state.checkoutPoints=maxPoints;pointInput.value=String(maxPoints);}else if(Number(pointInput.value||0)>maxPoints){pointInput.value=String(maxPoints);state.checkoutPoints=maxPoints;}}const pointValue=pointDiscount(state.checkoutPoints);setText('inlineDiscountTotal',money(discount+pointValue));setText('inlineCheckoutTotal',money(Math.max(0,subtotal-discount-pointValue)));
  }
  ...
  function bindEvents(){
    ...
    document.addEventListener('input',function(event){
      ...
      else if(event.target.closest('#checkoutFormInline')&&event.target.name==='discount'){state.checkoutDiscount=Math.max(0,Number(event.target.value||0));updateInlineCheckoutTotals();}
