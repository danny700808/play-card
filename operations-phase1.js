(function(global){
  ...
  const VERSION = '2026.08.29-v3-fixed-content-v1';
  ...
  const PRODUCT_RICH_CONTENT_STANDARD_VERSION = 'youzi-rich-product-content-v3';
  const PRODUCT_RICH_CONTENT_FEATURE_TARGET = 10;
  const PRODUCT_RICH_CONTENT_USAGE_TARGET = 10;
  const PRODUCT_RICH_CONTENT_POINT_MINIMUM_CHARACTERS = 16;
  const PRODUCT_RICH_CONTENT_POINT_RECOMMENDED_CHARACTERS = 20;
  const PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER = '商品圖片與規格僅供參考，實際內容以收到的實體商品為準。';
  const PRODUCT_DESCRIPTION_LAYOUT_VERSION = 'youzi-interleaved-description-v2';
  const PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE = 'refresh-description-media';
  const PRODUCT_DESCRIPTION_MEDIA_REFRESH_INSTRUCTIONS = '原地修改同一 SKU 與既有平台商品：重新整理並驗證官網、蝦皮、MOMO、酷澎的商品介紹與圖片。所有處理方式固定使用圖文穿插：商品介紹與特色→商品圖一→商品規格→商品圖二→使用建議→商品圖三→其餘商品圖→實體商品免責句→兩張固定介紹圖；兩張固定介紹圖是詳細介紹絕對最後內容，橫向商品圖最後一張固定為店址圖。蝦皮必須實際確認文字與全部圖片均已保存；酷澎與 MOMO 可編輯時同步完成；缺圖不得判定完成。';
  ...
  const PRODUCT_BRAND_TEMPLATE_CONTRACT = Object.freeze({
    ...
    storefrontPortrait:Object.freeze({assetUrl:'product-listing-brand-template-portrait.png',assetSha256:'e3fe984de916395accec67a9bf251429e271c908e45608d2d45781759be5cb90',widthPx:750,heightPx:1000,aspectRatio:'3:4'}),
...
async function loadPlatformLocalAgent(){
  ...
  function normalizeProductListingQueueRow(raw,docId){
    const source=raw&&typeof raw==='object'?raw:{},status=clean(source.batchQueueStatus),listingTargetScope=normalizeProductListingTargetScope(source.listingTargetScope);
    return {productId:clean(source.productId||docId),productSku:clean(source.productSku),productName:clean(source.productName),productImageUrl:safeUrl(source.productImageUrl),workflowPurpose:clean(source.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE:'',listingTargetScope:listingTargetScope,listingTargetPlatforms:productListingTargetPlatforms(listingTargetScope),batchQueueStatus:PRODUCT_LISTING_QUEUE_STATUSES.includes(status)?status:'',batchQueuedAt:source.batchQueuedAt||'',batchQueueUpdatedAt:source.batchQueueUpdatedAt||'',batchQueueError:clean(source.batchQueueError),batchRunId:clean(source.batchRunId),batchPosition:Number(source.batchPosition||0)};
  }
...
function renderOverviewV7(){
  ...
  function productMergeSelectorHtml(p){
    if(!state.productRecentOnly)return '';
    const selected=productMergeSelectedIds().includes(p.docId);
    return '<button type="button" class="ops-product-merge-selector '+(selected?'selected':'')+'" data-action="product-merge-select" data-id="'+attr(p.docId)+'" aria-pressed="'+(selected?'true':'false')+'"><span>'+(selected?'✓':'＋')+'</span>'+(selected?'已選':'合併')+'</button>';
  }
  ...
  function productListingQueueDrawerHtml(){
    ...
    const cards=rows.map(function(row,index){const product=catalogById(row.productId),image=safeUrl(row.productImageUrl||(product&&product.imageUrl)),name=clean(row.productName||(product&&(product.originalName||product.name)))||'未命名商品',sku=clean(row.productSku||(product&&product.sku))||'未設定',status=clean(row.batchQueueStatus),targetLabel=row.workflowPurpose===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹｜全部平台':productListingTargetLabel(row.listingTargetScope);return '<article class="ops-listing-queue-item"><div class="ops-listing-queue-order">'+(index+1)+'</div>'+(image?'<img src="'+attr(image)+'" alt="">':'<div class="ops-listing-queue-no-image">無圖</div>')+'<div class="ops-listing-queue-copy"><b>'+escapeHtml(name)+'</b><small>SKU '+escapeHtml(sku)+'｜'+escapeHtml(targetLabel)+(row.batchQueuedAt?'｜加入 '+escapeHtml(dateTimeText(row.batchQueuedAt)):'')+'</small>'+(row.batchQueueError?'<em>'+escapeHtml(row.batchQueueError)+'</em>':'')+'</div><span class="ops-listing-queue-status '+attr(status)+'">'+escapeHtml(productListingQueueStatusLabel(status))+'</span><div class="ops-listing-queue-actions"><button class="ops-button ghost small" type="button" data-action="product-listing-case-open" data-id="'+attr(row.productId)+'">查看</button><button class="ops-button ghost small" type="button" data-action="product-listing-queue-remove" data-id="'+attr(row.productId)+'">移除</button></div></article>';}).join('');
    return '<div class="ops-listing-queue"><div class="ops-listing-queue-summary"><div><b>'+formatNumber(rows.length)+' 件待網路上架</b><small>會依每件已選的平台照順序處理，一件完成後再接下一件。</small></div><button class="ops-button primary" type="button" data-action="product-listing-queue-start">開始處理全部</button></div><div class="ops-listing-queue-list">'+cards+'</div><div id="productListingQueueStatus" aria-live="polite"></div></div>';
  }
  ...
  function productListingBatchActivationPrompt(rows,runId){
    return [
      ...
      '開始每件時保留同一 productId 與 listingTargetScope 並建立該件不可變預檢快照；完成該件所選通路工作後把 batchQueueStatus 設為 completed、batchCompletedAt 寫入完成時間。單件永久失敗時把 batchQueueStatus 設為 failed、batchQueueError 寫明原因，然後繼續下一件。',
      '按下「開始處理全部」已授權依序執行這份批次。'
    ].join('\n');
  }
  async function startProductListingQueue(){
    let rows=productListingQueueRows().filter(function(row){return row.batchQueueStatus!=='processing';});if(!rows.length)throw new Error('目前沒有等待處理的商品');
    ...
    rows.forEach(function(row,index){batch.set(state.db.collection(COLLECTIONS.listingCases).doc(row.productId),{batchQueueStatus:'processing',batchRunId:runId,batchPosition:index+1,batchQueueError:'',batchQueueUpdatedAt:serverTime,caseStatus:'waiting-codex',updatedAt:serverTime,updatedBy:userLabel(),version:VERSION},{merge:true});});
    await batch.commit();
  ...
  function productMergeToolbarHtml(){
    if(!state.productRecentOnly)return '';
    const selected=productMergeSelectedIds(),count=selected.length;
    return '<div class="ops-product-merge-toolbar"><div><b>合併／加入既有商品</b><span>直接勾選要放在同一商品下的所有編號；系統會把已有平台商品編號的商品放在第一筆並作為主商品。</span></div><div><span>已選 <b>'+count+'</b> 件</span>'+(count?'<button class="ops-button ghost small" type="button" data-action="product-merge-clear">清除</button>':'')+'<button class="ops-button primary" type="button" data-action="product-merge-open" '+(count<2?'disabled':'')+'>合併所選商品</button></div></div>';
  }
...
function ensureSalesClock(){
  ...
  function productListingDescriptionStatus(value){
    const text=clean(value),lines=text.replace(/\r/g,'').split('\n').map(clean).filter(Boolean),featureHeader=lines.some(function(line){return /^商品特色[：:]?$/.test(line);}),usageHeader=lines.some(function(line){return /^(?:使用方式(?:／適用情境)?|適用情境|使用重點|使用建議)[：:]?$/.test(line);}),specHeader=lines.some(function(line){return /^商品規格[：:]?$/.test(line);});let activeSection='',featureCount=0,usageCount=0,specificationCount=0,featurePoints=[],usagePoints=[];lines.forEach(function(line){if(/^商品特色[：:]?$/.test(line)){activeSection='features';return;}if(/^(?:使用方式(?:／適用情境)?|適用情境|使用重點|使用建議)[：:]?$/.test(line)){activeSection='usage';return;}if(/^商品規格[：:]?$/.test(line)){activeSection='specifications';return;}if(/^(?:包裝內容|適用對象|注意事項)[：:]?$/.test(line)){activeSection='';return;}const listItem=/^(?:\d+[.、]|[-•●])\s*\S+/.test(line),point=line.replace(/^(?:\d+[.、]|[-•●])\s*/,'');if(activeSection==='features'&&listItem){featureCount+=1;featurePoints.push(point);}if(activeSection==='usage'&&listItem){usageCount+=1;usagePoints.push(point);}if(activeSection==='specifications'&&(listItem||/[：:]/.test(line)))specificationCount+=1;});const genericOnly=/(?:本商品為柚子樂器販售的樂器或樂器配件|此商品尚待依精確型號、條碼、案件圖片與可驗證網路資料完成正式商品介紹)/.test(text)||(!featureHeader&&!usageHeader&&!specHeader&&/(?:商品內容與規格以|如需確認尺寸、相容性或包裝內容)/.test(text));
    const missing=[];
    if(!text)missing.push('商品介紹');
    if(!featureHeader||featureCount<PRODUCT_RICH_CONTENT_FEATURE_TARGET)missing.push('可驗證商品特色 10 點');
    if(!usageHeader||usageCount<PRODUCT_RICH_CONTENT_USAGE_TARGET)missing.push('使用方式／適用情境 10 點');
    if(!specHeader||specificationCount<1)missing.push('可驗證商品規格');
    const pointLengthReady=featurePoints.concat(usagePoints).every(function(point){return point.length>=PRODUCT_RICH_CONTENT_POINT_MINIMUM_CHARACTERS;});
    if(!pointLengthReady)missing.push('每個重點須為完整句子，約 '+PRODUCT_RICH_CONTENT_POINT_RECOMMENDED_CHARACTERS+' 個中文字');
    if(genericOnly)missing.push('通用備援文案尚未改寫');
    const targetComplete=featureCount>=PRODUCT_RICH_CONTENT_FEATURE_TARGET&&usageCount>=PRODUCT_RICH_CONTENT_USAGE_TARGET;
    return {ready:!!text&&!genericOnly&&featureHeader&&usageHeader&&specHeader&&specificationCount>0&&targetComplete&&pointLengthReady,genericOnly:genericOnly,featureCount:featureCount,usageCount:usageCount,specificationCount:specificationCount,featureTarget:PRODUCT_RICH_CONTENT_FEATURE_TARGET,usageTarget:PRODUCT_RICH_CONTENT_USAGE_TARGET,recommendedPointCharacters:PRODUCT_RICH_CONTENT_POINT_RECOMMENDED_CHARACTERS,targetComplete:targetComplete,standardVersion:PRODUCT_RICH_CONTENT_STANDARD_VERSION,missing:Array.from(new Set(missing))};
  }
  ...
  function productRichContentStandardHtml(row){
    ...
    return '<section class="ops-callout '+(ready?'green':'yellow')+'"><b>'+escapeHtml(heading)+'</b><br><span>'+escapeHtml(counts+'。'+rule)+'</span><br><span>先分析案件圖片、OCR 與原有文字，再以精確型號／條碼及可靠來源查證；資料不足就停止，不猜測湊數。</span><br><span>免責句固定放在詳細介紹最後兩張指定圖片之前；第二張指定圖片是絕對最後內容。</span></section>';
  }
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const sharedPlatformContent='<div class="ops-callout green"><b>四平台共用同一份已驗證內容</b><br><span>完整商品介紹會自動轉成 EasyStore 安全 HTML、蝦皮進階圖文、MOMO 原生圖文與酷澎安全 HTML／IMAGE＋TEXT；免責句後固定接兩張指定介紹圖並以第二張收尾。</span></div>';
  ...
  function requestProductImageCollection(action,payload){
    ...
    return new Promise(function(resolve,reject){
      const sessionId=clean(payload&&payload.sessionId),timer=global.setTimeout(function(){if(!productImageCollectionPending||productImageCollectionPending.sessionId!==sessionId)return;productImageCollectionPending=null;reject(new Error('找不到收圖助手，請先更新到柚子掌櫃助手 0.3.34 並重新整理頁面。'));},3500);
    ...
    if(message.type===PRODUCT_IMAGE_COLLECTION.sessionAck){
      ...
      if(payload.ok===true){
        if(pending.action==='start'&&!productImageCollectionVersionAtLeast(payload.extensionVersion,PRODUCT_IMAGE_COLLECTION.minimumVersion)){
          ...
          pending.reject(new Error('目前收圖助手版本過舊，請更新到 0.3.34，重新載入後再開始搜圖。'));
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const cards=orderedKeys.map(function(key){
      ...
      const helper=key==='shopee'&&row.autofillPayload?'<div class="ops-card-actions"><button class="ops-button primary" type="button" data-action="product-shopee-autofill-open">安全開啟 EasyStore／蝦皮</button><a class="ops-button soft" href="youzi-easystore-shopee-autofill-v0.3.34.zip" download>下載／更新助手 0.3.34</a></div><small>'+(shopeeHomeLogisticsNeedsManualConfirmation?'會先套用 EasyStore 進階圖文介紹，再帶入分類、品牌與商品屬性；一般宅配的實際物流仍須確認。商品身分依本案已凍結的處理方式與中央平台 ID 執行，不會自行改成新增或修改。':'會先套用 EasyStore 進階圖文介紹；商品身分依本案已凍結的處理方式與中央平台 ID 執行，不先搜尋平台目錄，也不會自行改成新增或修改。')+'</small>':'';
  ...
  async function openShopeeAutofillHelper(){
    ...
    else toast('助手沒有收到商品資料',clean(result&&result.error)||'請安裝或更新到助手 0.3.34，重新整理營運中心後，再按一次「安全開啟 EasyStore／蝦皮」。','warning');
    return result;
  }
  ...
  function productListingRequiredImageRoles(targetScope){
    const platforms=productListingTargetPlatforms(targetScope),roles=['cleanMain'];
    if(platforms.includes('easyStore'))roles.push('storefrontPortrait');
    if(platforms.includes('shopee'))roles.push('brandedHero');
    return roles;
  }
  function productListingCodexMediaSnapshot(raw,product,targetScope){
    ...
    productListingRequiredImageRoles(targetScope).forEach(function(role){if(sourceImageUrls.length&&!readyRows.some(function(row){return row.roles.includes(role);}))missingRequiredRoles.push(role);});
  ...
  function productListingCodexPreparedCase(raw,product,targetScope){
    const source=raw&&typeof raw==='object'?raw:{},normalized=normalizeProductListingCase(source,product),media=productListingCodexMediaSnapshot(source,product,targetScope);
  ...
  async function loadProductListingCodexHandoffCase(id,targetScope){
    const productId=clean(id),product=catalogById(productId);if(!productId||!product)throw new Error('找不到合併商品的中央主檔：'+productId);
    const snap=await state.db.collection(COLLECTIONS.listingCases).doc(productId).get(),raw=snap.exists?snap.data()||{}:{},media=productListingCodexMediaSnapshot(raw,product,targetScope);
    return Object.assign({productId:productId,sku:clean(raw.productSku||product.sku),name:clean(raw.productName||product.originalName||product.name),caseExists:snap.exists,representativeSourceImageUrl:safeUrl(raw.variantChildImageUrl||raw.variantGroupPrimaryImageUrl),preparedCase:productListingCodexPreparedCase(raw,product,targetScope)},media);
  }
  function mergeProductListingCodexQueuedMedia(item,queued,targetScope){
    ...
    productListingRequiredImageRoles(targetScope).forEach(function(role){if(sourceImageUrls.length&&!readyRows.some(function(image){return image.roles.includes(role);}))missingRequiredRoles.push(role);});
  ...
  function productListingSharedCompletedRows(cases){
    const groups=(Array.isArray(cases)?cases:[]).map(function(item){const prepared=item&&item.preparedCase||{},allRows=readyProductListingImageRows(prepared),conflicts=productListingConflictingRoleUrls(allRows),allowed=new Set(normalizeProductResearchSourceUrls(item&&item.gallerySourceImageUrls)),rows=allRows.filter(function(row){return !conflicts.has(row.url)&&(!allowed.size||allowed.has(row.sourceImageUrl)||allowed.has(row.url));});return rows.slice().sort(function(a,b){const ar=a.roles.includes('cleanMain')?0:a.roles.includes('storefrontPortrait')?1:a.roles.includes('brandedHero')?2:3,br=b.roles.includes('cleanMain')?0:b.roles.includes('storefrontPortrait')?1:b.roles.includes('brandedHero')?2:3;return ar-br||a.sourceOrder-b.sourceOrder;});}),result=[],seen=new Set(),covered=new Set();
    function push(row,groupIndex){if(!row||seen.has(row.url)||result.length>=PRODUCT_GROUP_LISTING_IMAGE_MAX)return false;seen.add(row.url);result.push(row);if(Number.isInteger(groupIndex))covered.add(groupIndex);return true;}
    function findRole(role,excludedGroups){const excluded=new Set(Array.isArray(excludedGroups)?excludedGroups:[excludedGroups]);for(let groupIndex=0;groupIndex<groups.length;groupIndex+=1){if(excluded.has(groupIndex))continue;const row=groups[groupIndex].find(function(candidate){return candidate.roles.includes(role);});if(row)return {row:row,groupIndex:groupIndex};}return null;}
    const clean=findRole('cleanMain',-1);if(clean)push(clean.row,clean.groupIndex);
    const branded=findRole('brandedHero',groups.length>1&&clean?[clean.groupIndex]:[])||findRole('brandedHero',[]);if(branded)push(branded.row,branded.groupIndex);
    const occupied=[clean,branded].filter(Boolean).map(function(item){return item.groupIndex;}),storefront=findRole('storefrontPortrait',groups.length>occupied.length?occupied:[])||findRole('storefrontPortrait',[]);if(storefront)push(storefront.row,storefront.groupIndex);
    groups.forEach(function(rows,groupIndex){if(covered.has(groupIndex)||result.length>=PRODUCT_GROUP_LISTING_IMAGE_MAX)return;const representative=rows.find(function(row){return row.roles.includes('cleanMain');})||rows.find(function(row){return row.roles.includes('variantRepresentative');})||rows.find(function(row){return row.roles.includes('localizedDetail')||row.roles.includes('specification');})||rows[0];push(representative,groupIndex);});
    let index=0;while(result.length<PRODUCT_GROUP_LISTING_IMAGE_MAX&&groups.some(function(rows){return index<rows.length;})){groups.forEach(function(rows,groupIndex){push(rows[index],groupIndex);});index+=1;}
    return result;
  }
  function productListingPlatformImagePlan(rows,frozenSourceImageUrls){
    ...
    const pool=(Array.isArray(rows)?rows:[]).filter(function(row){return row&&row.url&&row.sourceImageUrl&&row.url!==row.sourceImageUrl&&!frozenSources.has(row.url);}).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);
    ...
    const cleanFirst=cleanRows[0]||null,brandedFirst=brandedRows[0]||null,storefrontFirst=storefrontRows[0]||null,rest=pool.map(function(row){return row.url;});
    ...
    const easyStore=unique([storefrontFirst&&storefrontFirst.url,cleanFirst&&cleanFirst.url].concat(rest));
    const shopee=unique([brandedFirst&&brandedFirst.url,cleanFirst&&cleanFirst.url].concat(rest));
  ...
  function productListingDecisionContract(canonicalCategoryDecision){
    return {
      version:1,mode:'deterministic-workflow-with-structured-judgment',immutableForJob:true,
  ...
  async function loadProductListingCodexHandoffSnapshot(product){
    ...
    const cases=await Promise.all(ids.map(function(caseId){return loadProductListingCodexHandoffCase(caseId,normalized.listingTargetScope);})),queueById=new Map(queue.map(function(item){return [clean(item&&item.productId),item||{}];}));
    cases.forEach(function(item){const queued=queueById.get(item.productId)||{};mergeProductListingCodexQueuedMedia(item,queued,normalized.listingTargetScope);item.representativeSourceImageUrl=safeUrl(queued.representativeSourceImageUrl)||item.representativeSourceImageUrl;item.gallerySourceImageUrls=normalizeProductResearchSourceUrls(queued.gallerySourceImageUrls||item.gallerySourceImageUrls).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);item.preparedCase.gallerySourceImageUrls=item.gallerySourceImageUrls.slice();item.representativeCompletedImageUrl=productListingRoleImageUrl(item.preparedCase,'cleanMain',item.representativeSourceImageUrl);});
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '橫向商品圖片固定保留最多 6 張商品圖，最後一張固定加入 product-listing-store-promo.png 店址圖；店址圖只能是橫向圖庫最後一張，不得拿來當商品首圖、MOMO 廣告圖或專推圖。',
      '詳細介紹固定依序為：商品介紹與商品特色→第 1 張商品圖→商品規格→第 2 張商品圖→使用方式／適用情境與使用建議→第 3 張商品圖→其餘商品圖→顧客版實體圖（若有）→固定免責句「商品圖片與規格僅供參考，實際內容以收到的實體商品為準。」→product-listing-description-promo-1.jpg→product-listing-description-promo-2.jpg。兩張固定介紹圖是詳細介紹的絕對最後兩個內容區塊，第二張後面不得再有文字或圖片；免責句固定緊接在第一張固定介紹圖之前。商品圖不足三個區塊時先從本案已保存來源、官網既有圖或精確商品來源補齊並完成繁體化；仍不足時可依上方橫向商品圖順序重複使用已完成商品圖，重複是允許的。不得把店址圖或兩張固定介紹圖拿來補三個商品圖區塊。',
      'EasyStore 官網必須先保存完整安全 HTML 與全部固定圖片順序；蝦皮必須使用同一官網內容並實際核對文字與全部預期圖片都已保存，畫面仍顯示 0 張、只匯入文字或按鈕成功都不算完成。酷澎可編輯詳細說明時必須保存同一圖文順序並重開確認；MOMO 必須用原生圖文編輯器保存同一順序、重開確認圖片仍在，且專推圖仍使用另一張合規乾淨商品圖。任何平台缺少文字、商品圖、店址尾圖、兩張固定介紹尾圖或免責句，都不得標記 verified。',
      '[案件摘要]',
      caseLines.join('\n'),
      '[兩階段固定快照]',
      ...
      '有幾張 physicalImageUrls 就在平台圖文介紹容量內使用幾張，固定接在一般繁體完成介紹圖之後、最終實體商品免責句之前；EasyStore、蝦皮進階描述、MOMO 圖文介紹與酷澎詳情都沿用同一順序。實體圖永遠不得成為平台第 1 張主圖。',
      '[圖片一次定案]',
      '每個案件最多 20 張 selectedReferenceImageUrls；每一張來源只做一輪完整檢查與台灣繁體化，同輪完成簡轉繁、大陸用語改成台灣常用說法、錯字／亂碼、被切半的文字或殘缺裝飾的移除或重排及必要版面調整。generatedListingImages 的每一個完成輸出都必須保留 sourceImageUrl、roles 與 assetFlags（至少明列 containsLogo、containsText、containsContactInfo、containsQrCode、greenBrandTemplate、momoPromotionEligible）；同一 sourceImageUrl 可以有多個完成輸出，不得用 Map(source→單一 url) 覆蓋；(sourceImageUrl, role) 必須唯一，而且同一完成圖 URL 不得同時兼任互斥的 cleanMain、storefrontPortrait 與 brandedHero。roles 只准使用 cleanMain、storefrontPortrait、brandedHero、localizedDetail、specification、variantRepresentative；本次只強制完成目標通路需要的角色：'+requiredImageRoles.join('、')+'。',
      '同一輪圖片處理必須自動製作本次目標通路需要的全部平台成品，不得把來源原圖直接當完成圖，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 固定使用 product-listing-brand-template-portrait.png，尺寸 750×1000 px、3:4；包含蝦皮時 brandedHero 固定使用 product-listing-brand-template-square.png，尺寸 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1；另依已查證事實製作 localizedDetail、specification、細項代表圖及 MOMO 專推圖。兩張品牌母版版本固定為 '+PRODUCT_BRAND_TEMPLATE_CONTRACT.version+'，必須保存 templateVersion、templateAssetSha256 與 templateComposition；缺少或雜湊不符就在進平台前停止。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是純白底、無品牌框、無新增 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得加入任何文字；商品本體原有且不可分離的印字必須保留，商品最長邊至少占 80%，在不裁切商品下以 90～95% 為目標。storefrontPortrait 與 brandedHero 必須從上述不可變母版合成，綠色頁首背景、頁首高度、紅色標語「有音樂的生活更有風格」、右上圓形柚子樂器 Logo、下方淺色面板與細綠邊都不得改色、縮窄、移位、換字或重畫；只准在下方內容面板內更換商品、已查證文字及最多 2 個有來源依據的細節小圖。storefrontPortrait 用 3～5 個已查證賣點；brandedHero 用 1～3 個已查證短賣點。兩者不得加入地址、電話、QR Code、價格、導外訊息、左下娃娃、PIC COLLAGE 或虛構功能／配件，也不得含殘缺文字。',
      ...
      '[共用圖片池與平台角色]',
      '全部編號的完成輸出公平合併為同一個共用池，整組最多 12 個不同完成圖 URL，不是每個編號各 12 張。上架圖片預覽的勾選範圍是唯一允許上架的圖片範圍；未勾選的來源與完成圖不得加入任何平台圖庫。在勾選範圍內只優先保留本次需要的 '+requiredImageRoles.join('、')+'，再依各細項公平補足。介面第一次會自動勾選最多 12 張且至少保留 1 張；保存原圖時先依 sourceImageUrl 找到完成輸出，保存完成圖 URL 時可直接使用。不得使用簡體原圖。',
      ...
      '[商品內容]',
      '商品標題依「品牌＋型號＋商品種類＋重要規格或材質」組成；可靠的品牌與型號優先，共同標題只寫共同事實，差異寫細項名稱。標題、內文、圖卡都不得加入「柚子樂器」或任何未指定店名、導外訊息；只有上述三張管理者指定固定圖片可保留原有門市與服務資訊。AI 第一步必須逐張分析案件原始圖片、圖片可辨識文字（OCR）與原有商品文字，再以精確型號或條碼確認身分，接著依序參考製造商官方規格／說明書、台灣代理商或經銷商資料，及多個可信的使用指南與真實使用經驗；不得跳過案件資料直接生成，也不得把單一匿名評論當作規格事實。每個特色、使用重點與規格屬性都必須在 fieldEvidence 指向來源。每個案件的 productDescriptionStatus 不是 ready，或 productDescription 只有通用提醒、商品編號、免責文字時，一律視為內容尚未完成；Codex 必須先整理並寫回每一份案件，後端重讀確認後才可建立 preparedSnapshot。新商品與既有商品都必須完成後才可結束本次發布，並保留同一 SKU 與平台商品編號。正式內容包含「商品特色」「使用方式／適用情境」「商品規格」三段：固定 10 點不重複、具體且可驗證的商品介紹／特色，固定 10 點有來源的使用方法、適用情境或使用心得，每點約 20 個中文字，並完整列出所有查證得到的平台屬性、尺寸、材質、段數、重量、內容物與細項差異。資料不足時必須繼續查證；仍不足就標示缺資料並停止發布，不得猜測、湊數或用通用備援文案代替。保固只填平台保固欄，不寫進介紹。本次 '+targetNamesText+'沿用同一份已完成介紹並轉成各自格式；免責句固定放在兩張指定介紹圖之前，而兩張指定介紹圖是詳細介紹絕對最後內容。',
      '蝦皮的進階商品描述必須在進入平台前完整準備：EasyStore body_html、介紹圖片、圖片順序、免責句、最後兩張固定介紹圖與內容指紋全部寫入同一份 preparedSnapshot。進入 EasyStore 官方蝦皮通路頁後只做一次輕量功能檢查；立即開啟「進階商品描述」並按「使用 EasyStore 的產品描述」，把官網文字與介紹圖片一起帶入。按鈕成功不等於完成：必須核對編輯器已有文字及全部預期圖片，免責句緊接在兩張固定介紹圖之前，而兩張固定介紹圖是絕對最後內容；若只帶入文字或漏圖，立即在同一編輯器補入同一 preparedSnapshot 的缺漏完成圖並恢復固定順序，再核對一次。禁止誤報完成或發布純文字版本，也不得重新分析或改寫文案。',
      '同一份已驗證商品介紹只整理一次，進站前只產生 listingTargetPlatforms 對應的固定交付格式並寫入 preparedPlatformFieldPlan：EasyStore 使用安全完整 HTML；蝦皮使用進階圖文描述，不貼原始 HTML且不得純文字發布；MOMO 使用原生文字與圖片區塊；酷澎優先使用安全 HTML，平台不接受原始 HTML 時改送保持相同順序的 IMAGE／TEXT contentDetails。MOMO 圖文與專推素材最多 20 張，每張寬 1000px、高 1500px 以下且小於 500KB。單一商品、同款多細項、合併既有商品與加入既有細項都沿用相同內容計畫；平台頁只負責套用，不得重新寫文案、漏圖片或另建第二套內容。',
      '[固定發布與核對]',
      ...
      '同一批後續每件商品及每個平台階段開始前只做輕量存活檢查，不重新掃描整頁。右側內建瀏覽器既有且可讀的已登入頁籤是第一順位工作階段證據；單一深層網址、頁籤或當下連結卡住不能判定平台進不去。必須依序嘗試目前已登入頁籤、同一官方網址的新工作頁籤、平台固定入口、同一草稿網址、桌面捷徑或瀏覽器歷史、專案紀錄／先前成功上下文中的入口，最後才使用保存帳密重新登入。恢復後沿用原 jobId、SKU、草稿 ID、preparedSnapshot 與 currentStage 續做，不得建立重複商品。不得因可恢復的頁籤或登入失效詢問是否重開、是否登入或是否繼續，也不得停止或回報該平台完成；只有缺少無法自動補齊的資料、MOMO OTP、CAPTCHA、平台明確拒絕、帳號停用或保存帳密確實被拒絕時才停止。',
      ...
      'EasyStore 圖片優先使用既有上傳方式；若本機檔案選擇器未把檔案帶入，直接在同一商品頁改用 generatedListingImages 的公開完成圖網址批次加入，不得重做圖片。物流必須先依包裝長寬高與重量判斷：大型商品關閉超商；蝦皮大型商品只保留符合材積級距的新竹物流，MOMO 固定甲指第三方 000001 新竹物流，酷澎使用已確認實際交由新竹物流／新竹貨運承運的對應自有或第三方宅配。嘉里大榮、黑貓及其他未配合承運商一律關閉；小型商品才依 preparedPlatformFieldPlan 開啟已核實超商選項。',
      ...
      'MOMO 專推圖固定走同一草稿的商品詳細介紹編輯器：平台外先從 preparedPlatformFieldPlan.momo.preparedFields.firstSubmitMediaGate 取得已完成專推圖、唯一素材檔名與指紋；進站後按「上傳圖片」→「從素材銀行選擇」→需要時先上傳該完成圖→以唯一檔名精確搜尋→選取並確認。插入前先檢查同一素材是否已存在，避免重複。主圖或廣告圖不能當成專推圖證據；商品主圖、廣告用圖、編輯器專推圖三處完成並儲存後，只重開同一草稿一次確認專推圖仍存在，隨即第一次發布，不得先送空缺版本再回頭補。發布後以 MOMO 品號或原廠編號精確核對一次。',
  ...
  function productListingCodexActivationPrompt(product,snapshot){
    ...
    return [
      ...
      '工作用途：'+(clean(snapshot&&snapshot.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹':'標準上架')+'；四種商品處理方式一律套用 '+PRODUCT_DESCRIPTION_LAYOUT_VERSION+' 固定圖文編排，缺少任何固定圖片不得完成。',
