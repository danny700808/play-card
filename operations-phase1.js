...
function renderOverviewV7(){
  ...
  function physicalProductImageTrayHtml(urls,productId){
    const id=clean(productId),rows=normalizeProductResearchSourceUrls(urls).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX),remaining=Math.max(0,PRODUCT_PHYSICAL_IMAGE_MAX-rows.length);
    return '<div class="ops-listing-physical-inline"><div id="productPhysicalImageTray" class="ops-listing-physical-tray">'+rows.map(function(url,index){return '<article class="ops-listing-physical-thumb"><a href="'+attr(url)+'" target="_blank" rel="noopener"><img src="'+attr(url)+'" alt="實體圖 '+(index+1)+'"><strong>實體圖</strong></a><button type="button" data-action="product-physical-image-remove" data-id="'+attr(id)+'" data-url="'+attr(url)+'" aria-label="移除實體圖 '+(index+1)+'">×</button></article>';}).join('')+(remaining?'<label class="ops-listing-physical-add"><input id="productPhysicalImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden><b>＋ 實體圖</b></label>':'')+'</div><div id="productPhysicalImageUploadStatus" aria-live="polite"></div></div>';
  }
...
function ensureSalesClock(){
  ...
  function productSupplierQuickLinksHtml(){
    return '<div class="ops-listing-collector-links"><a href="https://www.taobao.com/" target="_blank" rel="noopener">開啟淘寶</a><a href="https://www.1688.com/" target="_blank" rel="noopener">開啟 1688</a></div>';
  }
  ...
  function productListingModeSectionHtml(p,row){
    const parent=row.variantParentProductId?catalogById(row.variantParentProductId):null,suggestedAttribute=productVariantAttributeSuggestion(p,parent),storedAttribute=clean(row.variantAttributeName),attributeName=!storedAttribute||(storedAttribute==='款式'&&suggestedAttribute!=='規格')?suggestedAttribute:(storedAttribute==='款式'?'規格':storedAttribute),intent=normalizeProductListingIntent(row.listingIntent,row,p),policy=productListingIntentPolicy(intent);
    const mappings=[['官網',productPlatformMappingId(p,'easyStore')],['蝦皮',productPlatformMappingId(p,'shopee')],['MOMO',productPlatformMappingId(p,'momo')],['酷澎',productPlatformMappingId(p,'coupang')]].filter(function(item){return item[1];}).map(function(item){return item[0]+' '+item[1];}).join('｜')||'尚未保存平台商品編號';
    function choice(value,title,detail){return '<label class="'+(intent===value?'selected':'')+'"><input type="radio" name="listingIntent" value="'+attr(value)+'" '+(intent===value?'checked':'')+'><span><b>'+escapeHtml(title)+'</b><small>'+escapeHtml(detail)+'</small></span></label>';}
    return '<div class="ops-listing-mode-card"><div class="ops-listing-mode-heading">這次要怎麼處理商品</div><div class="ops-listing-mode-options" role="radiogroup" aria-label="商品處理方式">'
      +choice('create-single','新增獨立商品','一個 SKU 建立一個全新商品')
      +choice('create-group','新增多細項商品','多個顏色、尺寸或規格放在同一商品')
      +choice('add-variant','加入既有商品細項','保留原商品，再加入新的細項與介紹')
      +choice('update-existing','修改既有商品','同一 SKU 原地修正，不建立第二筆')+'</div>'
      ...
      +'<div class="ops-listing-variant-fields '+(intent==='add-variant'?'':'is-hidden')+'"><div class="ops-field ops-listing-variant-search-step"><label class="ops-required">先搜尋要加入的原商品</label><input class="ops-input" type="search" name="variantParentSearch" autocomplete="off" value="'+attr(productVariantParentSearchValue(p.docId,row.variantParentProductId))+'" placeholder="輸入商品編號或名稱"><input type="hidden" name="variantParentProductId" value="'+attr(row.variantParentProductId)+'"><div id="productVariantParentSearchResults" class="ops-listing-variant-search-results"></div></div><div id="productVariantParentSelection" class="ops-listing-variant-selection '+(parent?'':'is-hidden')+'"><div class="ops-field"><label class="ops-required">這組商品以什麼區分</label><input class="ops-input" name="variantAttributeName" value="'+attr(attributeName)+'" placeholder="例如：顏色、尺寸、規格"></div><div id="productVariantParentPreview">'+productVariantParentPreviewHtml(parent,p,row)+'</div></div></div>'
      +'<div class="ops-listing-update-fields '+(intent==='update-existing'?'':'is-hidden')+'"><div class="ops-callout green"><b>只修改同一件商品</b><br><span>'+escapeHtml(mappings)+'；系統會保留沒有要求修改的內容，並阻止重複建立。</span></div></div>'
      +'<div class="ops-field ops-listing-intent-instructions"><div class="ops-listing-field-heading"><label id="listingChangeInstructionsLabel">這次要增加或修改的內容</label><button class="ops-button ghost small" type="button" data-action="product-listing-speech" data-target="listingChangeInstructions">🎙 說話輸入</button></div><textarea class="ops-textarea compact" name="listingChangeInstructions" placeholder="把要增加、合併或修正的地方寫在這裡">'+escapeHtml(row.listingChangeInstructions)+'</textarea><small id="listingChangeInstructionsHint">'+escapeHtml(policy.description)+'</small></div></div>';
  }
  ...
  function updateProductListingMode(form){
    if(!form)return;const selected=query('[name="listingIntent"]:checked',form),intent=PRODUCT_LISTING_INTENTS.includes(clean(selected&&selected.value))?clean(selected.value):'create-single',adding=intent==='add-variant',groupEnabled=intent==='create-group',updating=intent==='update-existing',variantFields=query('.ops-listing-variant-fields',form),groupWrapper=query('.ops-listing-group-wrapper',form),updateFields=query('.ops-listing-update-fields',form),mode=query('[name="listingMode"]',form),groupToggle=query('[name="variantGroupEnabled"]',form),policy=productListingIntentPolicy(intent);
    if(mode)mode.value=adding?'add-variant':'independent';if(groupToggle)groupToggle.value=groupEnabled?'1':'0';if(variantFields)variantFields.classList.toggle('is-hidden',!adding);if(groupWrapper)groupWrapper.classList.toggle('is-hidden',!groupEnabled);if(updateFields)updateFields.classList.toggle('is-hidden',!updating);
    ...
    const instructions=query('[name="listingChangeInstructions"]',form),label=byId('listingChangeInstructionsLabel'),hint=byId('listingChangeInstructionsHint');if(instructions){instructions.required=updating;instructions.placeholder=updating?'例如：舊規格有誤，依新照片替換舊圖並改成新版介紹':adding?'例如：新增藍色款，將藍色圖片與差異補進原本介紹':'可補充商品特色、各細項差異或上架注意事項';}if(label)label.textContent=updating?'要修改或替換哪裡（必填）':adding?'新細項要增加到原商品的內容':'這次要增加或整理的內容';if(hint)hint.textContent=policy.description;
