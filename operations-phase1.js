...
function ensureSalesClock(){
  ...
  function productListingCodexHandoffPrompt(product,draft){
    return [
      ...
      '請先從案件讀取已選圖片、圖片修改要求、商品網址、價格、庫存、物流與細項設定；平台售價空白時自動沿用商品資料的共同網路售價、網路售價或門市售價，不要因平台售價欄位空白而停止。分類限定在樂器／樂器配件內自動判斷。圖片只做一次完整檢查與台灣繁體化，所有簡體字改為繁體字，中國大陸用語改為台灣常用說法。細項代表圖欄位保存的是原圖選擇，必須依 generatedListingImages 的 sourceImageUrl 對應到完成圖 url，四通路只能使用這張繁體完成圖；找不到對應完成圖時要停止，不可直接上架簡體原圖。不要重新呼叫網站的 OpenAI 文案或圖片 API。資料齊全後直接完成四通路發布。'
    ].join('\n');
  }
