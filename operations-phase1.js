(function(global){
  ...
  const VERSION = '2026.07.11-v5.1-member-full-flow';
  ...
  function productCard(p){
    const image=p.imageUrls[0]||'',reward=p.saleRewardPercent==null?membershipRuleForDate(new Date()).rewardPercent:p.saleRewardPercent;
    return '<button type="button" class="ops-product-card ops-product-card-full" data-action="product-edit" data-id="'+attr(p.docId)+'"><div class="ops-product-image">'+(image?'<img loading="lazy" src="'+attr(image)+'" alt="'+attr(p.originalName||p.name)+'" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"><div class="placeholder" style="display:none">無圖</div>':'<div class="placeholder">無圖</div>')+'</div><div class="ops-product-body"><div class="ops-product-key"><div><span>商品編號</span><b>'+escapeHtml(p.sku||'未設定')+'</b></div><div><span>庫存</span><b>'+escapeHtml(formatNumber(p.currentStock))+'</b></div></div><div class="ops-product-text"><span>Excel 原始名稱</span><b>'+escapeHtml(p.originalName||p.name||'未命名')+'</b></div><div class="ops-product-text online"><span>網路名稱</span><b>'+escapeHtml(p.onlineName||'未配對')+'</b></div><div class="ops-product-detail-grid"><div><span>門市定價</span><b>'+money(p.storePrice)+'</b></div><div><span>網路售價</span><b>'+money(p.onlinePrice)+'</b></div><div><span>進貨成本</span><b>'+money(p.latestPurchaseCost)+'</b></div><div><span>平均成本</span><b>'+money(p.averageCost)+'</b></div><div><span>可售庫存</span><b>'+formatNumber(p.availableStock)+'</b></div><div><span>會員回饋</span><b>'+formatNumber(reward)+'%</b></div></div></div></button>';
  }
