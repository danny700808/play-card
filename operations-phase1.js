(function(global){
  ...
  const PRODUCT_BRAND_CREATIVE_STYLE_CATALOG = Object.freeze([
    ['bold-coral-impact','珊瑚撞色','海報撞色'],['light-industrial','淺鋼工業','工業'],['sunrise-racing','晨光競速','速度'],['bright-warning','明亮警示','強調'],['daylight-rock','日光搖滾','搖滾'],
    ['pastel-street','粉彩街頭','街頭'],['athletic-white','運動白場','運動'],['orange-impact','橘色衝擊','熱力'],['comic-bright','明亮漫畫','漫畫'],['oversized-type-light','亮底巨字','字體'],
    ['pale-wilderness','淡野蒼涼','蒼涼'],['mist-city','霧城日常','城市'],['sunrise-camp','朝陽露營','戶外'],['sand-travel','沙色旅行','旅行'],['high-key-monochrome','高調單色','極簡'],
    ['matte-sage','霧鼠尾草','自然'],['winter-daylight','冬日亮光','冷冽'],['light-vintage','淡彩復古','復古'],['bright-film','明亮底片','底片'],['eastern-negative-space','東方留白','東方'],
    ['sunshine-lifestyle','陽光生活','生活'],['weekend-travel','週末出走','旅行'],['urban-commute','城市通勤','都會'],['campus-youth','校園青春','青春'],['warm-family','暖日家庭','溫暖'],
    ['natural-organic','自然有機','自然'],['handmade-collage','手作拼貼','手作'],['outdoor-picnic','戶外野餐','樂活'],['cafe-editorial','咖啡編輯','編輯'],['retro-lifestyle','復古生活','復古'],
    ['ivory-copper-premium','象牙銅質感','質感'],['cream-gold-premium','奶油金質感','質感'],['bright-magazine','明亮雜誌','雜誌'],['swiss-grid','瑞士格線','現代'],['minimal-luxury','清亮極簡','極簡'],
    ['blueprint-light','淺藍圖紙','技術'],['clean-tech','清爽科技','科技'],['product-lab','明亮實驗室','專業'],['museum-catalog','博物館目錄','典藏'],['daylight-showcase','日光展售','展示'],
    ['cream-vinyl','奶油黑膠','音樂'],['daylight-live','白晝現場','現場'],['soundwave-light','明亮聲波','音樂'],['pastel-synth','粉彩合成器','電子'],['daytime-jazz','日間爵士','爵士'],
    ['classical-ivory','象牙古典','古典'],['indie-festival','清亮獨立祭','獨立'],['analog-cream','奶油類比','類比'],['instrument-workshop','樂器工房','工藝'],['music-storybook','音樂故事書','故事']
  ].map(function(row){return Object.freeze({id:row[0],name:row[1],family:row[2]});}));
  const PRODUCT_BRAND_TEMPLATE_CONTRACT = Object.freeze({
    ...
    logo:Object.freeze({anchor:'upper-right',diameterRatioOfCanvasHeight:0.28,overlapHeaderAndContent:true,selectedReference:'approved-large-overlap-version',onlyElementAllowedToCrossHeaderBoundary:true,mayNotCoverProductOrPrimaryCopy:true}),
    contentPanel:Object.freeze({narrowGreenBorderRequired:true,borderStyle:'continuous-thin-rounded-green-outline',borderWidthPx:{minimum:3,maximum:6},allCreativeLayersMustStayInsideBorder:true,minimumLightAreaRatio:0.65,maximumDarkAreaRatio:0.35,darkFullBleedForbidden:true}),
    creativeStyleSystem:Object.freeze({version:'youzi-light-commercial-style-catalog-v1',catalogSize:50,selectionMode:'random-without-replacement',resetOnlyAfterAllStylesUsed:true,preventImmediateRepeatAcrossCycles:true,assignmentScope:'root-product-group',sameStyleAcrossAspectRatios:true,sameStyleAcrossVariants:true,allowedAspectRatios:['1:1','4:3'],styles:PRODUCT_BRAND_CREATIVE_STYLE_CATALOG}),
    storefrontPortrait:Object.freeze({assetUrl:'product-listing-brand-template-portrait.png',assetSha256:'e3fe984de916395accec67a9bf251429e271c908e45608d2d45781759be5cb90',widthPx:1000,heightPx:750,aspectRatio:'4:3'}),
    brandedHero:Object.freeze({assetUrl:'product-listing-brand-template-square.png',assetSha256:'9cb2f59fd4d4d9b28257c2527c804cb7b135451953de65dd567198ec11cb9145',widthPx:1000,heightPx:1000,aspectRatio:'1:1'})
  });
...
const DEFAULT_PLATFORM_FEE_SETTINGS = {
  ...
  function normalizedProductBrandCreativeStyleAssignment(value){
    ...
    return {catalogVersion:PRODUCT_BRAND_TEMPLATE_CONTRACT.creativeStyleSystem.version,styleId:style.id,styleName:style.name,family:style.family,selectionMode:'persisted-random-without-replacement',sameStyleAcrossAspectRatios:true,sameStyleAcrossVariants:true,assignedAt:clean(source.assignedAt)};
  }
  async function reserveProductBrandCreativeStyle(productId,fallbackValue){
    ...
    return state.db.runTransaction(async function(transaction){
      ...
      const styleId=remaining.shift(),style=productBrandCreativeStyle(styleId),assignment={catalogVersion:PRODUCT_BRAND_TEMPLATE_CONTRACT.creativeStyleSystem.version,styleId:style.id,styleName:style.name,family:style.family,selectionMode:'persisted-random-without-replacement',sameStyleAcrossAspectRatios:true,sameStyleAcrossVariants:true,cycle:cycle,assignedAt:new Date().toISOString()};
...
function ensureSalesClock(){
  ...
  function readyProductListingImageRows(listingCase){
    ...
    return generated.filter(function(row){const brandRole=row.roles.find(function(role){return role==='storefrontPortrait'||role==='brandedHero';}),profile=brandRole&&PRODUCT_BRAND_TEMPLATE_CONTRACT[brandRole];return row.status==='ready'&&(row.localizationStatus==='completed'||row.qaStatus==='approved')&&!!row.sourceImageUrl&&!allSourceUrls.has(row.url)&&(!profile||(row.templateVersion===PRODUCT_BRAND_TEMPLATE_CONTRACT.version&&row.templateAssetSha256===profile.assetSha256&&row.templateComposition===PRODUCT_BRAND_TEMPLATE_CONTRACT.composition));});
  }
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '同一輪圖片處理只產生本次目標通路需要的首圖版型，不得替未選通路多做圖片，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 使用 1000×750 px、4:3；包含蝦皮時 brandedHero 使用 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1。兩張品牌首圖版本固定為 '+PRODUCT_BRAND_TEMPLATE_CONTRACT.version+'，必須保存 templateVersion、templateAssetSha256、templateComposition 與 brandCreativeStyleAssignment；缺少或不符就在進平台前停止。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是純白底、無品牌框、無新增 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得加入任何文字；商品本體原有且不可分離的印字必須保留，商品最長邊至少占 80%，在不裁切商品下以 90～95% 為目標。storefrontPortrait 與 brandedHero 的綠色頁首固定占整張高度 1/9；紅色標語「有音樂的生活更有風格」與圓形柚子樂器 Logo 的圖樣、文字、顏色不可改。Logo 固定採管理者選定的大型跨界版本，置於右上並向下壓入內容區；只有 Logo 可跨頁首邊界，且不得遮住商品或主要文字。下方四周固定保留連續、清楚可見的細綠色圓角框，所有照片、色塊、文字與裝飾都要停在框線內。內容區至少 65% 為奶油白、白、淺灰或明亮照片，禁止深色滿版；文字可使用不同但真實可讀的商業字體，不限定微軟正黑體。每個根商品從 50 種淺色商業風格以 random-without-replacement 抽一種，50 種未用完不得重複；同一商品的 1:1 與 4:3、同組所有細項都沿用 preflightSnapshot.brandCreativeStyleAssignment，只做比例重排。storefrontPortrait 與 brandedHero 均固定使用 3 個已查證特色；不足 3 個時先補查證資料，仍不足就停止主圖完成，不得虛構。商品本體不得被 Logo、標題、色塊或裝飾遮住。兩者不得加入地址、電話、QR Code、價格、導外訊息、左下娃娃、PIC COLLAGE 或殘缺文字。',
