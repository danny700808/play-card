...
function ensureSalesClock(){
  ...
  function productVariantImagePickerOptionsHtml(productId,selectedUrl,sourceImages,role){
    ...
    if(!rows.length)return '<small class="ops-listing-variant-picker-empty">尚無圖片；可把其他細項的圖片拖到這裡，或按「為此細項收圖」。</small>';
  ...
  function productVariantImagePickerHtml(product,selectedUrl,sourceImages,role){
    ...
    return '<div class="ops-listing-variant-image-actions"><strong>'+(completed?'繁體代表圖已完成':'請從原圖指定代表圖（必選）')+'</strong><small>'+(completed?'這個編號已使用完成圖，不再顯示簡體來源圖。':'上架時只會使用這張圖對應的繁體完成版，不會直接使用簡體原圖。')+'</small></div><div class="ops-listing-variant-image-picker" data-variant-image-picker data-id="'+attr(id)+'">'+(completed?'':productVariantImagePickerOptionsHtml(id,selectedUrl,sourceImages,role))+'</div>';
  }
  ...
  function productVariantRepresentativeCardHtml(product,options){
    options=options||{};const selected=safeUrl(options.selectedUrl),missing=!selected,role=clean(options.role),sourceImages=normalizeProductResearchSourceUrls(options.sourceImages),completed=clean(product&&product.imageSource)==='localized-clean-main',previewImages=completed?productVariantImages(product).slice(0,1):sourceImages.length&&selected?[selected]:productVariantImages(product).slice(0,1);
    return '<article class="ops-listing-variant-item '+(options.className||'')+'" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-field="'+attr(options.fieldName||'')+'" data-variant-image-dropzone="1"'+(options.primary?' data-variant-primary="1"':'')+'><div class="ops-listing-variant-thumbs" data-variant-image-preview>'+productVariantImageThumbsHtml(product,previewImages)+'</div><div class="ops-listing-variant-item-main"><span class="ops-listing-variant-kind">'+escapeHtml(options.kind||'細項商品')+'</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b>'+(options.valueLabel?'<label class="ops-listing-variant-name-field">'+escapeHtml(options.valueLabel)+'<input class="ops-input" '+(options.valueName?'name="'+attr(options.valueName)+'"':'')+' value="'+attr(options.value||'')+'" placeholder="留白時依商品資料自動判斷" '+(options.valueDisabled?'disabled':'')+'></label>':'')+'<input type="hidden" name="'+attr(options.fieldName||'variantImageUrl')+'" value="'+attr(selected)+'">'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status>'+(missing?'尚未選擇代表圖':'代表圖已選擇')+'</span><div class="ops-listing-variant-quick-actions"><button class="ops-button soft" type="button" data-action="product-variant-image-collection" data-id="'+attr(product.docId)+'">為此細項收圖</button><button class="ops-button soft danger '+(selected?'':'is-hidden')+'" type="button" data-action="product-variant-image-clear" data-id="'+attr(product.docId)+'" data-role="'+attr(role)+'">× 清除代表圖</button><small>也可把其他細項的圖片拖到此區</small></div>'+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div></article>';
  }
  function productVariantParentPreviewHtml(parent,child,row){
    ...
    return '<div class="ops-listing-variant-pair">'+productVariantRepresentativeCardHtml(parent,{kind:'原商品',role:'add-parent',fieldName:'variantParentImageUrl',selectedUrl:row&&row.variantParentImageUrl,sourceImages:parentSources,valueLabel:'細項顯示名稱（人工優先）',valueName:'variantParentAttributeValue',value:row&&row.variantParentAttributeValue||productVariantSuggestion(parent)||'原款'})+productVariantRepresentativeCardHtml(child,{kind:'現在要加入的商品',role:'add-child',fieldName:'variantChildImageUrl',selectedUrl:row&&row.variantChildImageUrl,sourceImages:childSources,valueLabel:'細項顯示名稱（人工優先）',valueName:'variantAttributeValue',value:row&&row.variantAttributeValue||productVariantSuggestion(child)})+'</div>';
  }
  function productVariantGroupItemHtml(product,item){
    const selected=normalizeProductResearchSourceUrls(item&&item.imageUrls)[0]||'',sourceImages=normalizeProductResearchSourceUrls(item&&item.sourceImageUrls).length?normalizeProductResearchSourceUrls(item.sourceImageUrls):cachedProductListingSourceImages(product.docId),missing=!selected,role='group-child-'+product.docId,completed=clean(product&&product.imageSource)==='localized-clean-main',previewImages=completed?productVariantImages(product).slice(0,1):sourceImages.length&&selected?[selected]:normalizeProductResearchSourceUrls(item&&item.completedImageUrls).concat(productVariantImages(product)).slice(0,1);
    return '<article class="ops-listing-variant-item" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-dropzone="1"><div class="ops-listing-variant-thumbs" data-variant-image-preview>'+productVariantImageThumbsHtml(product,previewImages)+'</div><div class="ops-listing-variant-item-main"><span class="ops-listing-variant-kind">同款商品</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b><label class="ops-listing-variant-name-field">細項顯示名稱（人工優先）<input class="ops-input" data-variant-group-value value="'+attr(item&&item.attributeValue||productVariantSuggestion(product))+'" placeholder="留白時依商品資料自動判斷"></label><textarea data-variant-group-images hidden>'+escapeHtml(selected)+'</textarea><textarea data-variant-completed-images hidden>'+escapeHtml(normalizeProductResearchSourceUrls(item&&item.completedImageUrls).join('\n'))+'</textarea>'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status>'+(missing?'尚未選擇代表圖':'代表圖已選擇')+'</span><div class="ops-listing-variant-quick-actions"><button class="ops-button soft" type="button" data-action="product-variant-image-collection" data-id="'+attr(product.docId)+'">為此細項收圖</button><button class="ops-button soft danger '+(selected?'':'is-hidden')+'" type="button" data-action="product-variant-image-clear" data-id="'+attr(product.docId)+'" data-role="'+attr(role)+'">× 清除代表圖</button><small>也可把上方或其他細項圖片拖到此區</small></div>'+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div><button class="ops-icon-button" type="button" data-action="product-variant-group-remove" data-id="'+attr(product.docId)+'" aria-label="移除細項">×</button></article>';
  }
  function productVariantGroupPrimaryItemHtml(product,row,attributeValue){
    return productVariantRepresentativeCardHtml(product,{kind:'目前商品',className:'ops-listing-variant-primary-item',role:'group-primary',primary:true,fieldName:'variantGroupPrimaryImageUrl',selectedUrl:row&&row.variantGroupPrimaryImageUrl,sourceImages:row&&row.referenceImageUrls,valueLabel:'細項顯示名稱（人工優先）',valueName:'variantGroupPrimaryValue',value:attributeValue});
  }
  ...
  function productVariantImageProcessingQueueHtml(queue){
    ...
    return '<section class="ops-listing-variant-processing"><div class="ops-listing-variant-processing-heading"><b>這組商品要處理的全部圖片</b><small>每個編號的圖片都會先完成繁體化；整組最多優先選 12 張，沒有勾選時由系統平均挑選各細項適合的完成圖。</small></div><div class="ops-listing-variant-processing-groups">'+queue.map(function(item){const source=item.sourceImageUrls||[],completed=item.completedImageUrls||[],images=completed.length?completed:source,status=completed.length?'已完成 '+completed.length+' 張':source.length?'待處理 '+source.length+' 張':'尚未加入圖片',selected=new Set(item.gallerySourceImageUrls||[]);return '<div class="ops-listing-variant-processing-group"><div><b>'+escapeHtml(item.sku||'未設定')+'</b><span>'+escapeHtml(item.name)+'</span><em>'+escapeHtml(status)+'</em></div><div>'+images.map(function(url){const checked=selected.has(url);return '<label class="'+(checked?'selected':'')+'"><input type="checkbox" name="variantGallerySourceImageUrls" data-product-id="'+attr(item.productId)+'" value="'+attr(url)+'" '+(checked?'checked':'')+'><img src="'+attr(url)+'" alt="'+attr(item.sku||'商品')+'"><span>'+(checked?'✓ 優先上架':'選為上架圖')+'</span></label>';}).join('')+'</div></div>';}).join('')+'</div></section>';
  }
  ...
  async function copyProductVariantReferenceImage(form,targetProductId,url,role){
    ...
    try{
      const response=await global.fetch(sourceUrl,{credentials:'omit'});if(!response.ok)throw new Error('圖片下載失敗（HTTP '+response.status+'）');const blob=await response.blob(),mime=/^image\/(?:jpeg|png|webp)$/.test(clean(blob.type))?clean(blob.type):'image/jpeg',extension=mime==='image/png'?'png':mime==='image/webp'?'webp':'jpg',file=new File([blob],'shared-variant-'+Date.now()+'.'+extension,{type:mime});
      const uploaded=await uploadProductVariantReferenceImages(form,id,[file]),copied=normalizeProductResearchSourceUrls(uploaded)[0];if(!copied)throw new Error('圖片已送出但沒有取得保存網址');
      await selectProductVariantRepresentativeImage(form,id,copied,role);toast('圖片已複製到細項',clean(catalogById(id)&&catalogById(id).sku)+'｜後續會以這個細項自己的來源完成繁體化','success');
  ...
  async function startProductImageCollection(form,targetProductId){
    ...
    try{await requestProductImageCollection('start',session);productImageCollectionUploadFailures=[];productImageCollectionSession=session;updateProductImageCollectionUi(form,'商品 '+session.sku+' 已鎖定；現在到供應商頁按 Ctrl＋Shift＋Y 或使用右鍵選單。');toast('框選收圖已啟動',session.sku+'｜最多 '+session.maxImages+' 張','success');}
  ...
  async function stopProductImageCollection(form){
    ...
    catch(error){stopError=error;if(form)updateProductImageCollectionUi(form,errorMessage(error),true);}
    try{await drainProductImageCollectionUploads(form);}
    finally{productImageCollectionSession=null;}
    if(stopError)throw stopError;
    updateProductImageCollectionUi(form,'已停止收圖；已加入的圖片都已保存。');
  }
  ...
  async function receiveProductImageCollectionFile(payload){
    ...
    }catch(error){postProductImageCollectionFileAck({requestId:requestId,ok:false,error:errorMessage(error),count:form?productImageCollectionCount(form,productId):0});updateProductImageCollectionUi(form,errorMessage(error),true);throw error;}
  }
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      'cleanMain 是無品牌框、無 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得含任何文字；brandedHero 才使用方形 1:1「柚子樂器淺色商業展示版」，保留頂端薄荷綠品牌區、紅色標語與右上標誌，移除左下寶寶及 PIC COLLAGE，商品去背後約占 55～65%，搭配 1～3 個已查證短賣點及最多 2 個有來源依據的輔助視覺。淺色展示底板左右必須各縮進約 14～16%，讓官網 3:4 裁切後仍看得到細綠邊；右上標誌也要完整放在中央安全區內。不得加入價格、聯絡資訊、浮水印、虛構功能／配件、被切半的文字或殘缺裝飾。',
