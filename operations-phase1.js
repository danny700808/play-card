(function(global){
  ...
  const VERSION = '2026.07.11-v4.0-member-pos';
  ...
  function render(){
    ...
    const renderers={overview:renderOverview,products:renderProducts,sales:renderSalesV4,customers:renderCustomersV4,receivables:renderReceivables,purchases:renderPurchases,rentals:renderRentals,cases:renderCases,finance:renderFinance,sync:renderSync,connection:renderConnection};
  ...
  async function saveCheckoutV4(form){
    if(!state.cart.length)throw new Error('銷售清單是空的');const data=new FormData(form),discount=Math.max(0,numberOrNull(data.get('discount'))||0),soldAt=new Date(clean(data.get('soldAt'))),customerId=clean(data.get('customerId')),paymentChoice=clean(data.get('paymentStatus'))||'paid',requestedPoints=Math.max(0,Math.floor(numberOrNull(data.get('pointsToRedeem'))||0));if(Number.isNaN(soldAt.getTime()))throw new Error('成交時間格式不正確');if(paymentChoice!=='paid'&&!customerId)throw new Error('未收款必須選擇客戶');if(paymentChoice!=='paid'&&requestedPoints>0)throw new Error('未收款不能使用點數');
    ...
    await writeAudit('完成現場銷售','storeSale',saleRef.id,saleNo);state.cart=[];state.posSearch='';state.selectedCustomerId='';closeDrawer();toast('現場銷售完成',saleNo,'success');await loadAll(true);
  }
  ...
  async function saveQuickIncome(form){
    ...
    await writeAudit('新增收入','quickIncome',ref.id,no+'｜'+money(amount));state.salesMode='product';state.selectedCustomerId='';closeDrawer();toast('收入已儲存',no,'success');await loadAll(true);
  }
  ...
  function handleAction(action,el){
    ...
    if(action==='checkout') return checkoutDrawerV4();
  ...
  async function handleSubmit(form){
    ...
    try{
      ...
      else if(form.id==='checkoutFormV4') await saveCheckoutV4(form);
