(function(global){
  ...
  const VERSION = '2026.07.11-v4.1-inline-checkout';
  ...
  function render(){
    ...
    const renderers={overview:renderOverview,products:renderProducts,sales:renderSalesV5,customers:renderCustomersV4,receivables:renderReceivables,purchases:renderPurchases,rentals:renderRentals,cases:renderCases,finance:renderFinance,sync:renderSync,connection:renderConnection};
  ...
  function membershipRuleActive(){
    const s=state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS;if(!s.enabled)return false;const today=dateText(new Date());return (!s.validFrom||today>=s.validFrom)&&(!s.validTo||today<=s.validTo);
  }
  function calculateEarnPoints(amount){const s=state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS;if(!membershipRuleActive()||Number(s.earnAmount)<=0)return 0;return Math.floor(Math.max(0,Number(amount||0))/Number(s.earnAmount))*Math.max(0,Math.floor(Number(s.earnPoints||0)));}
  function pointDiscount(points){const s=state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS;if(Number(s.redeemPoints)<=0)return 0;return Math.floor(Math.max(0,Number(points||0))/Number(s.redeemPoints))*Math.max(0,Number(s.redeemAmount||0));}
  function maxRedeemablePoints(customer,subtotal){
    if(!customer||customer.customerType!=='member'||!membershipRuleActive())return 0;const s=state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS,step=Math.max(1,Math.floor(Number(s.redeemPoints||1))),balance=Math.floor(Math.max(0,Number(customer.pointBalance||0))/step)*step,percent=Math.max(0,Math.min(100,Number(s.maxRedeemPercent||0))),maxDiscount=Math.max(0,Number(subtotal||0))*percent/100,units=Number(s.redeemAmount)>0?Math.floor(maxDiscount/Number(s.redeemAmount)):0,maxByAmount=units*step,result=Math.min(balance,maxByAmount);return result>=Math.max(step,Math.floor(Number(s.minRedeemPoints||1)))?result:0;
  }
  ...
  function renderInlineCheckout(){
    if(!state.cart.length)return '';const subtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),c=selectedCustomer(),walkin=!c||state.posCustomerMode==='walkin',settings=state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS,maxPoints=walkin?0:maxRedeemablePoints(c,Math.max(0,subtotal-Number(state.checkoutDiscount||0))),points=Math.min(maxPoints,Math.max(0,Math.floor(Number(state.checkoutPoints||0)))),pointValue=pointDiscount(points),total=Math.max(0,subtotal-Math.max(0,Number(state.checkoutDiscount||0))-pointValue),status=walkin?'paid':state.checkoutPaymentStatus;
    state.checkoutPoints=points;if(walkin){state.checkoutPaymentStatus='paid';state.checkoutReceived='';}
  ...
  async function saveCheckoutV4(form){
    ...
    await state.db.runTransaction(async function(tx){
      ...
      const prepared=[];let subtotal=0,costTotal=0,unknownCostQty=0;snaps.forEach(function(snap,index){if(!snap.exists)throw new Error('商品主檔不存在：'+state.cart[index].name);const raw=snap.data()||{},item=state.cart[index],current=Number(raw.currentStock||0),qty=Math.max(1,Math.round(Number(item.qty||0)));if(current<qty)throw new Error(item.name+' 庫存不足，目前 '+current);const unitPrice=Math.max(0,Number(item.unitPrice||0)),fifo=consumeFifo(raw,qty);subtotal+=qty*unitPrice;costTotal+=fifo.costTotal;unknownCostQty+=fifo.unknownCostQty;prepared.push({ref:refs[index],item:item,qty:qty,current:current,unitPrice:unitPrice,fifo:fifo});});
      const customer=customerRaw?normalizeCustomer(Object.assign({__id:customerId},customerRaw)):null,maxPoints=maxRedeemablePoints(customer,Math.max(0,subtotal-discount)),redeemStep=Math.max(1,Math.floor(Number((state.membershipSettings||DEFAULT_MEMBERSHIP_SETTINGS).redeemPoints||1)));if(requestedPoints>maxPoints)throw new Error('可使用點數不足');if(requestedPoints%redeemStep!==0)throw new Error('點數請依設定單位使用');const pointsRedeemed=requestedPoints,pointValue=pointDiscount(pointsRedeemed),total=Math.max(0,subtotal-discount-pointValue),enteredReceived=numberOrNull(data.get('receivedAmount')),receivedAmount=paymentChoice==='paid'?total:Math.min(total,Math.max(0,enteredReceived||0)),actualStatus=receivedAmount>=total?'paid':receivedAmount>0?'partial':'unpaid',pendingPoints=customer&&customer.customerType==='member'?calculateEarnPoints(total):0,pointsEarned=actualStatus==='paid'?pendingPoints:0,grossProfit=total-costTotal;
      tx.set(saleRef,{saleNo:saleNo,soldAt:soldAt,items:prepared.map(function(x){return {productId:x.item.productId,name:x.item.name,sku:x.item.sku,imageUrl:x.item.imageUrl||'',qty:x.qty,unitPrice:x.unitPrice,lineTotal:x.qty*x.unitPrice,lineCost:x.fifo.costTotal,fifoBreakdown:x.fifo.breakdown,unknownCostQty:x.fifo.unknownCostQty};}),subtotal:subtotal,manualDiscount:discount,pointDiscount:pointValue,discount:discount+pointValue,total:total,costTotal:costTotal,grossProfit:grossProfit,unknownCostQty:unknownCostQty,costMethod:'FIFO',paymentMethod:clean(data.get('paymentMethod')),paymentStatus:actualStatus,receivedAmount:receivedAmount,customerId:customer?customer.id:'',customerName:customer?customer.name:'',customerType:customer?customer.customerType:'walk_in',memberNo:customer?customer.memberNo:'',pricingTier:customer?customer.pricingTier:'retail',pointsEarned:pointsEarned,pendingPointsEarned:actualStatus==='paid'?0:pendingPoints,pointsRedeemed:pointsRedeemed,note:'',status:'completed',createdAt:serverTimestamp(),createdBy:userLabel(),version:VERSION});
    ...
    await writeAudit('完成現場銷售','storeSale',saleRef.id,saleNo);state.cart=[];state.posSearch='';state.selectedCustomerId='';state.posCustomerMode='walkin';state.posMemberSearch='';state.checkoutPaymentMethod='現金';state.checkoutPaymentStatus='paid';state.checkoutDiscount=0;state.checkoutPoints=0;state.checkoutReceived='';toast('現場銷售完成',saleNo,'success');await loadAll(true);
  }
  ...
  async function saveSaleEdit(form){
    ...
    await state.db.runTransaction(async function(tx){
      ...
      tx.set(saleRef,{items:prepared.map(function(x){return {productId:x.requested.productId,name:clean(x.oldItem.name),sku:clean(x.oldItem.sku),imageUrl:clean(x.oldItem.imageUrl),qty:x.requested.qty,unitPrice:x.requested.unitPrice,lineTotal:x.lineTotal,lineCost:x.fifo.costTotal,fifoBreakdown:x.fifo.breakdown,unknownCostQty:x.fifo.unknownCostQty};}),subtotal:subtotal,manualDiscount:manualDiscount,pointDiscount:pointValue,discount:manualDiscount+pointValue,total:total,costTotal:costTotal,grossProfit:total-costTotal,unknownCostQty:unknownCostQty,paymentMethod:paymentMethod,paymentStatus:paymentStatus,receivedAmount:receivedAmount,pointsEarned:newEarned,pendingPointsEarned:newPending,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
  ...
  function handleAction(action,el){
    ...
    if(action==='pos-customer-mode'){state.posCustomerMode=el.dataset.mode||'walkin';if(state.posCustomerMode==='walkin'){state.selectedCustomerId='';state.posMemberSearch='';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;}return render();}
    if(action==='pos-member-select'){state.selectedCustomerId=el.dataset.id||'';state.posCustomerMode='member';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;return render();}
    ...
    if(action==='membership-settings') return openMembershipSettings();
  ...
  function bindEvents(){
    ...
    document.addEventListener('input',function(event){
      ...
      else if(event.target.closest('#checkoutFormInline')&&event.target.name==='pointsToRedeem'){state.checkoutPoints=Math.max(0,Math.floor(Number(event.target.value||0)));updateInlineCheckoutTotals();}
