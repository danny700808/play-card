(function(global){
  ...
  const VERSION = '2026.09.03-full-commercial-poster-v1';
  ...
  const PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER = '商品圖片與文字說明僅供參考；不同批次的包裝、印刷、配色或細節可能略有差異，實際內容以收到的商品為準。';
  const PRODUCT_WARRANTY_SUPPORT_NOTICE = '保固會依商品類型而有所不同。耗材及正常使用產生的自然耗損不在一般保固範圍；若商品附有原廠保固，則以原廠提供的保固時間與方式為主。收到商品若發現新品本身有異常，歡迎聯絡我們協助確認與處理。';
  ...
  const PRODUCT_DESCRIPTION_MEDIA_REFRESH_INSTRUCTIONS = '[適用範圍：共用｜狀態：已確認] 原地修改同一 SKU 與既有平台商品：重新整理並驗證所選通路的商品介紹與圖片。固定圖文順序為商品介紹與特色→商品圖一→商品規格→商品圖二→使用建議→商品圖三→其餘商品圖→實體商品說明→出貨與保固說明→兩張固定介紹圖；橫向商品圖最後一張固定為店址圖。[適用範圍：EasyStore／蝦皮｜狀態：已實頁驗證] 蝦皮必須實際確認文字、全部圖片、固定說明與最後兩張介紹圖均已保存；缺圖不得判定完成。';
  const PRODUCT_LISTING_PLATFORM_ORDER = ['momo','coupang','easyStore','shopee'];
  const PRODUCT_LISTING_TARGET_SCOPES = {
    all:{label:'全部平台',platforms:['momo','coupang','easyStore','shopee']},
    momo:{label:'MOMO',platforms:['momo']},
    coupang:{label:'酷澎',platforms:['coupang']},
    website:{label:'官網',platforms:['easyStore','shopee']}
  };
  ...
  const PRODUCT_BRAND_TEMPLATE_CONTRACT = Object.freeze({
    ...
    slogan:'有音樂的生活更有風格',logoIdentity:'youzi-round-green-logo',
    header:Object.freeze({heightRatio:1/9,background:'approved-youzi-green',sloganAndLogoArtworkMustRemainUnchanged:true}),
    logo:Object.freeze({anchor:'upper-right',diameterRatioOfCanvasHeight:0.28,overlapHeaderAndContent:true,selectedReference:'approved-large-overlap-version',onlyElementAllowedToCrossHeaderBoundary:true,mayNotCoverProductOrPrimaryCopy:true,layer:'topmost',mustCoverBorderAtOverlap:true}),
    contentPanel:Object.freeze({narrowGreenBorderRequired:true,borderStyle:'continuous-thin-rounded-green-outline',borderWidthPx:{minimum:3,maximum:6},borderLayer:'below-logo',borderMayNotCrossLogoArtwork:true,allCreativeLayersMustStayInsideBorder:true,minimumLightAreaRatio:0.65,maximumDarkAreaRatio:0.35,darkFullBleedForbidden:true}),
    creativeStyleSystem:Object.freeze({version:'youzi-full-commercial-poster-style-catalog-v2',commercialPosterStandardVersion:'youzi-full-commercial-poster-v1',renderProofVersion:'youzi-brand-creative-render-v2',renderedStyleMustMatchAssignment:true,commercialPosterVisualQaRequired:true,oldInformationCardDesignsAccepted:false,fullCommercialPosterStageRequired:true,approvedReferenceStandard:'approved-commercial-poster-pair-2026-09-02',styleMustControlWholeComposition:true,forbiddenFallbacks:['generic-three-box-layout','flat-information-card','plain-canvas-with-labels','style-name-or-color-swap-only','reused-identical-layout-across-style-ids'],catalogSize:50,selectionMode:'random-without-replacement',resetOnlyAfterAllStylesUsed:true,preventImmediateRepeatAcrossCycles:true,assignmentScope:'root-product-group',sameStyleAcrossAspectRatios:true,sameStyleAcrossVariants:true,allowedAspectRatios:['1:1','4:3'],styles:PRODUCT_BRAND_CREATIVE_STYLE_CATALOG}),
    ...
    storefrontPortrait:Object.freeze({assetUrl:'product-listing-brand-template-portrait.png',assetSha256:'dff948f29b8374897a08f1ee78c15fcdf3db4c0caf6eff60e0d40eeb8fbda9ce',widthPx:1000,heightPx:750,aspectRatio:'4:3'}),
    brandedHero:Object.freeze({assetUrl:'product-listing-brand-template-square.png',assetSha256:'5f0e7743102c00b97befef0c240ec67d76686c1ed0e69dd8aa741cad4a73a1fe',widthPx:1000,heightPx:1000,aspectRatio:'1:1'})
  });
  ...
  async function saveFastStateCache(){
    try{
      ...
      const payload={savedAt:Date.now(),loadedAt:state.loadedAt?state.loadedAt.toISOString():'',fullLoadedAt:state.fullLoadedAt?state.fullLoadedAt.toISOString():'',data:{
        ...
        internalProducts:state.internalProducts,catalog:state.catalog,productListingQueue:state.productListingQueue,rentals:state.rentals,rentalLedgers:state.rentalLedgers,sales:state.sales,incomes:state.incomes,purchases:state.purchases,inventory:state.inventory,suppliers:state.suppliers,
...
async function loadPlatformLocalAgent(){
  ...
  function productListingPlatformLabel(platform){
    const labels={easyStore:'EasyStore 官網',shopee:'官方蝦皮',momo:'MOMO',coupang:'酷澎'};
    return labels[clean(platform)]||clean(platform)||'未指定通路';
  }
  function productListingFailureRows(source){
    ...
    return PRODUCT_LISTING_PLATFORM_ORDER.map(function(platform){
      ...
      return {platform:platform,status:clean(row.status)||'failed',reason:clean(row.reason||row.message||row.error)||'平台尚未完成',retryable:row.retryable!==false,retryAt:row.retryAt||row.nextRetryAt||row.availableAt||'',lastAttemptAt:row.lastAttemptAt||row.failedAt||''};
    }).filter(Boolean);
  }
  ...
  async function loadProductListingQueue(){
    const started=Date.now();
    try{
      const snapshots=await Promise.all(PRODUCT_LISTING_QUEUE_STATUSES.map(function(status){return state.db.collection(COLLECTIONS.listingCases).where('batchQueueStatus','==',status).limit(1000).get();})),seen=new Map();
      ...
      state.diagnostics.push({collection:COLLECTIONS.listingCases+'(batch-queue)',ok:true,count:state.productListingQueue.length,ms:Date.now()-started});
    }catch(error){state.productListingQueue=[];state.diagnostics.push({collection:COLLECTIONS.listingCases+'(batch-queue)',ok:false,count:0,ms:Date.now()-started,error:errorMessage(error)});}
  }
...
function renderOverviewV7(){
  ...
  const attentionKinds=(pendingPlatform?1:0)+(openReceivables.length?1:0)+(lowStock.length?1:0);
  ...
  const attentionHtml=alerts.length?alerts.join(''):'<div class="ops-v8-attention-empty"><b>目前沒有需要立即處理的項目</b><span>主要營運狀態正常。</span></div>';
  const bottomHtml='<div class="ops-v8-overview-bottom ops-v8-overview-bottom-single"><section class="ops-card"><div class="ops-v8-section-head"><div><h2>需要注意</h2><p>訂單待確認、應收帳款與低庫存會集中顯示</p></div>'+(attentionKinds?'<span class="ops-tag yellow">'+formatNumber(attentionKinds)+' 類</span>':'<span class="ops-tag green">正常</span>')+'</div><div class="ops-v8-attention-list">'+attentionHtml+'</div></section></div>';
  ...
  function productHasPlatformMapping(p){
    return ['easyStore','shopee','momo','coupang'].some(function(key){return !!productPlatformMappingId(p,key);});
  }
  function normalizeProductListingIntent(value,source,product){
    const explicit=clean(value);
    if(PRODUCT_LISTING_INTENTS.includes(explicit))return explicit;
    const row=source&&typeof source==='object'?source:{};
    if(clean(row.listingMode)==='add-variant')return 'add-variant';
    if(clean(row.listingMode)==='merge-existing')return 'merge-existing';
    if(row.variantGroupEnabled===true)return 'create-group';
    return productHasPlatformMapping(product)?'update-existing':'create-single';
  }
  function productListingIntentPolicy(intent){
    const value=PRODUCT_LISTING_INTENTS.includes(clean(intent))?clean(intent):'create-single';
    const rules={
      'create-single':{label:'新增獨立商品',contentAction:'create-complete-content',identityAction:'create-one-new-product',description:'建立一個全新的商品；不可誤連到相似舊商品。'},
      'create-group':{label:'新增多細項商品',contentAction:'create-shared-content-with-variant-differences',identityAction:'create-one-parent-with-closed-variant-set',description:'建立一個主商品與完整細項集合；共同資料只填一次，各細項保留自己的 SKU、名稱、價格、庫存與代表圖。'},
      'merge-existing':{label:'合併／加入既有商品',contentAction:'merge-variant-group-into-existing-content',identityAction:'reuse-existing-primary-and-attach-selected-variants',description:'自動沿用已上架商品為主商品，把勾選的其他編號加入同一細項群組；保留既有平台編號、內容與各 SKU 的獨立庫存。'},
      'add-variant':{label:'加入既有商品細項',contentAction:'merge-new-variant-into-existing-content',identityAction:'append-one-variant-to-exact-parent',description:'保留原商品與舊細項，再加入新的細項、圖片與介紹；不得覆蓋舊內容或另建主商品。'},
      'update-existing':{label:'修改既有商品',contentAction:'replace-outdated-content-in-place',identityAction:'update-exact-sku-and-platform-id',description:'鎖定同一 SKU 與平台編號，依修改要求替換錯誤或過時內容；不得追加重複版本或建立第二筆商品。'}
    };
    return Object.assign({intent:value,preserveUnmentionedContent:value==='merge-existing'||value==='add-variant'||value==='update-existing',neverCreateDuplicate:true},rules[value]);
  }
  function productPlatformStatus(p,key){
    ...
    if(mappingId&&status==='unknown')status='mapped';
  ...
  function productPlatformStatusHtml(p,compact){
    ...
    return '<div class="ops-product-platform-status '+(compact?'compact':'')+'">'+PRODUCT_LISTING_PLATFORMS.map(function(platform){
      const status=productPlatformStatus(p,platform.key),row=productPlatformPresence(p,platform.key),pending=['queued','pending-review','draft'].includes(status.status),pendingLabel=status.status==='pending-review'?'審核中':status.status==='draft'?'草稿':'待確認',title=[platform.label,pending?pendingLabel:row.exists?'有':'沒有',row.listingId?'編號 '+row.listingId:'',row.inferredFrom,pending?'點一下重新查驗':row.exists?'':'點一下準備補上架'].filter(Boolean).join('｜');
  ...
  function productPlatformListingCount(p){
    return PRODUCT_LISTING_PLATFORMS.reduce(function(total,platform){return total+(productPlatformMappingId(p,platform.key)?1:0);},0);
  }
  function productMergeSelectedIds(){
    const seen=new Set();
    state.productMergeSelection=(Array.isArray(state.productMergeSelection)?state.productMergeSelection:[]).map(clean).filter(function(id){if(!id||seen.has(id)||!catalogById(id))return false;seen.add(id);return true;});
    return state.productMergeSelection.slice();
  }
  function productHasCompletedListingImage(p){
    const images=normalizeProductResearchSourceUrls([].concat(p&&p.imageUrls||[],p&&p.completedListingImageUrls||[],p&&p.imageUrl||''));
    return images.length>0;
  }
  function productHasCompletedFourChannelListing(p){
    return PRODUCT_LISTING_PLATFORMS.every(function(platform){const status=productPlatformStatus(p,platform.key).status;return !['queued','pending-review','draft'].includes(status)&&productPlatformPresence(p,platform.key).exists;});
  }
  function productNeedsRecentListingWork(p){
    return !productHasCompletedFourChannelListing(p);
  }
  ...
  function productNeedsEasyStoreVariantImage(p){
    return !!(p&&p.matchedOnline&&Number(p.easyStoreProductVariantCount||0)>1&&!normalizeProductResearchSourceUrls(p.variantImageUrls).length);
  }
  ...
  function productFiltered(){
    ...
    }else if(state.productSort==='missing-variant-image'){
      rows=rows.slice().sort(function(a,b){
        const aPriority=productNeedsEasyStoreVariantImage(a)?0:1,bPriority=productNeedsEasyStoreVariantImage(b)?0:1;
        if(aPriority!==bPriority) return aPriority-bPriority;
        return compareCatalogSku(a,b);
      });
    }else{
  ...
  async function advanceFixedV3AfterShopeeStatus(product,statuses){
    ...
    if(clean(job.workflowVersion)!==PRODUCT_LISTING_WORKFLOW_VERSION)throw new Error('這筆工作不是目前固定版 v3 網路上架流程，不能沿用舊工作。');
  ...
  async function saveProductPlatformStatus(form){
    ...
    PRODUCT_LISTING_PLATFORMS.forEach(function(platform){const rawUrl=clean(data.get('platformUrl_'+platform.key)),url=safeUrl(rawUrl),status=clean(data.get('platformStatus_'+platform.key))||'unknown',previous=p.platformListingStatus&&p.platformListingStatus[platform.key]||{},pendingReview=platform.key==='coupang'&&status==='pending-review';if(rawUrl&&!url)throw new Error(platform.label+'商品頁網址格式不正確');statuses[platform.key]={status:status,listingId:clean(data.get('platformListingId_'+platform.key)),url:url,note:clean(data.get('platformNote_'+platform.key)),lastCheckedAt:now,lastCheckedBy:userLabel(),reviewSubmittedAt:pendingReview?(previous.reviewSubmittedAt||now):previous.reviewSubmittedAt||null,nextReviewCheckAt:pendingReview?new Date(Date.now()+24*60*60*1000):null,finalReviewCheckAt:pendingReview?new Date(Date.now()+48*60*60*1000):null};});
    ...
    const progression=await advanceFixedV3AfterShopeeStatus(p,statuses);
    closeDrawer();toast('平台狀態已更新',progression&&!progression.skipped&&clean(progression.currentStage)==='completed'?'蝦皮已核對，本次所選通路工作完成。':p.sku||p.originalName||p.name,'success');await loadProductsOnly(true);
  }
  ...
  function physicalProductImageTrayHtml(urls,productId){
    const id=clean(productId),rows=normalizeProductResearchSourceUrls(urls).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX);
    return '<div class="ops-listing-physical-inline '+(rows.length?'':'is-empty')+'">'+(rows.length?'<div class="ops-listing-physical-heading">實體圖</div>':'')+'<div id="productPhysicalImageTray" class="ops-listing-physical-tray">'+rows.map(function(url,index){return '<article class="ops-listing-physical-thumb"><a href="'+attr(url)+'" target="_blank" rel="noopener"><img src="'+attr(url)+'" alt="實體圖 '+(index+1)+'"><strong>實體圖</strong></a><button type="button" data-action="product-physical-image-remove" data-id="'+attr(id)+'" data-url="'+attr(url)+'" aria-label="移除實體圖 '+(index+1)+'">×</button></article>';}).join('')+'</div><div id="productPhysicalImageUploadStatus" aria-live="polite"></div></div>';
  }
  function refreshPhysicalProductImageTray(productId){
    const form=byId('productListingCaseForm'),id=clean(productId),product=catalogById(id),tray=byId('productPhysicalImageTray');
    if(!form||clean(form.dataset.id)!==id||!product||!tray)return;
    const wrapper=tray.closest('.ops-listing-physical-inline');if(wrapper)wrapper.outerHTML=physicalProductImageTrayHtml(product.physicalImageUrls,id);
  }
  function loadPhysicalPhotoImage(file){
    if(global.createImageBitmap)return global.createImageBitmap(file);
    return new Promise(function(resolve,reject){const url=URL.createObjectURL(file),image=new Image();image.onload=function(){URL.revokeObjectURL(url);resolve(image);};image.onerror=function(){URL.revokeObjectURL(url);reject(new Error('無法讀取這張實體照片'));};image.src=url;});
  }
  async function physicalPhotoLabeledBlob(file){
    const image=await loadPhysicalPhotoImage(file),sourceWidth=Number(image.width||image.naturalWidth),sourceHeight=Number(image.height||image.naturalHeight);
    if(!sourceWidth||!sourceHeight){if(image.close)image.close();throw new Error('無法判斷實體照片尺寸');}
    const width=Math.max(1,Math.round(sourceWidth)),height=Math.max(1,Math.round(sourceHeight)),canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;
    ...
    const labelHeight=Math.max(72,Math.min(Math.round(height*0.13),Math.round(width*0.22))),fontSize=Math.max(34,Math.round(labelHeight*0.5));
    context.fillStyle='rgba(16,24,40,.78)';context.fillRect(0,height-labelHeight,width,labelHeight);context.fillStyle='#fff';context.textAlign='center';context.textBaseline='middle';context.font='700 '+fontSize+'px -apple-system,BlinkMacSystemFont,"Noto Sans TC","Microsoft JhengHei",sans-serif';context.fillText('實體圖',width/2,height-labelHeight/2);
    return new Promise(function(resolve,reject){canvas.toBlob(function(blob){if(blob)resolve(blob);else reject(new Error('無法產生已標示的實體圖'));},'image/jpeg',0.94);});
  }
  async function uploadPhysicalProductPhoto(productId,file,options){
    ...
    if(state.physicalPhotoBusy)throw new Error('上一張照片仍在保存中');
    if(!global.firebase||!global.firebase.storage)throw new Error('圖片上傳服務尚未載入，請重新整理頁面。');
    if(!['image/jpeg','image/png','image/webp'].includes(clean(file.type)))throw new Error('請直接用手機相機拍攝 JPG、PNG 或 WebP 圖片');
    if(Number(file.size||0)>15*1024*1024)throw new Error('每張圖片不可超過 15 MB');
    if((product.physicalImageUrls||[]).length>=PRODUCT_PHYSICAL_IMAGE_MAX)throw new Error('這件商品的實體圖已達上限');
    await requireEasyStoreManagerAuth();state.physicalPhotoBusy=true;
    const status=byId(clean(options.statusElementId)||'physicalPhotoUploadStatus');if(status)status.innerHTML='<div class="ops-product-ai-status running"><span class="ops-product-ai-spinner"></span><div><b>正在保存原照並加上「實體圖」標示</b><small>'+escapeHtml(product.sku||product.originalName||product.name)+'</small></div></div>';
    try{
      const extension=file.type==='image/png'?'png':file.type==='image/webp'?'webp':'jpg',createdAt=new Date().toISOString(),baseName=Date.now()+'-'+Math.random().toString(36).slice(2,9),originalStoragePath='ops-product-listing-cases/'+id+'/physical/'+baseName+'-original.'+extension,labeledStoragePath='ops-product-listing-cases/'+id+'/physical/'+baseName+'-labeled.jpg',storage=global.firebase.app().storage(),labeledBlob=await physicalPhotoLabeledBlob(file);
      const originalRef=storage.ref().child(originalStoragePath),labeledRef=storage.ref().child(labeledStoragePath);
      const originalSnapshot=await originalRef.put(file,{contentType:file.type,customMetadata:{productId:id,productSku:clean(product.sku),photoType:'physical-original',uploadedBy:userLabel()}}),originalUrl=await originalSnapshot.ref.getDownloadURL();
      const labeledSnapshot=await labeledRef.put(labeledBlob,{contentType:'image/jpeg',customMetadata:{productId:id,productSku:clean(product.sku),photoType:'physical-labeled',labelText:'實體圖',uploadedBy:userLabel()}}),url=await labeledSnapshot.ref.getDownloadURL(),record={url:url,labeledUrl:url,originalUrl:originalUrl,storagePath:labeledStoragePath,labeledStoragePath:labeledStoragePath,originalStoragePath:originalStoragePath,labelApplied:true,labelText:'實體圖',createdAt:createdAt,createdBy:userLabel(),source:clean(options.source)||'mobile-camera'},fv=fieldValue();
      if(!fv||typeof fv.arrayUnion!=='function')throw new Error('圖片資料庫功能尚未載入');
      const common={productId:id,productSku:clean(product.sku),productName:clean(product.originalName||product.name),physicalImageUrls:fv.arrayUnion(url),physicalOriginalImageUrls:fv.arrayUnion(originalUrl),physicalImages:fv.arrayUnion(record),physicalImagesUpdatedAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION};
      await Promise.all([state.db.collection(COLLECTIONS.listingCases).doc(id).set(common,{merge:true}),state.db.collection(COLLECTIONS.products).doc(id).set({physicalImageUrls:fv.arrayUnion(url),physicalOriginalImageUrls:fv.arrayUnion(originalUrl),physicalImages:fv.arrayUnion(record),physicalImagesUpdatedAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true})]);
      product.physicalImageUrls=normalizeProductResearchSourceUrls((product.physicalImageUrls||[]).concat(url)).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX);product.physicalOriginalImageUrls=normalizeProductResearchSourceUrls((product.physicalOriginalImageUrls||[]).concat(originalUrl)).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX);product.physicalImages=(product.physicalImages||[]).concat(record).slice(-PRODUCT_PHYSICAL_IMAGE_MAX);if(product.internal){product.internal.physicalImageUrls=product.physicalImageUrls.slice();product.internal.physicalOriginalImageUrls=product.physicalOriginalImageUrls.slice();product.internal.physicalImages=product.physicalImages.slice();}
      await writeAudit('新增商品實體圖片','product',id,(product.sku||id)+'｜實體圖 '+product.physicalImageUrls.length+' 張');
      refreshPhysicalProductImageTray(id);const currentStatus=byId(clean(options.statusElementId)||'physicalPhotoUploadStatus');if(currentStatus)currentStatus.innerHTML='<div class="ops-product-ai-status completed"><span>✓</span><div><b>實體圖已保存</b><small>'+escapeHtml(product.sku||product.originalName||product.name)+'｜原照已保留，顧客版已清楚標示</small></div></div>';
      toast('實體圖已保存',(product.sku||product.originalName||product.name)+'｜會放在商品詳情，不會取代主圖','success');
      return url;
    }catch(error){throw error;
    }finally{state.physicalPhotoBusy=false;}
  }
  async function removePhysicalProductPhoto(productId,url){
    const id=clean(productId),target=safeUrl(url),product=catalogById(id);if(!id||!target||!product)throw new Error('找不到要移除的實體圖');
    await requireEasyStoreManagerAuth();const caseRef=state.db.collection(COLLECTIONS.listingCases).doc(id),productRef=state.db.collection(COLLECTIONS.products).doc(id),snapshots=await Promise.all([caseRef.get(),productRef.get()]);
    function filteredPayload(snapshot){const raw=snapshot.exists?snapshot.data()||{}:{},records=(Array.isArray(raw.physicalImages)?raw.physicalImages:[]),removed=records.filter(function(row){return safeUrl(row&&[row.url,row.labeledUrl].find(Boolean))===target;}),remaining=records.filter(function(row){return safeUrl(row&&[row.url,row.labeledUrl].find(Boolean))!==target;}),originalsToRemove=removed.map(function(row){return safeUrl(row&&row.originalUrl);}).filter(Boolean);return {payload:{physicalImageUrls:normalizeProductResearchSourceUrls(raw.physicalImageUrls).filter(function(item){return item!==target;}),physicalOriginalImageUrls:normalizeProductResearchSourceUrls(raw.physicalOriginalImageUrls).filter(function(item){return !originalsToRemove.includes(item);}),physicalImages:remaining,physicalImagesUpdatedAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},originalsToRemove:originalsToRemove};}
    const caseData=filteredPayload(snapshots[0]),productData=filteredPayload(snapshots[1]);await Promise.all([caseRef.set(caseData.payload,{merge:true}),productRef.set(productData.payload,{merge:true})]);
    product.physicalImageUrls=productData.payload.physicalImageUrls.slice(0,PRODUCT_PHYSICAL_IMAGE_MAX);product.physicalOriginalImageUrls=productData.payload.physicalOriginalImageUrls.slice(0,PRODUCT_PHYSICAL_IMAGE_MAX);product.physicalImages=productData.payload.physicalImages.slice(-PRODUCT_PHYSICAL_IMAGE_MAX);if(product.internal){product.internal.physicalImageUrls=product.physicalImageUrls.slice();product.internal.physicalOriginalImageUrls=product.physicalOriginalImageUrls.slice();product.internal.physicalImages=product.physicalImages.slice();}
    refreshPhysicalProductImageTray(id);await writeAudit('移除商品實體圖片','product',id,(product.sku||id)+'｜剩餘 '+product.physicalImageUrls.length+' 張');toast('實體圖已移出商品',product.sku||product.originalName||product.name,'success');
  }

  function productListingQueueRows(){
    return sortProductListingQueue((state.productListingQueue||[]).filter(function(row){return PRODUCT_LISTING_QUEUE_STATUSES.includes(clean(row.batchQueueStatus));}));
  }
  function upsertProductListingQueueState(row){
    const normalized=normalizeProductListingQueueRow(row,row&&row.productId),rows=(state.productListingQueue||[]).filter(function(item){return clean(item.productId)!==normalized.productId;});
    if(normalized.batchQueueStatus)rows.push(normalized);
    state.productListingQueue=sortProductListingQueue(rows);
  }
  function productListingQueueStatusLabel(status){
    if(status==='processing')return '已帶入 Codex';
    if(status==='failed')return '可再次處理';
    return '等待處理';
  }
  function productListingQueueRetryTime(row){
    const explicit=dateFrom(row&&row.batchNextRetryAt),failureTimes=(row&&row.batchPlatformFailures||[]).map(function(item){return dateFrom(item.retryAt);}).filter(Boolean),times=(explicit?[explicit]:[]).concat(failureTimes);
    if(!times.length)return null;
    return new Date(Math.max.apply(null,times.map(function(value){return value.getTime();})));
  }
  function productListingQueueRetryReady(row){
    const retryAt=productListingQueueRetryTime(row);
    return !retryAt||retryAt.getTime()<=Date.now();
  }
  function productListingQueueFailureHtml(row){
    const failures=row&&row.batchPlatformFailures||[];
    if(!failures.length)return row&&row.batchQueueError?'<div class="ops-listing-queue-failure"><span>未完成</span><b>'+escapeHtml(row.batchQueueError)+'</b></div>':'';
    return '<div class="ops-listing-queue-failures">'+failures.map(function(item){const retryAt=dateFrom(item.retryAt),wait=retryAt&&retryAt.getTime()>Date.now()?'可於 '+dateTimeText(retryAt)+' 後再處理':item.retryable?'現在可再次處理':'需先補正資料後再處理';return '<div class="ops-listing-queue-failure"><span>'+escapeHtml(productListingPlatformLabel(item.platform))+'</span><b>'+escapeHtml(item.reason)+'</b><small>'+escapeHtml(wait)+'</small></div>';}).join('')+'</div>';
  }
  function productListingQueueCardHtml(row,index){
    const product=catalogById(row.productId),image=safeUrl(row.productImageUrl||(product&&product.imageUrl)),name=clean(row.productName||(product&&(product.originalName||product.name)))||'未命名商品',sku=clean(row.productSku||(product&&product.sku))||'未設定',status=clean(row.batchQueueStatus),targetLabel=row.workflowPurpose===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹｜全部平台':productListingTargetLabel(row.listingTargetScope),retryAt=productListingQueueRetryTime(row),waiting=status==='failed'&&retryAt&&retryAt.getTime()>Date.now(),statusLabel=waiting?'等待可重試時間':productListingQueueStatusLabel(status);
  ...
  function productListingQueueDrawerHtml(){
    ...
    if(!rows.length)return emptyHtml('目前沒有待網路上架商品','先進入任一商品的「準備上架」，直接按所需平台的「加入待處理」。','<button class="ops-button primary" type="button" data-action="drawer-close">關閉</button>');
    const first=rows.filter(function(row){return row.batchQueueStatus!=='failed';}),retry=rows.filter(function(row){return row.batchQueueStatus==='failed';}),ready=rows.filter(function(row){return row.batchQueueStatus!=='processing'&&productListingQueueRetryReady(row);}),sections=[];
    if(first.length)sections.push('<section class="ops-listing-queue-group"><div class="ops-listing-queue-group-head"><b>第一次待處理</b><small>'+formatNumber(first.length)+' 件</small></div><div class="ops-listing-queue-list">'+first.map(productListingQueueCardHtml).join('')+'</div></section>');
    if(retry.length)sections.push('<section class="ops-listing-queue-group retry"><div class="ops-listing-queue-group-head"><b>上次未完成，可再次處理</b><small>'+formatNumber(retry.length)+' 件｜會保留同一商品與已完成通路</small></div><div class="ops-listing-queue-list">'+retry.map(productListingQueueCardHtml).join('')+'</div></section>');
  ...
  async function addProductListingToQueue(form,targetScope,workflowPurpose){
    if(!form||form.dataset.queueRunning==='1')return;
    const id=clean(form.dataset.id),product=catalogById(id),purpose=clean(workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE:'',listingTargetScope=purpose?normalizeProductListingTargetScope('all'):normalizeProductListingTargetScope(targetScope),listingTargetPlatforms=productListingTargetPlatforms(listingTargetScope),button=query('[data-action="product-listing-queue-add"][data-scope="'+listingTargetScope+'"][data-purpose="'+purpose+'"]',form);if(!id||!product)throw new Error('找不到商品上架資料');
    applyProductListingWorkflowPurpose(form,purpose);
    form.dataset.queueRunning='1';if(button){button.disabled=true;button.classList.add('is-busy');const label=query('b',button);if(label)label.textContent='正在加入…';}
    try{
      await finishProductImageCollectionBeforeHandoff(form);
      await saveProductListingCase(form,false,true,true);
      const serverTime=serverTimestamp(),payload={workflowPurpose:purpose,listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,batchQueueStatus:'queued',batchQueuedAt:serverTime,batchQueueUpdatedAt:serverTime,batchQueuedBy:userLabel(),batchQueueError:'',batchPlatformFailures:{},batchRetryPlatforms:[],batchNextRetryAt:null,batchAttemptCount:0,batchRunId:'',batchPosition:0,updatedAt:serverTime,updatedBy:userLabel(),version:VERSION};
      await state.db.collection(COLLECTIONS.listingCases).doc(id).set(payload,{merge:true});
      upsertProductListingQueueState({productId:id,productSku:product.sku,productName:product.originalName||product.name,productImageUrl:product.imageUrl,workflowPurpose:purpose,listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,batchQueueStatus:'queued',batchQueuedAt:new Date(),batchQueueUpdatedAt:new Date()});
      form.dataset.queueStatus='queued';form.dataset.targetScope=listingTargetScope;form.dataset.workflowPurpose=purpose;form.dataset.dirty='0';
      queryAll('[data-action="product-listing-queue-add"]',form).forEach(function(tile){const selected=clean(tile.dataset.scope)===listingTargetScope&&clean(tile.dataset.purpose)===purpose;tile.disabled=selected;tile.classList.toggle('selected',selected);const label=query('b',tile);if(label)label.textContent=selected?'已加入待處理':'加入待處理';});
      toast('已加入待處理',clean(product.sku)+'｜'+(purpose?'修改圖片及網路介紹':productListingTargetLabel(listingTargetScope))+'｜之後可從商品資訊上方一次開始處理','success');
    }finally{
      form.dataset.queueRunning='0';
      if(button){
        const selected=clean(form.dataset.queueStatus)==='queued'&&clean(form.dataset.targetScope)===listingTargetScope&&clean(form.dataset.workflowPurpose)===purpose,label=query('b',button);
        button.classList.remove('is-busy');button.classList.toggle('selected',selected);button.disabled=selected||clean(form.dataset.queueStatus)==='processing';
        if(label)label.textContent=selected?'已加入待處理':'加入待處理';
      }
    }
  }
  async function removeProductListingFromQueue(productId){
    const id=clean(productId),row=productListingQueueRows().find(function(item){return item.productId===id;});if(!id||!row)return;
    await requireEasyStoreManagerAuth();
    await state.db.collection(COLLECTIONS.listingCases).doc(id).set({batchQueueStatus:'',batchQueueUpdatedAt:serverTimestamp(),batchQueueError:'',batchRunId:'',batchPosition:0,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
    state.productListingQueue=(state.productListingQueue||[]).filter(function(item){return item.productId!==id;});
    openDrawer('待網路上架商品','已儲存的商品會依各自指定的平台，依序帶入 Codex 處理。',productListingQueueDrawerHtml());toast('已從待處理移除',row.productSku||row.productName,'success');
  }
  function productListingBatchActivationPrompt(rows,runId){
    return [
      '[商品上架批次待處理]',
      '批次編號：'+runId,
      '請在同一個 Codex 對話中依下列順序處理；一件完成後才處理下一件：',
      rows.map(function(row,index){return (index+1)+'. opsProductListingCases/'+row.productId+'｜'+clean(row.productSku)+'｜'+clean(row.productName)+'｜目標：'+productListingTargetLabel(row.listingTargetScope)+'｜工作：'+(clean(row.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹':'標準上架');}).join('\n'),
      '每一件都沿用專案內 youzi-four-channel-listing-v3 固定規則與該案件已保存的圖片、文字修改、圖片修改、細項、價格、庫存及物流資料，但只能處理該件 listingTargetPlatforms 指定的通路；未選通路不得建立、修改或送出。不得把不同商品資料混在一起，也不得重複詢問例行確認。',
      '每件完成上架後、開始下一件以前，必須以該件完整精確 SKU 在 EasyStore 官網、官方蝦皮通路、MOMO 與酷澎正式商品清單各查一次，直接回寫中央商品四個 platformListingStatus、listingId、lastCheckedAt 與原因。正式存在寫 active／有，確定找不到或拒絕寫 missing／沒有；酷澎仍在審核寫 pending-review，並排定送審後 24 與 48 小時重查，不可誤寫 active。',
  ...
  async function startProductListingQueue(){
    let rows=productListingQueueRows().filter(function(row){return row.batchQueueStatus!=='processing'&&productListingQueueRetryReady(row);});if(!rows.length){const waiting=productListingQueueRows().map(productListingQueueRetryTime).filter(Boolean).sort(function(a,b){return a-b;})[0];throw new Error(waiting?'目前商品仍在平台限制等待期；最早可於 '+dateTimeText(waiting)+' 再次處理。':'目前沒有等待處理的商品');}
    if(rows.length>50)throw new Error('一次最多處理 50 件，請先移除這次不處理的商品');
    await requireEasyStoreManagerAuth();
    const runId='listing-batch-'+Date.now().toString(36),batch=state.db.batch(),serverTime=serverTimestamp();
    rows.forEach(function(row,index){batch.set(state.db.collection(COLLECTIONS.listingCases).doc(row.productId),{batchQueueStatus:'processing',batchRunId:runId,batchPosition:index+1,batchLastAttemptAt:serverTime,batchAttemptCount:Number(row.batchAttemptCount||0)+1,batchQueueUpdatedAt:serverTime,caseStatus:'waiting-codex',updatedAt:serverTime,updatedBy:userLabel(),version:VERSION},{merge:true});});
    await batch.commit();
    rows=rows.map(function(row,index){return Object.assign({},row,{batchQueueStatus:'processing',batchRunId:runId,batchPosition:index+1,batchQueueUpdatedAt:new Date()});});rows.forEach(upsertProductListingQueueState);
    const prompt=productListingBatchActivationPrompt(rows,runId),threadUrl=productListingCodexThreadUrl(prompt),copied=await copyProductListingCodexPrompt(prompt),status=byId('productListingQueueStatus');
    if(status)status.innerHTML='<div class="ops-product-ai-status completed"><span>✓</span><div><b>已帶入 '+rows.length+' 件商品</b><small>Codex 會照清單順序逐件處理'+(copied?'；批次文字也已複製':'')+'。</small></div></div>';
    await writeAudit('批次交給 Codex 對話處理','productListingBatch',runId,rows.map(function(row){return row.productSku||row.productId;}).join('、'));
...
function ensureSalesClock(){
  ...
  function productListingSuggestedDefaults(p){
    ...
    if(/(?:吉他)?腳踏板|足凳|腳凳|效果器|調音器|節拍器|移調夾|撥片|琴弦|導線|連接線|轉接頭/i.test(name))return {shopeeCategoryPath:'愛好與收藏品 > 樂器與樂器配件 > 樂器配件',shippingDecision:'convenience',packageLengthCm:40,packageWidthCm:30,packageHeightCm:10,packageWeightKg:1,packageMeasurementMode:'estimated'};
  ...
  function productListingAutomaticPrice(p,platformKey){
    const values=[p&&p[platformKey],p&&p.sharedOnlinePrice,p&&p.onlinePrice,p&&p.storePrice].map(numberOrNull);
    return values.find(function(value){return value!=null;});
  }
  function productListingDescriptionStatus(value){
    const text=clean(value),lines=text.replace(/\r/g,'').split('\n').map(clean).filter(Boolean),featureHeader=lines.some(function(line){return /^商品特色[：:]?$/.test(line);}),usageHeader=lines.some(function(line){return /^(?:使用方式(?:／適用情境)?|適用情境|使用重點|使用建議)[：:]?$/.test(line);}),specHeader=lines.some(function(line){return /^商品規格[：:]?$/.test(line);});let activeSection='',featureCount=0,usageCount=0,specificationCount=0;lines.forEach(function(line){if(/^商品特色[：:]?$/.test(line)){activeSection='features';return;}if(/^(?:使用方式(?:／適用情境)?|適用情境|使用重點|使用建議)[：:]?$/.test(line)){activeSection='usage';return;}if(/^商品規格[：:]?$/.test(line)){activeSection='specifications';return;}if(/^(?:包裝內容|適用對象|注意事項)[：:]?$/.test(line)){activeSection='';return;}const listItem=/^(?:\d+[.、]|[-•●])\s*\S+/.test(line);if(activeSection==='features'&&listItem)featureCount+=1;if(activeSection==='usage'&&listItem)usageCount+=1;if(activeSection==='specifications'&&(listItem||/[：:]/.test(line)))specificationCount+=1;});const genericOnly=/(?:本商品為柚子樂器販售的樂器或樂器配件|此商品尚待依精確型號、條碼、案件圖片與可驗證網路資料完成正式商品介紹)/.test(text)||(!featureHeader&&!usageHeader&&!specHeader&&/(?:商品內容與規格以|如需確認尺寸、相容性或包裝內容)/.test(text));
    const missing=[];
    if(!text)missing.push('商品介紹');
    if(!featureHeader||featureCount<1)missing.push('可驗證商品特色');
    if(!usageHeader||usageCount<1)missing.push('使用方式／適用情境');
    if(!specHeader||specificationCount<1)missing.push('可驗證商品規格');
    if(genericOnly)missing.push('通用備援文案尚未改寫');
    return {ready:!!text&&!genericOnly&&featureHeader&&featureCount>0&&usageHeader&&usageCount>0&&specHeader&&specificationCount>0,genericOnly:genericOnly,featureCount:featureCount,usageCount:usageCount,specificationCount:specificationCount,featureTarget:PRODUCT_RICH_CONTENT_FEATURE_TARGET,usageTarget:PRODUCT_RICH_CONTENT_USAGE_TARGET,targetComplete:featureCount>=PRODUCT_RICH_CONTENT_FEATURE_TARGET&&usageCount>=PRODUCT_RICH_CONTENT_USAGE_TARGET,standardVersion:PRODUCT_RICH_CONTENT_STANDARD_VERSION,missing:Array.from(new Set(missing))};
  }
  function productRichContentLifecycle(intent,description){
    const status=productListingDescriptionStatus(description),legacyUpgrade=clean(intent)==='update-existing';
    return {standardVersion:PRODUCT_RICH_CONTENT_STANDARD_VERSION,mode:legacyUpgrade?'legacy-upgrade-in-place':'new-product-required',status:status.ready?'ready':(legacyUpgrade?'needs-upgrade':'required-before-first-publish'),featureTarget:PRODUCT_RICH_CONTENT_FEATURE_TARGET,usageTarget:PRODUCT_RICH_CONTENT_USAGE_TARGET,blockFirstPublishUntilReady:!legacyUpgrade,preserveExistingListingIdentity:legacyUpgrade,descriptionStatus:status};
  }
  function productListingAutomaticDescription(p){
    const name=clean(p&&((p.originalName)||(p.onlineName)||(p.name)))||'樂器商品',sku=clean(p&&p.sku);
    const intro=/(?:吉他)?腳踏板|足凳|腳凳/i.test(name)?'木製吉他腳踏板，適合演奏時支撐腳部使用；可依坐姿調整高度，使用後可摺疊收納。':'此商品尚待依精確型號、條碼、案件圖片與可驗證網路資料完成正式商品介紹。';
    const reminders=/(木製|櫸木|烏金木|實木)/.test(name)?'天然木材的木紋與色澤可能略有差異，屬正常現象。':'螢幕顯示可能造成些微色差，實際顏色以收到商品為準。';
    return [name,'',intro,'','商品規格','商品編號：'+(sku||'依商品標示'),'','注意事項','• '+reminders].join('\n');
  }
  ...
  function listingCaseNumberValue(raw,legacy,key,fallback){
    const value=numberOrNull(listingCaseValue(raw,legacy,key,null));
    return value==null?numberOrNull(fallback):value;
  }
  function normalizeProductListingImageRoles(row){
    const source=row&&typeof row==='object'?row:{},values=[].concat(source.roles||[],source.imageRoles||[],source.role||[]),seen=new Set();
    values.forEach(function(value){const role=clean(value);if(PRODUCT_LISTING_IMAGE_ROLES.includes(role))seen.add(role);});
    return Array.from(seen);
  }
  function normalizeProductListingImageFlags(row){
    const source=row&&typeof row==='object'?row:{},flags=source.assetFlags&&typeof source.assetFlags==='object'?source.assetFlags:{};
    return {
  ...
  function normalizeGeneratedListingImages(value){
    return (Array.isArray(value)?value:[]).map(function(row){
      ...
      return {id:clean(row&&row.id),url:url,status:clean(row&&row.status)||'candidate',mode:clean(row&&row.mode),model:clean(row&&row.model),qaStatus:clean(row&&row.qaStatus),localizationStatus:clean(row&&row.localizationStatus),qaModel:clean(row&&row.qaModel),qaAttempts:Math.max(0,Number(row&&row.qaAttempts)||0),sourceImageUrl:safeUrl(row&&row.sourceImageUrl),sourceOrder:Math.max(0,Number(row&&row.sourceOrder)||0),roles:normalizeProductListingImageRoles(row),assetFlags:normalizeProductListingImageFlags(row),templateVersion:clean(row&&row.templateVersion),templateAssetSha256:clean(row&&row.templateAssetSha256).toLowerCase(),templateComposition:clean(row&&row.templateComposition),creativeStyleAssignment:normalizedProductBrandCreativeStyleAssignment(row&&row.creativeStyleAssignment),brandRenderProof:row&&row.brandRenderProof&&typeof row.brandRenderProof==='object'?Object.assign({},row.brandRenderProof):null,brandCommercialPosterQa:row&&row.brandCommercialPosterQa&&typeof row.brandCommercialPosterQa==='object'?Object.assign({},row.brandCommercialPosterQa):null,instructions:clean(row&&row.instructions),createdAt:row&&row.createdAt||''};
    }).filter(Boolean).slice(-120);
  }
  function readyProductListingImageRows(listingCase){
    const source=listingCase&&typeof listingCase==='object'?listingCase:{},generated=normalizeGeneratedListingImages(source.generatedListingImages),allSourceUrls=new Set(normalizeProductResearchSourceUrls([].concat(source.referenceImageUrls||[],source.selectedReferenceImageUrls||[],generated.map(function(row){return row.sourceImageUrl;}))));
    const expectedStyle=normalizedProductBrandCreativeStyleAssignment(source.brandCreativeStyleAssignment);
    return generated.filter(function(row){const brandRole=row.roles.find(function(role){return role==='storefrontPortrait'||role==='brandedHero';}),profile=brandRole&&PRODUCT_BRAND_TEMPLATE_CONTRACT[brandRole],rowStyle=expectedStyle||row.creativeStyleAssignment;return row.status==='ready'&&(row.localizationStatus==='completed'||row.qaStatus==='approved')&&!!row.sourceImageUrl&&!allSourceUrls.has(row.url)&&(!profile||(row.templateVersion===PRODUCT_BRAND_TEMPLATE_CONTRACT.version&&row.templateAssetSha256===profile.assetSha256&&row.templateComposition===PRODUCT_BRAND_TEMPLATE_CONTRACT.composition&&!!row.creativeStyleAssignment&&productBrandCreativeRenderProofMatches(row.brandRenderProof,rowStyle)));});
  }
  function productListingConflictingRoleUrls(rows){
    const rolesByUrl=new Map();
    (Array.isArray(rows)?rows:[]).forEach(function(row){
      const roles=rolesByUrl.get(row.url)||new Set();
      row.roles.forEach(function(role){roles.add(role);});
      rolesByUrl.set(row.url,roles);
    });
    const conflicts=new Set();
    rolesByUrl.forEach(function(roles,url){if(['cleanMain','brandedHero','storefrontPortrait'].filter(function(role){return roles.has(role);}).length>1)conflicts.add(url);});
    return conflicts;
  }
  function productListingImageRowsForRole(listingCase,role){
    return readyProductListingImageRows(listingCase).filter(function(row){return row.roles.includes(role);});
  }
  function productListingRoleImageUrl(listingCase,role,sourceImageUrl){
    const sourceUrl=safeUrl(sourceImageUrl),rows=productListingImageRowsForRole(listingCase,role),match=sourceUrl?rows.find(function(row){return row.sourceImageUrl===sourceUrl;}):rows[0];
    return match?match.url:'';
  }
  function completedProductImageUrls(listingCase){
    const source=listingCase&&typeof listingCase==='object'?listingCase:{},listingUrls=normalizeProductResearchSourceUrls(source.listingImageUrls),allGenerated=readyProductListingImageRows(source),conflicts=productListingConflictingRoleUrls(allGenerated),generated=allGenerated.filter(function(row){return !conflicts.has(row.url);});
    ...
    return ordered.slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);
  }
  async function syncCompletedListingImagesToProduct(productId,listingCase){
    const id=clean(productId),product=catalogById(id),allReady=readyProductListingImageRows(listingCase),conflicts=productListingConflictingRoleUrls(allReady),ready=allReady.filter(function(row){return !conflicts.has(row.url);}),cleanRows=ready.filter(function(row){return row.roles.includes('cleanMain');}),detailRows=ready.filter(function(row){return !row.roles.includes('brandedHero')&&(row.roles.includes('localizedDetail')||row.roles.includes('specification'));}),images=normalizeProductResearchSourceUrls(cleanRows.concat(detailRows).map(function(row){return row.url;})).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);if(!id||!product||!images.length)return {images:images,changed:false};
    const current=productVariantImages(product),productChanged=product.imageUrl!==images[0]||JSON.stringify(current)!==JSON.stringify(images);
    const productRef=state.db.collection(COLLECTIONS.products).doc(id);
    if(productChanged)await productRef.set({imageUrl:images[0],imageUrls:images,parentImageUrls:[],variantImageUrls:[],completedListingImageUrls:images,imageSource:'localized-clean-main',completedListingImagesUpdatedAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
    const verifiedSnap=await productRef.get(),verified=verifiedSnap.exists?verifiedSnap.data()||{}:{},verifiedImages=normalizeProductResearchSourceUrls(verified.imageUrls||verified.completedListingImageUrls);
    if(verified.imageUrl!==images[0]||JSON.stringify(verifiedImages)!==JSON.stringify(images))throw new Error('繁體乾淨主圖已寫入但重新讀取不一致，已停止替換中央商品圖');
    await state.db.collection(COLLECTIONS.listingCases).doc(id).set({centralImageReferenceVerification:{status:'verified',cleanMainUrl:images[0],imageUrls:images,verifiedAt:serverTimestamp()},sourceImageRetentionPolicy:{mode:'metadata-only-after-required-binary-cleanup',sourceBinaryCleanupRequired:true,cleanupStatus:'blocked-until-all-central-variant-platform-references-verified',cleanupWorkerRequired:true,eligibleForDeletion:false},updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
    if(productChanged){product.imageUrl=images[0];product.imageUrls=images.slice();product.completedListingImageUrls=images.slice();product.imageSource='localized-clean-main';product.parentImageUrls=[];product.variantImageUrls=[];if(product.internal){product.internal.imageUrl=images[0];product.internal.imageUrls=images.slice();product.internal.completedListingImageUrls=images.slice();product.internal.imageSource='localized-clean-main';product.internal.parentImageUrls=[];product.internal.variantImageUrls=[];}}
    return {images:images,changed:productChanged,verified:true};
  }
  ...
  function productAutomaticVariantRepresentative(product){
    if(!product)return {url:'',source:'missing'};
    const variantImage=normalizeProductResearchSourceUrls(product.variantImageUrls)[0]||'';
    if(variantImage)return {url:variantImage,source:'easystore-variant'};
    if(clean(product.imageSource)==='localized-clean-main'){
      const completed=normalizeProductResearchSourceUrls([].concat(product.completedListingImageUrls||[],product.imageUrls||[],product.imageUrl||''))[0]||'';
      if(completed)return {url:completed,source:'localized-clean-main'};
    }
    const variantCount=Math.max(0,Number(product.easyStoreProductVariantCount)||0);
    if(product.matchedOnline&&variantCount===1){
      const singleMain=normalizeProductResearchSourceUrls([].concat(product.parentImageUrls||[],product.imageUrls||[],product.imageUrl||''))[0]||'';
      if(singleMain)return {url:singleMain,source:'easystore-single-main'};
    }
    return {url:'',source:'missing'};
  }
  function productAutomaticVariantStatusLabel(source){
    if(source==='easystore-variant')return '官網細項圖已自動帶入';
    if(source==='easystore-single-main')return '官網單品主圖已自動帶入';
    if(source==='localized-clean-main')return '繁體完成圖已自動帶入';
    if(source==='manual'||source==='auto')return '代表圖已選擇';
    return '尚未選擇代表圖';
  }
  function productListingRecoveredMedia(source,p){
    ...
    const selectedReferenceImageUrls=(storedSelected.length?storedSelected:completedSources.length?completedSources:referenceImageUrls).filter(function(url){return referenceImageUrls.includes(url);}).slice(0,PRODUCT_SELECTED_IMAGE_MAX);
  ...
  function normalizeProductListingCase(raw,p){
    ...
    const warrantyInfo=clean(listingCaseValue(source,legacy,'warrantyInfo',''))||'保固半年';
    const automaticName=clean(p&&((p.originalName)||(p.onlineName)||(p.name)));
    const productDescription=clean(listingCaseValue(source,legacy,'productDescription',''))||legacyCommonDescription||[legacyShortDescription,legacyFeatureList,legacySpecificationText].filter(Boolean).join('\n\n')||productListingAutomaticDescription(p,warrantyInfo);
    const listingIntent=normalizeProductListingIntent(source.listingIntent,source,p),listingMode=listingIntent==='add-variant'?'add-variant':listingIntent==='merge-existing'?'merge-existing':'independent',variantGroupEnabled=['create-group','merge-existing'].includes(listingIntent),listingTargetScope=normalizeProductListingTargetScope(source.listingTargetScope),listingTargetPlatforms=productListingTargetPlatforms(listingTargetScope);
    const normalized={
      caseStatus:clean(source.caseStatus)||'draft',batchQueueStatus:PRODUCT_LISTING_QUEUE_STATUSES.includes(clean(source.batchQueueStatus))?clean(source.batchQueueStatus):'',batchQueuedAt:source.batchQueuedAt||'',batchQueueUpdatedAt:source.batchQueueUpdatedAt||'',batchQueueError:clean(source.batchQueueError),batchRunId:clean(source.batchRunId),batchPosition:Number(source.batchPosition||0),
      workflowPurpose:clean(source.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE:'',listingIntent:listingIntent,listingIntentPolicy:productListingIntentPolicy(listingIntent),listingChangeInstructions:clean(source.listingChangeInstructions),listingMode:listingMode,variantParentProductId:clean(source.variantParentProductId),variantParentSku:clean(source.variantParentSku),variantParentName:clean(source.variantParentName),variantAttributeName:clean(source.variantAttributeName)||'規格',variantParentAttributeValue:clean(source.variantParentAttributeValue)||'原款',variantAttributeValue:clean(source.variantAttributeValue),variantParentImageUrl:safeUrl(source.variantParentImageUrl),variantChildImageUrl:safeUrl(source.variantChildImageUrl),variantGroupEnabled:variantGroupEnabled,variantGroupAttributeName:clean(source.variantGroupAttributeName)||'規格',variantGroupPrimaryValue:clean(source.variantGroupPrimaryValue),variantGroupPrimaryImageUrl:safeUrl(source.variantGroupPrimaryImageUrl),variantGroupItems:normalizeProductVariantGroupItems(source.variantGroupItems,p&&p.docId),variantGallerySelectionInitialized:source.variantGallerySelectionInitialized===true,
      sourceProductDescription:clean(source.sourceProductDescription),researchInstructions:clean(source.researchInstructions),referenceUrls:normalizeProductResearchSourceUrls(source.referenceUrls),referenceImageUrls:recoveredMedia.referenceImageUrls,selectedReferenceImageUrls:recoveredMedia.selectedReferenceImageUrls,gallerySourceImageUrls:normalizeProductResearchSourceUrls(source.gallerySourceImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),referenceImagesCleared:recoveredMedia.referenceImagesCleared,listingImageUrls:normalizeProductResearchSourceUrls(source.listingImageUrls),physicalImageUrls:normalizeProductResearchSourceUrls([].concat(source.physicalImageUrls||[],legacy.physicalImageUrls||[])).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX),physicalOriginalImageUrls:normalizeProductResearchSourceUrls([].concat(source.physicalOriginalImageUrls||[],legacy.physicalOriginalImageUrls||[])).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX),physicalImages:Array.isArray(source.physicalImages)?source.physicalImages:(Array.isArray(legacy.physicalImages)?legacy.physicalImages:[]),
      ...
      researchedProductName:clean(listingCaseValue(source,legacy,'researchedProductName',''))||automaticName,alternateNames:clean(listingCaseValue(source,legacy,'alternateNames','')),searchKeywords:clean(listingCaseValue(source,legacy,'searchKeywords','')),sellingPoints:clean(listingCaseValue(source,legacy,'sellingPoints','')),productDescription:productDescription,specificationText:legacySpecificationText,includedItems:clean(listingCaseValue(source,legacy,'includedItems','')),material:clean(listingCaseValue(source,legacy,'material','')),color:clean(listingCaseValue(source,legacy,'color','')),countryOfOrigin:clean(listingCaseValue(source,legacy,'countryOfOrigin','')),warrantyInfo:warrantyInfo,shortDescription:legacyShortDescription,commonProductDescription:legacyCommonDescription||productDescription,featureList:legacyFeatureList,faqText:clean(source.faqText),
      imagePlan:clean(source.imagePlan),imageGenerationInstructions:clean(source.imageGenerationInstructions),generatedListingImages:recoveredMedia.generatedListingImages,lastImageGenerationStatus:clean(source.lastImageGeneration&&source.lastImageGeneration.status),lastImageGenerationError:clean(source.lastImageGeneration&&source.lastImageGeneration.error),easyStoreHtml:clean(source.easyStoreHtml),shopeeTitle:clean(source.shopeeTitle)||automaticName,shopeeDescription:clean(source.shopeeDescription)||productDescription,shopeeRequiredNotes:clean(source.shopeeRequiredNotes),momoGoodsName:clean(source.momoGoodsName)||automaticName,momoSlogan:clean(source.momoSlogan),momoHtml:clean(source.momoHtml),momoRequiredNotes:clean(source.momoRequiredNotes),coupangTitle:clean(source.coupangTitle)||automaticName,coupangDescriptionHtml:clean(source.coupangDescriptionHtml),coupangRequiredNotes:clean(source.coupangRequiredNotes),
      ...
      listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,enabledEasyStoreShopee:listingTargetPlatforms.includes('easyStore'),enabledMomo:listingTargetPlatforms.includes('momo'),enabledCoupang:listingTargetPlatforms.includes('coupang'),
      shippingDecision:clean(listingCaseValue(source,legacy,'shippingDecision',''))||suggested.shippingDecision||'',packageLengthCm:listingCaseNumberValue(source,legacy,'packageLengthCm',suggested.packageLengthCm),packageWidthCm:listingCaseNumberValue(source,legacy,'packageWidthCm',suggested.packageWidthCm),packageHeightCm:listingCaseNumberValue(source,legacy,'packageHeightCm',suggested.packageHeightCm),packageWeightKg:listingCaseNumberValue(source,legacy,'packageWeightKg',suggested.packageWeightKg),packageMeasurementMode:clean(listingCaseValue(source,legacy,'packageMeasurementMode',''))||suggested.packageMeasurementMode||'',
    ...
    normalized.richContentStandard=productRichContentLifecycle(normalized.listingIntent,normalized.productDescription);
    normalized.richContentStandardVersion=normalized.richContentStandard.standardVersion;
    normalized.richContentMode=normalized.richContentStandard.mode;
    normalized.richContentStatus=normalized.richContentStandard.status;
    return normalized;
  }

  function productListingCaseIsCompleted(raw){
    const source=raw&&typeof raw==='object'?raw:{},publishState=source.publishState&&typeof source.publishState==='object'?source.publishState:{};
    return clean(source.caseStatus)==='published'||clean(publishState.status)==='completed'||clean(publishState.currentStage)==='completed';
  }
  function resetCompletedProductListingCase(row){
    const next=Object.assign({},row||{});
    next.caseStatus='draft';next.batchQueueStatus='';next.batchRunId='';next.batchPosition=0;next.workflowPurpose='';
    next.listingIntent='update-existing';next.listingIntentPolicy=productListingIntentPolicy('update-existing');next.listingMode='independent';next.listingChangeInstructions='';next.imageGenerationInstructions='';
    next.variantParentProductId='';next.variantParentSku='';next.variantParentName='';next.variantAttributeName='規格';next.variantParentAttributeValue='原款';next.variantAttributeValue='';next.variantParentImageUrl='';next.variantChildImageUrl='';
    next.variantGroupEnabled=false;next.variantGroupAttributeName='規格';next.variantGroupPrimaryValue='';next.variantGroupPrimaryImageUrl='';next.variantGroupItems=[];next.variantGallerySelectionInitialized=false;next.reopenedAfterPublish=true;
    return next;
  }
  ...
  function productRichContentStandardHtml(row){
    const lifecycle=row&&row.richContentStandard||productRichContentLifecycle(row&&row.listingIntent,row&&row.productDescription),status=lifecycle.descriptionStatus||{},legacy=lifecycle.mode==='legacy-upgrade-in-place',ready=lifecycle.status==='ready';
    const heading=ready?'固定商品內容已就緒':legacy?'既有商品待原地升級':'新商品首次上架前必須完成';
    const rule=legacy?'保留原 SKU 與各平台商品編號，只替換本次指定的舊內容。':'先完成可驗證內容，再進行第一次上架。';
    const counts='商品特色 '+Number(status.featureCount||0)+' / '+PRODUCT_RICH_CONTENT_FEATURE_TARGET+'｜使用重點 '+Number(status.usageCount||0)+' / '+PRODUCT_RICH_CONTENT_USAGE_TARGET+'｜規格屬性 '+Number(status.specificationCount||0)+' 項';
    return '<section class="ops-callout '+(ready?'green':'yellow')+'"><b>'+escapeHtml(heading)+'</b><br><span>'+escapeHtml(counts+'。'+rule)+'</span><br><span>以精確型號／條碼、官方規格與可信使用資料為依據；找不到就留空，不為湊數而猜測，也不在標題或內文加入店名。</span><br><span>[適用範圍：共用｜狀態：已確認] 詳細介紹結尾固定順序：實體商品說明 → 出貨與保固說明 → 最後兩張固定介紹圖。</span></section>';
  }
  ...
  function normalizeProductVariantGroupItems(value,currentProductId){
    ...
    return (Array.isArray(rows)?rows:[]).map(function(row){
      ...
      seen.add(productId);
      return {productId:productId,sku:clean(row&&row.sku),name:clean(row&&row.name),attributeValue:attributeValue,imageUrls:normalizeProductResearchSourceUrls(row&&row.imageUrls).slice(0,1),sourceImageUrls:normalizeProductResearchSourceUrls(row&&row.sourceImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),completedImageUrls:normalizeProductResearchSourceUrls(row&&row.completedImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),gallerySourceImageUrls:normalizeProductResearchSourceUrls(row&&row.gallerySourceImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),imageMatchStatus:clean(row&&row.imageMatchStatus)||'auto'};
    }).filter(Boolean).slice(0,20);
  }
  function productVariantImages(product){
    if(!product)return [];
    return normalizeProductResearchSourceUrls([].concat(product.variantImageUrls||[],product.imageUrls||[],product.imageUrl||'')).slice(0,PRODUCT_REFERENCE_IMAGE_MAX);
  }
  ...
  async function loadProductListingVariantMedia(productId){
    const id=clean(productId),product=catalogById(id);if(!id||!product)return {sourceImageUrls:[],completedImageUrls:[]};
    const snap=await state.db.collection(COLLECTIONS.listingCases).doc(id).get(),raw=snap.exists?snap.data()||{}:{},row=normalizeProductListingCase(raw,product),completed=completedProductImageUrls(row);
    productListingSourceImageCache.set(id,row.referenceImageUrls.slice());
    return {sourceImageUrls:row.referenceImageUrls.slice(),selectedSourceImageUrls:row.selectedReferenceImageUrls.slice(),completedImageUrls:completed,generatedListingImages:normalizeGeneratedListingImages(raw.generatedListingImages),caseExists:snap.exists};
  }
  ...
  function productVariantImagePickerOptionsHtml(productId,selectedUrl,sourceImages,role){
    ...
    if(!rows.length)return '<span class="ops-listing-variant-picker-empty">尚無圖片</span>';
    return rows.map(function(url,index){const active=url===selected;return '<span class="ops-listing-variant-picker-card"><button class="ops-listing-variant-picker-option '+(active?'selected':'')+'" type="button" draggable="true" data-action="product-variant-image-select" data-id="'+attr(id)+'" data-role="'+attr(role||'')+'" data-url="'+attr(url)+'" data-variant-drag-url="'+attr(url)+'" aria-label="指定來源圖片 '+(index+1)+'">'+(active?'<span>✓</span>':'')+'<img src="'+attr(url)+'" alt="可指定圖片 '+(index+1)+'"></button><button class="ops-listing-variant-source-remove" type="button" data-action="product-variant-source-image-remove" data-id="'+attr(id)+'" data-role="'+attr(role||'')+'" data-url="'+attr(url)+'" aria-label="刪除這張細項來源圖">×</button></span>';}).join('');
  }
  function productVariantImagePickerHtml(product,selectedUrl,sourceImages,role){
    const id=clean(product&&product.docId);
    const completed=clean(product&&product.imageSource)==='localized-clean-main'||(!normalizeProductResearchSourceUrls(sourceImages).length&&productVariantImages(product).length);
    return '<div class="ops-listing-variant-image-picker" data-variant-image-picker data-id="'+attr(id)+'">'+(completed?'<span class="ops-listing-variant-picker-empty">圖片已完成</span>':productVariantImagePickerOptionsHtml(id,selectedUrl,sourceImages,role))+'</div>';
  }
  function productVariantRepresentativePreviewHtml(product,selectedUrl,fallbackImages,role){
    const selected=safeUrl(selectedUrl),fallback=normalizeProductResearchSourceUrls(fallbackImages)[0]||'',preview=selected||fallback;
    return '<div class="ops-listing-variant-representative '+(preview?'selected':'empty')+'" data-variant-image-preview>'+(preview?'<img src="'+attr(preview)+'" alt="'+attr(product&&product.sku||'細項代表圖')+'">':'<span aria-hidden="true">＋</span>')+(selected?'<button type="button" data-action="product-variant-image-clear" data-id="'+attr(product&&product.docId)+'" data-role="'+attr(role||'')+'" aria-label="清除細項代表圖">×</button>':'')+'</div>';
  }
  ...
  function normalizeProductVariantSuggestion(value){
    const text=clean(value),colors={藍:'藍色',黑:'黑色',白:'白色',紅:'紅色',黃:'黃色',綠:'綠色',粉:'粉紅',紫:'紫色',橘:'橘色',橙:'橙色',棕:'棕色'};
  ...
  function productVariantSuggestion(product){
    ...
    const tail=text.match(/(?:^|[-－_｜|/／])\s*(馬卡藍|天空藍|深藍|淺藍|藍色?|黑色?|白色?|紅色?|黃色?|綠色?|粉紅|粉色|紫色?|橘色?|橙色?|棕色?|原木色|自然色|透明|玫瑰金|金色|銀色|\d+[A-Z])\s*$/i);
    if(tail)return normalizeProductVariantSuggestion(tail[1]);
    ...
    return match?normalizeProductVariantSuggestion(match[1]):clean(product.model)||('編號 '+clean(product.sku));
  }
  function productVariantResolvedValue(product,value){
    const stored=clean(value),sku=clean(product&&product.sku),suggested=productVariantSuggestion(product);
    if(!stored||stored===sku||stored==='編號 '+sku||/^編號\s*[A-Za-z0-9_-]+$/.test(stored))return suggested;
    return normalizeProductVariantSuggestion(stored);
  }
  ...
  function productVariantGroupAttributeSuggestion(product,items){
    const products=[product].concat((Array.isArray(items)?items:[]).map(function(item){return catalogById(item.productId);}).filter(Boolean)),values=products.map(productVariantSuggestion).filter(Boolean),unique=Array.from(new Set(values));
    if(unique.length>1&&unique.every(function(value){return /(藍|黑|白|紅|黃|綠|粉|紫|橘|橙|咖啡|棕|原木|自然|透明|金|銀)/.test(value);}))return '顏色';
    if(unique.length>1&&unique.every(function(value){return /(cm|mm|吋|英吋|號)/i.test(value);}))return '尺寸';
    if(unique.length>1&&unique.every(function(value){return /[A-GＡ-Ｇ][#♯b♭]?調/i.test(value);}))return '調性';
    return '規格';
  }
  ...
  function productSupplierShortcutsHtml(extraClass){
    return '<div class="ops-listing-supplier-shortcuts '+attr(extraClass||'')+'" aria-label="快速開啟圖片來源網站"><span>可開啟任何商品網頁</span><a href="https://www.taobao.com/" target="_blank" rel="noopener">淘寶網</a><a href="https://www.1688.com/" target="_blank" rel="noopener">1688</a></div>';
  }
  function productVariantCollectorHtml(productId,count){
    const id=clean(productId),active=!!(productImageCollectionSession&&productImageCollectionSession.active&&clean(productImageCollectionSession.productId)===id),current=Math.min(PRODUCT_IMAGE_COLLECTION.maxImages,Math.max(0,Number(count)||0));
    return '<div class="ops-listing-variant-collector"><button class="ops-button '+(active?'danger':'soft')+'" type="button" data-action="product-variant-image-collection" data-id="'+attr(id)+'">'+(active?'停止收圖':'收圖')+'</button><span data-variant-collector-count>目前 <b>'+current+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張</span></div>'+productSupplierShortcutsHtml('ops-listing-variant-supplier-shortcuts')+'<div class="ops-listing-variant-collector-status" data-variant-collector-status aria-live="polite">'+(active?'<span class="active">正在收圖</span>':'')+'</div>';
  }
  function productVariantRepresentativeCardHtml(product,options){
    options=options||{};const explicit=safeUrl(options.selectedUrl),automatic=productAutomaticVariantRepresentative(product),selected=explicit||automatic.url,origin=explicit?'manual':automatic.source,role=clean(options.role),sourceImages=normalizeProductResearchSourceUrls([].concat(selected?[selected]:[],options.sourceImages||[])),completed=clean(product&&product.imageSource)==='localized-clean-main'?productVariantImages(product).slice(0,1):[],missing=!selected&&!completed.length;
    return '<article class="ops-listing-variant-item '+(options.className||'')+'" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-field="'+attr(options.fieldName||'')+'" data-variant-image-origin="'+attr(origin)+'" data-variant-image-dropzone="1"'+(options.primary?' data-variant-primary="1"':'')+'><div class="ops-listing-variant-item-main"><div class="ops-listing-variant-key-fields">'+productVariantRepresentativePreviewHtml(product,selected,completed,role)+'<div class="ops-listing-variant-key-copy"><span class="ops-listing-variant-kind">'+escapeHtml(options.kind||'細項商品')+'</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b>'+(options.valueName?'<label class="ops-listing-variant-name-field">細項名稱<input class="ops-input" name="'+attr(options.valueName)+'" value="'+attr(options.value||'')+'" placeholder="例如：藍色" '+(options.valueDisabled?'disabled':'')+'></label>':'')+'</div></div><input type="hidden" name="'+attr(options.fieldName||'variantImageUrl')+'" value="'+attr(selected)+'">'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status hidden>'+escapeHtml(selected?productAutomaticVariantStatusLabel(origin):'尚未選代表圖')+'</span>'+productVariantCollectorHtml(product.docId,sourceImages.length)+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div></article>';
  }
  function productVariantParentPreviewHtml(parent,child,row){
    ...
    return '<div class="ops-listing-variant-pair">'+productVariantRepresentativeCardHtml(parent,{kind:'原商品',role:'add-parent',fieldName:'variantParentImageUrl',selectedUrl:row&&row.variantParentImageUrl,sourceImages:parentSources,valueName:'variantParentAttributeValue',value:productVariantResolvedValue(parent,row&&row.variantParentAttributeValue)||'原款'})+productVariantRepresentativeCardHtml(child,{kind:'現在要加入的商品',role:'add-child',fieldName:'variantChildImageUrl',selectedUrl:row&&row.variantChildImageUrl,sourceImages:childSources,valueName:'variantAttributeValue',value:productVariantResolvedValue(child,row&&row.variantAttributeValue)})+'</div>';
  }
  function productVariantGroupItemHtml(product,item){
    const explicit=normalizeProductResearchSourceUrls(item&&item.imageUrls)[0]||'',automatic=productAutomaticVariantRepresentative(product),selected=explicit||automatic.url,origin=explicit?(clean(item&&item.imageMatchStatus)||'manual'):automatic.source,sourceImages=normalizeProductResearchSourceUrls([].concat(selected?[selected]:[],normalizeProductResearchSourceUrls(item&&item.sourceImageUrls).length?item.sourceImageUrls:cachedProductListingSourceImages(product.docId))),role='group-child-'+product.docId,completed=clean(product&&product.imageSource)==='localized-clean-main',missing=!selected&&!completed,previewImages=selected?[selected]:completed?productVariantImages(product).slice(0,1):[];
    return '<article class="ops-listing-variant-item" data-product-id="'+attr(product.docId)+'" data-variant-role="'+attr(role)+'" data-variant-image-origin="'+attr(origin)+'" data-variant-image-dropzone="1"><div class="ops-listing-variant-item-main"><div class="ops-listing-variant-key-fields">'+productVariantRepresentativePreviewHtml(product,selected,previewImages,role)+'<div class="ops-listing-variant-key-copy"><span class="ops-listing-variant-kind">同款商品</span><b>'+escapeHtml(productVariantSearchLabel(product))+'</b><label class="ops-listing-variant-name-field">細項名稱<input class="ops-input" data-variant-group-value value="'+attr(productVariantResolvedValue(product,item&&item.attributeValue))+'" placeholder="例如：紅色"></label></div></div><textarea data-variant-group-images hidden>'+escapeHtml(selected)+'</textarea><textarea data-variant-completed-images hidden>'+escapeHtml(normalizeProductResearchSourceUrls(item&&item.completedImageUrls).join('\n'))+'</textarea>'+productVariantSourceHiddenHtml(sourceImages)+'<span class="'+(missing?'missing':'ok')+'" data-variant-image-match-status hidden>'+escapeHtml(selected?productAutomaticVariantStatusLabel(origin):'尚未選代表圖')+'</span>'+productVariantCollectorHtml(product.docId,sourceImages.length)+productVariantImagePickerHtml(product,selected,sourceImages,role)+'</div><button class="ops-icon-button" type="button" data-action="product-variant-group-remove" data-id="'+attr(product.docId)+'" aria-label="移除細項">×</button></article>';
  }
  function productVariantGroupPrimaryItemHtml(product,row,attributeValue,kind){
    return productVariantRepresentativeCardHtml(product,{kind:kind||'目前商品',className:'ops-listing-variant-primary-item',role:'group-primary',primary:true,fieldName:'variantGroupPrimaryImageUrl',selectedUrl:row&&row.variantGroupPrimaryImageUrl,sourceImages:row&&row.referenceImageUrls,valueName:'variantGroupPrimaryValue',value:attributeValue});
  }
  function productVariantGroupItemsFromForm(form){
    ...
    return queryAll('.ops-listing-variant-item[data-variant-role^="group-child-"]',form).map(function(card){
      ...
      const completedImageUrls=normalizeProductResearchSourceUrls(query('[data-variant-completed-images]',card)&&query('[data-variant-completed-images]',card).value);
      return {productId:product.docId,sku:clean(product.sku),name:clean(product.originalName||product.name),attributeValue:clean(query('[data-variant-group-value]',card)&&query('[data-variant-group-value]',card).value)||productVariantSuggestion(product),imageUrls:imageUrls.slice(0,1),sourceImageUrls:sourceImageUrls,completedImageUrls:completedImageUrls,imageMatchStatus:imageUrls.length?(clean(card.dataset.variantImageOrigin)||'manual'):'missing'};
    }).filter(Boolean);
  }
  function productVariantImageProcessingQueue(product,row){
    row=row&&typeof row==='object'?row:{};const queue=[],seen=new Set();
    function add(target,sourceImages,representativeSourceImageUrl,completedImages){
      if(!target||seen.has(target.docId))return;seen.add(target.docId);
      queue.push({productId:target.docId,sku:clean(target.sku),name:clean(target.originalName||target.name),sourceImageUrls:normalizeProductResearchSourceUrls(sourceImages).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),representativeSourceImageUrl:safeUrl(representativeSourceImageUrl),completedImageUrls:normalizeProductResearchSourceUrls(completedImages).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),gallerySourceImageUrls:[],gallerySelectionInitialized:false});
    }
    add(product,row.referenceImageUrls,row.listingMode==='add-variant'?row.variantChildImageUrl:row.variantGroupEnabled?row.variantGroupPrimaryImageUrl:'',normalizeProductResearchSourceUrls(row.completedImageUrls).length?row.completedImageUrls:completedProductImageUrls(row));if(queue[0]){queue[0].gallerySourceImageUrls=normalizeProductResearchSourceUrls(row.gallerySourceImageUrls);queue[0].gallerySelectionInitialized=row.variantGallerySelectionInitialized===true;}
    if(row.listingMode==='add-variant'&&row.variantParentProductId){const parent=catalogById(row.variantParentProductId);add(parent,cachedProductListingSourceImages(row.variantParentProductId),row.variantParentImageUrl,productVariantImages(parent));}
    if(row.variantGroupEnabled)(row.variantGroupItems||[]).forEach(function(item){const child=catalogById(item.productId);add(child,item.sourceImageUrls,item.imageUrls&&item.imageUrls[0],item.completedImageUrls||[]);const added=queue[queue.length-1];if(added&&added.productId===item.productId){added.gallerySourceImageUrls=normalizeProductResearchSourceUrls(item.gallerySourceImageUrls);added.gallerySelectionInitialized=row.variantGallerySelectionInitialized===true;}});
    if(row.variantGallerySelectionInitialized===true)queue.forEach(function(item){item.gallerySelectionInitialized=true;});
    return queue;
  }
  function productVariantImageProcessingQueueFromForm(form){
    if(!form)return [];const data=new FormData(form),product=catalogById(clean(form.dataset.id)),selections=productVariantGallerySelectionsFromForm(form),items=productVariantGroupItemsFromForm(form).map(function(item){item.gallerySourceImageUrls=selections.get(item.productId)||item.gallerySourceImageUrls||[];return item;}),row={listingMode:clean(data.get('listingMode'))==='add-variant'?'add-variant':'independent',variantParentProductId:clean(data.get('variantParentProductId')),variantParentImageUrl:safeUrl(data.get('variantParentImageUrl')),variantChildImageUrl:safeUrl(data.get('variantChildImageUrl')),variantGroupEnabled:data.get('variantGroupEnabled')==='1',variantGroupPrimaryImageUrl:safeUrl(data.get('variantGroupPrimaryImageUrl')),variantGroupItems:items,variantGallerySelectionInitialized:data.get('variantGallerySelectionInitialized')==='1',referenceImageUrls:normalizeProductResearchSourceUrls(data.get('referenceImageUrls')),completedImageUrls:normalizeProductResearchSourceUrls(data.get('currentCompletedImageUrls')),gallerySourceImageUrls:selections.get(clean(form.dataset.id))||[],generatedListingImages:[],listingImageUrls:[]};return productVariantImageProcessingQueue(product,row);
  }
  function productVariantGallerySelectionsFromForm(form){const result=new Map();let total=0;if(!form)return result;queryAll('[name="variantGallerySourceImageUrls"]:checked',form).forEach(function(input){const id=clean(input.dataset.productId),url=safeUrl(input.value);if(!id||!url||total>=PRODUCT_GROUP_LISTING_IMAGE_MAX)return;const rows=result.get(id)||[];if(!rows.includes(url)){rows.push(url);total+=1;}result.set(id,rows);});return result;}
  function productVariantImageProcessingQueueHtml(queue){
    queue=Array.isArray(queue)?queue:[];if(queue.length<2)return '';
    const rows=[],selectionInitialized=queue.some(function(item){return item.gallerySelectionInitialized===true;}),defaultSelected=new Set();
    if(!selectionInitialized){queue.forEach(function(item){const images=normalizeProductResearchSourceUrls((item.completedImageUrls||[]).length?item.completedImageUrls:item.sourceImageUrls);images.forEach(function(url){if(defaultSelected.size<PRODUCT_GROUP_LISTING_IMAGE_MAX)defaultSelected.add(url);});});}
    queue.forEach(function(item){const source=item.sourceImageUrls||[],completed=item.completedImageUrls||[],images=completed.length?completed:source,selected=new Set(item.gallerySourceImageUrls||[]);images.forEach(function(url){rows.push({productId:item.productId,sku:item.sku,url:url,checked:selectionInitialized?selected.has(url):defaultSelected.has(url)});});});
    return '<section class="ops-listing-variant-processing"><input type="hidden" name="variantGallerySelectionInitialized" value="1"><div class="ops-listing-variant-processing-heading"><b>上架圖片預覽</b></div><div class="ops-listing-variant-processing-flat">'+rows.map(function(item){return '<article class="'+(item.checked?'selected':'excluded')+'"><label><input type="checkbox" name="variantGallerySourceImageUrls" data-product-id="'+attr(item.productId)+'" value="'+attr(item.url)+'" '+(item.checked?'checked':'')+'><img src="'+attr(item.url)+'" alt="'+attr(item.sku||'商品')+'"><span>'+escapeHtml(item.sku||'')+(item.checked?' ✓':' 不上架')+'</span></label><button type="button" class="ops-listing-gallery-toggle" data-action="product-variant-gallery-toggle" aria-label="'+(item.checked?'排除這張上架圖':'恢復這張上架圖')+'">'+(item.checked?'×':'＋')+'</button></article>';}).join('')+'</div></section>';
  }
  function refreshProductVariantImageProcessingQueue(form){const box=query('#productVariantImageProcessingQueue',form);if(box)box.innerHTML=productVariantImageProcessingQueueHtml(productVariantImageProcessingQueueFromForm(form));}
  function productVariantGroupEditorHtml(p,row,intent){
    const merging=intent==='merge-existing',items=normalizeProductVariantGroupItems(row.variantGroupItems,p.docId),primary=productVariantResolvedValue(p,row.variantGroupPrimaryValue),suggestedAttribute=productVariantGroupAttributeSuggestion(p,items),storedAttribute=clean(row.variantGroupAttributeName),attributeName=!storedAttribute||(storedAttribute==='款式'&&suggestedAttribute!=='規格')?suggestedAttribute:(storedAttribute==='款式'?'規格':storedAttribute);
    return '<div class="ops-listing-group-fields">'+(merging?'<div class="ops-callout green"><b>已自動把既有商品放在第一筆</b><br><span>以下編號會合併到同一商品；編號、庫存與銷售記錄仍各自獨立。</span></div>':'')+'<div class="ops-field ops-listing-variant-search-step"><label class="ops-required">'+(merging?'繼續搜尋要合併的商品':'先搜尋要一起上架的商品')+'</label><div class="ops-inline-control"><input class="ops-input" type="search" name="variantGroupSearch" autocomplete="off" placeholder="輸入商品編號或名稱"><button class="ops-button soft" type="button" data-action="product-variant-group-search">搜尋</button></div><div id="productVariantGroupSearchResults" class="ops-listing-variant-search-results"></div></div><div id="productVariantGroupSelection" class="ops-listing-variant-selection '+(items.length?'':'is-hidden')+'"><div class="ops-form-grid"><div class="ops-field"><label class="ops-required">這組商品以什麼區分</label><input class="ops-input" name="variantGroupAttributeName" value="'+attr(attributeName)+'" placeholder="例如：顏色、尺寸、規格"></div></div><div class="ops-listing-variant-items ops-listing-variant-primary-list">'+productVariantGroupPrimaryItemHtml(p,row,primary,merging?'既有主商品':'目前商品')+'</div><input type="hidden" name="variantGroupItems" value="'+attr(JSON.stringify(items))+'"><div id="productVariantGroupItems" class="ops-listing-variant-items">'+items.map(function(item){const product=catalogById(item.productId);return product?productVariantGroupItemHtml(product,item):'';}).join('')+'</div></div></div>';
  }
  function productListingModeSectionHtml(p,row){
    const parent=row.variantParentProductId?catalogById(row.variantParentProductId):null,suggestedAttribute=productVariantAttributeSuggestion(p,parent),storedAttribute=clean(row.variantAttributeName),attributeName=!storedAttribute||(storedAttribute==='款式'&&suggestedAttribute!=='規格')?suggestedAttribute:(storedAttribute==='款式'?'規格':storedAttribute),intent=normalizeProductListingIntent(row.listingIntent,row,p);
    function choice(value,title){return '<label class="'+(intent===value?'selected':'')+'"><input type="radio" name="listingIntent" value="'+attr(value)+'" '+(intent===value?'checked':'')+'><span><b>'+escapeHtml(title)+'</b></span></label>';}
    return '<div class="ops-listing-mode-card"><div class="ops-listing-mode-heading">這次要怎麼處理商品</div><div class="ops-listing-mode-options" role="radiogroup" aria-label="商品處理方式">'
      +choice('create-single','新增獨立商品')
      +choice('create-group','新增多細項商品')
      +choice('merge-existing','合併／加入既有商品')
      +(intent==='add-variant'?choice('add-variant','加入既有商品細項（舊案件）'):'')
      +choice('update-existing','修改既有商品')+'</div>'
      +'<input type="hidden" name="listingMode" value="'+(intent==='add-variant'?'add-variant':intent==='merge-existing'?'merge-existing':'independent')+'"><input type="hidden" name="variantGroupEnabled" value="'+(['create-group','merge-existing'].includes(intent)?'1':'0')+'">'
      +'<div class="ops-listing-group-wrapper '+(['create-group','merge-existing'].includes(intent)?'':'is-hidden')+'" data-listing-group-intent="'+attr(intent)+'">'+productVariantGroupEditorHtml(p,row,intent)+'</div>'
      +'<div class="ops-listing-variant-fields '+(intent==='add-variant'?'':'is-hidden')+'"><div class="ops-field ops-listing-variant-search-step"><label class="ops-required">先搜尋要加入的原商品</label><input class="ops-input" type="search" name="variantParentSearch" autocomplete="off" value="'+attr(productVariantParentSearchValue(p.docId,row.variantParentProductId))+'" placeholder="輸入商品編號或名稱"><input type="hidden" name="variantParentProductId" value="'+attr(row.variantParentProductId)+'"><div id="productVariantParentSearchResults" class="ops-listing-variant-search-results"></div></div><div id="productVariantParentSelection" class="ops-listing-variant-selection '+(parent?'':'is-hidden')+'"><div class="ops-field"><label class="ops-required">這組商品以什麼區分</label><input class="ops-input" name="variantAttributeName" value="'+attr(attributeName)+'" placeholder="例如：顏色、尺寸、規格"></div><div id="productVariantParentPreview">'+productVariantParentPreviewHtml(parent,p,row)+'</div></div></div></div>';
  }
  ...
  function renderProductVariantGroupItems(form,items){
    ...
    renderProductVariantGroupSearch(form);refreshProductVariantImageProcessingQueue(form);updateProductImageCollectionUi(form);
  }
  function updateProductListingMode(form){
    if(!form)return;const selected=query('[name="listingIntent"]:checked',form),intent=PRODUCT_LISTING_INTENTS.includes(clean(selected&&selected.value))?clean(selected.value):'create-single',adding=intent==='add-variant',merging=intent==='merge-existing',groupEnabled=intent==='create-group'||merging,updating=intent==='update-existing',variantFields=query('.ops-listing-variant-fields',form),groupWrapper=query('.ops-listing-group-wrapper',form),mode=query('[name="listingMode"]',form),groupToggle=query('[name="variantGroupEnabled"]',form);
    if(groupEnabled&&groupWrapper&&clean(groupWrapper.dataset.listingGroupIntent)!==intent){
      const product=catalogById(clean(form.dataset.id)),items=productVariantGroupItemsFromForm(form),groupRow={variantGroupItems:items,variantGroupAttributeName:clean(query('[name="variantGroupAttributeName"]',form)&&query('[name="variantGroupAttributeName"]',form).value),variantGroupPrimaryValue:clean(query('[name="variantGroupPrimaryValue"]',form)&&query('[name="variantGroupPrimaryValue"]',form).value),variantGroupPrimaryImageUrl:safeUrl(query('[name="variantGroupPrimaryImageUrl"]',form)&&query('[name="variantGroupPrimaryImageUrl"]',form).value),referenceImageUrls:normalizeProductResearchSourceUrls(query('[name="referenceImageUrls"]',form)&&query('[name="referenceImageUrls"]',form).value)};
      if(product)groupWrapper.innerHTML=productVariantGroupEditorHtml(product,groupRow,intent);groupWrapper.dataset.listingGroupIntent=intent;
    }
    if(mode)mode.value=adding?'add-variant':merging?'merge-existing':'independent';if(groupToggle)groupToggle.value=groupEnabled?'1':'0';if(variantFields)variantFields.classList.toggle('is-hidden',!adding);if(groupWrapper)groupWrapper.classList.toggle('is-hidden',!groupEnabled);
    queryAll('.ops-listing-single-image-controls',form).forEach(function(control){control.classList.toggle('is-hidden',groupEnabled||adding);});
    queryAll('.ops-listing-mode-options>label',form).forEach(function(label){const input=query('[name="listingIntent"]',label);label.classList.toggle('selected',!!(input&&input.checked));});
    ...
    const instructions=query('[name="listingChangeInstructions"]',form),label=byId('listingChangeInstructionsLabel');if(instructions){instructions.required=updating;instructions.placeholder=updating?'例如：舊規格有誤，依新照片替換舊圖並改成新版介紹':adding?'例如：新增藍色款，將藍色圖片與差異補進原本介紹':merging?'例如：保留既有商品內容，加入所選細項的名稱、圖片與差異':'可補充商品特色、各細項差異或上架注意事項';}if(label)label.textContent=updating?'文字要修改或替換哪裡（必填）':adding?'新細項的文字要增加什麼':merging?'合併後的共同內容要怎麼整理':'文字要怎麼增加或修改';
    if(adding)resolveProductVariantParent(form);if(groupEnabled)renderProductVariantGroupSearch(form);
  }
  function applyProductListingWorkflowPurpose(form,purpose){
    if(!form)return '';
    const normalized=clean(purpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE:'',hidden=query('[name="workflowPurpose"]',form);
    if(hidden)hidden.value=normalized;form.dataset.workflowPurpose=normalized;
    if(normalized){
      const intent=query('[name="listingIntent"][value="update-existing"]',form),instructions=query('[name="listingChangeInstructions"]',form);
      if(intent)intent.checked=true;
      if(instructions)instructions.value=PRODUCT_DESCRIPTION_MEDIA_REFRESH_INSTRUCTIONS;
      updateProductListingMode(form);
    }
    return normalized;
  }
  function productListingActionGridHtml(row){
    const queueStatus=clean(row&&row.batchQueueStatus),currentScope=normalizeProductListingTargetScope(row&&row.listingTargetScope),currentPurpose=clean(row&&row.workflowPurpose),processing=queueStatus==='processing';
    const scopes=['all','momo','coupang','website'];
    const standard=scopes.map(function(scope){
      const meta=productListingTargetMeta(scope),selected=queueStatus==='queued'&&currentScope===scope&&!currentPurpose,queueDisabled=processing||selected;
      return '<button class="ops-listing-action-tile queue '+(selected?'selected':'')+'" type="button" data-action="product-listing-queue-add" data-scope="'+attr(scope)+'" data-purpose="" '+(queueDisabled?'disabled':'')+' aria-label="'+attr(meta.label+'加入待處理')+'"><span>'+escapeHtml(meta.label)+'</span><b>'+(selected?'已加入待處理':'加入待處理')+'</b></button>'
        +'<button class="ops-listing-action-tile immediate" type="button" data-action="product-listing-codex-complete" data-scope="'+attr(scope)+'" data-purpose="" '+(processing?'disabled':'')+' aria-label="'+attr(meta.label+'立即處理')+'"><span>'+escapeHtml(meta.label)+'</span><b>立即處理</b></button>';
    }).join('');
    const refreshSelected=queueStatus==='queued'&&currentPurpose===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE;
    const refresh='<div class="ops-listing-refresh-label"><b>修改圖片及網路介紹</b><small>原地更新四平台；固定圖文穿插、店址尾圖與兩張介紹尾圖。</small></div>'
      +'<button class="ops-listing-action-tile queue refresh '+(refreshSelected?'selected':'')+'" type="button" data-action="product-listing-queue-add" data-scope="all" data-purpose="'+PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE+'" '+(processing||refreshSelected?'disabled':'')+' aria-label="修改圖片及網路介紹加入待處理"><span>修改圖片及網路介紹</span><b>'+(refreshSelected?'已加入待處理':'加入待處理')+'</b></button>'
      +'<button class="ops-listing-action-tile immediate refresh" type="button" data-action="product-listing-codex-complete" data-scope="all" data-purpose="'+PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE+'" '+(processing?'disabled':'')+' aria-label="修改圖片及網路介紹立即處理"><span>修改圖片及網路介紹</span><b>立即處理</b></button>';
    return '<div class="ops-listing-action-grid" aria-label="選擇上架平台與處理方式">'+standard+refresh+'</div>';
  }
  function productListingCaseFormHtml(p,row,exists){
    ...
    const readySourceSet=new Set(readyProductListingImageRows(row).filter(function(imageRow){return imageRow.roles.length>0;}).map(function(imageRow){return imageRow.sourceImageUrl;})),visibleReferenceImageUrls=row.referenceImageUrls.filter(function(url){return !readySourceSet.has(url);}),hiddenCompletedSelections=row.selectedReferenceImageUrls.filter(function(url){return readySourceSet.has(url);}).map(function(url){return '<input type="hidden" name="selectedReferenceImageUrls" value="'+attr(url)+'">';}).join('');
    ...
    const physicalAtLimit=row.physicalImageUrls.length>=PRODUCT_PHYSICAL_IMAGE_MAX;
    const sourceSection='<textarea name="sourceProductDescription" hidden>'+escapeHtml(row.sourceProductDescription)+'</textarea><input type="hidden" name="researchInstructions" value="'+attr(row.researchInstructions)+'"><textarea name="referenceUrls" hidden>'+escapeHtml(row.referenceUrls.join('\n'))+'</textarea>'
      +'<div class="ops-listing-media-workspace"><div class="ops-listing-media-actions"><button class="ops-listing-media-action '+(collectorActive?'danger':'primary')+' ops-listing-single-image-controls" type="button" data-action="product-image-collection-toggle">'+(collectorActive?'停止搜圖':'開始搜圖')+'</button><label class="ops-listing-media-action soft ops-listing-single-image-controls"><input id="productReferenceImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden>選擇圖片上傳</label><label class="ops-listing-media-action physical '+(physicalAtLimit?'disabled':'')+'">'+(physicalAtLimit?'實體圖已滿':'<input id="productPhysicalImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden>實體圖')+'</label></div>'+productSupplierShortcutsHtml('ops-listing-single-image-controls')+'<span id="productImageCollectionCount" hidden>目前 <b>'+collectorCount+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張</span><div id="productImageCollectionStatus" class="ops-listing-media-status" aria-live="polite">'+(collectorActive?'<div class="ops-listing-media-message">正在搜圖；切到任何商品網頁即可點圖或框選截圖</div>':'')+'</div><div id="productReferenceImageUploadStatus" aria-live="polite"></div>'
      +'<textarea name="referenceImageUrls" hidden>'+escapeHtml(row.referenceImageUrls.join('\n'))+'</textarea><textarea name="currentCompletedImageUrls" hidden>'+escapeHtml(completedProductImageUrls(row).join('\n'))+'</textarea>'+hiddenCompletedSelections+'<div class="ops-listing-media-board"><div id="productReferenceImagePreview" class="ops-listing-single-image-controls">'+(visibleReferenceImageUrls.length?productReferenceImageSelectorHtml(visibleReferenceImageUrls,row.selectedReferenceImageUrls):readySourceSet.size?'<div class="ops-listing-image-empty">來源圖譜系已隱藏保存；畫面只顯示繁體完成圖。</div>':'')+'</div>'+physicalProductImageTrayHtml(row.physicalImageUrls,p.docId)+'<div id="productVariantImageProcessingQueue">'+productVariantImageProcessingQueueHtml(productVariantImageProcessingQueue(p,row))+'</div></div>'
      +'<div class="ops-listing-instruction-grid"><div class="ops-field"><div class="ops-listing-field-heading"><label id="listingChangeInstructionsLabel">文字要怎麼增加或修改</label><button class="ops-button ghost small" type="button" data-action="product-listing-speech" data-target="listingChangeInstructions">🎙 說話輸入</button></div><textarea class="ops-textarea compact" name="listingChangeInstructions" placeholder="可補充商品特色、各細項差異或上架注意事項">'+escapeHtml(row.listingChangeInstructions)+'</textarea></div><div class="ops-field"><div class="ops-listing-field-heading"><label>圖片要怎麼修改</label><button class="ops-button ghost small" type="button" data-action="product-listing-speech" data-target="imageGenerationInstructions">🎙 說話輸入</button></div><textarea class="ops-textarea compact" name="imageGenerationInstructions" placeholder="例如：簡體字改成台灣繁體；移除『含琴盒』；保留商品顏色">'+escapeHtml(row.imageGenerationInstructions)+'</textarea></div></div></div>';
    ...
    const commonSection='<textarea name="sellingPoints" hidden>'+escapeHtml(row.sellingPoints)+'</textarea><textarea name="shortDescription" hidden>'+escapeHtml(row.shortDescription)+'</textarea><textarea name="featureList" hidden>'+escapeHtml(row.featureList)+'</textarea><textarea name="specificationText" hidden>'+escapeHtml(row.specificationText)+'</textarea><textarea name="commonProductDescription" hidden>'+escapeHtml(row.commonProductDescription)+'</textarea>'+productIdentityReviewHtml(row)+'<div class="ops-form-grid"><div class="ops-field full"><label>上架商品名稱</label><input class="ops-input" name="researchedProductName" value="'+attr(row.researchedProductName)+'" placeholder="AI 會填入，你也可以自己修改"></div><div class="ops-field full"><label>完整商品介紹</label><textarea class="ops-textarea ops-product-description-textarea" name="productDescription" placeholder="先用 2～4 句介紹商品與適合對象。\n\n商品特色\n1. 可驗證特色一\n2. 可驗證特色二\n\n使用方式／適用情境\n1. 使用重點一\n2. 使用重點二\n\n商品規格\n品牌：\n型號：\n顏色：">'+escapeHtml(row.productDescription)+'</textarea><small>固定目標為 10 個可驗證特色、8 個使用重點與所有找得到的規格屬性；資料不足寧少勿猜。系統會轉成四平台格式，並把實體商品免責句固定放在最後。</small></div></div><details class="ops-listing-image-url-details"><summary>其他商品資料</summary><div class="ops-listing-advanced-body">'+commonDetails+'</div></details>';
    const mediaSection='<textarea name="imagePlan" hidden>'+escapeHtml(row.imagePlan)+'</textarea><textarea name="listingImageUrls" hidden>'+escapeHtml(listingImages.join('\n'))+'</textarea><div class="ops-listing-image-order"><b>中性商業首圖</b><span>商品圖</span><span>圖文介紹</span></div><div id="productListingImagePreview">'+(listingImages.length?productListingImagePreviewHtml(listingImages,'目前已完成圖片'):'<div class="ops-listing-image-empty">交給 Codex 後會在這裡保存完成圖片</div>')+'</div>';
    const sharedPlatformContent='<div class="ops-callout green"><b>四平台共用同一份已驗證內容</b><br><span>完整商品介紹會自動轉成 EasyStore 安全 HTML、蝦皮進階圖文、MOMO 原生圖文與酷澎安全 HTML；最後一句固定不變。</span></div>';
    const easySection='<textarea name="shopeeDescription" hidden>'+escapeHtml(row.productDescription)+'</textarea><textarea name="easyStoreHtml" hidden>'+escapeHtml(productDescriptionToSafeHtml(row.productDescription))+'</textarea><input type="hidden" name="enabledEasyStoreShopee" value="1"><div class="ops-callout green"><b>自動發布 EasyStore 與蝦皮</b><br><span>不需勾選，也不先搜尋平台目錄；系統嚴格依上方處理方式執行。新增商品遇到既有平台編號會停止，修改商品缺少既有編號也會停止，避免建立或覆蓋錯誤商品。</span></div>'+sharedPlatformContent+'<div class="ops-form-grid"><div class="ops-field full ops-product-name-field"><label>蝦皮商品標題</label><input class="ops-input" name="shopeeTitle" value="'+attr(row.shopeeTitle)+'"></div><div class="ops-field full"><label>EasyStore → 蝦皮分類</label><input class="ops-input" name="shopeeCategoryPath" value="'+attr(row.shopeeCategoryPath)+'" readonly><small>分類由商品類型與蝦皮固定模板產生；吉他不會跳過「弦樂器」。</small></div><div class="ops-field full"><label>平台另外需要補的欄位（如有）</label><textarea class="ops-textarea compact" name="shopeeRequiredNotes">'+escapeHtml(row.shopeeRequiredNotes)+'</textarea></div></div>'+productShopeeAttributeEditorHtml(p,row);
    const momoSection='<textarea name="momoHtml" hidden>'+escapeHtml(productDescriptionToSafeHtml(row.productDescription))+'</textarea><input type="hidden" name="enabledMomo" value="1"><div class="ops-callout green"><b>自動發布 MOMO</b><br><span>不需勾選；沿用同一個商品 SKU、商品售價與完成圖片。市價預設售價加 35%，品牌不在商品名稱重複。</span></div>'+sharedPlatformContent+'<div class="ops-form-grid"><div class="ops-field full"><label>MOMO 商品名稱（不重複品牌，最多 60 字）</label><input class="ops-input" name="momoGoodsName" maxlength="60" value="'+attr(row.momoGoodsName)+'"></div><div class="ops-field full"><label>MOMO 商品特色標語（最多 15 字）</label><input class="ops-input" name="momoSlogan" maxlength="15" value="'+attr(row.momoSlogan)+'"><small>留白時會從已驗證商品特色抽取；不會為填滿欄位發明內容。</small></div><div class="ops-field full"><label>MOMO 平台分類</label><input class="ops-input" name="momoCategoryCode" value="'+attr(row.momoCategoryCode)+'" placeholder="留白時自動判斷樂器／樂器配件分類"></div><div class="ops-field full"><label>平台另外需要補的欄位（如有）</label><textarea class="ops-textarea compact" name="momoRequiredNotes">'+escapeHtml(row.momoRequiredNotes)+'</textarea></div></div>';
    const coupangSection='<textarea name="coupangDescriptionHtml" hidden>'+escapeHtml(productDescriptionToSafeHtml(row.productDescription))+'</textarea><input type="hidden" name="enabledCoupang" value="1"><div class="ops-callout green"><b>自動發布酷澎</b><br><span>不需勾選；沿用同一個商品 SKU、商品售價與完成圖片。</span></div>'+sharedPlatformContent+'<div class="ops-form-grid"><div class="ops-field full"><label>酷澎商品標題</label><input class="ops-input" name="coupangTitle" value="'+attr(row.coupangTitle)+'"></div><div class="ops-field full"><label>酷澎分類</label><input class="ops-input" name="coupangCategoryCode" value="'+attr(row.coupangCategoryCode)+'" placeholder="留白時自動判斷樂器／樂器配件分類"></div><div class="ops-field full"><label>平台另外需要補的欄位（如有）</label><textarea class="ops-textarea compact" name="coupangRequiredNotes">'+escapeHtml(row.coupangRequiredNotes)+'</textarea></div></div>';
    ...
    const codexAction='<section class="ops-listing-codex-action" data-workflow-version="'+attr(PRODUCT_LISTING_WORKFLOW_VERSION)+'"><div class="ops-listing-codex-heading"><span>✦</span><div><b>網路上架處理</b><small>直接點一個方塊，不會再跳出第二層選單。</small></div></div>'+productListingActionGridHtml(row)+'<details class="ops-listing-completed-upload"><summary>進階回填完成圖</summary><label class="ops-button soft ops-listing-upload-button ops-listing-completed-upload-button" title="檔名須為 01-cleanMain.jpg、01-brandedHero.jpg 等來源順序與角色"><input id="productCompletedImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple aria-label="進階回填角色完成圖"><span>選擇完成圖</span></label><div id="productCompletedImageUploadStatus" aria-live="polite"></div></details><div id="productListingCodexStatus"></div></section>';
    ...
    return '<form id="productListingCaseForm" data-id="'+attr(p.docId)+'" data-exists="'+(exists?'1':'0')+'" data-queue-status="'+attr(row.batchQueueStatus)+'" data-target-scope="'+attr(row.listingTargetScope)+'" data-workflow-purpose="'+attr(row.workflowPurpose)+'" data-identity-manual-confirmed="'+(row.identityManualConfirmed?'1':'0')+'" data-ai-research-status="'+attr(row.aiResearchStatus)+'" data-image-generation-status="'+attr(row.lastImageGenerationStatus)+'">'
      ...
      +(row.reopenedAfterPublish?'<div class="ops-callout green"><b>這件商品已完成過上架</b><br><span>本次已回到單一商品修改狀態，只保留這個 SKU 自己的圖片與資料；上次暫時合併的其他細項不會再次帶入。</span></div>':'')
      +'<input type="hidden" name="workflowPurpose" value="'+attr(row.workflowPurpose||'')+'"><input type="hidden" name="internalSku" value="'+attr(p.sku||'')+'"><input type="hidden" name="internalName" value="'+attr(p.originalName||p.name||'')+'"><input type="hidden" name="easyStorePrice" value="'+attr(productListingAutomaticPrice(p,'easyStorePrice')==null?'':productListingAutomaticPrice(p,'easyStorePrice'))+'"><input type="hidden" name="momoPrice" value="'+attr(productListingAutomaticPrice(p,'momoPrice')==null?'':productListingAutomaticPrice(p,'momoPrice'))+'"><input type="hidden" name="coupangPrice" value="'+attr(productListingAutomaticPrice(p,'coupangPrice')==null?'':productListingAutomaticPrice(p,'coupangPrice'))+'"><input type="hidden" name="currentStock" value="'+attr(p.currentStock)+'"><input type="hidden" name="packageMeasurementMode" value="'+attr(row.packageMeasurementMode||'')+'">'
      +hiddenState+productListingModeSectionHtml(p,row)+productListingSection('圖片','',sourceSection,true)
      +productRichContentStandardHtml(row)+codexAction+advancedSection
      +'<div class="ops-drawer-footer"><button class="ops-button ghost" type="button" data-action="drawer-close">關閉</button></div></form>';
  }
  function productMergeSelectionProducts(){
    return productMergeSelectedIds().map(catalogById).filter(Boolean);
  }
  async function openProductMergeSelection(){
    const selected=productMergeSelectionProducts();
    if(selected.length<2)throw new Error('請至少勾選兩個要合併的商品編號');
    const selectedOrder=new Map(selected.map(function(product,index){return [product.docId,index];})),listed=selected.filter(productHasPlatformMapping).sort(function(a,b){return productPlatformListingCount(b)-productPlatformListingCount(a)||(selectedOrder.get(a.docId)||0)-(selectedOrder.get(b.docId)||0);}),primary=listed[0]||selected[0],children=selected.filter(function(product){return product.docId!==primary.docId;}).sort(compareCatalogSku),ordered=[primary].concat(children);
    state.productMergeSelection=ordered.map(function(product){return product.docId;});
    await openProductListingCase(primary.docId,{mergeProductIds:ordered.map(function(product){return product.docId;}),listingIntent:listed.length?'merge-existing':'create-group'});
  }
  async function openProductListingCase(id,options){
    ...
    try{
      ...
      const snap=await state.db.collection(COLLECTIONS.listingCases).doc(id).get(),raw=snap.exists?snap.data():null;let row=normalizeProductListingCase(raw,p);
      if(productListingCaseIsCompleted(raw)&&!(options&&((options.mergeProductIds&&options.mergeProductIds.length)||options.listingMode==='add-variant')))row=resetCompletedProductListingCase(row);
      const mergeIds=options&&Array.isArray(options.mergeProductIds)?Array.from(new Set(options.mergeProductIds.map(clean).filter(function(productId){return productId&&productId!==id&&catalogById(productId);}))).slice(0,20):[];
      if(mergeIds.length){
        const savedItems=new Map((row.variantGroupItems||[]).map(function(item){return [item.productId,item];}));
        row.listingIntent=options&&options.listingIntent==='merge-existing'?'merge-existing':'create-group';row.listingMode=row.listingIntent==='merge-existing'?'merge-existing':'independent';row.variantGroupEnabled=true;row.listingIntentPolicy=productListingIntentPolicy(row.listingIntent);row.variantGroupAttributeName=clean(row.variantGroupAttributeName)||productVariantGroupAttributeSuggestion(p,mergeIds.map(function(productId){return {productId:productId};}));row.variantGroupPrimaryValue=productVariantResolvedValue(p,row.variantGroupPrimaryValue);row.listingChangeInstructions=clean(row.listingChangeInstructions)||(row.listingIntent==='merge-existing'?'保留既有主商品與平台編號，把所選商品編號合併為同一組細項。':'把所選商品編號建立為同一組細項。');
        row.variantGroupItems=mergeIds.map(function(productId){const product=catalogById(productId),saved=savedItems.get(productId)||{};return Object.assign({},saved,{productId:productId,sku:clean(product.sku),name:clean(product.originalName||product.name),attributeValue:productVariantResolvedValue(product,saved.attributeValue)});});
      }
      if(completedProductImageUrls(row).length)await syncCompletedListingImagesToProduct(id,row);
      productListingSourceImageCache.set(id,row.referenceImageUrls.slice());
      await Promise.all(row.variantGroupItems.map(async function(item){const media=await loadProductListingVariantMedia(item.productId);item.sourceImageUrls=media.sourceImageUrls;item.completedImageUrls=media.completedImageUrls;}));
      if(row.variantParentProductId)await loadProductListingVariantMedia(row.variantParentProductId);
      if(options&&options.listingMode==='add-variant'){row.listingIntent='add-variant';row.listingMode='add-variant';row.variantGroupEnabled=false;row.listingIntentPolicy=productListingIntentPolicy('add-variant');}
  ...
  async function saveProductListingCase(form,previewAfter,keepOpen,silent){
    ...
    const data=new FormData(form),workflowPurpose=clean(data.get('workflowPurpose'))===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE:'',packageInfo=productPackageFromFormData(data,false),researchSourceRaw=clean(data.get('packageResearchSourceUrl')),researchSourceUrl=safeUrl(researchSourceRaw),listingIntent=normalizeProductListingIntent(data.get('listingIntent'),{listingMode:data.get('listingMode'),variantGroupEnabled:data.get('variantGroupEnabled')==='1'},p),listingIntentPolicy=productListingIntentPolicy(listingIntent),listingChangeInstructions=clean(data.get('listingChangeInstructions'));
    const mergeExisting=listingIntent==='merge-existing',listingMode=listingIntent==='add-variant'?'add-variant':mergeExisting?'merge-existing':'independent',variantParentProductId=clean(data.get('variantParentProductId')),variantParent=variantParentProductId?catalogById(variantParentProductId):null,variantAttributeName=clean(data.get('variantAttributeName')),variantParentAttributeValue=clean(data.get('variantParentAttributeValue'))||(variantParent?productVariantSuggestion(variantParent)||'原款':''),variantAttributeValue=clean(data.get('variantAttributeValue'))||productVariantSuggestion(p),variantParentImageUrl=safeUrl(data.get('variantParentImageUrl')),variantChildImageUrl=safeUrl(data.get('variantChildImageUrl'));
    const variantGroupEnabled=['create-group','merge-existing'].includes(listingIntent),variantGroupAttributeName=clean(data.get('variantGroupAttributeName')),variantGroupPrimaryValue=clean(data.get('variantGroupPrimaryValue'))||productVariantSuggestion(p),variantGroupPrimaryImageUrl=safeUrl(data.get('variantGroupPrimaryImageUrl')),gallerySelections=productVariantGallerySelectionsFromForm(form),variantGallerySelectionInitialized=data.get('variantGallerySelectionInitialized')==='1',variantGroupItems=variantGroupEnabled?productVariantGroupItemsFromForm(form).map(function(item){item.gallerySourceImageUrls=gallerySelections.get(item.productId)||[];return item;}):[],gallerySourceImageUrls=gallerySelections.get(id)||[];
    if(listingIntent==='update-existing'&&!listingChangeInstructions)throw new Error('請寫明這次要修正或替換的內容，系統才會安全地原地修改同一件商品');
    ...
    if(mergeExisting&&!productHasPlatformMapping(p))throw new Error('合併／加入既有商品需要把已有平台商品編號的商品作為第一筆主商品；請回到新增未上架一次勾選全部編號，系統會自動排列');
    if(variantGroupEnabled){
      if(!variantGroupAttributeName||!variantGroupPrimaryValue)throw new Error('請填寫這組商品的區分方式與目前商品的細項名稱');
    ...
    const productResearchStatus=clean(data.get('productResearchStatus'))||'not-searched',sourceUrls=formUrls('productResearchSourceUrls','商品研究來源'),referenceUrls=formUrls('referenceUrls','參考網址'),referenceImageUrls=formUrls('referenceImageUrls','參考圖片網址'),listingImageUrls=formUrls('listingImageUrls','上架圖片網址'),researchComplete=['researched','manual'].includes(productResearchStatus);let selectedReferenceImageUrls=normalizeProductResearchSourceUrls(data.getAll('selectedReferenceImageUrls')).filter(function(url){return referenceImageUrls.includes(url);});
    if(variantGroupEnabled)selectedReferenceImageUrls=referenceImageUrls.slice(0,PRODUCT_SELECTED_IMAGE_MAX);
    ...
    if(requiredCurrentRepresentativeSource&&referenceImageUrls.includes(requiredCurrentRepresentativeSource)&&!selectedReferenceImageUrls.includes(requiredCurrentRepresentativeSource)){
      if(selectedReferenceImageUrls.length>=PRODUCT_SELECTED_IMAGE_MAX)selectedReferenceImageUrls.pop();
      selectedReferenceImageUrls.push(requiredCurrentRepresentativeSource);
    }
    selectedReferenceImageUrls.splice(PRODUCT_SELECTED_IMAGE_MAX);
    ...
    const richContentStandard=productRichContentLifecycle(listingIntent,productDescription);
    ...
    const ref=state.db.collection(COLLECTIONS.listingCases).doc(id),payload={
      productId:id,productSku:p.sku,productName:p.originalName||p.name,productImageUrl:p.imageUrl||'',caseStatus:clean(data.get('caseStatus'))||'draft',workflowPurpose:workflowPurpose,descriptionLayoutVersion:PRODUCT_DESCRIPTION_LAYOUT_VERSION,listingIntent:listingIntent,listingIntentPolicy:listingIntentPolicy,listingChangeInstructions:listingChangeInstructions,listingMode:listingMode,variantParentProductId:listingMode==='add-variant'?variantParentProductId:'',variantParentSku:listingMode==='add-variant'?clean(variantParent.sku):'',variantParentName:listingMode==='add-variant'?clean(variantParent.originalName||variantParent.name):'',variantAttributeName:listingMode==='add-variant'?variantAttributeName:'',variantParentAttributeValue:listingMode==='add-variant'?variantParentAttributeValue:'',variantAttributeValue:listingMode==='add-variant'?variantAttributeValue:'',variantParentImageUrl:listingMode==='add-variant'?variantParentImageUrl:'',variantChildImageUrl:listingMode==='add-variant'?variantChildImageUrl:'',variantGroupEnabled:variantGroupEnabled,variantGroupAttributeName:variantGroupEnabled?variantGroupAttributeName:'',variantGroupPrimaryValue:variantGroupEnabled?variantGroupPrimaryValue:'',variantGroupPrimaryImageUrl:variantGroupEnabled?variantGroupPrimaryImageUrl:'',variantGroupItems:variantGroupItems,variantGallerySelectionInitialized:variantGallerySelectionInitialized,variantImageProcessingQueue:productVariantImageProcessingQueueFromForm(form),sharedVariantGalleryMaximum:PRODUCT_GROUP_LISTING_IMAGE_MAX,
      sourceProductDescription:clean(data.get('sourceProductDescription')),researchInstructions:clean(data.get('researchInstructions')),referenceUrls:referenceUrls,referenceImageUrls:referenceImageUrls,selectedReferenceImageUrls:selectedReferenceImageUrls,gallerySourceImageUrls:gallerySourceImageUrls,listingImageUrls:listingImageUrls,
      ...
      easyStoreHtml:sharedProductHtml,shopeeTitle:clean(data.get('shopeeTitle')),shopeeDescription:productDescription,shopeeRequiredNotes:clean(data.get('shopeeRequiredNotes')),shopeeCategoryPath:clean(data.get('shopeeCategoryPath')),easyStoreShopeeDecision:decisionValues.easyStoreShopeeDecision,
      ...
      priceSnapshot:{easyStore:productListingAutomaticPrice(p,'easyStorePrice'),momo:productListingAutomaticPrice(p,'momoPrice'),coupang:productListingAutomaticPrice(p,'coupangPrice')},stockSnapshot:numberOrNull(p.currentStock)==null?0:numberOrNull(p.currentStock),schemaVersion:13,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION
    };
    ...
    if(variantGroupEnabled){
      ...
      variantGroupItems.forEach(function(item){
        ...
        const childSources=normalizeProductResearchSourceUrls(item.sourceImageUrls||item.imageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),childImages=normalizeProductResearchSourceUrls(item.completedImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX);
        const childPayload=Object.assign({},payload,{
          productId:child.docId,productSku:child.sku,productName:child.originalName||child.name,productImageUrl:item.imageUrls&&item.imageUrls[0]||child.imageUrl||productVariantImages(child)[0]||'',listingIntent:'add-variant',listingIntentPolicy:productListingIntentPolicy('add-variant'),listingChangeInstructions:'建立同一主商品的細項集合：'+variantGroupAttributeName+'／'+item.attributeValue,listingMode:'add-variant',variantParentProductId:id,variantParentSku:clean(p.sku),variantParentName:clean(p.originalName||p.name),variantAttributeName:variantGroupAttributeName,variantParentAttributeValue:variantGroupPrimaryValue,variantAttributeValue:item.attributeValue,variantParentImageUrl:variantGroupPrimaryImageUrl,variantChildImageUrl:item.imageUrls&&item.imageUrls[0]||'',variantGroupEnabled:false,variantGroupAttributeName:'',variantGroupPrimaryValue:'',variantGroupPrimaryImageUrl:'',variantGroupItems:[],variantGallerySelectionInitialized:variantGallerySelectionInitialized,variantImageProcessingQueue:[{productId:child.docId,sku:clean(child.sku),name:clean(child.originalName||child.name),sourceImageUrls:childSources,representativeSourceImageUrl:item.imageUrls&&item.imageUrls[0]||'',completedImageUrls:childImages,gallerySourceImageUrls:item.gallerySourceImageUrls||[],gallerySelectionInitialized:variantGallerySelectionInitialized}],referenceImageUrls:childSources,selectedReferenceImageUrls:childSources,gallerySourceImageUrls:item.gallerySourceImageUrls||[],listingImageUrls:childImages,priceSnapshot:{easyStore:productListingAutomaticPrice(child,'easyStorePrice'),momo:productListingAutomaticPrice(child,'momoPrice'),coupang:productListingAutomaticPrice(child,'coupangPrice')},stockSnapshot:numberOrNull(child.currentStock)==null?0:numberOrNull(child.currentStock),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION
        });
    ...
    form.dataset.exists='1';await writeAudit('儲存商品上架資料','productListingCase',id,p.sku+'｜'+listingIntentPolicy.label+(variantGroupEnabled?'｜同款 '+(variantGroupItems.length+1)+' 個細項':''));
  ...
  function refreshProductVariantImageTarget(form,productId,urls){
    ...
    refreshProductVariantImagePickers(form);refreshProductVariantImageProcessingQueue(form);
  }
  function refreshProductVariantImagePickers(form){
    ...
    queryAll('.ops-listing-variant-item[data-product-id]',form).forEach(function(card){
      const id=clean(card.dataset.productId),product=catalogById(id),sourceImages=productVariantCardSourceImages(form,card),completedImages=sourceImages.length?[]:productVariantImages(product),hidden=productVariantImageHidden(card);let selected=safeUrl(hidden&&hidden.value);
      if(selected&&sourceImages.length&&!sourceImages.includes(selected)){selected='';if(hidden)hidden.value='';card.dataset.variantImageOrigin='missing';}
      const preview=query('[data-variant-image-preview]',card),status=query('[data-variant-image-match-status]',card),picker=query('[data-variant-image-picker]',card),collectorCount=query('[data-variant-collector-count]',card);
      if(preview){const previewUrl=completedImages.length?completedImages[0]:selected;preview.classList.toggle('selected',!!previewUrl);preview.classList.toggle('empty',!previewUrl);preview.innerHTML=previewUrl?'<img src="'+attr(previewUrl)+'" alt="'+attr(product&&product.sku||'細項代表圖')+'">'+(selected?'<button type="button" data-action="product-variant-image-clear" data-id="'+attr(id)+'" data-role="'+attr(card.dataset.variantRole||'')+'" aria-label="清除細項代表圖">×</button>':''):'<span aria-hidden="true">＋</span>';}
      if(status){status.className=selected||completedImages.length?'ok':'missing';status.textContent=completedImages.length?'繁體代表圖已完成':selected?productAutomaticVariantStatusLabel(clean(card.dataset.variantImageOrigin)||'manual'):'尚未選擇代表圖';}
      if(picker)picker.innerHTML=productVariantImagePickerOptionsHtml(id,selected,sourceImages,card.dataset.variantRole);
      if(collectorCount)collectorCount.innerHTML='目前 <b>'+sourceImages.length+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張';
      card.classList.toggle('is-required-missing',!(selected||completedImages.length));
    });
  }
  async function selectProductVariantRepresentativeImage(form,productId,url,role){
    ...
    const hidden=productVariantImageHidden(card);if(hidden)hidden.value=selected;card.dataset.variantImageOrigin='manual';
  ...
  async function clearProductVariantRepresentativeImage(form,productId,role){
    if(!form)throw new Error('找不到商品上架資料');const id=clean(productId),card=productVariantImageTargetCard(form,id,role);if(!card)throw new Error('找不到要清除的細項');
    const hidden=productVariantImageHidden(card);if(hidden)hidden.value='';card.dataset.variantImageOrigin='missing';form.dataset.dirty='1';refreshProductVariantImagePickers(form);refreshProductVariantImageProcessingQueue(form);
    if(!state.db)throw new Error('商品圖片資料庫尚未載入，請重新整理頁面。');
    await state.db.collection(COLLECTIONS.listingCases).doc(clean(form.dataset.id)).set({variantParentImageUrl:safeUrl(query('[name="variantParentImageUrl"]',form)&&query('[name="variantParentImageUrl"]',form).value),variantChildImageUrl:safeUrl(query('[name="variantChildImageUrl"]',form)&&query('[name="variantChildImageUrl"]',form).value),variantGroupPrimaryImageUrl:safeUrl(query('[name="variantGroupPrimaryImageUrl"]',form)&&query('[name="variantGroupPrimaryImageUrl"]',form).value),variantGroupItems:productVariantGroupItemsFromForm(form),updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
    toast('代表圖已清除',clean(catalogById(id)&&catalogById(id).sku)+'｜可重新點選或拖入圖片','success');
  }
  async function copyProductVariantReferenceImage(form,targetProductId,url,role){
    if(!form)throw new Error('找不到商品上架資料');const id=clean(targetProductId),sourceUrl=safeUrl(url),existing=productVariantImageTargetUrls(form,id);if(!id||!sourceUrl||!productVariantImageTargetAllowed(form,id))throw new Error('找不到要接收圖片的細項');
    if(existing.includes(sourceUrl))return selectProductVariantRepresentativeImage(form,id,sourceUrl,role);
  ...
  async function uploadProductVariantReferenceImages(form,productId,files){
    ...
    if(id===clean(form.dataset.id)){const uploaded=await uploadProductReferenceImages(form,files);refreshProductVariantImageTarget(form,id);return uploaded;}
    if(!global.firebase||!global.firebase.storage)throw new Error('圖片上傳服務尚未載入，請重新整理頁面。');
    ...
    if(status)status.innerHTML='<div class="ops-product-ai-status completed"><span>✓</span><div><b>此細項圖片已加入</b><small>'+uploaded.length+' 張已指定給 '+escapeHtml(product.sku||'未設定')+'。</small></div></div>';toast('細項圖片已上傳',clean(product.sku)+'｜'+uploaded.length+' 張','success');return uploaded;
  }
  ...
  async function uploadProductCompletedListingImages(form,files){
    ...
    await requireEasyStoreManagerAuth();
    const id=clean(form.dataset.id);if(!id)throw new Error('找不到商品上架資料');
    const selected=Array.from(files).slice(0,PRODUCT_REFERENCE_IMAGE_MAX*PRODUCT_LISTING_IMAGE_ROLES.length),sourceImages=normalizeProductResearchSourceUrls(new FormData(form).getAll('selectedReferenceImageUrls')).slice(0,PRODUCT_SELECTED_IMAGE_MAX);
    if(!sourceImages.length)throw new Error('請先保留至少一張來源商品圖片');
    const assignments=selected.map(function(file){const name=clean(file&&file.name),match=name.match(/^(\d{1,2})[-_ ](cleanMain|brandedHero|storefrontPortrait|localizedDetail|specification|variantRepresentative)(?:[-_ .]|$)/i);if(!match)throw new Error('v3 完成圖檔名需為「來源順序-角色」，例如 01-cleanMain.jpg、01-storefrontPortrait.jpg、01-brandedHero.jpg');const sourceIndex=Number(match[1])-1,role=PRODUCT_LISTING_IMAGE_ROLES.find(function(value){return value.toLowerCase()===match[2].toLowerCase();});if(sourceIndex<0||sourceIndex>=sourceImages.length||!role)throw new Error('完成圖 '+name+' 的來源順序或角色不正確');return {file:file,sourceIndex:sourceIndex,role:role};}),assignmentKeys=new Set();
    assignments.forEach(function(item){const key=item.sourceIndex+'|'+item.role;if(assignmentKeys.has(key))throw new Error('同一來源的 '+item.role+' 角色只能有一張完成圖');assignmentKeys.add(key);});
    sourceImages.forEach(function(url,index){if(!assignments.some(function(item){return item.sourceIndex===index&&!['brandedHero','storefrontPortrait'].includes(item.role);}))throw new Error('來源第 '+(index+1)+' 張缺少 cleanMain／localizedDetail／specification／variantRepresentative 完成圖');});
    if(!assignments.some(function(item){return item.role==='cleanMain';})||!assignments.some(function(item){return item.role==='storefrontPortrait';})||!assignments.some(function(item){return item.role==='brandedHero';}))throw new Error('整組完成圖至少需要獨立的 cleanMain、storefrontPortrait 與 brandedHero，三種版型不能共用同一 URL');
    selected.forEach(function(file){if(!['image/jpeg','image/png','image/webp'].includes(clean(file&&file.type)))throw new Error('只支援 JPG、PNG 或 WebP 圖片');if(Number(file&&file.size||0)>8*1024*1024)throw new Error('每張圖片不可超過 8 MB');});
    for(let imageIndex=0;imageIndex<assignments.length;imageIndex+=1){
      const assignment=assignments[imageIndex],profile=PRODUCT_BRAND_TEMPLATE_CONTRACT[assignment.role];
      if(!profile)continue;
      const bitmap=await loadPhysicalPhotoImage(assignment.file);
      const dimensions={width:Number(bitmap.width||bitmap.naturalWidth||0),height:Number(bitmap.height||bitmap.naturalHeight||0)};if(typeof bitmap.close==='function')bitmap.close();
      if(dimensions.width!==profile.widthPx||dimensions.height!==profile.heightPx)throw new Error(assignment.role+' 必須使用固定母版尺寸 '+profile.widthPx+'×'+profile.heightPx+'；目前是 '+dimensions.width+'×'+dimensions.height);
    }
    ...
    for(let index=0;index<assignments.length;index+=1){
      const file=assignments[index].file,extension=(file.type==='image/png'?'png':file.type==='image/webp'?'webp':'jpg'),path='ops-product-listing-cases/'+id+'/completed/'+Date.now()+'-'+index+'-'+Math.random().toString(36).slice(2,9)+'.'+extension;
    ...
    const listingInput=query('[name="listingImageUrls"]',form),instructions=clean(query('[name="imageGenerationInstructions"]',form)&&query('[name="imageGenerationInstructions"]',form).value),createdAt=new Date().toISOString(),brandCreativeStyleAssignment=await reserveProductBrandCreativeStyle(id,null),brandRenderProof=productBrandCreativeRenderProof(brandCreativeStyleAssignment);
    const sharedListingUrls=uploaded.slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);
    if(listingInput)listingInput.value=sharedListingUrls.join('\n');
    form.dataset.dirty='1';await saveProductListingCase(form,false,true,true);
    const generated=uploaded.map(function(url,index){const assignment=assignments[index],richHero=assignment.role==='brandedHero'||assignment.role==='storefrontPortrait',cleanMain=assignment.role==='cleanMain',template=PRODUCT_BRAND_TEMPLATE_CONTRACT[assignment.role]||null;return {id:'codex-chat-'+Date.now()+'-'+index,url:url,status:'ready',mode:'codex-chat-single-pass',model:'Codex 對話圖片處理',qaStatus:'approved',localizationStatus:'completed',qaModel:'人工確認後上傳',qaAttempts:0,sourceImageUrl:sourceImages[assignment.sourceIndex],sourceOrder:assignment.sourceIndex+1,roles:[assignment.role],assetFlags:{containsLogo:richHero,containsText:richHero,containsContactInfo:false,containsQrCode:false,greenBrandTemplate:richHero,momoPromotionEligible:cleanMain},templateVersion:template?PRODUCT_BRAND_TEMPLATE_CONTRACT.version:'',templateAssetSha256:template?template.assetSha256:'',templateComposition:template?PRODUCT_BRAND_TEMPLATE_CONTRACT.composition:'',creativeStyleAssignment:template?brandCreativeStyleAssignment:null,brandRenderProof:template?brandRenderProof:null,brandCommercialPosterQa:template?{approved:true,fullCommercialPosterDesign:true,genericInformationCardLayoutFound:false,assignedStyleControlsWholeComposition:true,productIntegratedAsHero:true,strongCommercialHierarchy:true,threeFeaturesIntegrated:true,productUnobscured:true,fixedHeaderAndLogoIntact:true,borderDoesNotCrossLogo:true,issues:[],summary:'管理者已在 Codex 對話確認完整商業海報完成圖。',verificationSource:'human-approved-codex-completed-image-upload'}:null,instructions:instructions,createdAt:createdAt};});
    const completedCase={brandCreativeStyleAssignment:brandCreativeStyleAssignment,generatedListingImages:generated,listingImageUrls:sharedListingUrls,referenceImageUrls:sourceImages,selectedReferenceImageUrls:sourceImages,referenceImagesCleared:false,sourceImageRetentionPolicy:{mode:'metadata-only-after-required-binary-cleanup',sourceBinaryCleanupRequired:true,cleanupStatus:'blocked-until-all-references-verified',cleanupWorkerRequired:true,eligibleForDeletion:false},lastImageGeneration:{status:'completed',processingMode:'codex-chat-single-pass',processingPasses:1,requestedCount:sourceImages.length,completedCount:sourceImages.length,outputCount:uploaded.length,processedCount:sourceImages.length,verifiedCount:sourceImages.length,failedCount:0,sourceImageUrls:sourceImages,instructions:instructions,completedAt:serverTimestamp()},lastImageGenerationError:'',mediaDecision:'accepted',updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION};
    await state.db.collection(COLLECTIONS.listingCases).doc(id).set(completedCase,{merge:true});
    await syncCompletedListingImagesToProduct(id,completedCase);
    const completedInput=query('[name="currentCompletedImageUrls"]',form);if(completedInput)completedInput.value=sharedListingUrls.join('\n');refreshProductVariantImageProcessingQueue(form);
    const generatedBox=query('#productAiImageGenerationStatus',form),preview=byId('productListingImagePreview');if(generatedBox)generatedBox.innerHTML='<div class="ops-product-ai-status completed"><span>✓</span><div><b>Codex 完成圖已採用</b><small>'+uploaded.length+' 張角色輸出已保存；本次上架共用池最多使用 '+sharedListingUrls.length+' 張。</small></div></div>';if(preview)preview.innerHTML=productListingImagePreviewHtml(sharedListingUrls,'準備上架圖片');
    ...
    toast('繁體完成圖已同步',uploaded.length+' 張已回寫商品主圖並可直接進入本次網路上架。','success');
  }
  ...
  function updateProductImageCollectionUi(form,message,isError,targetProductId){
    if(!form)return;const id=clean(form.dataset.id),targetId=clean(targetProductId)||clean(productImageCollectionSession&&productImageCollectionSession.productId)||id,active=!!(productImageCollectionSession&&productImageCollectionSession.active&&targetId===id),count=productImageCollectionCount(form,id),button=query('[data-action="product-image-collection-toggle"]',form),countBox=query('#productImageCollectionCount',form),status=query('#productImageCollectionStatus',form),singleControls=query('.ops-listing-single-image-controls',form),singleMode=!singleControls||!singleControls.classList.contains('is-hidden');
    if(button){button.textContent=active?'停止搜圖':'開始搜圖';button.classList.toggle('primary',!active);button.classList.toggle('danger',active);button.disabled=count>=PRODUCT_IMAGE_COLLECTION.maxImages&&!active;}
    if(countBox)countBox.innerHTML='目前 <b>'+count+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張';
    queryAll('[data-action="product-variant-image-collection"]',form).forEach(function(targetButton){
      const buttonId=clean(targetButton.dataset.id),buttonActive=!!(productImageCollectionSession&&productImageCollectionSession.active&&buttonId===targetId),buttonCount=productImageCollectionCount(form,buttonId),card=targetButton.closest('.ops-listing-variant-item'),cardCount=card&&query('[data-variant-collector-count]',card),cardStatus=card&&query('[data-variant-collector-status]',card),isTarget=buttonId===targetId;
      targetButton.textContent=buttonActive?'停止收圖':'收圖';targetButton.classList.toggle('danger',buttonActive);targetButton.classList.toggle('soft',!buttonActive);targetButton.disabled=buttonCount>=PRODUCT_IMAGE_COLLECTION.maxImages&&!buttonActive;
      if(cardCount)cardCount.innerHTML='目前 <b>'+buttonCount+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張';
      if(cardStatus){
        if(!isTarget)cardStatus.innerHTML='';
        else if(isError)cardStatus.innerHTML='<span class="error">'+escapeHtml(message||'收圖未啟動')+'</span>';
        else if(buttonActive)cardStatus.innerHTML='<span class="active">正在收圖</span>';
        else if(message){const compact=/收滿/.test(message)?'已收滿 20 張':/停止|結束/.test(message)?'已停止收圖':/加入|截圖/.test(message)?'圖片已加入，可繼續收圖':message;cardStatus.innerHTML='<span class="done">'+escapeHtml(compact)+'</span>';}
        else cardStatus.innerHTML='';
      }
    });
    if(status&&!singleMode)status.innerHTML='';
    else if(status){
      if(isError)status.innerHTML='<div class="ops-listing-media-message error">'+escapeHtml(message||'搜圖未啟動')+'</div>';
      else if(active)status.innerHTML='<div class="ops-listing-media-message">正在搜圖</div>';
      else if(message)status.innerHTML='<div class="ops-listing-media-message">'+escapeHtml(message)+'</div>';
  ...
  async function stopProductImageCollection(form){
    const session=productImageCollectionSession;if(!session){updateProductImageCollectionUi(form,'已停止收圖',false,clean(form&&form.dataset.id));return;}
    let stopError=null;
    try{await requestProductImageCollection('stop',{sessionId:session.sessionId,productId:session.productId});}
    catch(error){stopError=error;if(form)updateProductImageCollectionUi(form,errorMessage(error),true,session.productId);}
    try{await drainProductImageCollectionUploads(form);}
    finally{productImageCollectionSession=null;}
    if(stopError)throw stopError;
    updateProductImageCollectionUi(form,'已停止收圖；已加入的圖片都已保存。',false,session.productId);
  }
  ...
  async function receiveProductImageCollectionFile(payload){
    ...
    try{
      ...
      if(count>=PRODUCT_IMAGE_COLLECTION.maxImages){productImageCollectionSession.active=false;productImageCollectionSession.stoppedReason='full';updateProductImageCollectionUi(form,'已收滿 20 張，收圖模式自動結束。',false,productId);}
      else updateProductImageCollectionUi(form,'圖片已加入；可繼續到任何商品網頁點圖或框選。',false,productId);
      postProductImageCollectionFileAck({requestId:requestId,ok:true,count:count});
    }catch(error){postProductImageCollectionFileAck({requestId:requestId,ok:false,error:errorMessage(error),count:form?productImageCollectionCount(form,productId):0});updateProductImageCollectionUi(form,errorMessage(error),true,productId);throw error;}
  }
  function queueProductImageCollectionFile(payload){
    productImageCollectionPendingUploads+=1;productImageCollectionDeliverySequence+=1;
    const next=productImageCollectionUploadChain.catch(function(){return null;}).then(function(){return receiveProductImageCollectionFile(payload);});
    productImageCollectionUploadChain=next.catch(function(error){productImageCollectionUploadFailures.push({requestId:clean(payload&&payload.requestId),error:errorMessage(error)});throw error;}).finally(function(){productImageCollectionPendingUploads=Math.max(0,productImageCollectionPendingUploads-1);});
    return productImageCollectionUploadChain;
  }
  async function finishProductImageCollectionBeforeHandoff(form){
    if(productImageCollectionSession&&productImageCollectionSession.active&&form&&productVariantImageTargetAllowed(form,productImageCollectionSession.productId))await stopProductImageCollection(form);
    else await drainProductImageCollectionUploads(form);
  }
  ...
  let productImageCollectionResumeProductId='';
  async function resumeProductImageCollectionForm(session){
    const productId=clean(session&&session.productId);if(!productId||productImageCollectionResumeProductId===productId)return;
    const current=byId('productListingCaseForm');if(current&&productVariantImageTargetAllowed(current,productId))return;
    productImageCollectionResumeProductId=productId;
    try{
      if((location.hash||'').split('?')[0]!=='#products')location.hash='products';
      for(let attempt=0;attempt<24&&!catalogById(productId);attempt+=1){
        if(!state.loading&&state.db)await loadProductsOnly(true);
        if(!catalogById(productId))await new Promise(function(resolve){global.setTimeout(resolve,250);});
      }
      if(!catalogById(productId))throw new Error('找不到目前收圖商品');
      await openProductListingCase(productId,{skipAutoResearch:true});
      const restored=byId('productListingCaseForm');if(restored)updateProductImageCollectionUi(restored,'正在收圖',false,productId);
    }finally{productImageCollectionResumeProductId='';}
  }
  function selectProductReferenceImages(form,selected){
    if(!form)return;
    queryAll('[name="selectedReferenceImageUrls"]',form).forEach(function(input,index){input.checked=selected&&index<PRODUCT_SELECTED_IMAGE_MAX;const card=input.closest('label');if(card){card.classList.toggle('selected',input.checked);const mark=query('.ops-listing-source-check',card);if(mark)mark.textContent=input.checked?'✓':'';}});form.dataset.dirty='1';refreshProductListingImagePreviews(form);
  }
  async function removeProductReferenceImage(form,url){
    ...
    if(productImageCollectionSession&&productImageCollectionSession.active&&productImageCollectionSession.productId===clean(form.dataset.id)){
      ...
      updateProductImageCollectionUi(form,'已刪除一張；可繼續到任何商品網頁點圖或框選截圖。');
    }
    toast('圖片已刪除','已從這件準備上架商品移除。','success');
  }
  async function removeProductVariantReferenceImage(form,productId,url,role){
    if(!form)return;const id=clean(productId),sourceUrl=safeUrl(url);if(!id||!sourceUrl)throw new Error('找不到要刪除的細項圖片');
    if(id===clean(form.dataset.id))return removeProductReferenceImage(form,sourceUrl);
    const card=productVariantImageTargetCard(form,id,role),existing=productVariantCardSourceImages(form,card),urls=existing.filter(function(value){return value!==sourceUrl;});
    if(!card||urls.length===existing.length)throw new Error('這張圖片已經不在目前細項裡');
    const hidden=productVariantImageHidden(card);if(hidden&&safeUrl(hidden.value)===sourceUrl)hidden.value='';
    refreshProductVariantImageTarget(form,id,urls);form.dataset.dirty='1';await persistProductVariantReferenceImages(form,id,urls,urls);
    if(productImageCollectionSession&&productImageCollectionSession.active&&productImageCollectionSession.productId===id){
      productImageCollectionSession.currentCount=urls.length;
      global.postMessage({source:PRODUCT_IMAGE_COLLECTION.source,type:PRODUCT_IMAGE_COLLECTION.start,payload:productImageCollectionSession},global.location.origin);
      updateProductImageCollectionUi(form,'已刪除一張；可繼續到任何商品網頁點圖或框選截圖。',false,id);
    }
    toast('細項圖片已刪除',clean(catalogById(id)&&catalogById(id).sku)+'｜已從這個細項移除','success');
  }
  ...
  function productListingDraftFromForm(form){
    ...
    const productDescription=clean(data.get('productDescription'))||clean(data.get('commonProductDescription'))||[clean(data.get('shortDescription')),clean(data.get('featureList')),clean(data.get('specificationText'))].filter(Boolean).join('\n\n'),sharedHtml=productDescriptionToSafeHtml(productDescription),listingIntent=normalizeProductListingIntent(data.get('listingIntent'),{listingMode:data.get('listingMode'),variantGroupEnabled:data.get('variantGroupEnabled')==='1'},product);
    const richContentStandard=productRichContentLifecycle(listingIntent,productDescription);
    return {
      ...
      listingIntent:listingIntent,listingIntentPolicy:productListingIntentPolicy(listingIntent),listingChangeInstructions:clean(data.get('listingChangeInstructions')),listingMode:listingIntent==='add-variant'?'add-variant':listingIntent==='merge-existing'?'merge-existing':'independent',variantParentProductId:clean(data.get('variantParentProductId')),variantParent:catalogById(clean(data.get('variantParentProductId'))),variantAttributeName:clean(data.get('variantAttributeName')),variantParentAttributeValue:clean(data.get('variantParentAttributeValue')),variantAttributeValue:clean(data.get('variantAttributeValue')),variantParentImageUrl:safeUrl(data.get('variantParentImageUrl')),variantChildImageUrl:safeUrl(data.get('variantChildImageUrl')),variantGroupEnabled:['create-group','merge-existing'].includes(listingIntent),variantGroupAttributeName:clean(data.get('variantGroupAttributeName')),variantGroupPrimaryValue:clean(data.get('variantGroupPrimaryValue')),variantGroupPrimaryImageUrl:safeUrl(data.get('variantGroupPrimaryImageUrl')),variantGroupItems:productVariantGroupItemsFromForm(form),variantGallerySelectionInitialized:data.get('variantGallerySelectionInitialized')==='1',
      identityStatus:clean(data.get('identityStatus')),identityDecision:normalizeProductListingDecision(data.get('identityDecision')),productResearchStatus:clean(data.get('productResearchStatus'))||'not-searched',productResearchSourceUrls:normalizeProductResearchSourceUrls(data.get('productResearchSourceUrls')),alternateNames:clean(data.get('alternateNames')),searchKeywords:clean(data.get('searchKeywords')),sellingPoints:clean(data.get('sellingPoints')),productDescription:productDescription,specificationText:clean(data.get('specificationText')),includedItems:clean(data.get('includedItems')),material:clean(data.get('material')),color:clean(data.get('color')),countryOfOrigin:clean(data.get('countryOfOrigin')),warrantyInfo:clean(data.get('warrantyInfo')),shortDescription:clean(data.get('shortDescription')),commonProductDescription:productDescription,featureList:clean(data.get('featureList')),faqText:clean(data.get('faqText')),commonContentDecision:normalizeProductListingDecision(data.get('commonContentDecision')),richContentStandard:richContentStandard,
      referenceUrls:normalizeProductResearchSourceUrls(data.get('referenceUrls')),referenceImageUrls:normalizeProductResearchSourceUrls(data.get('referenceImageUrls')),gallerySourceImageUrls:Array.from(productVariantGallerySelectionsFromForm(form).values()).flat().slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX),variantImageProcessingQueue:productVariantImageProcessingQueueFromForm(form),sharedVariantGalleryMaximum:PRODUCT_GROUP_LISTING_IMAGE_MAX,listingImageUrls:normalizeProductResearchSourceUrls(data.get('listingImageUrls')).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX),physicalImageUrls:normalizeProductResearchSourceUrls(product&&product.physicalImageUrls).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX),physicalOriginalImageUrls:normalizeProductResearchSourceUrls(product&&product.physicalOriginalImageUrls).slice(0,PRODUCT_PHYSICAL_IMAGE_MAX),physicalImages:Array.isArray(product&&product.physicalImages)?product.physicalImages.slice(-PRODUCT_PHYSICAL_IMAGE_MAX):[],imagePlan:clean(data.get('imagePlan')),mediaDecision:normalizeProductListingDecision(data.get('mediaDecision')),
      enabledEasyStoreShopee:true,easyStoreShopeeDecision:normalizeProductListingDecision(data.get('easyStoreShopeeDecision')),easyStoreHtml:sharedHtml,shopeeTitle:clean(data.get('shopeeTitle'))||clean(product&&((product.originalName)||(product.onlineName)||(product.name))),shopeeDescription:productDescription,shopeeCategoryPath:clean(data.get('shopeeCategoryPath')),shopeeAttributeValues:productShopeeAttributesFromForm(form),
      enabledMomo:true,momoDecision:normalizeProductListingDecision(data.get('momoDecision')),momoGoodsName:clean(data.get('momoGoodsName'))||clean(product&&((product.originalName)||(product.onlineName)||(product.name))),momoSlogan:clean(data.get('momoSlogan')),momoHtml:sharedHtml,momoCategoryCode:clean(data.get('momoCategoryCode')),
      enabledCoupang:true,coupangDecision:normalizeProductListingDecision(data.get('coupangDecision')),coupangTitle:clean(data.get('coupangTitle'))||clean(product&&((product.originalName)||(product.onlineName)||(product.name))),coupangDescriptionHtml:sharedHtml,coupangCategoryCode:clean(data.get('coupangCategoryCode')),
      easyStorePrice:numberOrNull(data.get('easyStorePrice'))==null?productListingAutomaticPrice(product,'easyStorePrice'):numberOrNull(data.get('easyStorePrice')),momoPrice:numberOrNull(data.get('momoPrice'))==null?productListingAutomaticPrice(product,'momoPrice'):numberOrNull(data.get('momoPrice')),coupangPrice:numberOrNull(data.get('coupangPrice'))==null?productListingAutomaticPrice(product,'coupangPrice'):numberOrNull(data.get('coupangPrice')),stock:numberOrNull(data.get('currentStock')),
  ...
  function productResearchReady(draft){
    const lifecycle=draft.richContentStandard||productRichContentLifecycle(draft.listingIntent,draft.productDescription);
    return !!((draft.researchedName||draft.name)&&lifecycle.descriptionStatus&&lifecycle.descriptionStatus.ready);
  }
  function productListingReadiness(draft){
    ...
    const intentReady=draft.listingIntent!=='update-existing'||!!draft.listingChangeInstructions;
    ...
    const easy=!draft.enabledEasyStoreShopee||!!(intentReady&&draft.sku&&listingName&&common&&media&&draft.easyStoreHtml&&draft.shopeeTitle&&draft.shopeeDescription&&draft.shopeeCategoryPath&&draft.easyStorePrice!=null&&shopeeLogisticsReady&&shopeeAttributesReady&&variantReady&&groupReady&&parentPlatformReady('easyStore'));
    const momo=!draft.enabledMomo||!!(intentReady&&draft.sku&&common&&media&&draft.momoGoodsName&&draft.momoHtml&&draft.momoPrice!=null&&packageReady&&variantReady&&groupReady&&parentPlatformReady('momo'));
    const coupang=!draft.enabledCoupang||!!(intentReady&&draft.sku&&common&&media&&draft.coupangTitle&&draft.coupangDescriptionHtml&&draft.coupangPrice!=null&&packageReady&&variantReady&&groupReady&&parentPlatformReady('coupang'));
    ...
    return {common:common,media:media,packageReady:packageReady,shopeeLogisticsReady:shopeeLogisticsReady,shopeeLogisticsManualConfirmationRequired:shopeeLogisticsManualConfirmationRequired,shopeeAttributesReady:shopeeAttributesReady,shopeeAttributeMissing:shopeeAttributeMissing,intentReady:intentReady,variantReady:variantReady,groupReady:groupReady,parentPlatformReady:parentPlatformReady,easy:easy,momo:momo,coupang:coupang,selected:selected,all:selected.length>0&&intentReady&&easy&&momo&&coupang};
  }
  ...
  function openProductListingPreview(form){
    ...
    const contentRows=[
      productListingCheckRow('本次處理規則',draft.listingIntentPolicy.label+'｜'+draft.listingIntentPolicy.description,readiness.intentReady,'修改既有商品時必須寫明要替換的內容'),productListingCheckRow('上架商品名稱',listingName,!!listingName),productListingCheckRow('固定商品內容標準','特色 '+draft.richContentStandard.descriptionStatus.featureCount+' / '+PRODUCT_RICH_CONTENT_FEATURE_TARGET+'｜使用重點 '+draft.richContentStandard.descriptionStatus.usageCount+' / '+PRODUCT_RICH_CONTENT_USAGE_TARGET+'｜規格 '+draft.richContentStandard.descriptionStatus.specificationCount+' 項',contentReady,draft.richContentStandard.mode==='legacy-upgrade-in-place'?'既有商品原地升級，不改 SKU 與平台編號':'新商品第一次上架前必須完成'),productListingCheckRow('完整商品介紹',draft.productDescription?'已填 '+draft.productDescription.length+' 字':'',contentReady),productListingCheckRow('固定最後一句',PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER,true),productListingCheckRow('上架圖片',draft.imageCount+' 張'+(draft.physicalImageUrls.length?'＋實體圖 '+draft.physicalImageUrls.length+' 張':''),hasImages)
    ];
    ...
    const easyRows=[
      productListingCheckRow('SKU',draft.sku,!!draft.sku),productListingCheckRow('商品識別',draft.listingIntentPolicy.identityAction+'｜禁止建立重複商品',true),productListingCheckRow('蝦皮標題',draft.shopeeTitle,!!draft.shopeeTitle),productListingCheckRow('EasyStore 商品內容',draft.easyStoreHtml?'已填 '+draft.easyStoreHtml.length+' 字':'',!!draft.easyStoreHtml),productListingCheckRow('蝦皮分類',draft.shopeeCategoryPath,!!draft.shopeeCategoryPath),productListingCheckRow('蝦皮分類屬性',readiness.shopeeAttributesReady?'已依分類模板準備':readiness.shopeeAttributeMissing.join('、'),readiness.shopeeAttributesReady,'請先回到上架資料確認保固欄位'),productListingCheckRow('EasyStore 售價',money(draft.easyStorePrice),draft.easyStorePrice!=null),productListingCheckRow(readiness.shopeeLogisticsManualConfirmationRequired?'外箱資料':'物流與包裝',shippingText+(packageText?'｜'+packageText:'')+(readiness.shopeeLogisticsManualConfirmationRequired?'｜蝦皮實際物流尚未自動選好':''),readiness.shopeeLogisticsReady,packageReady?'目前物流選擇或尺寸仍需人工確認':'請填寫外箱尺寸與重量')
    ];
    const momoRows=[
      productListingCheckRow('單一 SKU',draft.sku,!!draft.sku),productListingCheckRow('MOMO 商品名稱',draft.momoGoodsName,!!draft.momoGoodsName),productListingCheckRow('MOMO 商品內容',draft.momoHtml?'已填 '+draft.momoHtml.length+' 字':'',!!draft.momoHtml),productListingCheckRow('MOMO 分類',draft.momoCategoryCode||'自動判斷（限定樂器／樂器配件）',true),productListingCheckRow('MOMO 售價',money(draft.momoPrice),draft.momoPrice!=null),productListingCheckRow('MOMO 固定配送','甲指（第三方）000001｜台中市圓環東路347號｜新竹物流',true),productListingCheckRow('物流與包裝',shippingText+(packageText?'｜'+packageText:''),packageReady)
    ];
    const coupangRows=[
      productListingCheckRow('賣家 SKU',draft.sku,!!draft.sku),productListingCheckRow('酷澎標題',draft.coupangTitle,!!draft.coupangTitle),productListingCheckRow('酷澎商品內容',draft.coupangDescriptionHtml?'已填 '+draft.coupangDescriptionHtml.length+' 字':'',!!draft.coupangDescriptionHtml),productListingCheckRow('酷澎分類',draft.coupangCategoryCode||'自動判斷（限定樂器／樂器配件）',true),productListingCheckRow('酷澎售價',money(draft.coupangPrice),draft.coupangPrice!=null),productListingCheckRow('物流與包裝',shippingText+(packageText?'｜'+packageText:''),packageReady)
    ];
    ...
    const body='<div class="ops-callout '+(readiness.all?'green':'')+'"><b>'+(readiness.all?'這件商品的上架資料已齊全':'還有幾個上架欄位需要補')+'</b><br><span>紅色驚嘆號就是需要補的地方；不需要再做身分、來源或採用審核。</span></div>'+homeLogisticsNotice
      ...
      +(draft.enabledEasyStoreShopee?productPlatformDraftCard('EasyStore → 蝦皮',{ok:easyOk,label:easyOk&&readiness.shopeeLogisticsManualConfirmationRequired?'可送出・待選物流':'',type:readiness.shopeeLogisticsManualConfirmationRequired?'yellow':''},easyRows,'不預先搜尋平台目錄；嚴格依本案處理方式執行，條件不符就停止，不會自行改成新增或修改。'):'')
      ...
      +'</div><div class="ops-drawer-footer"><button class="ops-button primary" type="button" data-action="product-listing-case-open" data-id="'+attr(draft.id)+'">返回並使用 Codex v3 固定流程</button></div>';
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const detail=hasFailure?'已完成或已排隊的平台不會再建立第二筆；同一 job 會從失敗階段續做，只有送出結果不明時才精確查同 SKU。':needsInput?'補完必填資料後，必須從營運中心建立新的 v3 快照；舊快照不會混入。':'已依案件指定的商品處理方式與中央平台 ID 執行，沒有擅自改成新增或修改。'+(shopeeHomeLogisticsNeedsManualConfirmation?' 一般宅配的蝦皮實際物流仍須在 EasyStore 人工選擇。':'');
    const body='<div class="ops-callout '+(!hasFailure&&!needsInput?'green':'yellow')+'"><b>'+escapeHtml(headline)+'</b><br><span>'+escapeHtml(detail)+'</span></div><div class="ops-listing-platform-grid">'+(cards||emptyHtml('沒有平台結果','請返回商品上架資料重試。'))+'</div><div class="ops-drawer-footer"><button class="ops-button ghost" type="button" data-action="drawer-close">關閉</button>'+(!needsInput&&!hasFailure?'<button class="ops-button soft" type="button" data-action="product-platform-recheck" data-id="'+attr(draft.id)+'">重新檢查四平台</button>':'')+'<button class="ops-button primary" type="button" data-action="product-listing-case-open" data-id="'+attr(draft.id)+'">'+(needsInput||hasFailure?'返回修改':'查看上架資料')+'</button></div>';
  ...
  async function refreshProductListingHandoffMediaFromDatabase(form){
    if(!form)return new Map();
    const currentId=clean(form.dataset.id),ids=[currentId];
    const data=new FormData(form),parentId=clean(data.get('variantParentProductId'));if(parentId&&!ids.includes(parentId))ids.push(parentId);
    productVariantGroupItemsFromForm(form).forEach(function(item){if(item.productId&&!ids.includes(item.productId))ids.push(item.productId);});
    const entries=await Promise.all(ids.map(async function(id){return [id,await loadProductListingVariantMedia(id)];})),mediaByProduct=new Map(entries);
    const current=mediaByProduct.get(currentId)||{sourceImageUrls:[],selectedSourceImageUrls:[],completedImageUrls:[],generatedListingImages:[]},sourceInput=query('[name="referenceImageUrls"]',form),completedInput=query('[name="currentCompletedImageUrls"]',form),listingInput=query('[name="listingImageUrls"]',form),databaseSources=normalizeProductResearchSourceUrls(current.sourceImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX),databaseSelected=normalizeProductResearchSourceUrls(current.selectedSourceImageUrls).filter(function(url){return databaseSources.includes(url);}).slice(0,PRODUCT_SELECTED_IMAGE_MAX),databaseCompleted=normalizeProductResearchSourceUrls(current.completedImageUrls).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);
    if(sourceInput)sourceInput.value=databaseSources.join('\n');
    if(completedInput)completedInput.value=databaseCompleted.join('\n');
    if(listingInput)listingInput.value=databaseCompleted.join('\n');
    queryAll('.ops-listing-variant-item[data-product-id]',form).forEach(function(card){
      const id=clean(card.dataset.productId),media=mediaByProduct.get(id);if(!media||id===currentId)return;
      const source=query('[data-variant-source-images]',card),completed=query('[data-variant-completed-images]',card);
      if(source)source.value=normalizeProductResearchSourceUrls(media.sourceImageUrls).slice(0,PRODUCT_REFERENCE_IMAGE_MAX).join('\n');
      if(completed)completed.value=normalizeProductResearchSourceUrls(media.completedImageUrls).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX).join('\n');
    });
    const readySourceSet=new Set(readyProductListingImageRows({generatedListingImages:current.generatedListingImages}).filter(function(row){return row.roles.length>0;}).map(function(row){return row.sourceImageUrl;})),visibleSources=databaseSources.filter(function(url){return !readySourceSet.has(url);}),hiddenSelected=databaseSelected.filter(function(url){return readySourceSet.has(url);}).map(function(url){return '<input type="hidden" name="selectedReferenceImageUrls" value="'+attr(url)+'">';}).join(''),referenceBox=byId('productReferenceImagePreview'),listingBox=byId('productListingImagePreview');
    if(referenceBox)referenceBox.innerHTML=hiddenSelected+(visibleSources.length?productReferenceImageSelectorHtml(visibleSources,databaseSelected):readySourceSet.size?'<div class="ops-listing-image-empty">來源圖譜系已隱藏保存；畫面只顯示繁體完成圖。</div>':'');
    if(listingBox)listingBox.innerHTML=productListingImagePreviewHtml(databaseCompleted,'上架圖片');
    refreshProductVariantImagePickers(form);refreshProductVariantImageProcessingQueue(form);
  ...
  function setProductListingCodexUi(form,status,title,detail){
    ...
    form.inert=running;form.setAttribute('aria-busy',running?'true':'false');
    queryAll('[data-action="product-listing-codex-complete"]',form).forEach(function(button){
      const active=running&&clean(button.dataset.scope)===clean(form.dataset.activeScope)&&clean(button.dataset.purpose)===clean(form.dataset.workflowPurpose);button.disabled=running;button.setAttribute('aria-busy',active?'true':'false');button.classList.toggle('is-busy',active);
    });
  ...
  function productListingRequiredImageRoles(targetScope,includeGroupHeroes){
    const platforms=productListingTargetPlatforms(targetScope),roles=['cleanMain'];
    if(includeGroupHeroes!==false&&platforms.includes('easyStore'))roles.push('storefrontPortrait');
    if(includeGroupHeroes!==false&&platforms.includes('shopee'))roles.push('brandedHero');
    return roles;
  }
  function productListingCodexMediaSnapshot(raw,product,targetScope,includeGroupHeroes){
    const source=raw&&typeof raw==='object'?raw:{},generated=normalizeGeneratedListingImages(source.generatedListingImages),allReadyRows=readyProductListingImageRows(source),conflictingRoleUrls=productListingConflictingRoleUrls(allReadyRows),readyRows=allReadyRows.filter(function(row){return !conflictingRoleUrls.has(row.url);}),last=source.lastImageGeneration&&typeof source.lastImageGeneration==='object'?source.lastImageGeneration:{};
    const references=normalizeProductResearchSourceUrls(source.referenceImageUrls),selected=normalizeProductResearchSourceUrls(source.selectedReferenceImageUrls).filter(function(url){return references.includes(url)||!references.length;}),lineage=normalizeProductResearchSourceUrls([].concat(selected,references,last.sourceImageUrls||[],generated.map(function(row){return row.sourceImageUrl||'';}))),sourceImageUrls=(selected.length?selected:references.length?references:lineage).slice(0,PRODUCT_SELECTED_IMAGE_MAX),completedImageUrls=completedProductImageUrls(source).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX),readyRowsBySource=new Map();
    readyRows.forEach(function(row){const rows=readyRowsBySource.get(row.sourceImageUrl)||[];rows.push(row);readyRowsBySource.set(row.sourceImageUrl,rows);});
    const usableRoles=['cleanMain','localizedDetail','specification','variantRepresentative'],pendingSourceImageUrls=sourceImageUrls.filter(function(url){const rows=readyRowsBySource.get(url)||[];return !rows.some(function(row){return row.roles.some(function(role){return usableRoles.includes(role);});});}),missingRequiredRoles=[];
    productListingRequiredImageRoles(targetScope,includeGroupHeroes).forEach(function(role){if(sourceImageUrls.length&&!readyRows.some(function(row){return row.roles.includes(role);}))missingRequiredRoles.push(role);});
    if(conflictingRoleUrls.size)missingRequiredRoles.push('hero-role-conflict');
    const representativeSourceImageUrl=safeUrl(source.variantChildImageUrl||source.variantGroupPrimaryImageUrl),representativeRows=representativeSourceImageUrl?(readyRowsBySource.get(representativeSourceImageUrl)||[]):[];
    if(representativeSourceImageUrl&&!representativeRows.some(function(row){return row.roles.includes('cleanMain');}))missingRequiredRoles.push('variantRepresentative-cleanMain');
    const imageStatus=!sourceImageUrls.length?(completedImageUrls.length?'missing-lineage':'missing-source'):pendingSourceImageUrls.length?(completedImageUrls.length?'partially-localized':'pending-localization'):missingRequiredRoles.length?'roles-pending':'ready',retentionSource=source.sourceImageRetentionPolicy&&typeof source.sourceImageRetentionPolicy==='object'?source.sourceImageRetentionPolicy:{},referencesVerified=retentionSource.allReferencesVerified===true,eligibleForDeletion=referencesVerified&&imageStatus==='ready',cleanupStatus=clean(retentionSource.cleanupStatus)||(eligibleForDeletion?'required':'blocked-until-all-central-variant-platform-references-verified');
    const sourceImageRecords=sourceImageUrls.map(function(url){const rows=readyRowsBySource.get(url)||[];return {sourceImageUrl:url,outputUrls:normalizeProductResearchSourceUrls(rows.map(function(row){return row.url;})),roles:Array.from(new Set([].concat.apply([],rows.map(function(row){return row.roles;})))),deletionStatus:eligibleForDeletion?'eligible-metadata-only':'retain-until-all-references-verified'};});
    return {sourceImageUrls:sourceImageUrls,sourceLineageImageUrls:lineage.slice(0,PRODUCT_REFERENCE_IMAGE_MAX),pendingSourceImageUrls:pendingSourceImageUrls,completedImageUrls:completedImageUrls,readyImageRows:readyRows,missingRequiredRoles:Array.from(new Set(missingRequiredRoles)),gallerySourceImageUrls:normalizeProductResearchSourceUrls(source.gallerySourceImageUrls).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX),representativeSourceImageUrl:representativeSourceImageUrl,imageStatus:imageStatus,centralProductImageCount:productVariantImages(product).length,sourceImageRetention:{mode:'metadata-only-after-required-binary-cleanup',sourceBinaryCleanupRequired:true,cleanupWorkerRequired:true,cleanupStatus:cleanupStatus,referencesVerified:referencesVerified,eligibleForDeletion:eligibleForDeletion,records:sourceImageRecords}};
  }
  function productListingCodexPreparedCase(raw,product,targetScope,includeGroupHeroes){
    const source=raw&&typeof raw==='object'?raw:{},normalized=normalizeProductListingCase(source,product),media=productListingCodexMediaSnapshot(source,product,targetScope,includeGroupHeroes);
    const descriptionStatus=productListingDescriptionStatus(normalized.productDescription);
    return {
      productId:clean(product&&product.docId),sku:clean(source.productSku||product&&product.sku),name:clean(source.productName||product&&product.originalName||product&&product.name),
      sourceProductDescription:clean(source.sourceProductDescription),researchInstructions:clean(source.researchInstructions),productResearchSourceUrls:normalizeProductResearchSourceUrls(source.productResearchSourceUrls),referenceUrls:normalizeProductResearchSourceUrls(source.referenceUrls),imageGenerationInstructions:clean(source.imageGenerationInstructions),
      referenceImageUrls:media.sourceLineageImageUrls.slice(0,PRODUCT_REFERENCE_IMAGE_MAX),selectedReferenceImageUrls:media.sourceImageUrls.slice(),gallerySourceImageUrls:media.gallerySourceImageUrls.slice(),generatedListingImages:normalizeGeneratedListingImages(source.generatedListingImages),listingImageUrls:media.completedImageUrls.slice(),physicalImageUrls:normalized.physicalImageUrls.slice(),physicalOriginalImageUrls:normalized.physicalOriginalImageUrls.slice(),physicalImages:normalized.physicalImages.slice(-PRODUCT_PHYSICAL_IMAGE_MAX),physicalImagePolicy:{preserveOriginal:true,customerFacingDerivative:'label-only',labelText:'實體圖',aiEditingForbidden:true,placement:'description-only-after-completed-images-before-final-disclaimer',neverUseAsMainImage:true},imageRoleAssignments:media.readyImageRows.map(function(row){return {sourceImageUrl:row.sourceImageUrl,url:row.url,roles:row.roles.slice(),assetFlags:Object.assign({},row.assetFlags)};}),sourceImageRetention:media.sourceImageRetention,
      brand:normalized.brand,model:normalized.model,barcode:normalized.barcode,material:normalized.material,color:normalized.color,countryOfOrigin:normalized.countryOfOrigin,productDescription:normalized.productDescription,productDescriptionStatus:descriptionStatus,richContentStandard:productRichContentLifecycle(normalized.listingIntent,normalized.productDescription),specificationText:normalized.specificationText,includedItems:normalized.includedItems,
      listingIntent:normalized.listingIntent,listingIntentPolicy:normalized.listingIntentPolicy,listingChangeInstructions:normalized.listingChangeInstructions,listingMode:normalized.listingMode,variantParentProductId:normalized.variantParentProductId,variantAttributeName:normalized.variantAttributeName,variantParentAttributeValue:normalized.variantParentAttributeValue,variantAttributeValue:normalized.variantAttributeValue,variantGroupEnabled:normalized.variantGroupEnabled,variantGroupAttributeName:normalized.variantGroupAttributeName,variantGroupPrimaryValue:normalized.variantGroupPrimaryValue,
      shopeeCategoryPath:normalized.shopeeCategoryPath,shopeeAttributeValues:normalized.shopeeAttributeValues,momoCategoryCode:normalized.momoCategoryCode,coupangCategoryCode:normalized.coupangCategoryCode,
      shippingDecision:normalized.shippingDecision,packageLengthCm:normalized.packageLengthCm,packageWidthCm:normalized.packageWidthCm,packageHeightCm:normalized.packageHeightCm,packageWeightKg:normalized.packageWeightKg,
      priceSnapshot:source.priceSnapshot&&typeof source.priceSnapshot==='object'?source.priceSnapshot:{},stockSnapshot:numberOrNull(source.stockSnapshot)==null?Number(product&&product.currentStock||0):numberOrNull(source.stockSnapshot),schemaVersion:Number(source.schemaVersion)||0,recordVersion:clean(source.version)
    };
  }
  async function loadProductListingCodexHandoffCase(id,targetScope,includeGroupHeroes){
    const productId=clean(id),product=catalogById(productId);if(!productId||!product)throw new Error('找不到合併商品的中央主檔：'+productId);
    const snap=await state.db.collection(COLLECTIONS.listingCases).doc(productId).get(),raw=snap.exists?snap.data()||{}:{},media=productListingCodexMediaSnapshot(raw,product,targetScope,includeGroupHeroes);
    return Object.assign({productId:productId,sku:clean(raw.productSku||product.sku),name:clean(raw.productName||product.originalName||product.name),caseExists:snap.exists,representativeSourceImageUrl:safeUrl(raw.variantChildImageUrl||raw.variantGroupPrimaryImageUrl),preparedCase:productListingCodexPreparedCase(raw,product,targetScope,includeGroupHeroes)},media);
  }
  function mergeProductListingCodexQueuedMedia(item,queued,targetScope,includeGroupHeroes){
    const source=item&&typeof item==='object'?item:{},row=queued&&typeof queued==='object'?queued:{},queuedSources=normalizeProductResearchSourceUrls(row.sourceImageUrls),caseSources=normalizeProductResearchSourceUrls(source.sourceImageUrls),sourceImageUrls=normalizeProductResearchSourceUrls([].concat(caseSources,queuedSources)).slice(0,PRODUCT_SELECTED_IMAGE_MAX);
    if(!queuedSources.length||sourceImageUrls.length===caseSources.length)return source;
    const completedImageUrls=normalizeProductResearchSourceUrls(source.completedImageUrls),prepared=source.preparedCase&&typeof source.preparedCase==='object'?source.preparedCase:{},readyRows=readyProductListingImageRows(prepared),pendingSourceImageUrls=sourceImageUrls.filter(function(url){return !readyRows.some(function(image){return image.sourceImageUrl===url&&image.roles.some(function(role){return ['cleanMain','localizedDetail','specification','variantRepresentative'].includes(role);});});}),missingRequiredRoles=[];
    productListingRequiredImageRoles(targetScope,includeGroupHeroes).forEach(function(role){if(sourceImageUrls.length&&!readyRows.some(function(image){return image.roles.includes(role);}))missingRequiredRoles.push(role);});
    const imageStatus=pendingSourceImageUrls.length?(completedImageUrls.length?'partially-localized':'pending-localization'):missingRequiredRoles.length?'roles-pending':(completedImageUrls.length?'ready':'pending-localization');
    source.sourceImageUrls=sourceImageUrls;
    source.sourceLineageImageUrls=normalizeProductResearchSourceUrls([].concat(source.sourceLineageImageUrls||[],sourceImageUrls));
    source.pendingSourceImageUrls=pendingSourceImageUrls;
    source.missingRequiredRoles=missingRequiredRoles;
    source.imageStatus=imageStatus;
    source.preparedCase=Object.assign({},prepared,{referenceImageUrls:source.sourceLineageImageUrls.slice(0,PRODUCT_REFERENCE_IMAGE_MAX),selectedReferenceImageUrls:sourceImageUrls.slice()});
    return source;
  }
  function productListingSharedCompletedRows(cases){
    const groups=(Array.isArray(cases)?cases:[]).map(function(item){const prepared=item&&item.preparedCase||{},allRows=readyProductListingImageRows(prepared),conflicts=productListingConflictingRoleUrls(allRows),allowed=new Set(normalizeProductResearchSourceUrls(item&&item.gallerySourceImageUrls)),rows=allRows.filter(function(row){return !conflicts.has(row.url)&&(!allowed.size||allowed.has(row.sourceImageUrl)||allowed.has(row.url));});return rows.slice().sort(function(a,b){const ar=a.roles.includes('cleanMain')?0:a.roles.includes('storefrontPortrait')?1:a.roles.includes('brandedHero')?2:3,br=b.roles.includes('cleanMain')?0:b.roles.includes('storefrontPortrait')?1:b.roles.includes('brandedHero')?2:3;return ar-br||a.sourceOrder-b.sourceOrder;});}),result=[],seen=new Set();
    function push(row){if(!row||seen.has(row.url)||result.length>=PRODUCT_GROUP_LISTING_IMAGE_MAX)return false;seen.add(row.url);result.push(row);return true;}
    const root=groups[0]||[];
  ...
  function productListingPlatformImagePlan(rows,frozenSourceImageUrls){
    const frozenSources=new Set(normalizeProductResearchSourceUrls(frozenSourceImageUrls));
    const incoming=(Array.isArray(rows)?rows:[]).filter(function(row){return row&&row.url&&row.sourceImageUrl&&row.url!==row.sourceImageUrl&&!frozenSources.has(row.url);}),firstStorefront=incoming.find(function(row){return row.roles.includes('storefrontPortrait');}),firstBranded=incoming.find(function(row){return row.roles.includes('brandedHero');}),nonHero=incoming.filter(function(row){return !row.roles.includes('storefrontPortrait')&&!row.roles.includes('brandedHero');}),pool=[firstStorefront,firstBranded].concat(nonHero).filter(Boolean).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);
    const unique=function(values){return normalizeProductResearchSourceUrls(values).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);};
    const byRole=function(role){return pool.filter(function(row){return row.roles.includes(role);});};
    const cleanRows=byRole('cleanMain').filter(function(row){return !row.assetFlags.containsLogo&&!row.assetFlags.containsText&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode&&!row.assetFlags.greenBrandTemplate;});
    const brandedRows=byRole('brandedHero').filter(function(row){return (row.assetFlags.containsText||(row.assetFlags.containsLogo&&row.assetFlags.greenBrandTemplate))&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode;});
    const storefrontRows=byRole('storefrontPortrait').filter(function(row){return (row.assetFlags.containsText||(row.assetFlags.containsLogo&&row.assetFlags.greenBrandTemplate))&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode;});
    const cleanFirst=cleanRows[0]||null,brandedFirst=brandedRows[0]||null,storefrontFirst=storefrontRows[0]||null,rest=pool.map(function(row){return row.url;}),nonHeroUrls=pool.filter(function(row){return !row.roles.includes('storefrontPortrait')&&!row.roles.includes('brandedHero');}).map(function(row){return row.url;});
    const promoRow=pool.find(function(row){return (!cleanFirst||row.url!==cleanFirst.url)&&row.assetFlags.momoPromotionEligible===true&&!row.assetFlags.containsLogo&&!row.assetFlags.containsText&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode&&!row.assetFlags.greenBrandTemplate&&(row.roles.includes('cleanMain')||row.roles.includes('localizedDetail')||row.roles.includes('specification'));});
    const marketplaceRemainder=pool.filter(function(row){return !row.roles.includes('brandedHero')&&!row.roles.includes('storefrontPortrait');}).map(function(row){return row.url;});
    // EasyStore uses the locked portrait template as its only green brand image.
    // Do not append the square branded hero to the same gallery.
    const easyStore=unique([storefrontFirst&&storefrontFirst.url].concat(nonHeroUrls));
    const shopee=unique([brandedFirst&&brandedFirst.url].concat(nonHeroUrls));
    const coupang=unique([cleanFirst&&cleanFirst.url,brandedFirst&&brandedFirst.url].concat(marketplaceRemainder));
    const momo=unique([cleanFirst&&cleanFirst.url,promoRow&&promoRow.url,brandedFirst&&brandedFirst.url].concat(marketplaceRemainder));
    const momoSecondThird=momo.slice(1,3),orderedPromoRow=pool.find(function(row){return momoSecondThird.includes(row.url)&&row.assetFlags.momoPromotionEligible===true&&!row.assetFlags.containsLogo&&!row.assetFlags.containsText&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode&&!row.assetFlags.greenBrandTemplate&&(row.roles.includes('cleanMain')||row.roles.includes('localizedDetail')||row.roles.includes('specification'));});
    return {sharedCompletedImageUrls:unique(rest),easyStore:{imageUrls:easyStore,requiredFirstRole:'storefrontPortrait',ready:!!easyStore.length&&storefrontRows.some(function(row){return row.url===easyStore[0];})},shopee:{imageUrls:shopee,requiredFirstRole:'brandedHero',ready:!!shopee.length&&brandedRows.some(function(row){return row.url===shopee[0];})},coupang:{imageUrls:coupang,requiredFirstRole:'cleanMain',ready:!!coupang.length&&cleanRows.some(function(row){return row.url===coupang[0];}),brandedHeroAllowedAsSecondary:true,removeSecondaryBrandedHeroIfPlatformRejectsGalleryLogo:true},momo:{imageUrls:momo,requiredFirstRole:'cleanMain',ready:!!momo.length&&cleanRows.some(function(row){return row.url===momo[0];}),brandedHeroAllowedAsSecondary:true,promotionImageUrl:orderedPromoRow?orderedPromoRow.url:'',promotionImageReady:!!orderedPromoRow}};
  }
  function immutableProductListingSnapshot(value){
    if(!value||typeof value!=='object'||Object.isFrozen(value))return value;
    Object.keys(value).forEach(function(key){immutableProductListingSnapshot(value[key]);});return Object.freeze(value);
  }
  function productListingCanonicalCategoryDecision(product,cases,normalized){
    const text=clean([product&&product.originalName,product&&product.name,product&&product.category,normalized&&normalized.shopeeCategoryPath].concat((cases||[]).map(function(item){return [item&&item.name,item&&item.preparedCase&&item.preparedCase.sourceProductDescription].filter(Boolean).join(' ');})).filter(Boolean).join(' ')).normalize('NFKC').toLowerCase();
    let family='';
    if(/琴弦|吉他弦|貝斯弦|弦組|撥片|背帶|琴袋|琴盒|琴架|吉他架|鼓棒|鼓皮|調音器|節拍器|移調夾|導線|效果器|踏板|簧片|吹嘴|譜架|弱音器|拾音器/.test(text))family='樂器配件';
    else if(/電鋼琴|數位鋼琴|電子琴|鍵盤|合成器|鋼琴/.test(text))family='鍵盤樂器';
    else if(/爵士鼓|電子鼓|木箱鼓|非洲鼓|手鼓|鈴鼓|軍鼓|銅鈸|打擊樂器|木琴|鐵琴/.test(text))family='打擊樂器';
    else if(/薩克斯|長笛|短笛|單簧管|豎笛|雙簧管|小號|長號|法國號|直笛|陶笛|口琴|管樂器/.test(text))family='管樂器';
    else if(/電吉他|木吉他|古典吉他|民謠吉他|吉他|貝斯|烏克麗麗|小提琴|中提琴|大提琴|二胡|古箏|琵琶|弦樂器/.test(text))family='弦樂器';
    return {version:1,decidedOnceBeforePlatformNavigation:true,scope:'musical-instruments-and-accessories-only',canonicalFamily:family||'未判定',canonicalKey:family?'music:'+family:'',evidenceSummary:clean([product&&product.category,product&&product.originalName||product&&product.name].filter(Boolean).join('｜')).slice(0,500),forbidPerPlatformReclassification:true,remapOnlyWhenPlatformTaxonomyRejectsStoredMapping:true};
  }
  function productListingDecisionContract(canonicalCategoryDecision){
    return {
      version:2,mode:'deterministic-workflow-with-structured-judgment',immutableForJob:true,
      deterministicSteps:['freeze-case-input','validate-completed-image-lineage','sync-central-clean-images','prepare-platform-field-plan-once','run-momo-coupang-easystore-roots','run-shopee-after-easystore','verify-each-platform-once','cleanup-source-binaries-after-all-verification'],
      automaticFields:{priceFallbackOrder:['platform-price','shared-online-price','online-price','store-price'],warrantyDays:180,publishImmediately:true,momoThirdPartyLocationCode:'000001',centralImageRole:'cleanMain',easyStoreFirstImageRole:'storefrontPortrait',shopeeFirstImageRole:'brandedHero',coupangFirstImageRole:'cleanMain',momoFirstImageRole:'cleanMain',singleGreenBrandTemplatePerPlatformGallery:true,brandHeaderHeightRatio:1/9,brandLogoOverlap:'approved-large-overlap-version',brandLogoLayer:'topmost',brandBorderLayer:'below-logo',brandBorderMayNotCrossLogo:true,brandOuterBorder:'continuous-thin-rounded-green-outline',brandStyleCatalogVersion:PRODUCT_BRAND_TEMPLATE_CONTRACT.creativeStyleSystem.version,brandStyleRenderProofRequired:true,brandStyleSelection:'random-without-replacement-until-all-50-used',sameBrandStyleAcrossAspectRatiosAndVariants:true,easyStoreTaxable:false,easyStoreCompareAtMarkupPercent:35,easyStoreBarcodeOptional:true,easyStoreFeatureBulletMinChars:24,easyStoreFeatureBulletMaxChars:30,coupangAttributeNameMaxChars:25,coupangExcludedRedundantAttributes:['Parent Manufacturer Part Number','Manufacturer Part Number'],coupangContentType:'HTML',coupangContentDetailType:'TEXT',coupangContentDetailMustRemainNonEmpty:true,physicalImageTransform:'label-only-with-original-retained',physicalImagePlacement:'description-only-after-completed-images-before-fixed-notices-and-final-promos',descriptionRuleScope:'shared-content',easyStoreShopeeRuleScope:'EasyStore/Shopee',retryPolicy:'same-sku-same-draft-same-stage-only',routineConfirmation:'already-authorized-by-handoff'},
      judgmentFields:{listingContentMutation:{resolver:'selected-listing-intent',required:true,intents:['create-single','create-group','merge-existing','add-variant','update-existing'],mergeExistingRule:'reuse-listed-primary-and-attach-selected-variants-with-independent-stock',addVariantRule:'preserve-existing-and-merge-new-variant-content',updateExistingRule:'replace-only-requested-outdated-content-on-exact-sku-and-platform-id',neverCreateDuplicate:true},imageLocalization:{resolver:'codex-vision',required:true,output:['sourceImageUrl','url','roles','assetFlags'],ordinaryDetailMode:'text-localization-only',appliesToRoles:['localizedDetail','specification','variantRepresentative'],excludesRoles:['cleanMain','brandedHero','storefrontPortrait'],preserveExactly:['product','layout','composition','background','crop','aspect-ratio','colors','icons','information-hierarchy','text-position','typography-style'],allowedMutations:['simplified-to-traditional-chinese','remove-third-party-watermark-or-outbound-seller-information','remove-entire-truncated-text-block-or-retype-only-verified-complete-copy','format-normalization-without-recomposition'],forbiddenMutations:['redesign','recompose','change-background','change-product','add-marketing-copy','invent-feature','reorder-information','crop-product'],textReplacementMethod:'ocr-recognize-erase-original-text-and-retype-with-real-traditional-chinese-font',handDrawnOrPaintedGlyphsForbidden:true,preserveOriginalTypographyCharacterWhenPossible:true,blurryTinyOrIncompleteSourceAction:'find-clearer-complete-source-or-skip',blurrySourceUpscalingForbidden:true,newMarketingCopyForbidden:true,unchangedWhenNoSimplifiedTextOrForbiddenMarks:true,failClosedWhenPixelPreservationCannotBeVerified:true},heroSourceSelection:{resolver:'codex-vision',required:true,allowedRoles:['cleanMain','brandedHero','storefrontPortrait']},verifiedProductContent:{resolver:'codex-evidence',required:true,standardVersion:PRODUCT_RICH_CONTENT_STANDARD_VERSION,output:['title','description','brand','model','verified-features','verified-usage','verified-specifications','fieldEvidence'],requiredDescriptionSections:['商品特色','使用方式／適用情境','商品規格'],featureTarget:PRODUCT_RICH_CONTENT_FEATURE_TARGET,usageTarget:PRODUCT_RICH_CONTENT_USAGE_TARGET,requireExactModelOrBarcodeEvidence:true,fillEveryVerifiableAttribute:true,neverInventToReachTarget:true,forbidStoreNameInTitleAndCopy:true,fixedDescriptionNoticesBeforeFinalPromos:[PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER,PRODUCT_WARRANTY_SUPPORT_NOTICE],newProductRule:'complete-before-first-publish',existingProductRule:'upgrade-in-place-and-preserve-listing-identity',genericFallbackIsIncomplete:true,writeBackToEveryCaseBeforePreparedSnapshot:true},categoryAndAttributes:{resolver:'canonical-category-once-then-platform-mapping',required:true,scope:'musical-instruments-and-accessories-only',canonicalDecision:canonicalCategoryDecision||{},neverReclassifyInsideEachPlatform:true},variantNames:{resolver:'manager-value-first-then-codex',requiredWhen:'variant-group',neverOverrideManagerValue:true},shippingByDimensions:{resolver:'deterministic-once-before-platform-when-dimensions-known',mapPreparedDecisionToEveryPlatform:true,neverRejudgeInsideEachPlatform:true,requireEvidenceForOversizeDecision:true}},
      forbiddenJudgment:['invent-certification','invent-brand-or-model','invent-feature-or-accessory','change-sku-set','change-platform-order','create-replacement-on-retry','request-routine-second-confirmation'],
      unresolvedAction:'stop-before-platform-navigation-with-exact-field-reason'
    };
  }
  async function loadProductListingCodexHandoffSnapshot(product){
    const id=clean(product&&product.docId);if(!id)throw new Error('找不到商品上架資料');
    const rootSnap=await state.db.collection(COLLECTIONS.listingCases).doc(id).get();if(!rootSnap.exists)throw new Error('商品案件尚未保存，請重新開啟後再試');
    const raw=rootSnap.data()||{},normalized=normalizeProductListingCase(raw,product),queue=Array.isArray(raw.variantImageProcessingQueue)?raw.variantImageProcessingQueue:[],ids=[id];queue.forEach(function(item){const itemId=clean(item&&item.productId);if(itemId&&!ids.includes(itemId))ids.push(itemId);});normalizeProductVariantGroupItems(raw.variantGroupItems,id).forEach(function(item){if(item.productId&&!ids.includes(item.productId))ids.push(item.productId);});const parentId=clean(raw.variantParentProductId);if(parentId&&!ids.includes(parentId))ids.push(parentId);
    const cases=await Promise.all(ids.map(function(caseId){return loadProductListingCodexHandoffCase(caseId,normalized.listingTargetScope,caseId===id);})),queueById=new Map(queue.map(function(item){return [clean(item&&item.productId),item||{}];}));
    cases.forEach(function(item){const queued=queueById.get(item.productId)||{},isRoot=item.productId===id;mergeProductListingCodexQueuedMedia(item,queued,normalized.listingTargetScope,isRoot);item.representativeSourceImageUrl=safeUrl(queued.representativeSourceImageUrl)||item.representativeSourceImageUrl;item.gallerySourceImageUrls=normalizeProductResearchSourceUrls(queued.gallerySourceImageUrls||item.gallerySourceImageUrls).slice(0,PRODUCT_GROUP_LISTING_IMAGE_MAX);item.preparedCase.gallerySourceImageUrls=item.gallerySourceImageUrls.slice();item.representativeCompletedImageUrl=productListingRoleImageUrl(item.preparedCase,'cleanMain',item.representativeSourceImageUrl);});
    const createdAt=new Date().toISOString();
    const brandCreativeStyleAssignment=await reserveProductBrandCreativeStyle(id,raw.brandCreativeStyleAssignment);
    const previousWorkflowVersion=clean(raw.codexHandoff&&raw.codexHandoff.workflowVersion||raw.codexExecution&&raw.codexExecution.workflowVersion),sharedRows=productListingSharedCompletedRows(cases),frozenSourceImageUrls=normalizeProductResearchSourceUrls([].concat.apply([],cases.map(function(item){return item&&item.sourceImageUrls||[];}))),platformImagePlan=productListingPlatformImagePlan(sharedRows,frozenSourceImageUrls),canonicalCategoryDecision=productListingCanonicalCategoryDecision(product,cases,normalized),snapshot={
      workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,snapshotId:id+'-'+Date.now().toString(36),createdAt:createdAt,platformOrder:PRODUCT_LISTING_PLATFORM_ORDER.slice(),currentStage:'codex-preflight',immutableUntilUserEditsCase:true,
      replacesWorkflowVersion:previousWorkflowVersion&&previousWorkflowVersion!==PRODUCT_LISTING_WORKFLOW_VERSION?previousWorkflowVersion:'',legacySnapshotMayNotBeMixed:true,
      productId:id,sku:clean(product.sku),name:clean(product.originalName||product.name),workflowPurpose:normalized.workflowPurpose,descriptionLayoutVersion:PRODUCT_DESCRIPTION_LAYOUT_VERSION,brandTemplateContract:JSON.parse(JSON.stringify(PRODUCT_BRAND_TEMPLATE_CONTRACT)),brandCreativeStyleAssignment:brandCreativeStyleAssignment,listingTargetScope:normalized.listingTargetScope,listingTargetPlatforms:normalized.listingTargetPlatforms.slice(),listingIntent:normalized.listingIntent,listingIntentPolicy:normalized.listingIntentPolicy,listingChangeInstructions:normalized.listingChangeInstructions,listingMode:normalized.listingMode,variantGroupEnabled:normalized.variantGroupEnabled,richContentStandard:Object.assign({},normalized.richContentStandard),physicalImageUrls:normalized.physicalImageUrls.slice(),physicalOriginalImageUrls:normalized.physicalOriginalImageUrls.slice(),cases:cases,
      preparedData:{brand:normalized.brand,model:normalized.model,shopeeCategoryPath:normalized.shopeeCategoryPath,momoCategoryCode:normalized.momoCategoryCode,coupangCategoryCode:normalized.coupangCategoryCode,shippingDecision:normalized.shippingDecision,packageLengthCm:normalized.packageLengthCm,packageWidthCm:normalized.packageWidthCm,packageHeightCm:normalized.packageHeightCm,packageWeightKg:normalized.packageWeightKg,priceSnapshot:raw.priceSnapshot&&typeof raw.priceSnapshot==='object'?raw.priceSnapshot:{},stockSnapshot:numberOrNull(raw.stockSnapshot)==null?Number(product.currentStock||0):numberOrNull(raw.stockSnapshot)},
      ...
      executionPolicy:{workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,processIsFixedProductDataMayVary:true,descriptionLayoutVersion:PRODUCT_DESCRIPTION_LAYOUT_VERSION,descriptionLayoutAppliesToAllListingIntents:true,storePromoMustBeLastGalleryImage:true,easyStoreGroupedGalleryOfficialMaximum:9,easyStoreGroupedUniqueVariantImageMaximum:7,easyStoreGroupedVariantImageIdsMustPersist:true,easyStoreVariantImageBindingMode:'in-app-browser-admin-selector',easyStoreVariantImageIdApiWriteUnsupported:true,easyStoreVariantImageIdZeroBlocksCompletion:true,descriptionPromosMustBeLastTwoDescriptionImages:true,missingDescriptionImageBlocksCompletion:true,prepareOnce:true,singleHandoffAuthorization:true,finalSubmissionAuthorizedByHandoff:true,routineSecondConfirmationForbidden:true,continueAutomaticallyAfterEachVerifiedStage:true,prepareCompleteFieldPlanBeforeFirstPlatform:true,preparedFieldPlanIsImmutableForWholeJob:true,requireStructuredVerifiedDescriptionBeforePreparedSnapshot:true,genericFallbackDescriptionIsIncomplete:true,writeVerifiedDescriptionBackToEveryGroupedCase:true,reuseKnownRoutesAndFieldLocations:true,persistStableSelectorsAndFieldSemantics:true,applyFixedFieldsWithoutWholePageRescan:true,fillStableControlsInSingleSectionPass:true,validateSectionOnceAfterBatch:true,dynamicControlsSequentialWithinSection:true,inspectOnlyDynamicCategoryAttributesAndErrors:true,rescanCurrentSectionOnlyWhenLayoutSignatureChanges:true,fallbackToSectionRescanWithoutRestartingJob:true,skipPreSubmitCatalogSearchWhenNoPlatformId:true,backendFirst:true,desktopControlFallbackOnly:true,browserWorkspace:'codex-in-app-browser-only',neverUsePrimaryChrome:true,reuseAuthenticatedPlatformTabs:true,batchSessionPreflightBeforeFirstProduct:true,requireAllPlatformSessionsOperableBeforeBatchStart:true,lightweightSessionProbeBeforeEveryProductAndStage:true,autoRecoverSavedCredentialSessions:true,neverAskUserForRecoverableSessionRestart:true,keepMomoOtpAnchorTab:true,retrySameUrlInFreshInAppWorkTabWhenClaimedTabInputFails:true,allowSavedCredentialLoginRetry:true,stopForInteractiveAuthenticationOnly:true,newCaseBoundaryResetRequired:true,discardPreviousCaseUiState:true,openCanonicalPlatformEntryBeforeNewCase:true,bindNavigationToCurrentCaseSku:true,doNotResetSameSkuRetryDraft:true,croppedTextMustBeReflowedReplacedOrRemoved:true,neverReturnToResearchAfterPlatformStart:true,retrySameSkuDraftAndStage:true,verifyOnceAfterEachSubmit:true,verifyExactSkuOnlyWhenOutcomeUnknown:true,neverSearchWholePlatformCatalog:true,neverCreateReplacementOnRetry:true,shopeeWorkspace:'easystore-channel-then-seller-center-native-description',shopeeDescriptionMemoryOnlyUpload:false,shopeeLocalTempDownloadRequired:true,shopeeDesktopDownloadRequired:false,shopeeUserDesktopFolderForbidden:true,shopeeDescriptionUploadEntry:'商品描述/新增圖片/從電腦裝置上傳',shopeeOfficialZeroOfTwelveIsIncomplete:true,shopeeNeverDeleteUserFiles:true,shopeeLargeItemHctAndSellerDelivery:true,easyStoreImageUrlUploadFallback:true,fixedWarrantyDays:180,momoThirdPartyLocationCode:'000001',momoThirdPartyLocationRequired:true,momoMainAdAndPromotionImagesAreSeparate:true,momoPromotionRequiredBeforeFirstSubmit:true,momoPromotionImage:'clean-product-output-position-2-or-3-no-logo-contact-qr',momoPromotionPersistenceEvidence:'contenteditable-html-img-src-after-save-reopen',momoThirdPartyPersistenceEvidence:'000001-visible-after-save-reopen',sourceBinaryCleanupRequired:true,cleanupOnlyAfterAllCentralVariantPlatformReferencesVerified:true,cleanupWorkerRequired:true,retainSourceMetadataHashAndLineage:true}
    };
    snapshot.executionPolicy.groupHeroAssetsAreSingletons=true;
    snapshot.executionPolicy.brandCreativeStyleCatalogVersion=PRODUCT_BRAND_TEMPLATE_CONTRACT.creativeStyleSystem.version;
    snapshot.executionPolicy.brandCreativeStyleCatalogSize=PRODUCT_BRAND_CREATIVE_STYLE_CATALOG.length;
    snapshot.executionPolicy.brandCreativeStyleRandomWithoutReplacement=true;
    snapshot.executionPolicy.brandCreativeStyleResetOnlyAfterAllUsed=true;
    snapshot.executionPolicy.sameBrandCreativeStyleAcrossAspectRatios=true;
    snapshot.executionPolicy.sameBrandCreativeStyleAcrossVariants=true;
    snapshot.executionPolicy.brandCreativeStyleAssignment=brandCreativeStyleAssignment;
    snapshot.executionPolicy.groupStorefrontPortraitMax=1;
    snapshot.executionPolicy.groupBrandedHeroMax=1;
    snapshot.executionPolicy.variantCasesRequireCleanRepresentativeOnly=true;
  ...
  function requireProductListingCodexHandoffMedia(snapshot){
    const cases=Array.isArray(snapshot&&snapshot.cases)?snapshot.cases:[],seen=new Set(),invalid=cases.filter(function(item){const id=clean(item&&item.productId),sources=normalizeProductResearchSourceUrls(item&&item.sourceImageUrls),completed=normalizeProductResearchSourceUrls(item&&item.completedImageUrls),bad=!id||seen.has(id)||sources.length>PRODUCT_REFERENCE_IMAGE_MAX||completed.length>PRODUCT_GROUP_LISTING_IMAGE_MAX;seen.add(id);return bad;}),missing=cases.filter(function(item){return item.imageStatus==='missing-source'||item.imageStatus==='missing-lineage';});
    if(invalid.length)throw new Error('合併商品的圖片快照不完整或超過上限，已停止交接；請重新開啟案件建立 v3 快照');
    if(!cases.length||missing.length)throw new Error('圖片尚未完整保存到商品案件：'+(missing.map(function(item){return item.sku||item.productId;}).join('、')||'目前商品')+'。請先完成收圖或確認繁體完成圖仍保有來源圖對應，再按一次交給 Codex；系統不會拿同系列商品圖片替代，也不會使用無法追溯來源的圖片。');
  }
  function productListingCodexHandoffPrompt(product,snapshot){
    const cases=Array.isArray(snapshot&&snapshot.cases)?snapshot.cases:[],statusLabels={'missing-source':'缺少圖片','missing-lineage':'完成圖缺少來源對應','pending-localization':'等待繁體化','partially-localized':'部分已完成','roles-pending':'待完成圖片角色','ready':'繁體與角色均完成'},intent=clean(snapshot&&snapshot.listingIntent)||'create-single',intentPolicy=snapshot&&snapshot.listingIntentPolicy||productListingIntentPolicy(intent),listingTargetScope=normalizeProductListingTargetScope(snapshot&&snapshot.listingTargetScope),listingTargetPlatforms=productListingTargetPlatforms(listingTargetScope),targetNames=listingTargetPlatforms.map(function(platform){return platform==='easyStore'?'EasyStore 官網':platform==='shopee'?'蝦皮':platform==='coupang'?'酷澎':'MOMO';});
    const targetNamesText=targetNames.join('、'),requiredImageRoles=productListingRequiredImageRoles(listingTargetScope),selectedRoots=listingTargetPlatforms.filter(function(platform){return platform!=='shopee';}).map(function(platform){return platform==='easyStore'?'EasyStore':platform==='coupang'?'酷澎':'MOMO';}),targetPlatformCount=listingTargetPlatforms.length;
    ...
    return [
      '[固定流程]',
      '請接手全通路營運中心的待上架商品。這是唯一標準流程，workflowVersion 必須等於 '+PRODUCT_LISTING_WORKFLOW_VERSION+'；v2 或任何其他舊快照一律停止並要求從營運中心建立新的 v3 快照，不得沿用、混合或降級。商品資料可以不同，但只准執行 listingTargetPlatforms 指定的通路；未選通路不得建立、修改、排隊或送出。',
      '商品編號：'+clean(product.sku),
      '商品名稱：'+clean(product.originalName||product.name),
      '案件文件：opsProductListingCases/'+clean(product.docId),
      '固定流程版本：'+clean(snapshot&&snapshot.workflowVersion)+'；不可變預檢快照：'+clean(snapshot&&snapshot.snapshotId)+'。',
      '目標範圍：'+productListingTargetLabel(listingTargetScope)+'；目標通路：'+targetNamesText+'。這個選擇具有最高優先權，不得擴大到其他通路，也不需再勾選。',
      '[五種商品處理規則]',
      '本案已選：'+clean(intentPolicy.label)+'（'+intent+'）。管理者要求：'+(clean(snapshot&&snapshot.listingChangeInstructions)||'未另外補充')+'。必須只依這個 intent 執行，不得根據平台目前是否有編號自行改成另一種處理方式。',
      'create-single＝新增獨立商品：以本案唯一 SKU 建立一個完整新商品；任何通路若已保存既有平台商品 ID 就停止該通路，不得覆蓋舊商品或另建重複品。',
      'create-group＝新增多細項商品：建立一筆主商品與快照列出的完整封閉 SKU 集合；共同介紹只建立一次，各細項保留名稱、SKU、售價、庫存與代表圖，不得拆成多筆主商品。',
      'merge-existing＝合併／加入既有商品：以第一筆已有的平台 ID 為主商品，把快照列出的其他 SKU 合併為同一組細項；保留原平台商品與未要求變更的內容，各細項名稱、SKU、售價、庫存與代表圖獨立。若某通路沒有主商品平台 ID，才在該通路建立完整多細項商品。',
      'add-variant＝加入既有商品細項：以指定父商品平台 ID 為唯一目標，完整保留原有商品、舊細項及未要求變更的圖文，把新細項的圖片、差異與介紹合併追加到原商品的圖文介紹；不得覆蓋舊內容、不得另建父商品。',
      'update-existing＝修改既有商品：鎖定同一 SKU 與各通路既有平台 ID，依 listingChangeInstructions 與新圖片只替換錯誤、過時或改版的部分，刪除被取代的舊版本並保留未提到的內容；缺少目標平台 ID 就停止，不得建立新品、追加重複版本或改到相似商品。',
      '[固定圖文編排規則]',
      ...
      caseLines.join('\n'),
      '[兩階段固定快照]',
      'codexHandoff.preflightSnapshot 是不可變的 handoff input snapshot，只固定來源圖、使用者選擇、修改要求與交接當下資料。Codex 完成圖片後，把 generatedListingImages 寫回各商品案件，但不得修改這份輸入快照；第一次進入平台前，後端會重讀全部案件的最新完成輸出，驗證每個 sourceImageUrl 都屬於凍結輸入，再建立最終不可變的 job preparedSnapshot。來源不符、原圖冒充完成圖、缺角色或 assetFlags 時必須停止，尚未操作任何平台。',
      '最終 job preparedSnapshot 建立後，一次完成分類、品牌、屬性、物流、價格、庫存、圖片角色、細項資料及本次 '+targetPlatformCount+' 個目標通路的固定欄位表。'+targetNamesText+'及所有重試只准沿用該快照；頁面版型未改變時直接套用已準備欄位，不重新掃描整頁，只在分類屬性、版型簽章改變或平台錯誤時檢查當前區段。不得回營運中心重新研究、重寫文案、重修圖片或改走第二條路，也不得重新呼叫網站的 OpenAI 文案或圖片 API。',
      '[程式化與判斷邊界]',
      'preflightSnapshot.decisionContract 是唯一執行契約。deterministicSteps 與 automaticFields 全部由程式直接完成，不得在每一件商品重新思考；只有 judgmentFields 可以由 Codex 依案件證據判斷，且必須輸出契約指定的結構化欄位。管理者已填的細項名稱優先，不得覆寫。任何 judgmentFields 無法可靠判定時，必須在進入平台前以精確欄位與原因停止，不得猜測、不得改走舊流程。',
      '[實體圖固定規則]',
      'preparedCase.physicalOriginalImageUrls 是永遠保留的原始照片；physicalImageUrls 是系統只加上清楚「實體圖」三字的顧客版。實體圖不得交給圖片模型、不得去背、美化、換背景、改色、修字或加入其他文案，也不得計入來源圖、完成圖角色、首圖或平台商品輪播圖。',
      '有幾張 physicalImageUrls 就在平台圖文介紹容量內使用幾張，固定接在一般繁體完成介紹圖之後、兩段固定說明與最後兩張固定介紹圖之前；EasyStore、蝦皮進階描述、MOMO 圖文介紹與酷澎詳情都沿用同一順序。實體圖永遠不得成為平台第 1 張主圖。',
      '[圖片一次定案]',
      '每個案件最多 20 張 selectedReferenceImageUrls；一般來源截圖、localizedDetail、specification 與 variantRepresentative 只能做最小幅度繁體化，不得重新設計。先以 OCR 逐字辨識可讀簡體，乾淨移除原字，再用真實可讀且接近原風格的繁體中文字型重打回原位置；禁止用筆畫塗改、描字或生成像手畫的假字。商品、背景、構圖、裁切、比例、配色、圖示、資訊層級、文字位置與字體性格全部保持。來源包裝、商品圖或角落出現非商品品牌的「○○樂器」、店名、電話、網址、QR Code、店章或重複商家字樣，一律視為第三方商家浮水印並完整移除；商品本體的原廠品牌、型號與包裝原生印刷不得誤刪。半截文字整塊刪除，只有案件有已驗證完整內容時才可重打完整句；來源過小、模糊或不完整時改找同商品較清楚完整來源，找不到就跳過，不得放大假清晰。不得新增任何原圖沒有的行銷賣點。generatedListingImages 的每一個完成輸出都必須保留 sourceImageUrl、roles 與 assetFlags（至少明列 containsLogo、containsText、containsContactInfo、containsQrCode、greenBrandTemplate、momoPromotionEligible）；同一 sourceImageUrl 可以有多個完成輸出，不得用 Map(source→單一 url) 覆蓋；(sourceImageUrl, role) 必須唯一，而且同一完成圖 URL 不得同時兼任互斥的 cleanMain、storefrontPortrait 與 brandedHero。roles 只准使用 cleanMain、storefrontPortrait、brandedHero、localizedDetail、specification、variantRepresentative。本組只由根案件各產生一張 storefrontPortrait 與一張 brandedHero；所有細項案件不得重複產生這兩種整組品牌首圖，只需各自完成對應來源的 cleanMain／variantRepresentative。',
      '同一輪圖片處理只產生本次目標通路需要的首圖版型，不得替未選通路多做圖片，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 使用 1000×750 px、4:3；包含蝦皮時 brandedHero 使用 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1。兩張品牌首圖版本固定為 '+PRODUCT_BRAND_TEMPLATE_CONTRACT.version+'，必須保存 templateVersion、templateAssetSha256、templateComposition、brandCreativeStyleAssignment、商業海報視覺 QA 與 brandRenderProof；缺少、母版雜湊不符、未完成完整商業海報階段、被判定為資訊卡、視覺 QA 未通過、風格證明不符，或證明顯示框線穿過 Logo，就在進平台前停止。舊版母版、舊風格目錄與舊 render proof 一律失效，不得混入新案件。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是純白底、無品牌框、無新增 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得加入任何文字；商品本體原有且不可分離的印字必須保留，商品最長邊至少占 80%，在不裁切商品下以 90～95% 為目標。storefrontPortrait 與 brandedHero 必須先完成真正可刊登的商業商品海報，再於最後合成固定品牌圖層；不得跳過完整海報創作階段。綠色頁首固定占整張高度 1/9；紅色標語「有音樂的生活更有風格」與圓形柚子樂器 Logo 的圖樣、文字、顏色不可改。Logo 固定採管理者選定的大型跨界版本，置於右上並向下壓入內容區；只有 Logo 可跨頁首邊界，且不得遮住商品或主要文字。下方四周固定保留連續、清楚可見的細綠色圓角框；框線先畫、Logo 最後覆蓋，Logo 永遠在最上層，圓形 Logo 內不得看見穿過的框線。所有照片、色塊、文字與裝飾都要停在框線內。商品本體不得被 Logo、標題、色塊或裝飾遮住。每個根商品從 50 種完整商業海報藝術方向以 random-without-replacement 抽一種，50 種未用完不得重複；抽中的風格必須控制整個內容區的場景或圖像系統、商品融入方式、商業標題字體、三項特色的視覺敘事、材質、圖示與裝飾，不能只換 background、accents、色塊或記錄 layout 名稱。唯一核准完成度基準是 2026-09-02 最後確認的明亮工業商業海報與明亮生活雜誌海報；新作品可採不同風格，但整體性、第一眼層級、商業感及完成度不得降低。舊的柔和綠色資訊排版、白／淺色畫布加三個普通矩形或膠囊標籤、表格式規格卡、商品去背圖加幾行文字，以及只換配色卻沿用同一骨架，一律判定為失效資訊卡，禁止發布或作為備援。固定創作順序為藝術指導與場景概念→商品主視覺整合→商業標題字體→三特色視覺敘事→品牌圖層合成→獨立商業海報視覺 QA；任何一步不得略過。內容區至少 65% 為奶油白、白、淺灰或明亮照片，禁止深色滿版；文字可使用不同但真實可讀的商業字體，不限定微軟正黑體。storefrontPortrait 與 brandedHero 均固定使用 3 個已查證特色；不足 3 個時先補查證資料，仍不足就停止，不得虛構。同一商品的 1:1 與 4:3、同組所有細項都沿用 preflightSnapshot.brandCreativeStyleAssignment，但必須依比例重新構圖，不能生硬縮放、裁切或退回資訊卡。產圖後必須確認完整海報、不是資訊卡、風格控制全畫面、商品融入主視覺、第一眼層級清楚、三特色已整合、Logo 與框線正確；未全部通過就重做內容區，最多三次，仍失敗則保留待處理，不得發布簡化版。兩者不得加入地址、電話、QR Code、價格、導外訊息、左下娃娃、PIC COLLAGE 或殘缺文字。',
      '圖片完成並寫回案件後，必須把本案所有完成圖依上架順序排成一張聯絡表／總覽圖，並在目前 Codex 對話一次顯示；不得逐張分散發送。總覽圖只是進度回報，不是審核關卡；顯示後直接繼續建立 preparedSnapshot 與本次所選通路發布，不得要求使用者確認，也不得等待回覆或中斷流程。',
      '中央商品 imageUrl 與細項代表圖只能取對應來源的 cleanMain（variantRepresentative 也必須是同一來源的乾淨完成圖）；EasyStore 的 storefrontPortrait 與蝦皮的 brandedHero 只能另存平台圖庫，不得覆寫中央辨識主圖。圖片角色、來源譜系與平台圖片計畫只在進站前完整驗證一次；送出後不再逐張蒐集平台 CDN 網址或重比整個圖庫，但單一商品、同款多細項、併入既有商品及新增既有商品細項都必須從正式商品資料核對每個細項已有獨立小圖，缺圖或仍使用凍結來源原圖不得 verified。只有平台明確回報圖片錯誤、首圖缺失或仍顯示來源原圖時，才針對該平台圖片區重新檢查。',
      '[共用圖片池與平台角色]',
      '全部編號的完成輸出公平合併為同一個共用池，整組最多 12 個不同完成圖 URL，不是每個編號各 12 張。共用池內 storefrontPortrait 最多一張、brandedHero 最多一張，而且只能使用根案件的整組首圖；接著讓每個細項各取一張對應來源的 cleanMain／variantRepresentative，最後才輪流補真正不同的 localizedDetail 或 specification。不得把各細項重複套版的綠底首圖當成不同圖片補滿容量；EasyStore、蝦皮正式圖庫若出現第二張同角色品牌母版就視為錯誤並原地移除。上架圖片預覽的勾選範圍是唯一允許上架的圖片範圍；未勾選圖片不得加入平台圖庫，也不得使用簡體原圖。',
      '以下平台圖片規則只在該平台屬於 listingTargetPlatforms 時適用：EasyStore 首圖必須是 storefrontPortrait；蝦皮首圖必須是 brandedHero；酷澎與 MOMO 首圖必須是 cleanMain。storefrontPortrait 不得送到 MOMO 或酷澎；brandedHero 只能在 MOMO／酷澎第 2 張以後且不得含地址、電話或 QR Code。若平台明確回傳整個圖庫禁止 Logo／文字，才移除該平台的 brandedHero 次圖。找不到已選平台規定首圖時停止該平台，不得拿另一角色、店面宣傳圖或舊圖補位。',
      '酷澎所有選項／屬性名稱在進站前先驗證最長 25 字；Parent Manufacturer Part Number 與 Manufacturer Part Number 和正式 SKU、型號／產品編號重複，一開始就從送出資料排除。其餘平台分類要求的屬性名稱若超過 25 字，必須在送出前停止並回報精確欄位，不得等平台退件後才處理，也不得任意截斷正式分類屬性名稱。',
      '酷澎圖文介紹固定使用 contentsType=HTML、contentDetails.detailType=TEXT，且 contentDetails 必須至少保留一筆非空 HTML。移除某張平台專用圖片時只能精確移除該張 img 區塊，不得把整筆 contentDetails 清空；送出前若內容為空，必須在預檢停止，不得讓平台以「詳細內容子類型錯誤」退回。',
      '只有 MOMO 在 listingTargetPlatforms 時才套用本段：MOMO 第 2 或第 3 張必須先保留專推圖，整個 MOMO 圖庫最多再放一張安全的 brandedHero 次圖。專推圖只可從 MOMO 完成圖順序第 2 或第 3 張中選 roles 包含 cleanMain、localizedDetail 或 specification，且 assetFlags.momoPromotionEligible=true 的乾淨商品完成輸出；必須確認 containsLogo、containsText、containsContactInfo、containsQrCode、greenBrandTemplate 全為 false。進入 MOMO 前就在 preparedPlatformFieldPlan 產生專推圖網址、唯一素材檔名與指紋；不得等平台第一次退回「至少上傳 1 張專推圖」後才準備。',
      '只有 MOMO 在 listingTargetPlatforms 時才套用本段：MOMO 的商品主圖、廣告用圖與商品詳細介紹編輯器內的專推圖是三個互相獨立的必填位置；主圖或廣告用圖已出現，不能推定專推圖已完成。主圖固定 1～6 張、1:1、1000×1000 px、38KB～1MB；廣告用圖同為 1:1、1000×1000 px、38KB～1MB，且不得有標籤、外框或浮水印。單一商品、同款多細項與加入既有商品成為細項都使用同一 firstSubmitMediaGate；多細項必須先把完整 SKU、細項名稱、價格、庫存與各自代表圖寫入同一主商品，再由整筆主商品共用一張專推圖，不得逐細項重複上傳或繞過關卡。第一次送出前必須一次完成三處：主圖批次上傳、廣告用圖使用已準備的乾淨主圖、商品詳細介紹的每張圖先建立含 SKU、內容指紋與順序的唯一檔名，再使用「上傳圖片」→「此裝置上傳」放入精確文字邊界。若 MOMO 回報同名素材衝突，禁止改用素材銀行舊圖，因為該流程會重設編輯游標；應換新唯一檔名直接上傳。儲存同一草稿後只重開一次並確認圖文順序、第二張固定圖後無內容、甲指第三方 000001 仍保存，三處未齊全時禁止第一次發布。MOMO 商店分類最多 5 個，只保留與本商品直接相關的分類，送出前先驗證數量。',
      '[適用範圍：MOMO｜狀態：官方欄位已確認，帳號分類待登入實頁驗證] 品牌只選已驗證的精確品牌；平台前台會另外顯示品牌，所以 60 字商品名稱要移除開頭重複品牌。商品品號填完整 SKU，原廠編號優先填型號、無型號才填 SKU，條碼只填已驗證值。售價沿用快照；沒有已驗證較高市價時固定填售價 1.35 倍向上取整。商品其他資訊只填平台分類衍生的商品事實與法規資料，不是廣告文案；NCC、BSMI、產地、材質、配件與認證字號沒有證據就停止，不得猜測。商品特色標語與每筆特色內文都限 15 字，從已驗證商品特色前三點抽取，不為填滿而發明。店內商品分類須登入後先讀現有帳號選項，再依共同分類與商品類型選最多 5 個，禁止虛構名稱。售後保固固定勾選並填 180 天。',
      '[適用範圍：MOMO｜狀態：流程已確認，物流上限進頁再核對] preparedPlatformFieldPlan 必須在進頁前保存外箱長寬高與重量、常溫、第三方 000001、新竹物流及超商判斷；第三方固定保持啟用。保守預判材積合計不超過 101cm 且重量不超過 10kg 的小件可同時啟用超商，超過者只保留第三方，進頁後仍以 MOMO 當下可選限制為準。商品特色（專推）以原生區塊固定交錯為特色→圖一→規格→圖二→使用建議→圖三→其餘圖→實體說明→出貨與保固說明→最後兩張指定圖，保固說明必須緊貼最後兩圖上方；保存後重開同一草稿核對全部文字、圖片與順序。',
      '[商品內容]',
      '[適用範圍：共用內容｜狀態：已確認] 商品標題依「品牌＋型號＋商品種類＋重要規格或材質」組成；可靠的品牌與型號優先，共同標題只寫共同事實，差異寫細項名稱。標題、內文、圖卡都不得加入「柚子樂器」或任何未指定店名、導外訊息；只有上述三張管理者指定固定圖片可保留原有門市與服務資訊。先以精確型號或條碼確認商品身分，再依序參考製造商官方規格／說明書、台灣代理商或經銷商資料，及多個可信的使用指南與真實使用經驗；不得把單一匿名評論當作規格事實。每個特色、使用重點與規格屬性都必須在 fieldEvidence 指向來源；搜尋不到就留空。每個案件的 productDescriptionStatus 不是 ready，或 productDescription 只有通用提醒、商品編號、免責文字時，一律視為內容尚未完成；Codex 必須先整理並寫回每一份案件，後端重讀確認後才可建立 preparedSnapshot。新商品第一次上架前必須完成；既有商品以原地升級方式逐筆修正，保留同一 SKU 與平台商品編號。正式內容包含「商品特色」「使用方式／適用情境」「商品規格」三段：目標 10 點不重複、具體且可驗證特色，目標 8 點有來源的使用方式／使用心得，並完整列出所有找得到的平台屬性、尺寸、材質、段數、重量、內容物與細項差異；進入平台時再依固定圖文編排順序重排為介紹與特色、規格、使用建議。可驗證資料不足就少寫，不得猜測、湊數或用通用備援文案代替。平台保固欄仍依商品類型填寫；詳細介紹在所有商品圖後固定加入「'+PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER+'」，再加入「'+PRODUCT_WARRANTY_SUPPORT_NOTICE+'」，最後才放兩張固定介紹圖，兩張圖後不得再放文字或圖片。本次 '+targetNamesText+'沿用同一份已完成介紹並轉成各自格式。',
      '[適用範圍：EasyStore／蝦皮｜狀態：已實頁驗證] 蝦皮詳細介紹必須在進入平台前完整準備：特色、規格、使用說明、實體商品說明、出貨與保固說明、介紹圖片順序、最後兩張固定介紹圖與內容指紋全部寫入同一份 preparedSnapshot。EasyStore 官方通路頁只同步基本資料並明確略過描述匯入；送出後先把 preparedSnapshot 的全部完成圖下載到本案專用本機暫存目錄，再直接開啟 Seller Center 同一商品，交給蝦皮原生「商品描述 → 新增圖片 → 從電腦裝置上傳」欄位依文字邊界分批插入。不得只用遠端 URL 或記憶體 Blob，不得寫入桌面、開原生檔案選擇器、要求下載確認或刪除任何非助手建立的檔案。更新前必須從實際編輯器核對文字、精確圖片數、原生 CDN 上傳成功狀態、完整圖文交錯順序、出貨與保固說明直接位於最後兩張固定圖之前，且第二張固定圖是最後區塊；更新成功並重新載入仍存在才算完成。平台頁禁止重新分析或改寫文案；原生欄位不存在、版型改變或平台明確回報失敗時停止並保留同一筆工作供重試。',
      '[適用範圍：蝦皮｜狀態：已實頁驗證] 蝦皮分類與屬性必須在平台外先完成欄位計畫：分類頁會出現的屬性逐項對照商品事實與已驗證規格；固定半年保固、每包數量與新品狀態直接套用，NCC、BSMI、產地或材質等沒有來源的欄位不得猜測。分類連動後若頁面新增必填欄位，只補目前屬性區段並回寫平台欄位契約。價格同步只固定勾選「套用價格調整」，用途是讓蝦皮價格跟隨 EasyStore 價格；減號、加號、金額與百分比維持平台現值，不得由助手強制改動。',
      '同一份已驗證商品介紹只整理一次，進站前只產生 listingTargetPlatforms 對應的固定交付格式並寫入 preparedPlatformFieldPlan：EasyStore 與酷澎使用安全 HTML，把標題、段落、清單與繁體完成圖交錯排版；蝦皮使用進階圖文描述，不貼原始 HTML；MOMO 使用「商品特色（專推）」原生圖文編輯器，把相同文字與完成圖組成區塊，不假設平台接受任意 HTML 原碼。MOMO 圖文完成圖最多 20 張，每張寬 1000px、高 1500px 以下且不超過 500KB；外部嵌入只可使用 HTTPS 且支援 iframe 的網址。單一商品、同款多細項與加入既有細項都沿用相同內容計畫；平台頁只負責套用，不得重新寫文案、漏圖片或另建第二套內容。',
      '[固定發布與核對]',
      '整批商品第一次開始前，先在這個 Codex 內建瀏覽器只完成本次目標通路的工作階段預檢：'+targetNamesText+'必須已登入，且頁面可讀、可點擊、可輸入；未選通路不得登入、開頁或操作，已選通路未通過前不得開始第一件商品。MOMO 若需要 OTP／驗證碼，必須在整批開始前一次處理；酷澎與 EasyStore 若登入失效，應自動使用內建瀏覽器保存帳密重新登入、重開已知入口並驗證可操作，不得要求使用者處理可自動恢復的登入。',
      '同一批後續每件商品及每個平台階段開始前只做輕量存活檢查，不重新掃描整頁。右側內建瀏覽器既有且可讀的已登入頁籤是第一順位工作階段證據；單一深層網址載入失敗不能直接判定登出。若頁籤失效，先重新載入一次，再回到該平台固定入口確認登入地標；仍停在登入頁才使用同一內建瀏覽器已保存帳密直接按登入並重新確認地標。恢復後開啟完全相同的商品／草稿網址，沿用原 jobId、SKU、草稿 ID、preparedSnapshot 與 currentStage 續做。工作執行中不得因可恢復的頁籤或登入失效詢問是否重開、是否登入或是否繼續；只有 MOMO OTP、CAPTCHA、平台明確拒絕、帳號停用或保存帳密確實被拒絕時才停止。',
      '新案件邊界規則：每次接到與上一件不同的 SKU，先清除上一件商品留下的搜尋字、開啟中抽屜、編輯頁與未綁定新案件的暫存畫面；營運中心回到商品資訊／本案 SKU，並且只讓本次 listingTargetPlatforms 對應的頁面回到各自固定入口。只做一次輕量回正並以本案 productId、SKU、snapshotId 綁定後開始，不得打開未選通路，不得把上一件畫面當成資料錯誤；同一 SKU、同一草稿的重試則禁止回入口或重建草稿。',
      '瀏覽器只使用這個 Codex 對話旁邊的內建瀏覽器，沿用其中已登入的 EasyStore、酷澎與 MOMO 頁籤；不得操作使用者主要 Chrome，也不得在同一案件中途切換瀏覽器工作區。若使用者原本開啟的內建瀏覽器頁籤可讀取但點擊輸入持續失效，應在同一內建瀏覽器開一個工作頁籤載入完全相同網址、同一商品與同一 request／draft 身分續做；這只是輸入控制備援，不是第二條上架路徑，也不得重建商品。若登入狀態失效，可先用該內建瀏覽器已保存的帳號密碼重試登入；只有 OTP、驗證碼或登入失效且保存資料無法恢復時才停下交給使用者。',
      '按下「立即處理」已授權 '+targetNamesText+'的建立、修改與最後正式發布。分類、物流、保固、細項及平台送出都應依 preparedSnapshot 自動完成；營運中心與工作佇列不得再產生「確認上架／確認提交／套用細項」等第二次例行確認。各目標站獨立核對，listingTargetPlatforms 全部 verified 才完成整筆工作。',
      '速度規則：進入第一個平台前先產生本次目標通路完整欄位表，之後沿用已知網址、頁面區段與欄位位置，不得每站重新掃描整頁或重新思考固定欄位。半年保固、立即上架、MOMO 第三方 000001 與最後送出只在對應目標通路直接套用；只有商品分類、分類產生的必填屬性、依材積決定的超商選項及平台實際錯誤需要動態判斷。若頁面版型簽章改變，只重新辨識目前區段，不回頭重做前站。',
      '執行規則：本次根節點為 '+(selectedRoots.join('、')||'無')+'，資料預檢完成後可交錯啟動；若本次包含蝦皮，它只依賴 EasyStore，而且 EasyStore 一經正式核對 verified，就在同一 EasyStore 分支立刻進入官方蝦皮同步／編輯頁。瀏覽器的導航、點擊、輸入、檔案選擇、選圖與最後送出必須共用單一操作鎖，同一時間只操作一個頁面；某平台進入導航等待、圖片上傳、平台處理或審核等待後，立即釋放操作鎖去處理另一個已選平台，不得三頁同時輸入而搶焦點。',
      '本次已選平台都必須依 preparedPlatformFieldPlan 的 batchSections 整區批次填寫：每區只辨識一次欄位位置，文字、數字、原生選單及固定開關同批帶入，圖片同批上傳。固定欄位寫入後不逐欄回讀；只有分類連動、分類衍生屬性、特殊彈出選單與平台實際錯誤才做區段檢查。頁面簽章未變不得逐欄重掃、重驗或重做已完成區段。',
      'EasyStore 圖片優先使用既有上傳方式；若本機檔案選擇器未把檔案帶入，直接在同一商品頁改用 generatedListingImages 的公開完成圖網址批次加入，不得重做圖片。蝦皮大型商品必須同時開啟符合材積級距的新竹物流與「賣家宅配：大型/超重物品運送」（固定運費 NT$100、賣家不吸收），並關閉全家、7-ELEVEN、蝦皮店到店、店到家、嘉里與黑貓；小型商品才依 preparedPlatformFieldPlan 開啟已核實的超商選項。',
      '酷澎新增固定走已驗證的「以圖片建立」同一草稿：同款多細項必須在第一次產生前先上傳每一個細項各自的 cleanMain，再補繁體完成副圖；不得只上傳第一個細項的圖片。填好樂器葉分類及可驗證品牌後只按一次「產生商品資訊」。商品選項區必須以每一列為範圍，先寫入並確認唯一細項值，再批次填售價、庫存與賣家 SKU；不得用整頁動態 nth 順序跨列填寫。每列成為可銷售且完整 SKU 集合與 preparedPlatformFieldPlan 相同後，立即開啟自動儲存並儲存同一草稿，才可進入配送、人工繁體介紹與 TW_General 合規資料。細項欄位若呈唯讀或值無效，只能在尚未填寫細項商務、配送或介紹以前停在同一草稿；平台若自動複製前一列、把新細項改回舊值、產生重複唯讀值或失去列資料，同樣不得填商務欄位、不得建立替代商品，只可回同一草稿圖片步驟一次，補齊全部細項 cleanMain 後重新產生，並在選項值穩定後再填商務欄位。後續區段已填後禁止返回或重新產生，晚期錯誤只修正目前區段並保留已完成資料。最後按「建立產品」並以本案完整 SKU 集合在正式清單核對一次；不得整頁重跑、名稱廣搜或另建商品。審核中只記為已送審，不得誤報已上架。',
      '[適用範圍：酷澎｜狀態：已實頁驗證] 修改既有商品固定以精確賣家 SKU 與唯一 Vendor Inventory ID 開啟原商品。展開可輸入屬性一次，只填有證據的值；原聲吉他型態、琴弦材質、慣用手、產地與認證無來源就留空。已確認六弦時，「吉他琴橋線(弦)數」必須同時在商品基本資訊與每一個細項列的「編輯選項屬性」填單位「弦」、數值 6，並儲存細項屬性。「選項 檢查欄位」就回到該細項的編輯屬性視窗；「產品基本資訊 檢查欄位」就重讀商品層級。修改選項後必須重讀兩層，因為酷澎可能把屬性數值重設為 0；不得虛構 GTIN 或未知屬性。圖庫保留正確現有順序並把 product-listing-store-promo.png 放在最後補充圖。詳細說明從「已寫入HTML」列按「修改」，以 PC 預覽確認實體商品說明與出貨及保固說明位於最後兩張固定介紹圖之前，第二張圖後無任何內容；儲存、重開核對後才按「申請修改」。出現「修改申請完成」後以精確 SKU 核對同一 Vendor Inventory ID、價格、庫存與銷售／審核狀態；審核中才記 pending-review，不得誤報 active。',
      '商品要新增、成組、加入細項或原地修改，只依 preflightSnapshot.listingIntent 決定；不得再用「有沒有平台 ID」自行改變意圖。新增模式遇到既有 ID 必須停止，修改模式缺少既有 ID 必須停止；只有送出逾時或結果不明時，才以同一 SKU 或平台正式品號精確查詢一次，不得用名稱廣搜。',
      ...
      '平台售價空白時沿用共同網路售價、網路售價或門市售價；分類限定在樂器／樂器配件。本次已選平台各自保存狀態並可交錯完成；若包含官網，EasyStore verified 後立刻解除蝦皮依賴並在同一分支續做。全部沿用同一 jobId、最終 preparedSnapshot、preparedPlatformFieldPlan 與已選平台 stage。重試不得重新讀取案件或重建快照。每站送出後只以本案完全相同 SKU 做一次正式資料核對：平台商品編號、SKU、售價、庫存、送出／審核狀態及正式清單存在；同款多細項或加入既有細項還必須核對完整 SKU 集合、每列細項名稱、售價、庫存與指定小圖。任何必填內容缺漏或不一致都不能標記完成；直接回到同一商品或同一草稿，只補缺漏欄位後再次送出並以相同 SKU 核對，不得另建替代商品，也不得回頭重做已正確欄位。',
      '本件所選平台工作結束後，再以本案完整精確 SKU 對 EasyStore 官網、EasyStore 官方蝦皮通路、MOMO 與酷澎四個正式商品清單各查一次，並直接更新本組每個中央商品的 platformListingStatus、listingId、lastCheckedAt 與簡短原因。正式清單存在才寫 active／有；確定找不到、拒絕、受限制或已刪除寫 missing／沒有。酷澎正式清單顯示審核中時寫 pending-review 並保存送審後 24 小時與 48 小時重查時間，不得寫 active；這項狀態核對是每件上架的收尾規則，不得略過。',
      'MOMO 專推圖固定走同一草稿的商品詳細介紹編輯器：平台外先從 preparedPlatformFieldPlan.momo.preparedFields.firstSubmitMediaGate 取得全部完成圖、依 SKU＋內容指紋＋順序產生的唯一檔名與固定區塊計畫；進站後按「上傳圖片」→「此裝置上傳」，逐段插入精確文字邊界。全部文字先完成，最後兩張固定介紹圖再跳到整份編輯器末端並依圖一、圖二順序一次上傳；禁止沿用同名素材銀行舊圖，也禁止在第二張固定圖後留下文字或圖片。主圖或廣告圖不能當成專推圖證據；商品主圖、廣告用圖、編輯器專推圖三處完成並儲存後，只重開同一商品一次確認兩段固定原文各一次、全部 CDN 圖片、完整順序與第二張固定圖確為最後區塊，隨即第一次發布，不得先送空缺版本再回頭補。發布後以 MOMO 品號或原廠編號精確核對一次。',
      'MOMO 有庫存新品的名額規則固定由 preparedPlatformFieldPlan.momo.preparedFields.capacityGate 執行：第一次發布前先讀取正式上架數；未達 1000 件直接發布，不得下架任何舊品。只有目標庫存大於 0、確實是需要新名額的新品且正式上架數已達 1000 件時，才從正式上架清單選一筆非本 SKU、非本批次、非保留／置頂、無待處理訂單、正式庫存為 0 且沒有銷售紀錄的不重要商品暫時下架；優先使用明列低優先度者，其次零銷售且最久未更新者。只下架取得本次一個父商品名額所需的一筆，永遠不得刪除、不得動有庫存商品、不得另建替代草稿。下架後先核對該品號已在下架清單且上架數低於 1000，再沿用完全相同 jobId、preparedSnapshot、MOMO 品號與草稿發布新品；新品正式清單核對為上架後，保存被下架品號、前後數量與新品品號的名額回收回條。找不到安全候選才以確切原因停止。按下立即處理已涵蓋這項例行名額回收，不得再次詢問確認。',
      'MOMO 的「此帳號無此功能權限」必須依發生步驟分類，不得直接判定整個帳號無權限：若在素材銀行選取同名圖片時出現，視為素材路徑衝突，立即改用包含 SKU、細項與內容指紋的唯一檔名，從「此裝置上傳」直接上傳；不得再選同名素材、重新登入或重做圖片。若直接上傳已成功、只在已上架商品送出新增細項圖時出現，視為目前編輯路徑受限，保留同一 MOMO 品號、完整 SKU 集合與已上傳素材，轉入同一商品的規格異動流程續送；不得另建、複製、重傳或回營運中心重做。只有規格異動流程也明確拒絕一次後，才記錄為 account-permission-denied 永久阻擋。',
      '蝦皮官方通路頁開始操作前先收合內嵌客服聊天，避免遮住欄位或誤攔按鍵。細項代表圖必須從該 EasyStore 商品已存在的繁體完成圖庫選取，依完成圖 URL／素材 ID 對應各細項；不得開啟 Windows 原生選檔視窗。EasyStore 共用圖庫同步成功不代表蝦皮細項小圖已帶入，必須在官方同步頁每一個細項圖片格實際選入其對應繁體完成圖；新建同款商品、併入舊商品與加入既有細項都相同。所有細項圖片與由包裝重量換算的必填公克數完成後，才按「準備發布」與「上架」。',
      '禁止先全面瀏覽或搜尋任一平台商品清單。只有送出結果不明、回應逾時或精確狀態無法判定時，才以本案件完全相同 SKU 做一次精確查詢；不得用名稱廣搜或建立替代商品。蝦皮基本欄位與發布先使用 EasyStore 官方通路頁；詳細介紹固定接續 Seller Center 同一商品的原生圖片上傳與更新，不得切換成其他商品或第二份內容。',
      '[安全重試]',
      '圖片暫時讀不到、圖片仍在處理、網路逾時、HTTP 408／425／429／500／502／503／504、平台暫時錯誤、頁籤控制失效、被導回登入頁或一般登入工作階段到期時，只針對同一案件、同一 SKU、同一平台草稿與目前階段恢復登入後重試，再做一次精確 SKU 核對；只可從 generatedListingImages 或已驗證中央完成圖重新取得 URL。只有缺少無法自動補齊的必填資料、平台明確拒絕、MOMO OTP／驗證碼、CAPTCHA、帳號停用或保存帳密確實被拒絕才停止，並留下確切原因。',
    ].join('\n');
  }
  function productListingCodexActivationPrompt(product,snapshot){
    const cases=Array.isArray(snapshot&&snapshot.cases)?snapshot.cases:[],caseIds=cases.map(function(item){return clean(item&&item.productId);}).filter(Boolean),policy=snapshot&&snapshot.listingIntentPolicy||productListingIntentPolicy(snapshot&&snapshot.listingIntent),mode=clean(policy.label),listingTargetScope=normalizeProductListingTargetScope(snapshot&&snapshot.listingTargetScope),listingTargetPlatforms=productListingTargetPlatforms(listingTargetScope);
    return [
      '[固定流程 v3 短交接]',
      '請接手全通路營運中心待上架商品 '+clean(product.sku)+'｜'+clean(product.originalName||product.name)+'。',
      'workflowVersion：'+PRODUCT_LISTING_WORKFLOW_VERSION,
      '根案件：opsProductListingCases/'+clean(product.docId),
      '不可變預檢快照：'+clean(snapshot&&snapshot.snapshotId),
      '商品處理方式：'+mode+'（'+clean(snapshot&&snapshot.listingIntent)+'）',
      '工作用途：'+(clean(snapshot&&snapshot.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹':'標準上架')+'；五種商品處理方式一律套用 '+PRODUCT_DESCRIPTION_LAYOUT_VERSION+' 固定圖文編排，缺少任何固定圖片不得完成。',
      '本次上架範圍：'+productListingTargetLabel(listingTargetScope)+'；只准處理 '+listingTargetPlatforms.join('、')+'，未選通路禁止操作。',
      '本組案件：'+caseIds.map(function(id){return 'opsProductListingCases/'+id;}).join('、'),
      '完整固定規則、來源圖、修改要求、細項、價格、庫存與物流資料已保存在根案件 codexHandoff.prompt 與 codexHandoff.preflightSnapshot；請先讀取該案件，不得使用 v1、舊快照或另一路徑。',
      '先一次完成本組每個商品的繁體完成圖並依 sourceImageUrl 寫回各自 generatedListingImages；後端必須把每個商品與細項的 cleanMain 回寫中央商品並重讀驗證，才可依同一 preparedSnapshot 啟動本次已選通路。官網範圍包含 EasyStore，並在 EasyStore verified 後接蝦皮。',
      '每件所選通路工作完成後，仍須以本組完整精確 SKU 在 EasyStore 官網、官方蝦皮、MOMO、酷澎正式清單各查一次並回寫四個中央 platformListingStatus；酷澎審核中寫 pending-review 並排定 24／48 小時重查，只有正式核准上架才寫 active。',
      '按下立即處理已授權本次所選通路正式發布，不得再要求例行確認，也不得擴大範圍。'
    ].join('\n');
  }
  ...
  function productPlatformAuditPrompt(products,options){
    const scopedProducts=Array.isArray(products)?products.filter(Boolean):[],settings=options&&typeof options==='object'?options:{},single=scopedProducts.length===1&&!!settings.productId,publishedOnly=settings.publishedOnly===true,productCount=single?1:publishedOnly?scopedProducts.length:catalogRowsInSkuOrder().filter(function(product){return hasListingSku(product.sku);}).length,scopeText=single?('只檢查 '+clean(scopedProducts[0].sku)+'｜'+clean(scopedProducts[0].originalName||scopedProducts[0].name)+'，中央商品文件 opsInternalProducts/'+clean(scopedProducts[0].docId)+'。'):publishedOnly?'只檢查曾送出、已上架、審核中或已有平台編號的商品案件；不得掃描從未送出過的中央商品。':'必須從中央商品資訊頁讀取全部商品並檢查到完畢，不可抽查。';
    return [
      '[全通路商品狀態完整檢測]',
      '請立即接手全通路營運中心「商品資訊」的四平台有效上架狀態檢測，檢測完成後直接更新中央商品紅綠狀態；本次確認已授權讀取平台清單及修改中央商品狀態，不需逐筆或在中途再次詢問。',
      '中央系統：'+location.href,
      '檢測範圍：EasyStore 官網、EasyStore 官方蝦皮通路、MOMO 店+商品管理、Coupang Wing；目標商品共 '+productCount+' 筆。'+scopeText,
      ...
      '酷澎特別規則：平台清單仍顯示審核中時寫成 pending-review，不可寫 active，也不可寫 missing；同時保存第一次重查時間為送審後 24 小時、第二次重查時間為送審後 48 小時。重查發現已核准上架才改 active；發現審核未通過、拒絕或不可恢復才改 rejected／missing。',
      '更新要求：每一筆中央商品的 easyStore、shopee、momo、coupang 都寫入最新 platformListingStatus、正確 listingId、查驗時間與簡短原因；缺貨／一般下架寫明仍可恢復並保持綠色，酷澎審核中保持黃色，違規／拒絕寫明需重新上架並保持紅色。不得修改四平台商品內容、價格、庫存或上下架狀態。',
      '完成後回報四平台各自的「有／審核中／沒有／違規」總數、修正筆數、找不到及需要重新上架的 SKU 清單，並重新讀取中央頁面確認顏色已更新。'
    ].join('\n');
  }
  async function publishedProductPlatformAuditTargets(){
    const snapshot=await state.db.collection(COLLECTIONS.listingCases).limit(5000).get(),ids=new Set();
    snapshot.docs.forEach(function(doc){const row=doc.data()||{},publishState=row.publishState&&typeof row.publishState==='object'?row.publishState:{},status=clean(row.caseStatus);if(['submitted','published','waiting-codex'].includes(status)||clean(publishState.jobId)||clean(publishState.status))ids.add(doc.id);});
    catalogRowsInSkuOrder().forEach(function(product){if(productHasPlatformListing(product))ids.add(product.docId);});
    return Array.from(ids).map(catalogById).filter(function(product){return product&&hasListingSku(product.sku);});
  }
  async function startProductPlatformAudit(options){
    const settings=options&&typeof options==='object'?options:{},singleProduct=settings.productId?catalogById(clean(settings.productId)):null,products=singleProduct?[singleProduct]:settings.publishedOnly?await publishedProductPlatformAuditTargets():[];
    if(settings.productId&&!singleProduct)throw new Error('找不到要重新查驗的商品');
    if(settings.publishedOnly&&!products.length)throw new Error('目前沒有曾送出或審核中的商品需要重查');
    const title=singleProduct?'重新檢查這件商品':settings.publishedOnly?'重查已送出／審核中商品':'檢測全部網路商品',detail=singleProduct?'將以這個 SKU 在官網、蝦皮、MOMO 與酷澎正式清單逐一核對，並直接回寫最新狀態。':settings.publishedOnly?'只檢查曾送出、已有平台編號或仍在審核中的商品；適合在酷澎送審隔天或後天使用。':'將完整檢查 EasyStore 官網、蝦皮、MOMO 與酷澎，並依最新平台結果直接更新全部商品狀態。';
    const yes=await confirmAction(title,detail+' 缺貨、庫存 0 或一般下架仍算存在；酷澎審核中會維持黃色，違規、未通過或已刪除才會標成沒有。確認後會帶入 Codex 自動執行到完成。','同意並開始檢測');
    if(!yes)return;
    const scope=singleProduct?'one-product-four-platforms':settings.publishedOnly?'published-or-pending-products-four-platforms':'all-central-products-four-platforms',prompt=productPlatformAuditPrompt(products,{productId:singleProduct&&singleProduct.docId,publishedOnly:settings.publishedOnly===true}),threadUrl=productListingCodexThreadUrl(prompt),copied=await copyProductListingCodexPrompt(prompt);
    await state.db.collection(COLLECTIONS.settings).doc('productPlatformAudit').set({status:'requested',scope:scope,targetProductIds:singleProduct?[singleProduct.docId]:settings.publishedOnly?products.map(function(product){return product.docId;}):[],decisionPolicy:'valid-listing-including-out-of-stock-and-normal-inactive;pending-review-recheck-24h-48h;exclude-violations-rejected-restricted-deleted',authorizationGranted:true,noSecondConfirmation:true,requestedAt:serverTimestamp(),requestedBy:userLabel(),requestedByEmail:clean(state.user&&state.user.email).toLowerCase(),threadId:PRODUCT_LISTING_CODEX_THREAD_ID,threadUrl:threadUrl,sourcePageUrl:location.href,version:VERSION},{merge:true});
    await writeAudit('要求檢測網路商品狀態','productPlatformAudit','productPlatformAudit','EasyStore／蝦皮／MOMO／酷澎｜'+(singleProduct?singleProduct.sku:settings.publishedOnly?'曾送出 '+products.length+' 件':'全部中央商品')+'｜已授權完成後直接更新');
  ...
  function updateNineSeriesBookCoverProgress(result,latestRows){
    ...
    const matched=Math.max(0,Number(result&&result.matchedCount||0)),unresolved=Math.max(0,Number(result&&result.unresolvedCount||0)),failed=Math.max(0,Number(result&&result.failedCount||0));
    const percent=total?Math.min(100,Math.round(cursor*100/total)):100;
    const rows=(latestRows||[]).map(function(row){const label=row.status==='matched'?'已補齊':row.status==='unresolved'?'待確認':'處理失敗';return '<li><b>'+escapeHtml(row.sku||'未設定')+'</b><span>'+escapeHtml(label)+(row.matchedTitle?'｜'+row.matchedTitle:row.reason?'｜'+row.reason:'')+'</span></li>';}).join('');
    target.innerHTML='<div class="ops-callout green"><b>進度 '+formatNumber(cursor)+' / '+formatNumber(total)+'（'+percent+'%）</b><br><span>已補齊 '+formatNumber(matched)+' 本｜待確認 '+formatNumber(unresolved)+' 本｜失敗 '+formatNumber(failed)+' 本</span></div>'+(rows?'<ul class="ops-summary-list">'+rows+'</ul>':'');
  }
  async function startNineSeriesBookCoverBatch(){
    ...
    finally{nineSeriesBookCoverBatchBusy=false;renderKeepingViewport();}
  }
  ...
  async function handoffProductListingToCodex(form,targetScope){
    if(!form||form.dataset.codexRunning==='1')return null;
    const id=clean(form.dataset.id),product=catalogById(id),executionPlan=productListingExecutionPlan(targetScope||form.dataset.targetScope),listingTargetScope=executionPlan.listingTargetScope,listingTargetPlatforms=executionPlan.listingTargetPlatforms;if(!id||!product)throw new Error('找不到商品上架資料');
    form.dataset.codexRunning='1';form.dataset.activeScope=listingTargetScope;form.dataset.targetScope=listingTargetScope;
    try{
      await requireEasyStoreManagerAuth();
      ...
      await saveProductListingCase(form,false,true,true);
      await state.db.collection(COLLECTIONS.listingCases).doc(id).set({listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION},{merge:true});
      setProductListingCodexUi(form,'running','案件已保存，正在建立不可變快照','建立後只會處理本次選擇的通路，安全重試也沿用同一份資料。');
      const snapshot=await loadProductListingCodexHandoffSnapshot(product);requireProductListingCodexHandoffMedia(snapshot);
      setProductListingCodexUi(form,'running','來源快照已建立，正在保存固定 v3 工作','保存完成後會立即啟動'+productListingTargetLabel(listingTargetScope)+'；複製交接文字與稽核紀錄不會阻擋發布。');
      const prompt=productListingCodexHandoffPrompt(product,snapshot),activationPrompt=productListingCodexActivationPrompt(product,snapshot),threadUrl=productListingCodexThreadUrl(activationPrompt),serverTime=serverTimestamp(),platforms=executionPlan.platforms;
      await state.db.collection(COLLECTIONS.listingCases).doc(id).set({
        caseStatus:'waiting-codex',
        codexHandoff:{status:'pending',workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,threadId:PRODUCT_LISTING_CODEX_THREAD_ID,threadUrl:threadUrl,activationPrompt:activationPrompt,prompt:prompt,productId:id,productSku:clean(product.sku),productName:clean(product.originalName||product.name),listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,preflightSnapshot:snapshot,autoPublishAuthorization:{granted:true,scope:'fixed-v3-selected-channel-publish',listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,snapshotId:snapshot.snapshotId,grantedAt:serverTime,grantedBy:userLabel(),grantedByEmail:clean(state.user&&state.user.email).toLowerCase(),noSecondConfirmation:true},requestedAt:serverTime,requestedBy:userLabel(),sourcePageUrl:location.href},
        codexExecution:{workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,snapshotId:snapshot.snapshotId,currentStage:'codex-preflight',listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,platformOrder:executionPlan.platformOrder,executionGraph:{mode:'staggered-parallel',parallelRoots:executionPlan.parallelRoots,dependencies:executionPlan.dependencies},variantFieldExecution:{prepareAllVariantsBeforePlatformNavigation:true,reuseStableVariantFieldSemantics:true,reuseStableVariantSelectorsWhenLayoutSignatureMatches:true,rescanCurrentVariantSectionOnlyWhenLayoutSignatureChanges:true,neverRestartImageOrPreviousPlatformStages:true},browserScheduler:{interactionConcurrency:1,releaseInteractionLockDuringWait:true,sessionHeartbeatSeconds:180,keepAuthenticatedAnchorTabPerPlatform:true,loginRecovery:{automatic:true,currentAuthenticatedTabHasPriority:true,deepLinkFailureAloneDoesNotProveLogout:true,retryCanonicalEntryBeforeCredentialLogin:true,useSavedCredentialsBeforeRequestingUser:true,resumeSameJobSkuDraftAndStage:true,stopOnlyForOtpCaptchaOrRejectedCredentials:true},postSubmitRepair:{verifyRequiredFieldsOnExactSku:true,reapplyOnlyMissingFieldsToSameListing:true,resubmitAndVerifyAgain:true,neverCreateReplacementListing:true}},platforms:platforms,retrySameSkuDraftAndStage:true,neverRebuildPreflightDuringRetry:true,updatedAt:serverTime},
        updatedAt:serverTime,updatedBy:userLabel(),version:VERSION
      },{merge:true});
      if(form.dataset.queueStatus){
        await state.db.collection(COLLECTIONS.listingCases).doc(id).set({batchQueueStatus:'processing',batchQueueUpdatedAt:serverTime,batchQueueError:'',batchRunId:clean(form.dataset.batchRunId)||('listing-single-'+Date.now().toString(36)),batchPosition:1},{merge:true});
        upsertProductListingQueueState({productId:id,productSku:product.sku,productName:product.originalName||product.name,productImageUrl:product.imageUrl,listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,batchQueueStatus:'processing',batchQueuedAt:new Date(),batchQueueUpdatedAt:new Date(),batchPosition:1});form.dataset.queueStatus='processing';
      }
      ...
      if(completedMediaReady){
        setProductListingCodexUi(form,'running','完成圖已齊全，正在啟動'+productListingTargetLabel(listingTargetScope),'同一份 v3 快照只會處理本次選擇的通路。');
      ...
      setProductListingCodexUi(form,'completed','已授權'+productListingTargetLabel(listingTargetScope)+'自動上架',completedMediaReady?'本次所選通路工作已啟動。':'圖片仍待 Codex 一次定案；完成圖寫回後再按相同方塊，即會直接啟動本次所選通路。');
      global.setTimeout(function(){global.location.href=threadUrl;},100);
      return {status:publishResult?clean(publishResult.status)||'submitted':'pending',productId:id,listingTargetScope:listingTargetScope,listingTargetPlatforms:listingTargetPlatforms,workflowVersion:PRODUCT_LISTING_WORKFLOW_VERSION,snapshotId:snapshot.snapshotId,threadId:PRODUCT_LISTING_CODEX_THREAD_ID,threadUrl:threadUrl,activationPrompt:activationPrompt,promptCopied:copied,publishResult:publishResult};
    }catch(error){
  ...
  function productListingCodexResultDraft(product,listingCase){
    const source=listingCase&&typeof listingCase==='object'?listingCase:{},scope=normalizeProductListingTargetScope(source.listingTargetScope),platforms=productListingTargetPlatforms(scope);
    return {id:product.docId,sku:product.sku,name:product.originalName||product.name,researchedName:clean(source.researchedProductName),shippingDecision:clean(source.shippingDecision),listingTargetScope:scope,listingTargetPlatforms:platforms,enabledEasyStoreShopee:platforms.includes('easyStore'),enabledMomo:platforms.includes('momo'),enabledCoupang:platforms.includes('coupang')};
  }
  function productListingImageGenerationReady(listingCase,sourceImageUrls,instructions){
    const source=listingCase&&typeof listingCase==='object'?listingCase:{},phase=source.lastImageGeneration&&typeof source.lastImageGeneration==='object'?source.lastImageGeneration:{},expected=normalizeProductResearchSourceUrls(sourceImageUrls).slice(0,PRODUCT_SELECTED_IMAGE_MAX),completedSources=normalizeProductResearchSourceUrls(phase.sourceImageUrls).slice(0,PRODUCT_SELECTED_IMAGE_MAX),generated=normalizeGeneratedListingImages(source.generatedListingImages),listingUrls=normalizeProductResearchSourceUrls(source.listingImageUrls),readyBySource=new Map();
    generated.forEach(function(row){if(row.status==='ready'&&(row.localizationStatus==='completed'||row.qaStatus==='approved')&&row.sourceImageUrl&&row.url){const rows=readyBySource.get(row.sourceImageUrl)||[];rows.push(row);readyBySource.set(row.sourceImageUrl,rows);}});
    ...
    return expected.every(function(url){const rows=readyBySource.get(url)||[];return rows.some(function(row){return row.roles.some(function(role){return ['cleanMain','localizedDetail','specification','variantRepresentative'].includes(role);})&&listingUrls.includes(row.url);});});
  }
  ...
  async function resumeExplicitShopeeListingFromQuery(){
    ...
    if(!/^[A-Za-z0-9_-]{1,200}$/.test(jobId)||!/^[A-Za-z0-9_-]{1,200}$/.test(productId))throw new Error('指定的 v3 上架工作代碼不正確。');
    await requireEasyStoreManagerAuth();
    ...
    const stages=result&&result.stages&&typeof result.stages==='object'?result.stages:{},easyStoreStage=stages.easyStore&&typeof stages.easyStore==='object'?stages.easyStore:{},shopeeStage=stages.shopee&&typeof stages.shopee==='object'?stages.shopee:{};
    if(clean(easyStoreStage.status)!=='verified'||clean(shopeeStage.status)==='verified'||!payload)throw new Error('指定工作目前尚未進入可續跑的蝦皮階段。');
    if(clean(payload.workflowVersion)!==PRODUCT_LISTING_WORKFLOW_VERSION||clean(payload.jobId)!==jobId||clean(payload.productId)!==productId)throw new Error('蝦皮資料不屬於指定的固定 v3 工作，已停止續跑。');
    ...
    try{global.sessionStorage.setItem('youzi_v3_shopee_resume_'+jobId,'queued');}catch(error){}
  ...
  function productListingTransientFailure(value){
    ...
    if(/(?:缺少無法自動補齊|平台明確拒絕|OTP|驗證碼|captcha|保存帳密[^，。\n]{0,20}(?:拒絕|錯誤)|帳號[^，。\n]{0,12}停用|細項代表圖尚未完成繁體化)/i.test(message))return false;
    return /(?:HTTP\s+(?:408|425|429|500|502|503|504)|連線失敗|網路(?:錯誤|異常)|Gateway Timeout|timeout|timed out|temporar(?:y|ily)|登入(?:已)?失效|login expired|session expired|跳回登入|redirected[^,.;\n]{0,12}login|頁籤[^，。\n]{0,12}(?:失效|無法控制)|圖片[^，。\n]{0,24}(?:暫時|讀取|載入|下載|取得|找不到|處理中|逾時|失敗)|image[^,.;\n]{0,32}(?:fetch|load|download|not found|processing|timeout|failed))/i.test(message);
  }
  async function callProductListingPublishWithTransientRetry(productId,form){
    const delays=[0,3000,10000,30000];let lastResult=null,lastError=null;
    for(let attempt=0;attempt<delays.length;attempt+=1){
      if(delays[attempt]){
        setProductListingCodexUi(form,'running','平台或圖片暫時未完成，正在安全重試','等待後會以完全相同的案件、SKU 與平台草稿重試（第 '+(attempt+1)+'／'+delays.length+' 次）；不會建立重複商品。');
        await new Promise(function(resolve){global.setTimeout(resolve,delays[attempt]);});
      }
      try{
        lastResult=await callProductListingPublish(productId);lastError=null;
        if(!productListingTransientFailure(lastResult)||attempt===delays.length-1)return lastResult;
      }catch(error){
        lastError=error;if(!productListingTransientFailure(errorMessage(error))||attempt===delays.length-1)throw error;
      }
    }
  ...
  async function publishSavedProductListingCase(id){
    ...
    const snap=await state.db.collection(COLLECTIONS.listingCases).doc(id).get(),raw=snap.exists?snap.data()||{}:{};
    return confirmAndPublishProductListingCase({id:id,sku:product.sku,name:product.originalName||product.name,researchedName:clean(raw.researchedProductName),shippingDecision:clean(raw.shippingDecision),enabledEasyStoreShopee:true,enabledMomo:true,enabledCoupang:true});
  }
  ...
  async function syncEasyStoreApi(){
    ...
    try{
      ...
      clearAlert();
      toast('EasyStore API 同步完成','父商品 '+formatNumber(result.productCount)+'、規格 SKU '+formatNumber(result.variantCount)+'、配對 '+formatNumber(result.matchedCount)+'、細項圖已帶入 '+formatNumber(result.variantImageMatchedCount)+'、缺細項圖 '+formatNumber(result.variantImageMissingCount),'success');
      await loadAll(true);
    }catch(error){
      console.error(error);
      const message=errorMessage(error);
...
async function syncPlatformOrdersNow(){const yes=await confirmAction('要求店內電腦立即同步','系統會建立同步請求；店內電腦開機且背景程式正常時，最慢約 20 秒內接收。若同步正在進行，不會重複執行。','建立請求');if(!yes)return;const ref=state.db.collection(COLLECTIONS.platformSyncRequests).doc();await ref.set({requestId:ref.id,status:'pending',requestedAt:serverTimestamp(),requestedBy:userLabel(),source:'operations-hub',version:VERSION});toast('同步請求已建立','店內電腦會在約 20 秒內接收，完成後重新讀取即可查看結果。','success');}
  ...
  function handleAction(action,el){
    ...
    if(action==='product-listing-queue-start'){
      el.disabled=true;el.textContent='正在帶入…';
      return startProductListingQueue().catch(function(error){toast('批次尚未開始',errorMessage(error),'error');el.disabled=false;el.textContent='開始處理全部';});
    }
    if(action==='product-merge-select'){
      const id=clean(el.dataset.id),selected=productMergeSelectedIds(),index=selected.indexOf(id);if(!id)return;
      if(index>=0)selected.splice(index,1);else selected.push(id);state.productMergeSelection=selected;return renderKeepingViewport();
    }
    if(action==='product-merge-clear'){state.productMergeSelection=[];return renderKeepingViewport();}
    if(action==='product-merge-open'){
      el.disabled=true;return openProductMergeSelection().catch(function(error){toast('商品尚未合併',errorMessage(error),'error');el.disabled=false;});
    }
    if(action==='product-recent'){
      if(!closeProductEditorForListChange())return;
      state.productRecentOnly=!state.productRecentOnly;if(!state.productRecentOnly)state.productMergeSelection=[];state.productSeries='all';state.productFilter='all';state.productSearch='';state.productVisible=PRODUCT_PAGE_SIZE;return renderKeepingViewport();
    }
    ...
    if(action==='product-variant-group-add'){
      ...
      return loadProductListingVariantMedia(product.docId).then(function(media){const automatic=productAutomaticVariantRepresentative(product);items.push({productId:product.docId,sku:product.sku,name:product.originalName||product.name,attributeValue:productVariantSuggestion(product),imageUrls:automatic.url?[automatic.url]:[],sourceImageUrls:normalizeProductResearchSourceUrls([].concat(automatic.url?[automatic.url]:[],media.sourceImageUrls||[])),completedImageUrls:media.completedImageUrls,gallerySourceImageUrls:[],imageMatchStatus:automatic.url?automatic.source:'missing'});renderProductVariantGroupItems(form,items);const input=query('[name="variantGroupSearch"]',form);if(input){input.value='';input.focus();}form.dataset.dirty='1';}).catch(function(error){toast('商品圖片尚未載入',errorMessage(error),'error');});
    }
    ...
    if(action==='product-listing-codex-complete'){
      const form=byId('productListingCaseForm');applyProductListingWorkflowPurpose(form,el.dataset.purpose);
      return handoffProductListingToCodex(form,el.dataset.scope).catch(function(error){toast('Codex 待辦尚未建立',errorMessage(error),'error');});
    }
    ...
    if(action==='product-variant-image-collection'){
      el.disabled=true;
      return startProductImageCollection(byId('productListingCaseForm'),el.dataset.id).catch(function(error){toast('此細項收圖尚未啟動',errorMessage(error),'error');}).finally(function(){el.disabled=false;});
    }
    ...
    if(action==='product-variant-image-clear'){
      el.disabled=true;
      return clearProductVariantRepresentativeImage(byId('productListingCaseForm'),el.dataset.id,el.dataset.role).catch(function(error){toast('代表圖尚未清除',errorMessage(error),'error');}).finally(function(){el.disabled=false;});
    }
    if(action==='product-variant-source-image-remove'){
      el.disabled=true;
      return removeProductVariantReferenceImage(byId('productListingCaseForm'),el.dataset.id,el.dataset.url,el.dataset.role).catch(function(error){toast('細項圖片刪除未完成',errorMessage(error),'error');}).finally(function(){el.disabled=false;});
    }
    if(action==='product-variant-gallery-toggle'){
      const card=el.closest('article'),input=card&&query('[name="variantGallerySourceImageUrls"]',card),form=byId('productListingCaseForm');if(!input||!form)return;
      const checked=queryAll('[name="variantGallerySourceImageUrls"]:checked',form);if(input.checked&&checked.length<=1)return toast('至少保留一張上架圖','可改選其他圖片後，再排除這一張。','warning');
      if(!input.checked&&checked.length>=PRODUCT_GROUP_LISTING_IMAGE_MAX)return toast('最多上架 12 張','請先排除其他圖片。','warning');
      input.checked=!input.checked;form.dataset.dirty='1';refreshProductVariantImageProcessingQueue(form);return;
    }
    ...
    if(action==='product-listing-publish-saved'){
      toast('舊上架入口已停用','請回到「準備上架」並使用 Codex v3 固定流程；v2 工作不可混用。','warning');return;
    }
    if(action==='product-listing-publish-prepare'){
      toast('舊上架入口已停用','請使用「帶入這個 Codex 對話」建立 v3 不可變快照。','warning');return;
    }
...
function rerenderKeepingFocus(id,value){
  ...
  function bindEvents(){
    ...
    document.addEventListener('drop',function(event){
      ...
      const url=safeUrl(payload.url||event.dataTransfer.getData('text/plain')),targetId=clean(card.dataset.productId),targetRole=clean(card.dataset.variantRole);if(!url||!targetId)return;
      copyProductVariantReferenceImage(form,targetId,url,targetRole).catch(function(error){toast('圖片尚未拖入細項',errorMessage(error),'error');});
    });
    document.addEventListener('dragend',function(){queryAll('.ops-listing-variant-picker-option.is-dragging,.ops-listing-variant-item.is-drag-over').forEach(function(row){row.classList.remove('is-dragging','is-drag-over');});});
    ...
    document.addEventListener('change',function(event){
      ...
      if(event.target.matches('[name="variantGallerySourceImageUrls"]')){const checked=queryAll('[name="variantGallerySourceImageUrls"]:checked',listingCaseForm);if(!checked.length){event.target.checked=true;toast('至少保留一張上架圖','可改選其他圖片後，再排除這一張。','warning');}else if(checked.length>PRODUCT_GROUP_LISTING_IMAGE_MAX){event.target.checked=false;toast('最多上架 12 張','請先排除其他圖片。','warning');}refreshProductVariantImageProcessingQueue(listingCaseForm);return;}
      if(event.target.matches('#productListingCaseForm [name="listingIntent"]')){updateProductListingMode(listingCaseForm);return;}
      ...
      if(event.target.id==='productPhysicalImageUpload'){
        ...
        return (async function(){for(const file of files)await uploadPhysicalProductPhoto(productId,file,{source:'listing-case-camera-or-library',statusElementId:'productPhysicalImageUploadStatus'});})().catch(function(error){const status=byId('productPhysicalImageUploadStatus');if(status)status.innerHTML='<div class="ops-product-ai-status failed"><span>!</span><div><b>實體圖尚未保存</b><small>'+escapeHtml(errorMessage(error))+'</small></div></div>';toast('實體圖上傳失敗',errorMessage(error),'error');state.physicalPhotoBusy=false;});
      }
