...
function renderSalesV5(){
  ...
  function renderSalesHistoryV7(){
    const term=lower(state.saleInvoiceSearch).trim(),from=state.saleInvoiceFrom?new Date(state.saleInvoiceFrom+'T00:00:00'):null,to=state.saleInvoiceTo?new Date(state.saleInvoiceTo+'T23:59:59'):null,expanded=state.salesHistoryExpanded===true,entries=[];
    state.sales.forEach(function(sale){entries.push({kind:'sale',date:sale.soldAt,row:sale});});
    ...
    const rowHtml=visible.length?visible.map(function(entry){
      if(entry.kind==='income'){
        ...
        return '<tr><td>'+escapeHtml(dateTimeText(income.occurredAt))+'</td><td><b>'+escapeHtml(type)+'</b><br><small>'+escapeHtml(income.incomeNo||income.id)+'</small></td><td>'+escapeHtml(income.customerName||'門市散客')+'</td><td>'+escapeHtml(content)+'</td><td>'+statusTag(status,paymentStatus==='paid'?'green':'yellow')+'</td><td class="num"><b>'+money(income.amount)+'</b></td><td><button class="ops-button small ghost" data-action="income-edit" data-id="'+attr(income.id)+'">修改</button></td></tr>';
      }
      const sale=entry.row,internalUse=sale.saleType==='internalUse',items=(sale.items||[]),names=items.slice(0,2).map(function(item){return clean(item.name)||clean(item.sku)||'商品';}),itemText=names.join('、')+(items.length>2?' 等 '+formatNumber(items.length)+' 項':''),paymentStatus=clean(sale.paymentStatus)||'paid',status=internalUse?'已扣庫存':paymentStatus==='paid'?'已收清':paymentStatus==='partial'?'部分收款':'未收款',ownerText=internalUse?(sale.usageReason||'內部耗用／報廢'):(sale.customerName||'門市散客'),type=internalUse?'內部耗用／報廢':'商品銷售',amountHtml='<b>'+money(sale.total)+'</b>'+(internalUse?'<br><small>成本 '+money(sale.costTotal)+'</small>':'');
      return '<tr><td>'+escapeHtml(dateTimeText(sale.soldAt))+'</td><td><b>'+escapeHtml(type)+'</b><br><small>'+escapeHtml(sale.saleNo||sale.id)+'</small></td><td>'+escapeHtml(ownerText)+'</td><td>'+escapeHtml(itemText||'—')+(sale.returnStatus?'<br><small>已退回／處理中</small>':'')+'</td><td>'+statusTag(status,internalUse?'yellow':paymentStatus==='paid'?'green':'yellow')+'</td><td class="num">'+amountHtml+'</td><td><button class="ops-button small ghost" data-action="sale-edit" data-id="'+attr(sale.id)+'">修改</button></td></tr>';
    }).join(''):'';
    const table=rowHtml?'<div class="ops-table-wrap"><table class="ops-table ops-v8-sales-history-table"><thead><tr><th>時間</th><th>類型／單號</th><th>客戶／用途</th><th>內容</th><th>收款／狀態</th><th class="num">金額／成本</th><th></th></tr></thead><tbody>'+rowHtml+'</tbody></table></div>':emptyHtml('找不到符合條件的紀錄',expanded?'請調整搜尋文字或日期區間。':'完成商品銷售、維修、其他收入或內部耗用後會顯示最近紀錄。');
    return '<section class="ops-card ops-v8-sales-history"><div class="ops-v8-section-head"><div><h2>最近銷售、收入與耗用紀錄</h2><p>'+(expanded?'可使用日期與關鍵字查詢完整紀錄':'預設顯示最近 8 筆；每一筆都可按「修改」')+'</p></div><button class="ops-button small ghost" data-action="sales-history-toggle">'+(expanded?'收合紀錄':'展開全部紀錄')+'</button></div>'+toolbar+table+'</section>';
  }
  ...
  function renderSalesV7(){
    ...
    const modes='<div class="ops-sales-modes ops-v8-sales-modes"><button data-action="sales-mode" data-mode="product" class="'+(state.salesMode==='product'?'active':'')+'">商品銷售</button><button data-action="sales-mode" data-mode="repair" class="'+(state.salesMode==='repair'?'active':'')+'">維修收入</button><button data-action="sales-mode" data-mode="other" class="'+(state.salesMode==='other'?'active':'')+'">其他收入</button><button data-action="sales-mode" data-mode="usage" class="'+(usageMode?'active':'')+'">內部耗用／報廢</button></div>';
...
function ensureSalesClock(){
  ...
  function openSaleEdit(id){
    const sale=state.sales.find(function(x){return x.id===id;});if(!sale)return;if(sale.saleType==='internalUse'){
