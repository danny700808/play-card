(function(global){
  ...
  const VERSION = '2026.07.11-v5.3-sale-returns-points';
  const DASHBOARD_CACHE_KEY = 'youzi_ops_dashboard_member_full_v3';
  ...
  function renderOverviewV6(){
    ...
    const tabs='<div class="ops-range-tabs"><button class="'+(state.overviewRange==='today'?'active':'')+'" data-action="overview-range" data-range="today">今天</button><button class="'+(state.overviewRange==='month'?'active':'')+'" data-action="overview-range" data-range="month">本月</button><button class="'+(state.overviewRange==='year'?'active':'')+'" data-action="overview-range" data-range="year">今年</button><button class="'+(state.overviewRange==='custom'?'active':'')+'" data-action="overview-range" data-range="custom">自訂區間</button></div>';
    const custom=state.overviewRange==='custom'?'<div class="ops-range-custom"><label>開始日期<input class="ops-input" id="overviewFrom" type="date" value="'+attr(state.overviewFrom)+'"></label><label>結束日期<input class="ops-input" id="overviewTo" type="date" value="'+attr(state.overviewTo)+'"></label></div>':'';
    return tabs+custom+'<div class="ops-kpi-grid ops-today-kpis">'+kpi('商品銷售',money(productRevenue),'','＄')+kpi('維修收入',money(repairRevenue),'','修')+kpi('其他收入',money(otherRevenue),'','＋')+kpi('租賃收益',money(rentalRevenue),'','租')+kpi('總收入',money(revenue),'','合')+kpi('商品成本',money(cost),'','成本')+kpi(bounds.label+'賺多少',money(profit),'','↗')+kpi('賣出數量',formatNumber(qty),'','件')+'</div>';
  }
  ...
  function renderTodayInvoices(){
    const term=lower(state.saleInvoiceSearch),rows=state.sales.filter(function(x){const customer=state.customers.find(function(row){return row.id===x.customerId;});return !term||lower([x.saleNo,x.customerName,x.paymentMethod,customer&&customer.phone].concat(x.items.map(function(i){return i.name+' '+i.sku;})).join(' ')).includes(term);}).sort(function(a,b){return (dateFrom(b.soldAt)||0)-(dateFrom(a.soldAt)||0);}).slice(0,80);
    return '<section class="ops-card ops-invoice-list"><div class="ops-card-head"><h2>銷售紀錄</h2></div><input class="ops-input" id="saleInvoiceSearch" placeholder="搜尋單號、會員、電話或商品" value="'+attr(state.saleInvoiceSearch)+'"><div class="ops-invoice-rows">'+(rows.length?rows.map(function(x){return '<button type="button" data-action="sale-edit" data-id="'+attr(x.id)+'"><span>'+escapeHtml(dateText(x.soldAt))+'</span><b>'+escapeHtml(x.saleNo)+'</b><em>'+escapeHtml(x.customerName||'門市散客')+(x.returnStatus?'・已退貨':'')+'</em><strong>'+money(x.total)+'</strong><i>修改</i></button>';}).join(''):'<div class="ops-empty"><strong>尚無單據</strong></div>')+'</div></section>';
  }
  ...
  async function saveCheckoutV4(form){
    ...
    await writeAudit('完成現場銷售','storeSale',saleRef.id,saleNo);state.cart=[];state.posSearch='';state.selectedCustomerId='';state.posCustomerMode='walkin';state.posMemberSearch='';state.checkoutPaymentMethod='現金';state.checkoutPaymentStatus='paid';state.checkoutDiscount=0;state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;state.checkoutReceived='';toast('現場銷售完成',saleNo,'success');await loadAll(true);
  }
  ...
  function exportBackup(){
    const payload={exportedAt:new Date().toISOString(),version:VERSION,projectId:(global.APP_CONFIG&&APP_CONFIG.FIREBASE_CONFIG&&APP_CONFIG.FIREBASE_CONFIG.projectId)||'',onlineSource:state.onlineSource,data:{internalProducts:state.internalProducts,sales:state.sales,salesReturns:state.salesReturns,incomes:state.incomes,customers:state.customers,pointTransactions:state.pointTransactions,receivables:state.receivables,receivablePayments:state.receivablePayments,membershipSettings:state.membershipSettings,purchases:state.purchases,inventoryTransactions:state.inventory,rentalLedgers:state.rentalLedgers,cases:state.cases,expenses:state.expenses,syncJobs:state.syncJobs,auditLogs:state.audit}};
  ...
  function handleAction(action,el){
    ...
    if(action==='pos-customer-mode'){state.posCustomerMode=el.dataset.mode||'walkin';if(state.posCustomerMode==='walkin'){state.selectedCustomerId='';state.posMemberSearch='';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;}return render();}
    if(action==='pos-member-select'){state.selectedCustomerId=el.dataset.id||'';state.posCustomerMode='member';state.checkoutPaymentStatus='paid';state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;return render();}
    if(action==='pos-choice'){const name=el.dataset.name,value=el.dataset.value;if(name==='paymentMethod')state.checkoutPaymentMethod=value;if(name==='paymentStatus')state.checkoutPaymentStatus=value;if(name==='earnPointsEnabled')state.checkoutEarnPoints=value==='earn';if(name==='paymentStatus'&&value!=='partial')state.checkoutReceived='';return render();}
