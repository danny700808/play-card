(function(global){
  ...
  const VERSION = '2026.08.26-interleaved-description-v1';
...
function renderOverviewV7(){
  ...
  function productListingIntentPolicy(intent){
    ...
    return Object.assign({intent:value,preserveUnmentionedContent:value==='add-variant'||value==='update-existing',neverCreateDuplicate:true},rules[value]);
  }
...
function ensureSalesClock(){
  ...
  function normalizeProductListingCase(raw,p){
    ...
    const listingIntent=normalizeProductListingIntent(source.listingIntent,source,p),listingMode=listingIntent==='add-variant'?'add-variant':'independent',variantGroupEnabled=listingIntent==='create-group',listingTargetScope=normalizeProductListingTargetScope(source.listingTargetScope),listingTargetPlatforms=productListingTargetPlatforms(listingTargetScope);
    const normalized={
  ...
  function productListingModeSectionHtml(p,row){
    ...
    return '<div class="ops-listing-mode-card"><div class="ops-listing-mode-heading">這次要怎麼處理商品</div><div class="ops-listing-mode-options" role="radiogroup" aria-label="商品處理方式">'
      ...
      +choice('add-variant','加入既有商品細項')
      +choice('update-existing','修改既有商品')+'</div>'
      +'<input type="hidden" name="listingMode" value="'+(intent==='add-variant'?'add-variant':'independent')+'"><input type="hidden" name="variantGroupEnabled" value="'+(intent==='create-group'?'1':'0')+'">'
      +'<div class="ops-listing-group-wrapper '+(intent==='create-group'?'':'is-hidden')+'">'+productVariantGroupEditorHtml(p,row)+'</div>'
  ...
  function updateProductListingMode(form){
    if(!form)return;const selected=query('[name="listingIntent"]:checked',form),intent=PRODUCT_LISTING_INTENTS.includes(clean(selected&&selected.value))?clean(selected.value):'create-single',adding=intent==='add-variant',groupEnabled=intent==='create-group',updating=intent==='update-existing',variantFields=query('.ops-listing-variant-fields',form),groupWrapper=query('.ops-listing-group-wrapper',form),mode=query('[name="listingMode"]',form),groupToggle=query('[name="variantGroupEnabled"]',form);
    if(mode)mode.value=adding?'add-variant':'independent';if(groupToggle)groupToggle.value=groupEnabled?'1':'0';if(variantFields)variantFields.classList.toggle('is-hidden',!adding);if(groupWrapper)groupWrapper.classList.toggle('is-hidden',!groupEnabled);
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const sourceSection='<textarea name="sourceProductDescription" hidden>'+escapeHtml(row.sourceProductDescription)+'</textarea><input type="hidden" name="researchInstructions" value="'+attr(row.researchInstructions)+'"><textarea name="referenceUrls" hidden>'+escapeHtml(row.referenceUrls.join('\n'))+'</textarea>'
      +'<div class="ops-listing-media-workspace"><div class="ops-listing-media-actions"><button class="ops-listing-media-action '+(collectorActive?'danger':'primary')+' ops-listing-single-image-controls" type="button" data-action="product-image-collection-toggle">'+(collectorActive?'停止搜圖':'開始搜圖')+'</button><label class="ops-listing-media-action soft ops-listing-single-image-controls"><input id="productReferenceImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden>選擇圖片上傳</label><label class="ops-listing-media-action physical '+(physicalAtLimit?'disabled':'')+'">'+(physicalAtLimit?'實體圖已滿':'<input id="productPhysicalImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden>實體圖')+'</label></div><div class="ops-listing-supplier-shortcuts ops-listing-single-image-controls" aria-label="快速開啟圖片來源網站"><span>可開啟任何商品網頁</span><a href="https://www.taobao.com/" target="_blank" rel="noopener">淘寶網</a><a href="https://www.1688.com/" target="_blank" rel="noopener">1688</a></div><span id="productImageCollectionCount" hidden>目前 <b>'+collectorCount+'</b>／'+PRODUCT_IMAGE_COLLECTION.maxImages+' 張</span><div id="productImageCollectionStatus" class="ops-listing-media-status" aria-live="polite">'+(collectorActive?'<div class="ops-listing-media-message">正在搜圖；切到任何商品網頁即可點圖或框選截圖</div>':'')+'</div><div id="productReferenceImageUploadStatus" aria-live="polite"></div>'
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '[固定圖文編排規則]',
      '本案工作用途：'+(clean(snapshot&&snapshot.workflowPurpose)===PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE?'修改圖片及網路介紹':'標準商品上架')+'。不論本案選擇新增獨立商品、新增多細項商品、加入既有商品細項或修改既有商品，全部必須套用 '+PRODUCT_DESCRIPTION_LAYOUT_VERSION+'，不得因新舊商品或平台不同改用舊版純文字模式。',
...
async function syncPlatformOrdersNow(){const yes=await confirmAction('要求店內電腦立即同步','系統會建立同步請求；店內電腦開機且背景程式正常時，最慢約 20 秒內接收。若同步正在進行，不會重複執行。','建立請求');if(!yes)return;const ref=state.db.collection(COLLECTIONS.platformSyncRequests).doc();await ref.set({requestId:ref.id,status:'pending',requestedAt:serverTimestamp(),requestedBy:userLabel(),source:'operations-hub',version:VERSION});toast('同步請求已建立','店內電腦會在約 20 秒內接收，完成後重新讀取即可查看結果。','success');}
  ...
  function handleAction(action,el){
    ...
    if(action==='product-recent'){
      ...
      state.productRecentOnly=!state.productRecentOnly;state.productSeries='all';state.productFilter='all';state.productSearch='';state.productVisible=PRODUCT_PAGE_SIZE;return renderKeepingViewport();
    }
