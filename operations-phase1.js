(function(global){
  ...
  const VERSION = '2026.08.29-locked-brand-template-v1';
  ...
  const PRODUCT_RICH_CONTENT_STANDARD_VERSION = 'youzi-rich-product-content-v1';
  ...
  const PRODUCT_DESCRIPTION_LAYOUT_VERSION = 'youzi-interleaved-description-v1';
  const PRODUCT_DESCRIPTION_MEDIA_REFRESH_PURPOSE = 'refresh-description-media';
  const PRODUCT_DESCRIPTION_MEDIA_REFRESH_INSTRUCTIONS = '原地修改同一 SKU 與既有平台商品：重新整理並驗證官網、蝦皮、MOMO、酷澎的商品介紹與圖片。所有處理方式固定使用圖文穿插：商品介紹與特色→商品圖一→商品規格→商品圖二→使用建議→商品圖三→其餘商品圖→兩張固定介紹圖→實體商品免責句；橫向商品圖最後一張固定為店址圖。蝦皮必須實際確認文字與全部圖片均已保存；酷澎與 MOMO 可編輯時同步完成；缺圖不得判定完成。';
...
function ensureSalesClock(){
  ...
  function productRichContentStandardHtml(row){
    ...
    return '<section class="ops-callout '+(ready?'green':'yellow')+'"><b>'+escapeHtml(heading)+'</b><br><span>'+escapeHtml(counts+'。'+rule)+'</span><br><span>以精確型號／條碼、官方規格與可信使用資料為依據；找不到就留空，不為湊數而猜測，也不在標題或內文加入店名。</span><br><span>各平台內文最後一句固定為：'+escapeHtml(PRODUCT_PHYSICAL_PRODUCT_DISCLAIMER)+'</span></section>';
  }
  ...
  function requestProductImageCollection(action,payload){
    ...
    return new Promise(function(resolve,reject){
      const sessionId=clean(payload&&payload.sessionId),timer=global.setTimeout(function(){if(!productImageCollectionPending||productImageCollectionPending.sessionId!==sessionId)return;productImageCollectionPending=null;reject(new Error('找不到收圖助手，請先更新到柚子掌櫃助手 0.3.33 並重新整理頁面。'));},3500);
    ...
    if(message.type===PRODUCT_IMAGE_COLLECTION.sessionAck){
      ...
      if(payload.ok===true){
        if(pending.action==='start'&&!productImageCollectionVersionAtLeast(payload.extensionVersion,PRODUCT_IMAGE_COLLECTION.minimumVersion)){
          ...
          pending.reject(new Error('目前收圖助手版本過舊，請更新到 0.3.33，重新載入後再開始搜圖。'));
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const cards=orderedKeys.map(function(key){
      ...
      const helper=key==='shopee'&&row.autofillPayload?'<div class="ops-card-actions"><button class="ops-button primary" type="button" data-action="product-shopee-autofill-open">安全開啟 EasyStore／蝦皮</button><a class="ops-button soft" href="youzi-easystore-shopee-autofill-v0.3.33.zip" download>下載／更新助手 0.3.33</a></div><small>'+(shopeeHomeLogisticsNeedsManualConfirmation?'會先套用 EasyStore 進階圖文介紹，再帶入分類、品牌與商品屬性；一般宅配的實際物流仍須確認。商品身分依本案已凍結的處理方式與中央平台 ID 執行，不會自行改成新增或修改。':'會先套用 EasyStore 進階圖文介紹；商品身分依本案已凍結的處理方式與中央平台 ID 執行，不先搜尋平台目錄，也不會自行改成新增或修改。')+'</small>':'';
  ...
  async function openShopeeAutofillHelper(){
    ...
    else toast('助手沒有收到商品資料',clean(result&&result.error)||'請安裝或更新到助手 0.3.33，重新整理營運中心後，再按一次「安全開啟 EasyStore／蝦皮」。','warning');
    return result;
  }
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '詳細介紹固定依序為：商品介紹與商品特色→第 1 張商品圖→商品規格→第 2 張商品圖→使用方式／適用情境與使用建議→第 3 張商品圖→其餘商品圖→product-listing-description-promo-1.jpg→product-listing-description-promo-2.jpg→固定實體商品免責句。兩張固定介紹圖必須是最後兩張圖片；免責句是最後一個文字區塊。商品圖不足三張時先從本案已保存來源、官網既有圖或精確商品來源補齊並完成繁體化，禁止發布缺圖版本，也不得把店址圖或兩張固定介紹圖拿來補三張商品圖。',
      ...
      '同一輪圖片處理只產生本次目標通路需要的首圖版型，不得替未選通路多做圖片，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 固定使用 product-listing-brand-template-portrait.png，尺寸 750×1000 px、3:4；包含蝦皮時 brandedHero 固定使用 product-listing-brand-template-square.png，尺寸 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1。兩張品牌母版版本固定為 '+PRODUCT_BRAND_TEMPLATE_CONTRACT.version+'，必須保存 templateVersion、templateAssetSha256 與 templateComposition；缺少或雜湊不符就在進平台前停止。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      ...
      '[商品內容]',
      '商品標題依「品牌＋型號＋商品種類＋重要規格或材質」組成；可靠的品牌與型號優先，共同標題只寫共同事實，差異寫細項名稱。標題、內文、圖卡都不得加入「柚子樂器」或任何未指定店名、導外訊息；只有上述三張管理者指定固定圖片可保留原有門市與服務資訊。先以精確型號或條碼確認商品身分，再依序參考製造商官方規格／說明書、台灣代理商或經銷商資料，及多個可信的使用指南與真實使用經驗；不得把單一匿名評論當作規格事實。每個特色、使用重點與規格屬性都必須在 fieldEvidence 指向來源；搜尋不到就留空。每個案件的 productDescriptionStatus 不是 ready，或 productDescription 只有通用提醒、商品編號、免責文字時，一律視為內容尚未完成；Codex 必須先整理並寫回每一份案件，後端重讀確認後才可建立 preparedSnapshot。新商品第一次上架前必須完成；既有商品以原地升級方式逐筆修正，保留同一 SKU 與平台商品編號。正式內容包含「商品特色」「使用方式／適用情境」「商品規格」三段：目標 10 點不重複、具體且可驗證特色，目標 8 點有來源的使用方式／使用心得，並完整列出所有找得到的平台屬性、尺寸、材質、段數、重量、內容物與細項差異；進入平台時再依固定圖文編排順序重排為介紹與特色、規格、使用建議。可驗證資料不足就少寫，不得猜測、湊數或用通用備援文案代替。保固只填平台保固欄，不寫進介紹。本次 '+targetNamesText+'沿用同一份已完成介紹並轉成各自格式；所有文字、圖片與圖文區塊都完成後，再把「商品圖片與規格僅供參考，實際內容以收到的實體商品為準。」作為唯一且固定的最後一句，不得在它後面再放圖片、文字或店鋪宣傳。',
      '蝦皮的進階商品描述必須在進入平台前完整準備：EasyStore body_html、介紹圖片、圖片順序、固定最後免責句與內容指紋全部寫入同一份 preparedSnapshot。進入 EasyStore 官方蝦皮通路頁後只做一次輕量功能檢查；帳號有「進階商品描述」時立即開啟並按「使用 EasyStore 的產品描述」，把官網文字與介紹圖片一起帶入。按鈕成功不等於完成：必須核對編輯器已有文字及全部預期圖片，而免責句仍是最後一個區塊；若只帶入文字，立即把同一 preparedSnapshot 的缺漏完成圖補入免責句之前並再核對一次，仍缺圖才停止蝦皮，不得誤報完成或發布純文字版本。此時禁止重新分析、改寫文案或重排圖片；只有功能不存在、欄位版型改變或平台明確回報圖片／描述失敗時才停止處理該例外。',
