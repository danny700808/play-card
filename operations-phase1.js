...
function ensureSalesClock(){
  ...
  function renderProductVariantGroupItems(form,items){
    ...
    renderProductVariantGroupSearch(form);refreshProductVariantImageProcessingQueue(form);
  }
  ...
  function refreshProductVariantImagePickers(form){
    ...
    queryAll('.ops-listing-variant-item[data-product-id]',form).forEach(function(card){
      ...
      if(picker)picker.innerHTML=sourceImages.length?productVariantImagePickerOptionsHtml(id,selected,sourceImages,card.dataset.variantRole):'';
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '按下「帶入這個 Codex 對話」已同時授權四通路的建立、修改與最後正式發布。分類、物流、保固、細項及平台送出都應依 preparedSnapshot 自動完成；不得再詢問「確認上架／確認提交／套用細項」等例行確認。各站獨立核對，四站全部 verified 才完成整筆工作。',
