...
function renderOverviewV7(){
  ...
  const activeRentals=state.rentals.filter(rentalIsActive),dueRentals=activeRentals.filter(function(rental){const days=daysUntil(rental.endDate);return days!==null&&days>=0&&days<=30;});
  ...
  const syncAnomalies=inventorySyncAnomalies(),syncAnomalyCount=syncAnomalies.length;
  ...
  const rangeHtml='<div class="ops-v8-overview-range">'+tabs+monthPicker+custom+'<span>'+escapeHtml(bounds.label)+'資料</span></div>';
  const heroHtml='<section class="ops-card ops-v8-overview-hero"><div class="ops-v8-hero-primary"><span>全部通路營運結餘</span><strong class="'+(allBalance<0?'negative':'')+'">'+money(allBalance)+'</strong><small>門市、網路、租賃與補習班收入－已知商品與老師成本</small></div><div class="ops-v8-hero-secondary">'+summaryBox('目前營運收入',money(allCash))+summaryBox('已知直接成本',money(knownDirectCost))+summaryBox('現金流狀態',escapeHtml(cashStatus),openReceivables.length?'warning':'success')+'<small class="ops-v8-cash-note">'+escapeHtml(cashSub)+'</small></div></section>';

  const storeHtml='<section class="ops-card ops-v8-channel-card ops-v8-channel-store"><div class="ops-v8-channel-accent"></div><div class="ops-v8-channel-head"><div><h2>門市營運</h2><p>現場商品、維修與其他收入</p></div><button class="ops-button small soft" data-nav="sales">前往銷售</button></div><div class="ops-v8-channel-summary">'+summaryBox('門市實收',money(storeCash))+summaryBox('門市結餘',money(storeBalance),storeBalance<0?'warning':'success')+'</div><div class="ops-v8-metric-list">'+metricRow('商品銷售',money(productRevenue))+metricRow('維修／其他',money(repairRevenue+otherRevenue))+metricRow('商品成本',money(productCost))+metricRow('退貨退款',money(returnRefund))+'</div></section>';
  ...
  const rentalHtml='<section class="ops-card ops-v8-channel-card ops-v8-channel-rental"><div class="ops-v8-channel-accent"></div><div class="ops-v8-channel-head"><div><h2>租賃營運</h2><p>正式合約送出即認列租金與運費</p></div><button class="ops-button small ghost" data-nav="rentals">查看租賃</button></div><div class="ops-v8-channel-summary">'+summaryBox('租賃收入',money(rentalRevenue))+summaryBox('成立合約',formatNumber(rentals.length)+' 件','success')+'</div><div class="ops-v8-metric-list">'+metricRow('租金＋運費',money(rentalRevenue))+metricRow('目前租用中',formatNumber(activeRentals.length)+' 件')+metricRow('30 日內到期',formatNumber(dueRentals.length)+' 件')+metricRow('押金', '不列入營收')+'</div></section>';
  ...
  const alerts=[];
  if(syncAnomalyCount){const counts={};syncAnomalies.forEach(function(row){counts[row.platform]=Number(counts[row.platform]||0)+1;});const detail=Object.keys(counts).sort().map(function(name){return name+' '+counts[name]+' 項';}).join('、');alerts.push('<button type="button" class="ops-v8-attention-row" data-action="overview-sync-errors"><span class="ops-v8-attention-icon danger">同</span><span><b>庫存同步異常</b><small>'+formatNumber(syncAnomalyCount)+' 項'+(detail?'：'+escapeHtml(detail):'')+'</small></span><em>查看明細</em></button>');}
  if(pendingPlatform){const returnCount=pendingPlatformRows.filter(function(row){return row.processingStatus==='manual-return-review';}).length,matchCount=pendingPlatformRows.filter(function(row){return ['missing-sku','unmatched-sku','duplicate-sku'].includes(row.processingStatus);}).length,otherCount=Math.max(0,pendingPlatform-returnCount-matchCount);alerts.push('<button type="button" class="ops-v8-attention-row" data-action="overview-order-errors"><span class="ops-v8-attention-icon">網</span><span><b>平台訂單待確認</b><small>'+formatNumber(pendingPlatform)+' 筆：SKU '+formatNumber(matchCount)+'、退貨 '+formatNumber(returnCount)+'、其他 '+formatNumber(otherCount)+'</small></span><em>查看</em></button>');}
  if(openReceivables.length)alerts.push('<button type="button" class="ops-v8-attention-row" data-nav="receivables"><span class="ops-v8-attention-icon danger">帳</span><span><b>門市未收款</b><small>'+formatNumber(openReceivables.length)+' 筆，共 '+money(outstanding)+'</small></span><em>查看</em></button>');
  ...
  const bottomHtml='<div class="ops-v8-overview-bottom"><section class="ops-card"><div class="ops-v8-section-head"><div><h2>需要注意</h2><p>同步異常、訂單異常、未收款與低庫存會集中顯示</p></div>'+(attentionKinds?'<span class="ops-tag yellow">'+formatNumber(attentionKinds)+' 類</span>':'<span class="ops-tag green">正常</span>')+'</div><div class="ops-v8-attention-list">'+attentionHtml+'</div></section><section class="ops-card"><div class="ops-v8-section-head"><div><h2>快速入口</h2><p>直接進入最常使用的作業</p></div></div><div class="ops-v8-quick-grid"><button class="ops-button soft" data-nav="sales">現場銷售</button><button class="ops-button ghost" data-nav="sync">平台訂單</button><button class="ops-button ghost" data-nav="rentals">租賃營運</button><button class="ops-button ghost" data-action="injiaoyun-import">補習班同步</button></div></section></div>';
  ...
  function renderProducts(){
    ...
    return '<section class="ops-card ops-product-section"><div class="ops-product-title-row"><div class="ops-product-title-group"><h2>商品庫存</h2><div class="ops-product-title-stat">庫存成本總額：<b>'+money(inventoryValue)+'</b></div><button type="button" class="ops-product-low-button '+(lowActive?'active':'')+'" data-action="product-low-stock">低於安全庫存：<b>'+formatNumber(low)+' 項</b></button></div><div class="ops-card-actions"><button class="ops-button primary" data-action="product-new">新增商品</button><button class="ops-button soft" data-action="sync-easystore-api">同步圖片</button><button class="ops-button soft" data-action="open-import">'+(central?'更新商品資料':'匯入商品資料')+'</button></div></div>'+productSeriesTabs()+'<div class="ops-product-toolbar"><input class="ops-input" id="productSearch" placeholder="搜尋商品名稱或 SKU" value="'+attr(state.productSearch)+'"><select class="ops-select" id="productFilter"><option value="all">全部商品狀態</option><option value="unmatched">尚未配對</option><option value="no-image">沒有圖片</option><option value="missing-cost">缺少成本</option><option value="low">低庫存／缺貨</option><option value="in-stock">有庫存</option><option value="negative">負庫存</option></select><select class="ops-select" id="productSort"><option value="image">以圖片優先</option><option value="name">依名稱</option><option value="sku">依商品編號</option><option value="stock">依庫存</option><option value="cost">依成本</option><option value="price">依售價</option></select></div>'+editorHtml+(visible.length?'<div class="ops-products-grid">'+visible.map(productCard).join('')+'</div>'+(visible.length<rows.length?'<div class="ops-pagination"><button class="ops-button ghost" data-action="load-more-products">顯示更多</button></div>':''):emptyHtml(central?'找不到商品':'尚未建立商品',central?'請調整搜尋條件。':'請先新增商品或匯入商品資料。','<button class="ops-button primary" data-action="product-new">新增商品</button>'))+'</section>'+previewHtml;
  }
...
function posCustomerBarV5(){
  const c=selectedCustomer(),term=lower(state.posMemberSearch),members=state.customers.filter(function(x){return x.enabled&&x.customerType==='member'&&(!term||lower([x.name,x.phone,x.memberNo].join(' ')).includes(term));}).slice(0,12);
  let popover='';
  if(state.posCustomerMode==='member'&&state.posMemberPickerOpen){
    const resultHtml=term?(members.length?members.map(function(x){return '<button type="button" class="'+(x.id===state.selectedCustomerId?'active':'')+'" data-action="pos-member-select" data-id="'+attr(x.id)+'"><b>'+escapeHtml(x.name)+'</b><span>'+escapeHtml(x.phone||x.memberNo||'')+'</span><strong>'+formatNumber(x.pointBalance)+' 點</strong></button>';}).join(''):'<div class="ops-member-empty">找不到會員</div>'):'<div class="ops-member-empty">輸入姓名、電話或會員編號</div>';
  ...
  function renderInlineCheckout(){
    ...
    const subtotal=sum(state.cart,function(x){return x.qty*x.unitPrice;}),c=selectedCustomer(),walkin=!c||state.posCustomerMode==='walkin',settings=membershipRuleForDate(new Date()),manualDiscount=Math.max(0,Number(state.checkoutDiscount||0)),maxPoints=walkin?0:maxRedeemablePoints(c,Math.max(0,subtotal-manualDiscount));
    if(!walkin&&settings.redemptionMode==='auto'&&!state.checkoutPointsTouched)state.checkoutPoints=maxPoints;
    const points=Math.min(maxPoints,Math.max(0,Math.floor(Number(state.checkoutPoints||0)))),pointValue=pointDiscount(points),total=Math.max(0,subtotal-manualDiscount-pointValue),status=walkin?'paid':state.checkoutPaymentStatus,member=!!(c&&c.customerType==='member'),earnEnabled=member&&state.checkoutEarnPoints!==false;
    ...
    state.checkoutPoints=points;if(walkin){state.checkoutPaymentStatus='paid';state.checkoutReceived='';state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;}
    const memberSummary=member?'<div class="ops-member-payment-summary"><div><span>會員</span><b>'+escapeHtml(c.name)+' '+escapeHtml(c.memberNo||'')+'</b></div><div><span>可用點數</span><b>'+formatNumber(c.pointBalance)+' 點</b></div><div><span>本次折抵</span><b id="inlinePointDiscount">'+formatNumber(points)+' 點／'+money(pointValue)+'</b></div><div><span>折抵後剩餘</span><b id="inlinePointRemaining">'+formatNumber(remaining)+' 點</b></div><div><span>本次累積</span><b id="inlinePointsEarned">'+(earnEnabled?formatNumber(earned)+' 點':'不累積')+'</b></div></div><input type="hidden" name="earnPointsEnabled" value="'+(earnEnabled?'true':'false')+'"><div class="ops-checkout-block"><label>本次點數</label>'+posChoiceButtons('earnPointsEnabled',earnEnabled?'earn':'none',[{value:'earn',label:'累積點數'},{value:'none',label:'不累積點數'}])+'</div>':'<input type="hidden" name="earnPointsEnabled" value="false">';
    return '<form id="checkoutFormInline" class="ops-inline-checkout"><h3>結帳</h3><input type="hidden" name="customerId" value="'+attr(walkin?'':c.id)+'"><input type="hidden" name="paymentMethod" value="'+attr(state.checkoutPaymentMethod)+'"><input type="hidden" name="paymentStatus" value="'+attr(status)+'">'+memberSummary+'<div class="ops-checkout-block"><label>付款方式</label>'+posChoiceButtons('paymentMethod',state.checkoutPaymentMethod,[{value:'現金',label:'現金'},{value:'信用卡',label:'信用卡'},{value:'轉帳',label:'轉帳'}])+'</div><div class="ops-checkout-block"><label>收款狀態</label>'+posChoiceButtons('paymentStatus',status,walkin?[{value:'paid',label:'已收清'}]:[{value:'paid',label:'已收清'},{value:'partial',label:'部分收款'},{value:'unpaid',label:'未收款'}])+'</div>'+(status==='partial'?'<div class="ops-field"><label>本次已收</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="receivedAmount" value="'+attr(state.checkoutReceived)+'"></div>':'')+'<div class="ops-checkout-fields"><div class="ops-field"><label>額外折扣</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="discount" value="'+attr(state.checkoutDiscount)+'"></div>'+(member&&settings.redemptionMode!=='earn-only'?'<div class="ops-field"><label>點數折抵</label><input class="ops-input ops-number-clean" type="number" inputmode="numeric" min="0" max="'+maxPoints+'" step="'+Math.max(1,Math.floor(Number(settings.redeemPoints||1)))+'" name="pointsToRedeem" value="'+points+'"></div>':'<input type="hidden" name="pointsToRedeem" value="0">')+'</div><div class="ops-summary-list"><div class="ops-summary-line"><span>商品金額</span><b>'+money(subtotal)+'</b></div><div class="ops-summary-line"><span>折扣</span><b id="inlineDiscountTotal">'+money(manualDiscount+pointValue)+'</b></div><div class="ops-summary-line total"><span>應收金額</span><b id="inlineCheckoutTotal">'+money(total)+'</b></div></div><button class="ops-button primary wide" type="submit">確認結帳</button></form>';
  }
  function renderDirectIncomeV5(mode){
    const repair=mode==='repair',c=selectedCustomer(),walkin=!c||state.posCustomerMode==='walkin',status=walkin?'paid':state.checkoutPaymentStatus;if(walkin)state.checkoutPaymentStatus='paid';return '<section class="ops-card ops-direct-income"><form id="quickIncomeForm"><input type="hidden" name="occurredAt" value="'+inputDateTime(new Date())+'"><input type="hidden" name="customerId" value="'+attr(walkin?'':c.id)+'"><input type="hidden" name="paymentMethod" value="'+attr(state.checkoutPaymentMethod)+'"><input type="hidden" name="paymentStatus" value="'+attr(status)+'"><div class="ops-form-grid"><div class="ops-field full"><label>'+(repair?'維修項目':'收入項目')+'</label>'+(repair?'<input class="ops-input" name="category" value="維修收入" required>':'<div class="ops-choice-grid two"><button type="button" class="'+(state.incomeCategory==='未登錄商品'?'active':'')+'" data-action="income-category" data-value="未登錄商品">未登錄商品</button><button type="button" class="'+(state.incomeCategory==='其他收入'?'active':'')+'" data-action="income-category" data-value="其他收入">其他收入</button></div><input type="hidden" id="incomeCategory" name="category" value="'+attr(state.incomeCategory)+'">')+'</div><div class="ops-field full"><label>金額</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="amount" value="'+attr(state.directIncomeAmount)+'" required></div></div><div class="ops-checkout-block"><label>付款方式</label>'+posChoiceButtons('paymentMethod',state.checkoutPaymentMethod,[{value:'現金',label:'現金'},{value:'信用卡',label:'信用卡'},{value:'轉帳',label:'轉帳'}])+'</div><div class="ops-checkout-block"><label>收款狀態</label>'+posChoiceButtons('paymentStatus',status,walkin?[{value:'paid',label:'已收清'}]:[{value:'paid',label:'已收清'},{value:'partial',label:'部分收款'},{value:'unpaid',label:'未收款'}])+'</div>'+(status==='partial'?'<div class="ops-field"><label>本次已收</label><input class="ops-input ops-number-clean" type="number" inputmode="decimal" min="0" step="1" name="receivedAmount" value="'+attr(state.checkoutReceived)+'"></div>':'')+'<button class="ops-button primary wide" type="submit">確認</button></form></section>';
  }
...
function renderSalesV5(){
  ...
  function renderSalesHistoryV7(){
    const term=lower(state.saleInvoiceSearch).trim(),from=state.saleInvoiceFrom?new Date(state.saleInvoiceFrom+'T00:00:00'):null,to=state.saleInvoiceTo?new Date(state.saleInvoiceTo+'T23:59:59'):null,expanded=state.salesHistoryExpanded===true;
    const rows=state.sales.filter(function(sale){
      const soldAt=dateFrom(sale.soldAt),customer=state.customers.find(function(row){return row.id===sale.customerId;});
      if(from&&(!soldAt||soldAt<from))return false;
      if(to&&(!soldAt||soldAt>to))return false;
      return !term||lower([sale.saleNo,sale.customerName,sale.paymentMethod,sale.saleType,sale.usageReason,sale.usageNote,customer&&customer.phone].concat((sale.items||[]).map(function(item){return item.name+' '+item.sku;})).join(' ')).includes(term);
    }).sort(function(a,b){return (dateFrom(b.soldAt)||0)-(dateFrom(a.soldAt)||0);});
    const visible=rows.slice(0,expanded?120:5);
    const toolbar=expanded?'<div class="ops-v8-sales-history-tools"><input class="ops-input grow" id="saleInvoiceSearch" placeholder="搜尋單號、會員、用途、電話或商品" value="'+attr(state.saleInvoiceSearch)+'"><label><span>開始日期</span><input class="ops-input" id="saleInvoiceFrom" type="date" value="'+attr(state.saleInvoiceFrom)+'"></label><label><span>結束日期</span><input class="ops-input" id="saleInvoiceTo" type="date" value="'+attr(state.saleInvoiceTo)+'"></label><button class="ops-button small ghost" data-action="sale-history-reset-range">清除區間</button></div>':'';
    const rowHtml=visible.length?visible.map(function(sale){
      const internalUse=sale.saleType==='internalUse',items=(sale.items||[]),names=items.slice(0,2).map(function(item){return clean(item.name)||clean(item.sku)||'商品';}),itemText=names.join('、')+(items.length>2?' 等 '+formatNumber(items.length)+' 項':'');
      const paymentStatus=clean(sale.paymentStatus)||'paid',status=internalUse?'已扣庫存':paymentStatus==='paid'?'已收清':paymentStatus==='partial'?'部分收款':'未收款',ownerText=internalUse?(sale.usageReason||'內部耗用／報廢'):(sale.customerName||'門市散客');
      const amountHtml=internalUse?'<b>'+money(0)+'</b><br><small>成本 '+money(sale.costTotal)+'</small>':'<b>'+money(sale.total)+'</b>';
      return '<tr><td>'+escapeHtml(dateTimeText(sale.soldAt))+'</td><td><b>'+escapeHtml(sale.saleNo||sale.id)+'</b>'+(internalUse?'<br><small>內部耗用</small>':'')+'</td><td>'+escapeHtml(ownerText)+'</td><td>'+escapeHtml(itemText||'—')+(sale.returnStatus?'<br><small>已退回／處理中</small>':'')+'</td><td>'+statusTag(status,internalUse?'yellow':paymentStatus==='paid'?'green':'yellow')+'</td><td class="num">'+amountHtml+'</td><td><button class="ops-button small ghost" data-action="sale-edit" data-id="'+attr(sale.id)+'">'+(internalUse?'查看':'處理')+'</button></td></tr>';
    }).join(''):'';
    const table=rowHtml?'<div class="ops-table-wrap"><table class="ops-table ops-v8-sales-history-table"><thead><tr><th>時間</th><th>單號</th><th>客戶／用途</th><th>商品</th><th>收款／狀態</th><th class="num">金額／成本</th><th></th></tr></thead><tbody>'+rowHtml+'</tbody></table></div>':emptyHtml('找不到符合條件的銷售紀錄',expanded?'請調整搜尋文字或日期區間。':'完成銷售或內部耗用後會顯示最近紀錄。');
    return '<section class="ops-card ops-v8-sales-history"><div class="ops-v8-section-head"><div><h2>最近銷售與耗用紀錄</h2><p>'+(expanded?'可使用日期與關鍵字查詢完整紀錄':'預設只顯示最近 5 筆，降低畫面干擾')+'</p></div><button class="ops-button small ghost" data-action="sales-history-toggle">'+(expanded?'收合紀錄':'展開全部紀錄')+'</button></div>'+toolbar+table+'</section>';
  }

  function renderStockUsageForm(){
    if(!state.cart.length)return '';
    const reason=state.stockUsageReason||'店內自用',estimatedCost=estimateCartCost(),reasonOptions=['店內自用','消耗品','報廢','其他'];
    ...
    return '<form id="stockUsageForm" class="ops-inline-checkout ops-stock-usage-form"><h3>內部耗用／報廢</h3><div class="ops-callout"><b>這筆不會產生收入。</b><br>商品會依 FIFO 成本扣除庫存，並列為本次帳面損失；下一次同步時會把最新庫存更新到各平台。</div><div class="ops-checkout-block"><label>處理原因</label><div class="ops-choice-grid ops-stock-usage-reasons">'+reasonButtons+'</div><input type="hidden" name="usageReason" value="'+attr(reason)+'"></div><div class="ops-field"><label>備註</label><textarea class="ops-textarea" id="stockUsageNote" name="usageNote" placeholder="例如：門市展示使用、包裝耗材、損壞報廢">'+escapeHtml(state.stockUsageNote)+'</textarea></div><div class="ops-summary-list"><div class="ops-summary-line"><span>帳面收入</span><b>'+money(0)+'</b></div><div class="ops-summary-line"><span>預估庫存成本</span><b>'+money(estimatedCost)+'</b></div><div class="ops-summary-line total"><span>預估帳面損失</span><b>'+money(0-estimatedCost)+'</b></div></div><button class="ops-button primary wide" type="submit">確認扣除庫存</button></form>';
  }

  function renderSalesV7(){
    const products=state.catalog.filter(function(product){return product.initialized&&product.status!=='inactive';}),term=lower(state.posSearch).trim(),choices=term?products.filter(function(product){return lower([product.originalName,product.onlineName,product.sku,product.barcode,product.brand,product.category].join(' ')).includes(term);}).slice(0,30):[],usageMode=state.salesMode==='usage',cartSubtotal=usageMode?0:sum(state.cart,function(item){return item.qty*item.unitPrice;}),todaySales=todayRows(state.sales,function(sale){return sale.soldAt;}),regularSales=todaySales.filter(function(sale){return sale.saleType!=='internalUse';}),usageSales=todaySales.filter(function(sale){return sale.saleType==='internalUse';}),todayIncome=todayRows(state.incomes,function(income){return income.occurredAt;}),repairIncome=todayIncome.filter(function(income){return income.category==='維修收入';}),otherIncome=todayIncome.filter(function(income){return income.category!=='維修收入';});
    let main='';
    if(state.salesMode==='product'||usageMode){
      ...
      const cartHtml=state.cart.length?state.cart.map(function(item,index){const product=catalogById(item.productId);return '<div class="ops-cart-row"><div><b>'+escapeHtml(item.name)+'</b><small>編號 '+escapeHtml(item.sku||'')+'・庫存 '+formatNumber(product?product.currentStock:item.currentStock)+'</small></div><input class="ops-number-clean" aria-label="數量" type="number" min="1" step="1" value="'+item.qty+'" data-cart-qty="'+index+'">'+(usageMode?'<span class="ops-stock-usage-zero">0 元</span>':'<input class="ops-number-clean" aria-label="售價" type="number" min="0" step="1" value="'+item.unitPrice+'" data-cart-price="'+index+'">')+'<button class="ops-icon-button" data-action="cart-remove" data-index="'+index+'">×</button></div>';}).join(''):'<div class="ops-v8-cart-empty"><b>尚未選商品</b><span>從左側搜尋結果點選商品後，會加入本次'+(usageMode?'耗用／報廢':'銷售')+'。</span></div>';
      main='<div class="ops-v8-sales-workspace"><section class="ops-card ops-v8-sales-search-card"><div class="ops-v8-section-head"><div><h2>商品搜尋</h2><p>輸入商品編號或名稱，再點選結果加入'+(usageMode?'耗用清單':'銷售')+'</p></div><span class="ops-tag '+(usageMode?'yellow':'green')+'">'+(usageMode?'扣庫存':'主要操作')+'</span></div><div class="ops-toolbar ops-v8-sales-searchbar"><input class="ops-input grow ops-pos-search" id="posSearch" placeholder="商品編號／名稱" value="'+attr(state.posSearch)+'"><button class="ops-button ghost" data-action="pos-clear-search">清除</button></div><div class="ops-v8-sales-search-grid">'+posNumberPadHtml()+'<div class="ops-pos-products">'+productHtml+'</div></div></section><section class="ops-card ops-v8-sales-cart-card"><div class="ops-v8-section-head"><div><h2>'+(usageMode?'本次內部耗用／報廢':'本次銷售')+'</h2><p>'+(usageMode?'價格固定為 0 元，只扣庫存並認列成本':'商品、價格與收款集中在同一區')+'</p></div><button class="ops-button small ghost" data-action="cart-clear">清空</button></div><div class="ops-cart">'+cartHtml+'</div>'+(state.cart.length?'<div class="ops-summary-line total ops-v8-cart-subtotal"><span>'+(usageMode?'帳面收入':'商品金額')+'</span><b id="cartSubtotal">'+money(cartSubtotal)+'</b></div>':'')+(usageMode?renderStockUsageForm():renderInlineCheckout())+'</section></div>';
    }else{
      main='<div class="ops-v8-sales-income-mode">'+renderDirectIncomeV5(state.salesMode)+'</div>';
    }
    const productRevenue=sum(regularSales,function(sale){return sale.total;}),usageCost=sum(usageSales,function(sale){return sale.costTotal;}),repairRevenue=sum(repairIncome,function(income){return income.amount;}),otherRevenue=sum(otherIncome,function(income){return income.amount;}),totalRevenue=productRevenue+repairRevenue+otherRevenue;
    const customerRow=usageMode?'<section class="ops-card ops-stock-usage-banner"><div><h2>內部耗用／報廢</h2><p>適用於店內自用、消耗品、展示使用或報廢。這類紀錄不連結客戶，也不會累積會員點數。</p></div><div class="ops-v8-sales-clock"><span>現在時間</span><b id="opsSalesClock">'+formatSalesClock(new Date())+'</b></div></section>':'<div class="ops-v8-sales-customer-row">'+posCustomerBarV5()+'<div class="ops-v8-sales-clock"><span>現在時間</span><b id="opsSalesClock">'+formatSalesClock(new Date())+'</b></div></div>';
    const modes='<div class="ops-sales-modes ops-v8-sales-modes"><button data-action="sales-mode" data-mode="product" class="'+(state.salesMode==='product'?'active':'')+'">商品銷售</button><button data-action="sales-mode" data-mode="usage" class="'+(usageMode?'active':'')+'">內部耗用／報廢</button><button data-action="sales-mode" data-mode="repair" class="'+(state.salesMode==='repair'?'active':'')+'">維修收入</button><button data-action="sales-mode" data-mode="other" class="'+(state.salesMode==='other'?'active':'')+'">其他收入</button></div>';
    const totals='<section class="ops-v8-sales-totals"><div class="ops-kpi-grid">'+kpi('今日商品銷售',money(productRevenue),regularSales.length+' 筆','＄')+kpi('內部耗用成本',money(usageCost),usageSales.length+' 筆','耗')+kpi('維修收入',money(repairRevenue),repairIncome.length+' 筆','修')+kpi('其他收入',money(otherRevenue),otherIncome.length+' 筆','＋')+kpi('今日總收入',money(totalRevenue),'不含內部耗用','合')+'</div></section>';
    return '<div class="ops-v8-sales-page">'+customerRow+modes+main+totals+renderSalesHistoryV7()+'</div>';
  }
  ...
  async function saveCustomerV4(form){
    const id=clean(form.dataset.id),data=new FormData(form),type=clean(data.get('customerType')),phone=clean(data.get('phone')).replace(/[^0-9]/g,''),payload={name:clean(data.get('name')),phone:phone,email:clean(data.get('email')),customerType:type,memberNo:clean(data.get('memberNo'))||(type==='member'?uid('MEM'):''),pricingTier:clean(data.get('pricingTier')),creditLimit:numberOrNull(data.get('creditLimit'))||0,note:clean(data.get('note')),enabled:true,updatedAt:serverTimestamp(),updatedBy:userLabel(),version:VERSION};if(!payload.name)throw new Error('請填寫姓名或名稱');if(type==='member'&&!/^09\d{8}$/.test(phone))throw new Error('請輸入正確的行動電話');const duplicatePhone=state.customers.find(function(x){return x.id!==id&&clean(x.phone).replace(/[^0-9]/g,'')===phone;});if(phone&&duplicatePhone)throw new Error('這個行動電話已經是會員：'+duplicatePhone.name);let ref;if(id){ref=state.db.collection(COLLECTIONS.customers).doc(id);await ref.set(payload,{merge:true});}else{payload.pointBalance=0;payload.createdAt=serverTimestamp();payload.createdBy=userLabel();ref=await state.db.collection(COLLECTIONS.customers).add(payload);}if(form.dataset.returnSales==='1'){state.selectedCustomerId=ref.id;state.posCustomerMode='member';state.posMemberSearch=payload.phone||payload.name;state.posMemberPickerOpen=false;state.checkoutPoints=0;state.checkoutPointsTouched=false;state.checkoutEarnPoints=true;}await writeAudit(id?'更新客戶':'新增客戶','customer',ref.id,payload.name);closeDrawer();toast('客戶已儲存',payload.name,'success');await loadAll(true);
  }
  ...
  function renderPurchaseEntry(){
    ...
    const cartHtml=state.purchaseEntryCart.length?state.purchaseEntryCart.map(function(item,index){
      ...
      return '<div class="ops-purchase-entry-cart-row purchase-row" data-index="'+index+'"><input type="hidden" name="productId" value="'+attr(item.productId)+'"><div class="ops-purchase-entry-cart-title"><div><b>'+escapeHtml(product.originalName||product.name)+'</b><small>SKU '+escapeHtml(product.sku||'未設定')+'｜目前庫存 '+formatNumber(product.currentStock)+'</small></div><strong>'+money(qty*unitCost)+'</strong></div><div class="ops-purchase-entry-cart-fields"><label><span>數量</span><input class="ops-input" type="number" min="1" step="1" name="qty" value="'+qty+'" data-purchase-entry-qty="'+index+'" required></label><label><span>單位成本</span><input class="ops-input" type="number" min="0" step="0.01" name="unitCost" value="'+unitCost+'" data-purchase-entry-cost="'+index+'" required></label><button class="ops-icon-button" type="button" data-action="purchase-entry-remove" data-index="'+index+'" aria-label="移除商品">×</button></div></div>';
  ...
  function openStocktakeWorkspace(){resetStocktake();location.hash='stocktake';}
  ...
  function stocktakeSeriesTabs(){return '<div class="ops-purchase-series-tabs"><button type="button" class="'+(state.stocktakeSeries==='all'?'active':'')+'" data-action="stocktake-series" data-series="all">全部</button>'+PRODUCT_SERIES.map(function(row){return '<button type="button" class="'+(state.stocktakeSeries===row[0]?'active':'')+'" data-action="stocktake-series" data-series="'+row[0]+'">'+row[0]+' '+escapeHtml(row[1])+'</button>';}).join('')+'</div>';}
  ...
  async function saveStocktake(form){
    ...
    });await writeAudit(correction?'更正盤點紀錄':'完成庫存盤點','inventory',stocktakeNo,operator+'｜'+entries.length+' 項');resetStocktake();state.purchaseWorkspaceTab='adjust';location.hash='purchases';toast(correction?'盤點已更正':'盤點已完成',stocktakeNo,'success');await loadAll(true);
  }
  ...
  function renderPurchases(){
    const lowAll=state.catalog.filter(function(product){return product.initialized&&Number(product.currentStock||0)<=Number(product.safetyStock||0);}).sort(function(a,b){return Number(a.currentStock||0)-Number(b.currentStock||0);}),lowTerm=lower(state.purchaseLowSearch).trim(),lowRows=lowAll.filter(function(product){return !lowTerm||lower([product.originalName,product.name,product.sku].join(' ')).includes(lowTerm);});
    const currentPurchases=currentMonthRows(state.purchases,function(row){return row.receivedAt;}),monthPurchaseTotal=sum(currentPurchases,function(row){return row.totalCost;}),inventoryValue=sum(state.catalog,function(product){return Number(product.inventoryValue||0)||Number(product.averageCost||0)*Number(product.currentStock||0);}),todayInventory=todayRows(state.inventory,function(row){return row.occurredAt;}),recentPurchases=state.purchases.slice().sort(function(a,b){return (dateFrom(b.receivedAt)||0)-(dateFrom(a.receivedAt)||0);}).slice(0,40),adjustments=state.inventory.filter(function(row){return row.type==='adjustment';}).sort(function(a,b){return (dateFrom(b.occurredAt)||0)-(dateFrom(a.occurredAt)||0);}).slice(0,30),allowedTabs=['low','inbound','adjust','history'];if(!allowedTabs.includes(state.purchaseWorkspaceTab))state.purchaseWorkspaceTab='low';
    function workTab(tab,label,count){return '<button type="button" class="'+(state.purchaseWorkspaceTab===tab?'active':'')+'" data-action="purchase-worktab" data-tab="'+tab+'">'+escapeHtml(label)+(count!=null?'<span>'+formatNumber(count)+'</span>':'')+'</button>';}
    let panel='';
    if(state.purchaseWorkspaceTab==='low'){
      ...
      const table=recentPurchases.length?'<div class="ops-purchase-card-list">'+recentPurchases.map(function(row){const qty=sum(row.items||[],function(item){return item.qty;}),paid=row.paymentStatus==='paid';return '<article class="ops-purchase-history-card"><div class="ops-purchase-history-main"><div><b>'+escapeHtml(row.purchaseNo||row.id)+'</b><small>'+escapeHtml(dateTimeText(row.receivedAt))+(row.externalNo?'｜外部 '+escapeHtml(row.externalNo):'')+'</small></div><div><span>供應商</span><strong>'+escapeHtml(row.supplier||'未填供應商')+'</strong></div><div><span>品項／件數</span><strong>'+formatNumber((row.items||[]).length)+' 項／'+formatNumber(qty)+' 件</strong></div><div><span>總成本</span><strong>'+money(row.totalCost)+'</strong></div><div>'+statusTag(paid?'已付款':'未付款',paid?'green':'yellow')+(paid&&row.paymentDate?'<small>'+escapeHtml(dateText(row.paymentDate))+'</small>':'')+'</div><button class="ops-button small primary" data-action="purchase-edit" data-id="'+attr(row.id)+'">修改</button></div></article>';}).join('')+'</div>':emptyHtml('尚無進貨單','按「新增進貨」建立第一張進貨單。');panel='<div class="ops-v8-purchase-panel"><div class="ops-v8-inbound-summary"><div><span>本月進貨</span><b>'+money(monthPurchaseTotal)+'</b><small>'+formatNumber(currentPurchases.length)+' 張進貨單</small></div><div><span>供應商資料庫</span><b>'+formatNumber(state.suppliers.length)+' 家</b><small>可快速選擇聯絡與付款資訊</small></div><button class="ops-button ghost" data-action="supplier-manager">管理供應商</button><button class="ops-button primary" data-action="open-purchase">開始新增進貨</button></div><div class="ops-v8-subsection-head"><h3>最近進貨單</h3></div>'+table+'</div>';
    }else if(state.purchaseWorkspaceTab==='adjust'){
      const rows=adjustments.length?'<div class="ops-purchase-card-list">'+adjustments.map(function(row){return '<article class="ops-stocktake-history-card"><div><b>'+escapeHtml(row.productName||'未命名商品')+'</b><small>'+escapeHtml(dateTimeText(row.occurredAt))+'｜'+escapeHtml(row.sku||'')+'｜盤點人 '+escapeHtml(row.counterName||row.createdBy||'管理者')+'</small></div><strong class="'+(row.qtyChange>0?'plus':row.qtyChange<0?'minus':'')+'">'+(row.qtyChange>0?'+':'')+formatNumber(row.qtyChange)+'</strong><span>'+formatNumber(row.beforeStock)+' → '+formatNumber(row.afterStock)+'</span><button class="ops-button small ghost" data-action="stocktake-edit" data-id="'+attr(row.id)+'">更正</button></article>';}).join('')+'</div>':emptyHtml('尚無盤點紀錄','完成盤點後會顯示。');panel='<div class="ops-v8-purchase-panel"><div class="ops-stocktake-entry-actions"><div><b>盤點調整</b><p>盤點功能統一放在這裡：管理者可一次盤點多項，工讀生使用手機專用入口。</p></div><button class="ops-button ghost" data-action="inventory-count-settings">工讀生手機入口／密碼</button><button class="ops-button primary" data-action="open-adjustment">管理者開始盤點</button></div><div class="ops-v8-subsection-head"><h3>最近盤點紀錄</h3></div>'+rows+'</div>';
    }else{
      const inventoryRows=state.inventory.filter(function(row){const term=lower(state.inventorySearch);return !term||lower([row.productName,row.sku,row.referenceId,row.note].join(' ')).includes(term);}).sort(function(a,b){return (dateFrom(b.occurredAt)||0)-(dateFrom(a.occurredAt)||0);}).slice(0,160);panel='<div class="ops-v8-purchase-panel"><div class="ops-toolbar"><input class="ops-input grow" id="inventorySearch" placeholder="搜尋商品、SKU、來源或備註" value="'+attr(state.inventorySearch)+'"></div>'+(inventoryRows.length?'<div class="ops-table-wrap"><table class="ops-table"><thead><tr><th>時間</th><th>類型</th><th>商品</th><th class="num">異動</th><th class="num">異動後</th><th>來源／備註</th></tr></thead><tbody>'+inventoryRows.map(function(row){return '<tr><td>'+escapeHtml(dateTimeText(row.occurredAt))+'</td><td>'+escapeHtml(row.referenceType||row.type)+'</td><td><b>'+escapeHtml(row.productName)+'</b><br><small>'+escapeHtml(row.sku)+'</small></td><td class="num">'+(row.qtyChange>0?'+':'')+formatNumber(row.qtyChange)+'</td><td class="num">'+formatNumber(row.afterStock)+'</td><td>'+escapeHtml(row.referenceId)+'<br><small>'+escapeHtml(row.note)+'</small></td></tr>';}).join('')+'</tbody></table></div>':emptyHtml('尚無庫存流水',''))+'</div>';
    }
    const metrics='<div class="ops-kpi-grid ops-v8-purchase-kpis">'+kpi('低庫存待補',formatNumber(lowAll.length),'優先確認補貨','!')+kpi('本月進貨',money(monthPurchaseTotal),currentPurchases.length+' 張進貨單','⇧')+kpi('庫存價值',money(inventoryValue),'目前庫存成本','＄')+kpi('今日異動',formatNumber(todayInventory.length),'入庫、銷售與盤點','↕')+'</div>',tabs='<div class="ops-v8-worktabs">'+workTab('low','補貨待辦',lowAll.length)+workTab('inbound','進貨入庫')+workTab('adjust','盤點調整')+workTab('history','異動紀錄')+'</div>';
    return metrics+'<section class="ops-card ops-v8-purchase-workbench"><div class="ops-v8-section-head"><div><h2>進貨與庫存作業台</h2><p>進貨單、供應商、付款狀態與盤點紀錄集中管理</p></div><div class="ops-card-actions"><button class="ops-button primary" data-action="open-purchase">新增進貨</button></div></div>'+tabs+panel+'</section>';
  }
...
function platformOrderIsHidden(row){
  ...
  return ['ignored','ignored-freight','ignored-return'].includes(status);
}
...
function platformOrderIsEffective(row){
  return !!row&&!platformOrderIsHidden(row)&&row.reversalApplied!==true&&row.inventoryReversed!==true&&clean(row.processingStatus)!=='inventory-reversed';
}
...
function platformOrderProcessingLabel(row){
  const map={'inventory-applied':'已扣中央庫存','already-applied':'已處理','dry-run':'有效訂單／尚未扣庫存','unmatched-sku':'SKU 未配對','duplicate-sku':'SKU 重複','missing-sku':'缺少 SKU','ignored':'已排除','ignored-freight':'運費已排除','ignored-cancelled':'未成交／取消，未扣庫存','ignored-return':'退貨狀態，未扣庫存','manual-return-review':'退貨／退款請手動處理','missing-from-platform-review':'平台本次未再出現，等待複核','inventory-reversed':'取消／未付款，庫存已回補','reversal-error':'取消回補失敗','error':'處理失敗'};
...
function renderSync(){
  ...
  const agent=state.platformLocalAgent||{},agentHeartbeat=dateFrom(agent.lastHeartbeatAt),agentHasReported=!!agentHeartbeat,agentOnline=!!(agentHeartbeat&&(Date.now()-agentHeartbeat.getTime()<180000));
  const agentLabel=agentOnline?'店內同步程式運作中':agentHasReported?'店內同步程式目前未連線':'尚未收到店內同步程式回報';
  const agentColor=agentOnline?'green':agentHasReported?'yellow':'red';
  const agentSub=agentOnline?'最後回報 '+dateTimeText(agentHeartbeat):agentHasReported?'最後回報 '+dateTimeText(agentHeartbeat)+'；請確認店內電腦與背景監聽程式':'完成店內程式安裝並啟動後才會顯示在線';
  const runSummary=latestRun&&latestRun.summary||{},platformFetch=runSummary.platformFetch||{},syncErrors=inventorySyncAnomalies();
  const errorCounts={};syncErrors.forEach(function(row){errorCounts[row.platform]=Number(errorCounts[row.platform]||0)+1;});
  const errorSub=syncErrors.length?Object.keys(errorCounts).sort().map(function(name){return name+' '+errorCounts[name]+' 項';}).join('、'):'目前沒有待處理異常';
  function platformCard(name){
    const info=platformFetch[name]||{},status=info.status==='success'?'同步成功':info.status==='error'?'同步失敗':'等待首次同步',color=info.status==='success'?'green':info.status==='error'?'red':'yellow',cfg=platformFeeConfig(name),platformRows=validRows.filter(function(row){return row.platform===name;}),m=platformFeeMetrics(platformRows),platformQty=sum(platformRows,function(row){return row.quantity;}),platformOrders=new Set(platformRows.map(platformOrderGroupKey)).size;
    return '<section class="ops-card ops-platform-status-card"><div class="ops-card-head"><div><h3>'+escapeHtml(name)+'</h3><p>'+(info.lines!=null?'本次讀取 '+formatNumber(info.lines)+' 筆明細':'尚無同步紀錄')+'</p></div>'+statusTag(status,color)+'</div><div class="ops-summary-list"><div class="ops-summary-line"><span>成交金額</span><b>'+money(m.gross)+'</b></div><div class="ops-summary-line"><span>平台／金流費</span><b>'+money(m.variableFees)+'</b></div><div class="ops-summary-line"><span>固定費攤提</span><b>'+money(m.fixedFees)+'</b></div><div class="ops-summary-line"><span>商品成本</span><b>'+money(m.cost)+'</b></div><div class="ops-summary-line total"><span>預估毛利</span><b>'+money(m.profit)+'</b></div></div><div class="ops-platform-fee-mini">'+formatNumber(platformOrders)+' 單／'+formatNumber(platformQty)+' 件・抽成 '+formatNumber(cfg.commissionRate)+'%・金流 '+formatNumber(cfg.paymentRate)+'%・月固定 '+money(Number(cfg.monthlyFixedFee||0)+Number(cfg.monthlyAdvertisingFee||0))+'</div>'+(info.error?'<div class="ops-platform-error">'+escapeHtml(info.error)+'</div>':'')+'</section>';
  }
  ...
  const orderTable=rows.length?'<div class="ops-table-wrap ops-platform-orders-table"><table class="ops-table"><thead><tr><th>平台／成交時間</th><th>訂單</th><th>商品</th><th class="num">數量</th><th class="num">成交</th><th class="num">平台費</th><th class="num">固定費攤提</th><th class="num">成本</th><th class="num">預估毛利</th><th>中央庫存</th></tr></thead><tbody>'+rows.slice(0,500).map(function(row){
    ...
    if(row.processingStatus==='inventory-reversed')stockHtml+='<br><small>'+formatNumber(row.inventoryBeforeReversal)+' → '+formatNumber(row.inventoryAfterReversal)+'（已回補）</small>';
    else if(row.inventoryApplied)stockHtml+='<br><small>'+formatNumber(row.inventoryBefore)+' → '+formatNumber(row.inventoryAfter)+'</small>';
    if(row.missingFromPlatformCount)stockHtml+='<br><small>未出現 '+formatNumber(row.missingFromPlatformCount)+' 次</small>';
  ...
  const runRows=state.platformSyncRuns.slice().sort(function(a,b){return (dateFrom(b.startedAt)||0)-(dateFrom(a.startedAt)||0);}).slice(0,20),runTable=runRows.length?'<div class="ops-table-wrap"><table class="ops-table"><thead><tr><th>執行時間</th><th>方式</th><th>結果</th><th>訂單明細</th><th>新扣庫存</th><th>取消回補</th><th>待複核</th><th>差異商品</th><th>庫存同步</th><th>訊息</th></tr></thead><tbody>'+runRows.map(function(run){const summary=run.summary||{},processing=summary.processing||{},reconciliation=summary.reconciliation||{},inventorySync=summary.inventorySync||{};return '<tr><td>'+escapeHtml(dateTimeText(run.startedAt))+'</td><td>'+escapeHtml(run.trigger||'排程')+'</td><td>'+statusTag(run.status||'—',run.status==='completed'?'green':run.status==='completed-with-errors'?'yellow':'red')+'</td><td>'+formatNumber(summary.fetchedLines||0)+'</td><td>'+formatNumber(processing.applied||0)+'</td><td>'+formatNumber(processing.reversed||reconciliation.reversed||0)+'</td><td>'+formatNumber(reconciliation.reviewed||0)+'</td><td>'+formatNumber(summary.changedProducts||0)+'</td><td>'+formatNumber(inventorySync.success||0)+' 成功／'+formatNumber(inventorySync.errors||0)+' 異常</td><td><small>'+escapeHtml(run.error||((processing.errors||0)?'訂單處理異常 '+processing.errors+' 筆':(inventorySync.errors||0)?'庫存同步異常 '+inventorySync.errors+' 項':'完成'))+'</small></td></tr>';}).join('')+'</tbody></table></div>':emptyHtml('尚無同步紀錄','每天 14:00、20:30 或按立即同步後會出現。');
  const errorList=syncErrors.length?'<div class="ops-sync-anomaly-list">'+syncErrors.map(function(row){const platformColor=row.platform==='EasyStore'?'green':row.platform==='MOMO'?'blue':row.platform==='Coupang'?'yellow':'red';return '<article class="ops-sync-anomaly-row"><div class="ops-sync-anomaly-platform">'+statusTag(row.platform,platformColor)+'</div><div class="ops-sync-anomaly-main"><b>'+escapeHtml(row.productName||'未命名商品')+'</b><small>SKU '+escapeHtml(row.sku||'未設定')+'・目標庫存 '+formatNumber(row.targetStock)+'</small><strong>'+escapeHtml(row.reason)+'</strong>'+(row.message?'<p>'+escapeHtml(row.message)+'</p>':'')+'<small>最後嘗試 '+escapeHtml(dateTimeText(row.lastAttemptAt))+(row.runId?'・批次 '+escapeHtml(row.runId):'')+'</small></div><div class="ops-sync-anomaly-actions"><button class="ops-button small primary" data-action="inventory-anomaly-product" data-sku="'+attr(row.sku)+'">前往商品庫存</button></div></article>';}).join('')+'</div>':emptyHtml('目前沒有庫存同步異常','下一次同步若成功，原本的異常會自動從這裡消失。');
  const runPanel=state.platformSyncPanel==='runs'?'<section class="ops-card ops-platform-expand-panel"><div class="ops-card-head"><div><h2>同步紀錄</h2><p>最近 20 次店內同步執行結果</p></div><button class="ops-button small ghost" data-action="platform-sync-panel" data-panel="runs">收合</button></div>'+runTable+'</section>':'';
  const errorPanel=state.platformSyncPanel==='errors'?'<section class="ops-card ops-platform-expand-panel ops-platform-error-panel"><div class="ops-card-head"><div><h2>同步異常明細</h2><p>'+escapeHtml(errorSub)+'。完成平台 SKU 配對後，下一次同步會自動重試。</p></div><button class="ops-button small ghost" data-action="platform-sync-panel" data-panel="errors">收合</button></div>'+errorList+'</section>':'';
  const schedule='<div class="ops-platform-schedule"><div class="ops-platform-schedule-card"><span>上次同步</span><b>'+(latestRun?escapeHtml(dateTimeText(latestRun.startedAt)):'尚未執行')+'</b></div><div class="ops-platform-schedule-card"><span>下次同步</span><b>'+escapeHtml(nextPlatformSyncText())+'</b></div><div class="ops-platform-schedule-card"><span>執行位置</span><b>店內 Windows 電腦（固定 IP）</b></div><button type="button" class="ops-platform-schedule-card ops-platform-schedule-action '+(state.platformSyncPanel==='runs'?'active':'')+'" data-action="platform-sync-panel" data-panel="runs"><span>同步程式狀態</span>'+statusTag(agentLabel,agentColor)+'<small>'+escapeHtml(agentSub)+'</small><em>點擊查看同步紀錄</em></button><button type="button" class="ops-platform-schedule-card ops-platform-schedule-action '+(syncErrors.length?'has-error':'')+' '+(state.platformSyncPanel==='errors'?'active':'')+'" data-action="platform-sync-panel" data-panel="errors"><span>同步異常</span><b>'+formatNumber(syncErrors.length)+' 項</b><small>'+escapeHtml(errorSub)+'</small><em>點擊查看異常明細</em></button></div>';
  return '<section class="ops-card ops-platform-summary"><div class="ops-card-head"><div><h2>平台訂單、毛利與差異庫存同步</h2><p>有效訂單會計算成交、平台費、成本與毛利；中央庫存異動會同步至三個平台。同步異常可在上方直接展開查看與處理。</p></div><div class="ops-card-actions"><button class="ops-button ghost" data-action="platform-fee-settings">費用設定</button><button class="ops-button primary" data-action="platform-sync-now">立即同步</button></div></div>'+schedule+'</section>'+runPanel+errorPanel+'<div class="ops-grid-3 ops-platform-status-grid">'+platformCard('EasyStore')+platformCard('MOMO')+platformCard('Coupang')+'</div><section class="ops-card ops-platform-order-list"><div class="ops-card-head"><div><h2>'+escapeHtml(bounds.label)+'網路銷售</h2><p>只顯示並合計目前有效的訂單；已取消、未付款與重複明細不會出現在這裡。</p></div></div>'+rangeTabs+'<div class="ops-kpi-grid ops-platform-kpis">'+kpi('成交金額',money(fees.gross),orderCount+' 筆訂單','＄')+kpi('平台與金流費',money(fees.variableFees),'依各平台設定','費')+kpi('固定費用攤提',money(fees.fixedFees),'月費＋廣告費','固')+kpi('商品成本',money(fees.cost),'中央 FIFO／估算成本','成本')+kpi('預估毛利',money(fees.profit),'成交－全部費用－成本','利')+kpi('銷售件數',formatNumber(qty),'有效成交數量','件')+'</div>'+platformTabs+'<div class="ops-toolbar ops-platform-search"><input class="ops-input grow" id="platformOrderSearch" placeholder="搜尋訂單編號、SKU、商品、客戶或狀態" value="'+attr(state.platformOrderSearch)+'"></div>'+orderTable+'</section>';
}
...
function ensureSalesClock(){
  ...
  async function saveStockUsage(form){
    ...
    await state.db.runTransaction(async function(tx){
      ...
      const prepared=[];let costTotal=0,unknownCostQty=0;
      snaps.forEach(function(snap,index){if(!snap.exists)throw new Error('商品主檔不存在：'+state.cart[index].name);const raw=snap.data()||{},item=state.cart[index],current=Number(raw.currentStock||0),qty=Math.max(1,Math.round(Number(item.qty||0)));if(current<qty)throw new Error(item.name+' 庫存不足，目前 '+current);const fifo=consumeFifo(raw,qty);costTotal+=fifo.costTotal;unknownCostQty+=fifo.unknownCostQty;prepared.push({ref:refs[index],raw:raw,item:item,qty:qty,current:current,fifo:fifo});});
      tx.set(saleRef,{saleNo:usageNo,soldAt:occurredAt,saleType:'internalUse',usageReason:usageReason,usageNote:usageNote,items:prepared.map(function(x){return {productId:x.item.productId,name:x.item.name,sku:x.item.sku,imageUrl:x.item.imageUrl||'',qty:x.qty,unitPrice:0,lineTotal:0,lineCost:x.fifo.costTotal,fifoBreakdown:x.fifo.breakdown,unknownCostQty:x.fifo.unknownCostQty,rewardPercent:0};}),subtotal:0,manualDiscount:0,pointDiscount:0,discount:0,total:0,costTotal:costTotal,grossProfit:0-costTotal,unknownCostQty:unknownCostQty,costMethod:'FIFO',paymentMethod:'內部耗用',paymentStatus:'paid',receivedAmount:0,customerId:'',customerName:'',customerType:'internal',memberNo:'',pricingTier:'internal',earnPointsEnabled:false,pointsEarned:0,pendingPointsEarned:0,pointsRedeemed:0,note:[usageReason,usageNote].filter(Boolean).join('｜'),status:'completed',createdAt:serverTimestamp(),createdBy:userLabel(),version:VERSION});
    ...
    await writeAudit('內部耗用／報廢','internalUse',saleRef.id,usageNo+'｜'+usageReason);state.cart=[];state.posSearch='';state.stockUsageReason='店內自用';state.stockUsageNote='';toast('庫存已扣除',usageNo+'｜'+usageReason,'success');await loadAll(true);
  }
...
async function syncPlatformOrdersNow(){const yes=await confirmAction('要求店內電腦立即同步','系統會建立同步請求；店內電腦開機且背景程式正常時，最慢約 20 秒內接收。若同步正在進行，不會重複執行。','建立請求');if(!yes)return;const ref=state.db.collection(COLLECTIONS.platformSyncRequests).doc();await ref.set({requestId:ref.id,status:'pending',requestedAt:serverTimestamp(),requestedBy:userLabel(),source:'operations-hub',version:VERSION});toast('同步請求已建立','店內電腦會在約 20 秒內接收，完成後重新讀取即可查看結果。','success');}
  ...
  function handleAction(action,el){
    ...
    if(action==='overview-order-errors'){state.platformSyncPanel='';state.platformOrderRange='all';state.platformOrderPlatform='all';state.platformOrderSearch='';location.hash='sync';return;}
    if(action==='inventory-anomaly-product'){state.productSearch=clean(el.dataset.sku);state.productFilter='all';state.productSeries='all';state.productVisible=PRODUCT_PAGE_SIZE;location.hash='products';return;}
