(function(global){
  ...
  const VERSION = '2026.09.02-listing-retry-queue-v1';
...
function ensureSalesClock(){
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '同一輪圖片處理只產生本次目標通路需要的首圖版型，不得替未選通路多做圖片，也不得進平台後才重新裁切或設計。若本次包含 EasyStore，storefrontPortrait 使用 1000×750 px、4:3；包含蝦皮時 brandedHero 使用 1000×1000 px、1:1；包含 MOMO／酷澎時 cleanMain 為 1000×1000 px、1:1。兩張品牌首圖版本固定為 '+PRODUCT_BRAND_TEMPLATE_CONTRACT.version+'，必須保存 templateVersion、templateAssetSha256、templateComposition、brandCreativeStyleAssignment 與 brandRenderProof；缺少、母版雜湊不符、風格證明不符，或證明顯示框線穿過 Logo，就在進平台前停止。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。來源截圖在分析前先建立標準化工作副本：長邊以 1600～2000 px 為宜、目標 1800 px，超過 2400 px 才縮小，較小來源不放大，並保留 sourceImageUrl 與雜湊譜系。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是純白底、無品牌框、無新增 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得加入任何文字；商品本體原有且不可分離的印字必須保留，商品最長邊至少占 80%，在不裁切商品下以 90～95% 為目標。storefrontPortrait 與 brandedHero 的綠色頁首固定占整張高度 1/9；紅色標語「有音樂的生活更有風格」與圓形柚子樂器 Logo 的圖樣、文字、顏色不可改。Logo 固定採管理者選定的大型跨界版本，置於右上並向下壓入內容區；只有 Logo 可跨頁首邊界，且不得遮住商品或主要文字。下方四周固定保留連續、清楚可見的細綠色圓角框；框線先畫、Logo 最後覆蓋，Logo 永遠在最上層，圓形 Logo 內不得看見穿過的框線。所有照片、色塊、文字與裝飾都要停在框線內。內容區至少 65% 為奶油白、白、淺灰或明亮照片，禁止深色滿版；文字可使用不同但真實可讀的商業字體，不限定微軟正黑體。每個根商品從 50 種淺色商業風格以 random-without-replacement 抽一種，50 種未用完不得重複；抽中的 background、accents 與 layout 必須實際控制畫面，不得只記錄名稱或退回每件相同的通用模板。同一商品的 1:1 與 4:3、同組所有細項都沿用 preflightSnapshot.brandCreativeStyleAssignment，只做比例重排。storefrontPortrait 與 brandedHero 均固定使用 3 個已查證特色；不足 3 個時先補查證資料，仍不足就停止主圖完成，不得虛構。商品本體不得被 Logo、標題、色塊或裝飾遮住。兩者不得加入地址、電話、QR Code、價格、導外訊息、左下娃娃、PIC COLLAGE 或殘缺文字。',
