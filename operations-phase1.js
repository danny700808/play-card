(function(global){
  ...
  const VERSION = '2026.08.25-listing-action-grid';
...
function ensureSalesClock(){
  ...
  function productListingDescriptionStatus(value){
    const text=clean(value),lines=text.replace(/\r/g,'').split('\n').map(clean).filter(Boolean),featureHeader=lines.some(function(line){return /^商品特色[：:]?$/.test(line);}),usageHeader=lines.some(function(line){return /^(?:使用方式|適用情境)[：:]?$/.test(line);}),specHeader=lines.some(function(line){return /^商品規格[：:]?$/.test(line);});let inFeatureSection=false,featureCount=0;lines.forEach(function(line){if(/^商品特色[：:]?$/.test(line)){inFeatureSection=true;return;}if(/^(?:使用方式|適用情境|商品規格|包裝內容|適用對象|注意事項)[：:]?$/.test(line)){inFeatureSection=false;return;}if(inFeatureSection&&/^(?:\d+[.、]|[-•●])\s*\S+/.test(line))featureCount+=1;});const genericOnly=/本商品為柚子樂器販售的樂器或樂器配件/.test(text)||(!featureHeader&&!usageHeader&&!specHeader&&/(?:商品內容與規格以|如需確認尺寸、相容性或包裝內容)/.test(text));
    const missing=[];
    if(!text)missing.push('商品介紹');
    if(!featureHeader||featureCount<1)missing.push('可驗證商品特色');
    if(!usageHeader)missing.push('使用方式／適用情境');
    if(!specHeader)missing.push('商品規格');
    if(genericOnly)missing.push('通用備援文案尚未改寫');
    return {ready:!!text&&!genericOnly&&featureHeader&&featureCount>0&&usageHeader&&specHeader,genericOnly:genericOnly,featureCount:featureCount,missing:Array.from(new Set(missing))};
  }
  function productListingAutomaticDescription(p){
    ...
    const intro=/(?:吉他)?腳踏板|足凳|腳凳/i.test(name)?'木製吉他腳踏板，適合演奏時支撐腳部使用；可依坐姿調整高度，使用後可摺疊收納。':'本商品為柚子樂器販售的樂器或樂器配件，尚待依案件圖片與可驗證資料完成正式商品介紹。';
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const mediaSection='<textarea name="imagePlan" hidden>'+escapeHtml(row.imagePlan)+'</textarea><textarea name="listingImageUrls" hidden>'+escapeHtml(listingImages.join('\n'))+'</textarea><div class="ops-listing-image-order"><b>淺色商業首圖</b><span>商品圖</span><span>店面宣傳</span></div><div id="productListingImagePreview">'+(listingImages.length?productListingImagePreviewHtml(listingImages,'目前已完成圖片'):'<div class="ops-listing-image-empty">交給 Codex 後會在這裡保存完成圖片</div>')+'</div>';
  ...
  async function uploadProductCompletedListingImages(form,files){
    ...
    const generated=uploaded.map(function(url,index){const assignment=assignments[index],branded=assignment.role==='brandedHero'||assignment.role==='storefrontPortrait',cleanMain=assignment.role==='cleanMain';return {id:'codex-chat-'+Date.now()+'-'+index,url:url,status:'ready',mode:'codex-chat-single-pass',model:'Codex 對話圖片處理',qaStatus:'approved',localizationStatus:'completed',qaModel:'人工確認後上傳',qaAttempts:0,sourceImageUrl:sourceImages[assignment.sourceIndex],sourceOrder:assignment.sourceIndex+1,roles:[assignment.role],assetFlags:{containsLogo:branded,containsText:branded,containsContactInfo:false,containsQrCode:false,greenBrandTemplate:branded,momoPromotionEligible:cleanMain},instructions:instructions,createdAt:createdAt};});
  ...
  async function startProductImageCollection(form,targetProductId){
    ...
    try{await requestProductImageCollection('start',session);productImageCollectionUploadFailures=[];productImageCollectionSession=session;updateProductImageCollectionUi(form,'正在收圖',false,id);toast('框選收圖已啟動',session.sku+'｜最多 '+session.maxImages+' 張','success');}
  ...
  async function receiveProductImageCollectionFile(payload){
    ...
    try{
      ...
      else updateProductImageCollectionUi(form,'剛剛的截圖已加入；可繼續到供應商頁框選。',false,productId);
  ...
  async function removeProductVariantReferenceImage(form,productId,url,role){
    ...
    if(productImageCollectionSession&&productImageCollectionSession.active&&productImageCollectionSession.productId===id){
      productImageCollectionSession.currentCount=urls.length;
      global.postMessage({source:PRODUCT_IMAGE_COLLECTION.source,type:PRODUCT_IMAGE_COLLECTION.start,payload:productImageCollectionSession},global.location.origin);
      updateProductImageCollectionUi(form,'已刪除一張；可繼續到供應商頁框選截圖。',false,id);
    }
  ...
  function openProductListingPreview(form){
    ...
    const contentRows=[
      productListingCheckRow('本次處理規則',draft.listingIntentPolicy.label+'｜'+draft.listingIntentPolicy.description,readiness.intentReady,'修改既有商品時必須寫明要替換的內容'),productListingCheckRow('上架商品名稱',listingName,!!listingName),productListingCheckRow('完整商品介紹',draft.productDescription?'已填 '+draft.productDescription.length+' 字':'',!!draft.productDescription),productListingCheckRow('上架圖片',draft.imageCount+' 張'+(draft.physicalImageUrls.length?'＋實體圖 '+draft.physicalImageUrls.length+' 張':''),hasImages)
    ];
  ...
  function productListingPlatformImagePlan(rows,frozenSourceImageUrls){
    ...
    const brandedRows=byRole('brandedHero').filter(function(row){return row.assetFlags.containsLogo&&row.assetFlags.greenBrandTemplate&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode;});
    const storefrontRows=byRole('storefrontPortrait').filter(function(row){return row.assetFlags.containsLogo&&row.assetFlags.containsText&&row.assetFlags.greenBrandTemplate&&!row.assetFlags.containsContactInfo&&!row.assetFlags.containsQrCode;});
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '有幾張 physicalImageUrls 就在平台圖文介紹容量內使用幾張，固定接在一般繁體完成介紹圖之後、兩張店面服務宣傳圖之前；EasyStore、蝦皮進階描述、MOMO 圖文介紹與酷澎詳情都沿用同一順序。實體圖永遠不得成為平台第 1 張主圖。',
      '[圖片一次定案]',
      ...
      'cleanMain 是無品牌框、無 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得含任何文字。storefrontPortrait 使用 3:4「柚子樂器淺色商業展示版」：頂端薄荷綠品牌區必須從 product-listing-main-template.jpg 以確定性像素覆蓋方式直接合成，圖片模型只准生成下方商品內容區，禁止重畫、仿畫、換字或另生 Logo。固定標頭完整保留「有音樂的生活更有風格」標語與右上圓形柚子樂器標誌原本的高度、寬度及位置，禁止壓窄、拉伸、裁切或重新排版；只有下方商品展示區可依商品調整。淺色內容底板四周保留細綠邊，移除左下寶寶及 PIC COLLAGE；用清楚商業層級呈現 3～5 個已查證賣點，最多 2 個有來源依據的輔助視覺。brandedHero 同樣必須以固定母版像素覆蓋品牌頁首，圖片模型只能生成 1:1 方形版下方內容；商品去背後約占 55～65%，只放 1～3 個已查證短賣點。三者都不得加入價格、聯絡資訊、浮水印或虛構功能／配件；不得含殘缺文字。',
