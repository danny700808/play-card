...
function renderOverviewV7(){
  ...
  function productCard(p){
    ...
    const urls=[mainImage,variantImage].filter(Boolean),reward=p.saleRewardPercent==null?membershipRuleForDate(new Date()).rewardPercent:p.saleRewardPercent;
    ...
    return '<button type="button" class="ops-product-card ops-product-card-full" data-action="product-edit" data-id="'+attr(p.docId)+'"><div class="ops-product-image-grid '+(urls.length<2?'single':'')+'">'+imageHtml+'</div><div class="ops-product-body"><div class="ops-product-sku-row"><span>商品編號</span><b>'+escapeHtml(p.sku||'未設定')+'</b></div><div class="ops-product-name-rows"><span>商品名稱</span><b>'+escapeHtml(cleanOnline)+'</b></div>'+(variant?'<div class="ops-product-variant-row"><span>細項名稱</span><b>'+escapeHtml(variant)+'</b></div>':'')+'<div class="ops-product-stock"><span>目前庫存</span><b>'+escapeHtml(formatNumber(p.currentStock))+'</b></div><div class="ops-product-detail-grid"><div><span>門市定價</span><b>'+money(p.storePrice)+'</b></div><div><span>網路售價</span><b>'+money(p.onlinePrice)+'</b></div><div><span>進貨成本</span><b>'+money(p.latestPurchaseCost)+'</b></div><div><span>平均成本</span><b>'+money(p.averageCost)+'</b></div><div><span>會員回饋</span><b>'+formatNumber(reward)+'%</b></div></div></div></button>';
  }
