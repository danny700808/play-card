...
function ensureSalesClock(){
  ...
  async function saveProductListingCase(form,previewAfter,keepOpen,silent){
    ...
    const variantGroupEnabled=listingMode==='independent'&&data.get('variantGroupEnabled')==='1',variantGroupAttributeName=clean(data.get('variantGroupAttributeName')),variantGroupPrimaryValue=clean(data.get('variantGroupPrimaryValue'))||productVariantSuggestion(p),variantGroupPrimaryImageUrl=safeUrl(data.get('variantGroupPrimaryImageUrl')),gallerySelections=productVariantGallerySelectionsFromForm(form),variantGroupItems=variantGroupEnabled?productVariantGroupItemsFromForm(form).map(function(item){item.gallerySourceImageUrls=gallerySelections.get(item.productId)||[];return item;}):[],gallerySourceImageUrls=gallerySelections.get(id)||[];
  ...
  function updateProductImageCollectionUi(form,message,isError,targetProductId){
    ...
    if(status&&!singleMode)status.innerHTML='';
      ...
      if(isError)status.innerHTML='<div class="ops-product-ai-status failed"><span>!</span><div><b>'+(productImageCollectionSession&&productImageCollectionSession.active?'圖片尚未加入':'收圖未啟動')+'</b><small>'+escapeHtml(message||'請確認已安裝最新版柚子掌櫃助手。')+'</small><div class="ops-listing-collector-links"><a href="youzi-easystore-shopee-autofill-v0.3.24.zip" download>下載／更新助手 0.3.24</a></div></div></div>';
  ...
  function requestProductImageCollection(action,payload){
    ...
    return new Promise(function(resolve,reject){
      const sessionId=clean(payload&&payload.sessionId),timer=global.setTimeout(function(){if(!productImageCollectionPending||productImageCollectionPending.sessionId!==sessionId)return;productImageCollectionPending=null;reject(new Error('找不到收圖助手，請先更新到柚子掌櫃助手 0.3.24 並重新整理頁面。'));},3500);
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const cards=orderedKeys.map(function(key){
      ...
      const helper=key==='shopee'&&row.autofillPayload?'<div class="ops-card-actions"><button class="ops-button primary" type="button" data-action="product-shopee-autofill-open">安全開啟 EasyStore／蝦皮</button><a class="ops-button soft" href="youzi-easystore-shopee-autofill-v0.3.24.zip" download>下載／更新助手 0.3.24</a></div><small>'+(shopeeHomeLogisticsNeedsManualConfirmation?'會帶入分類、品牌與商品屬性；一般宅配的實際物流仍須確認。商品身分固定使用中央平台 ID；沒有 ID 就建立本次唯一的新 SKU。':'商品身分固定使用中央平台 ID；有 ID 就更新，沒有 ID 就建立本次唯一的新 SKU，不先搜尋平台目錄。')+'</small>':'';
  ...
  async function openShopeeAutofillHelper(){
    ...
    else toast('助手沒有收到商品資料',clean(result&&result.error)||'請安裝或更新到助手 0.3.24，重新整理營運中心後，再按一次「安全開啟 EasyStore／蝦皮」。','warning');
    return result;
  }
  ...
  function productListingCodexHandoffPrompt(product,snapshot){
    ...
    return [
      ...
      '同一輪圖片處理固定產生三個首圖版型，不得進平台後才重新裁切或設計：EasyStore storefrontPortrait 為 750×1000 px、3:4；蝦皮 brandedHero 為 1000×1000 px、1:1；MOMO／酷澎 cleanMain 為 1000×1000 px、1:1。全部使用 sRGB、優先 JPEG、每張不超過 1,000,000 bytes。只有平台明確拒絕並回傳可識別的圖片規格錯誤時，才可從同一完成圖產生相容輸出，並把新規格回寫固定規則。',
      'cleanMain 是無品牌框、無 Logo、無地址／電話／QR Code、可辨識真實商品的乾淨完成圖，而且不得含任何文字。storefrontPortrait 使用 3:4「柚子樂器淺色商業展示版」：保留頂端薄荷綠品牌區、紅色標語與右上標誌，淺色內容底板四周保留細綠邊，移除左下寶寶及 PIC COLLAGE；用清楚商業層級呈現 3～5 個已查證賣點，最多 2 個有來源依據的輔助視覺。brandedHero 使用相同品牌語言的 1:1 方形版，商品去背後約占 55～65%，只放 1～3 個已查證短賣點。三者都不得加入價格、聯絡資訊、浮水印或虛構功能／配件；不得含殘缺文字。',
      ...
      'MOMO 的商品主圖、廣告用圖與圖文編輯器專推圖是三個互相獨立的必填位置；主圖或廣告用圖已出現，不能推定專推圖已完成。第一次送出前就必須把合格完成圖分別套入需要的位置，並在儲存後重開同一草稿，確認專推圖 img src 與甲指第三方 000001 都仍存在；不得等平台第一次拒絕後才補專推圖或出貨地。',
      '[商品內容]',
      ...
      '交錯並行規則：MOMO、酷澎、EasyStore 是三個獨立根節點，資料預檢完成後一起啟動；蝦皮只依賴 EasyStore。瀏覽器的導航、點擊、輸入、檔案選擇、選圖與最後送出必須共用單一操作鎖，同一時間只操作一個頁面；某平台進入導航等待、圖片上傳、平台處理或審核等待後，立即釋放操作鎖去處理另一平台，不得等待完一整站才開始下一站，也不得三頁同時輸入而搶焦點。',
      ...
      '酷澎固定走已驗證的「以圖片建立」同一草稿：先上傳 cleanMain 與繁體完成副圖，再填樂器葉分類及可驗證品牌，按一次「產生商品資訊」，接著依 preparedPlatformFieldPlan 一次填完細項、售價、庫存、賣家 SKU、配送、人工繁體介紹與 TW_General 合規資料，最後按「建立產品」並以完全相同 SKU 在正式清單核對一次。產生後的細項欄位若呈唯讀或值無效，只回同一草稿圖片步驟重新產生一次；不得整頁重跑、名稱廣搜或另建商品。審核中只記為已送審，不得誤報已上架。',
      ...
      '平台售價空白時沿用共同網路售價、網路售價或門市售價；分類限定在樂器／樂器配件。MOMO、酷澎、EasyStore 各自保存狀態並可用任意順序完成，蝦皮只在 EasyStore verified 後啟動；全部沿用同一 jobId、最終 preparedSnapshot、preparedPlatformFieldPlan 與各平台 stage。重試不得重新讀取案件或重建快照。只有 job schema、目前 automationPolicy、執行圖、最終完成圖快照、穩定 fingerprint 與案件 handoff input snapshot 完全一致的未完成 v2 工作可復用；任何早期 v2 或舊資料不符都要拒絕。每站送出後只核對一次正式清單與正式商品資料，確認相同 SKU、價格、庫存、圖片角色及狀態；成功提示本身不算完成。單一根平台尚未完成不得阻擋其他根平台，只有蝦皮固定為 blocked-by-dependency，直到 EasyStore verified。',
      'MOMO 專推圖固定走同一草稿的素材銀行：開啟素材銀行 → 上傳文件 → 第二層「上傳素材」 → 以檔名精確搜尋 → 選取並確認。暫存後重開同一草稿時，不得依賴會變動的編輯器 element id；必須從目前 contenteditable 的 HTML 確認仍存在素材銀行圖片 img src，才可發布。發布後返回「上架」分頁，以 MOMO 品號或原廠編號精確核對一次。',
