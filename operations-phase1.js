...
function platformOrderIsHidden(row){
  ...
  return ['ignored','ignored-freight'].includes(status);
}
...
function platformOrderDetailDate(label,value){return '<div class="ops-order-timeline-row"><span>'+escapeHtml(label)+'</span><b>'+(value?escapeHtml(dateTimeText(value)):'尚無資料')+'</b></div>';}
...
function renderSync(){
  ...
  const returnRows=visiblePlatformOrders(state.platformOrders).filter(function(row){return platformOrderHasReturn(row)&&clean(row.returnHandlingStatus)!=='completed';}),returnOrderCount=new Set(returnRows.map(platformOrderGroupKey)).size;
  const returnAlert=returnRows.length?'<section class="ops-card ops-platform-return-alert"><div><span>退貨待處理</span><b>'+formatNumber(returnRows.length)+' 項商品／'+formatNumber(returnOrderCount)+' 筆訂單</b><small>只在平台訂單頁處理，不會塞進營運總覽。</small></div><button class="ops-button primary" data-action="platform-return-filter">'+(state.platformOrderIssueFilter==='returns'?'顯示全部訂單':'查看退貨訂單')+'</button></section>':'';
...
async function syncPlatformOrdersNow(){const yes=await confirmAction('要求店內電腦立即同步','系統會建立同步請求；店內電腦開機且背景程式正常時，最慢約 20 秒內接收。若同步正在進行，不會重複執行。','建立請求');if(!yes)return;const ref=state.db.collection(COLLECTIONS.platformSyncRequests).doc();await ref.set({requestId:ref.id,status:'pending',requestedAt:serverTimestamp(),requestedBy:userLabel(),source:'operations-hub',version:VERSION});toast('同步請求已建立','店內電腦會在約 20 秒內接收，完成後重新讀取即可查看結果。','success');}
  ...
  function handleAction(action,el){
    ...
    if(action==='platform-return-filter'){state.platformOrderIssueFilter=state.platformOrderIssueFilter==='returns'?'all':'returns';state.platformOrderRange='all';return renderKeepingViewport();}
