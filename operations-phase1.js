...
function ensureSalesClock(){
  ...
  function productVariantParentPreviewHtml(parent,child,row){
    ...
    return '<div class="ops-listing-variant-pair">'+productVariantRepresentativeCardHtml(parent,{kind:'原商品',role:'add-parent',fieldName:'variantParentImageUrl',selectedUrl:row&&row.variantParentImageUrl,sourceImages:parentSources,valueName:'variantParentAttributeValue',value:row&&row.variantParentAttributeValue||productVariantSuggestion(parent)||'原款'})+productVariantRepresentativeCardHtml(child,{kind:'現在要加入的商品',role:'add-child',fieldName:'variantChildImageUrl',selectedUrl:row&&row.variantChildImageUrl,sourceImages:childSources,valueName:'variantAttributeValue',value:row&&row.variantAttributeValue||productVariantSuggestion(child)})+'</div>';
  }
