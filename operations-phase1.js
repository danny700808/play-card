(function(global){
  ...
  const VERSION = '2026.08.14-cross-platform-status-v1';
...
function ensureSalesClock(){
  ...
  function productListingCaseFormHtml(p,row,exists){
    ...
    const sourceSection='<div class="ops-callout green ops-listing-intake-guide"><b>圖片來源可選一種，也可以兩種一起用</b><br><span>貼商品網址讓系統自動挑圖，或直接上傳你已存好的圖片；確認勾選後，按下方按鈕就會接續完成文案與圖片。</span></div><div class="ops-callout"><b>固定圖片格式已套用</b><br><span>第一張自動合成柚子樂器綠色模板（保留標語與商標、移除左下寶寶）；中間圖片逐張轉為台灣繁體；最後一張固定放實體店面宣傳。商品介紹最後另放兩張柚子樂器服務圖。</span></div>'
      ...
      +'<div class="ops-listing-intake-card full"><div class="ops-listing-intake-heading"><span>A</span><div><b>貼商品網址，自動抓圖</b><small>貼 1～3 個原廠、供應商、淘寶、1688 或其他商品頁；一行一個網址。系統會去除重複圖並優先挑約 8 張。</small></div></div><div class="ops-field full"><label>商品網址</label><textarea class="ops-textarea compact" name="referenceUrls" placeholder="一行一個網址，最多貼 3 個\nhttps://...">'+escapeHtml(row.referenceUrls.join('\n'))+'</textarea></div><div class="ops-listing-url-import full"><button class="ops-button soft" type="button" data-action="product-source-images-import">從網址抓圖，挑選約 8 張</button><small>若供應商頁需要登入，系統會再找相同品牌、型號與顏色的公開頁面。</small></div><div id="productReferenceImageImportStatus" class="full"></div></div>'
      +'<div class="ops-listing-upload-box full"><div class="ops-listing-intake-heading"><span>B</span><div><b>直接上傳自己的圖片</b><small>支援 JPG、PNG、WebP；可和網址抓到的圖片混用，最多保留 10 張。</small></div></div><label class="ops-button soft"><input id="productReferenceImageUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple hidden>選擇圖片上傳</label></div><div id="productReferenceImageUploadStatus" class="full"></div>'
  ...
  async function importProductListingImagesFromUrls(form){
    ...
    if(status)status.innerHTML='<div class="ops-product-ai-status running"><span class="ops-product-ai-spinner"></span><div><b>正在挑選約 8 張適合的商品圖片</b><small>先讀你貼的商品網址、去除重複圖片；若頁面被登入或驗證擋住，再依品牌、型號與顏色找公開頁面。</small></div></div>';
    try{
