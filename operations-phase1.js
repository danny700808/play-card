(function(global){
  ...
  const VERSION = '2026.08.12-shopee-autopublish-v7';
...
function ensureSalesClock(){
  ...
  function openProductListingPublishResult(result,draft){
    ...
    const cards=Object.keys(platforms).map(function(key){
      ...
      const helper=key==='shopee'&&row.autofillPayload?'<div class="ops-card-actions"><button class="ops-button primary" type="button" data-action="product-shopee-autofill-open">安全開啟 EasyStore／蝦皮</button><a class="ops-button soft" href="youzi-easystore-shopee-autofill-v0.3.5.zip" download>下載／更新助手 0.3.5</a></div><small>'+(shopeeHomeLogisticsNeedsManualConfirmation?'會帶入分類、品牌與商品屬性；一般宅配與新增／更新狀態仍須確認，助手不會誤建重複商品。':'已有商品只配對／更新；只有明確確認蝦皮沒有相同 SKU 時才允許建立新品。')+'</small>':'';
  ...
  async function openShopeeAutofillHelper(){
    ...
    else toast('助手沒有收到商品資料',clean(result&&result.error)||'請安裝或更新到助手 0.3.5，重新整理營運中心後，再按一次「安全開啟 EasyStore／蝦皮」。','warning');
    return result;
  }
