(function(global){
  ...
  function renderContractHtml(contract, opts){
    ...
    const hasFormalData=!!(contract.customerSubmittedFormalAt || contract.customerSignatureDataUrl || contract.signatureDataUrl || contract.customerSignatureImageDataUrl || contract.signatureImageDataUrl || contract.customerSignature || contract.signature || contract.customerIdImageWatermarkedDataUrl || contract.idImageWatermarkedDataUrl || contract.customerIdImageDataUrl || contract.idImageDataUrl || contract.idCardImageDataUrl || contract.customerIdCardImageDataUrl);
    ...
    const dateSource=clean(contract.contractDate||contract.officialConfirmedAt||contract.updatedAtText)||ymd(new Date());
    const dateText=rocDate(dateSource.slice(0,10)).replace(/^民國\s*/,'').replace(/\s+/g,'');
    const sig=clean(contract.customerSignatureDataUrl || contract.signatureDataUrl || contract.customerSignatureImageDataUrl || contract.signatureImageDataUrl || contract.customerSignature || contract.signature || contract.customerSignatureUrl || contract.signatureUrl || contract.signDataUrl || contract.signImageDataUrl);
    const idImage=clean(contract.customerIdImageWatermarkedDataUrl||contract.idImageWatermarkedDataUrl||contract.customerIdImageDataUrl||contract.idImageDataUrl||contract.idCardImageDataUrl||contract.idCardWatermarkedDataUrl||contract.customerIdCardImageDataUrl||contract.customerIdImageUrl||contract.idImageUrl||contract.idCardImageUrl);
