(function(global){
  ...
  function renderContractHtml(contract, opts){
    ...
    const itemHtml = items.length ? `<div class="equipment-list-contract equipment-list-contract--twocol">${items.map((x,i)=>`<div class="equipment-item-contract"><span class="equipment-no-contract">${i+1}.</span><span class="equipment-text-contract">${esc(x.name)}${x.note?`<span class="eq-note-contract">（${esc(x.note)}）</span>`:''}</span></div>`).join('')}</div>` : '依雙方確認設備清單';
    function periodRows(){
      const rows=[];
