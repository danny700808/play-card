(function(global){
  ...
  function renderContractHtml(contract, opts){
    ...
    const sig=clean(contract.customerSignatureDataUrl || contract.signatureDataUrl || contract.customerSignatureUrl || contract.signatureUrl);
    ...
    return `
      ...
      <div class="rental-contract-sheet">
        ...
        <div class="sign-grid">
          <div class="sign-card party-a"><b>甲方</b><br><span class="party-a-line">姓名 / 公司：${esc(partyAName)}</span><span class="party-a-line">身分證字號 / 統編：${esc(identity)}</span><span class="wide-line">地址：${esc(customerAddress)}</span><span class="party-a-line">電話：${esc(customerPhone)}</span><div class="sig-box"><b>甲方簽名：</b>${sig?`<img src="${esc(sig)}" class="sig-img">`:'尚未簽名'}</div></div>
