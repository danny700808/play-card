...
test('new teacher stop captures only its subject debt and duplicate stop does not rewrite it',async()=>{
 ...
 const ctx={requireSession:async()=>({teacherId:'t1'}),dateKey:v=>v,currentTaipeiDay:()=> '2026-09-10',assertPortalAdvanceDate(){},assertInput(){},teacherOwnsStudent:async()=>true,clean:v=>String(v||''),mirrorProfilesByIds:async(type,ids)=>{assert.deepEqual(Array.from(ids),[type==='students'?'s1':'t1']);return [{id:ids[0],name:type}]},mirrorRows:async type=>{assert(!['students','teachers'].includes(type));return type==='fixedCourses'?courses:[]},mirrorRowsByField:async()=>periods,sourceId:r=>r.id,eventTeacherId:r=>r.teacherId,eventSubjectId:r=>r.subjectId,eventStudentIds:r=>r.studentIds,tuitionOutstandingAmount:r=>r.balance,courseSourceIds:r=>[r.id],hash:v=>v,FieldValue:{serverTimestamp:()=>1,increment:()=>1},nowText:()=> '2026-09-10',scheduleVersionRef:()=>({id:'version'}),HttpsError:Error,db:{collection:()=>({doc:id=>({id,get:async()=>({exists:records.has(id),data:()=>records.get(id)})})}),batch:()=>({set:(ref,row)=>{writes.push(row);if(ref.id!=='version')records.set(ref.id,row)},commit:async()=>{}})}};
 vm.createContext(ctx);vm.runInContext(fn,ctx);
