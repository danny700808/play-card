...
for(const file of ['course-scheduler.js','operations-course-inline-runtime.js']){
 ...
 function evaluate(candidate,policy={},events=[]){const ctx={state:{},Set,scheduleHoursForDate:()=>({start:0,end:1440,closed:false}),isNonOccupyingEvent:()=>false,timeToMin:()=>780,numberOf:Number,roomById:()=>({allowedSubjectIds:['piano']}),subjectById:()=>({name:'吉他'}),crossedTimes:()=>['13:00'],slotPolicy:()=>policy,eventSharedResourceIds:()=>[],effectiveEventsForDate:()=>events,isHiddenEvent:()=>false,eventDisplayName:()=> '既有課程',unique:v=>[...new Set(v)]};vm.createContext(ctx);vm.runInContext(fn,ctx);return Array.from(ctx.eventConflictReasons({type:'fixed',start:'13:00',duration:60,roomId:'room',studentIds:[],...candidate},[]));}
