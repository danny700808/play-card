...
const legacyLineStart = onboardingBackend.indexOf('async function handleExternalTeacherLineEvent()');
...
assert(legacyLineStart >= 0);
assert(legacyLineFlow.includes('return false;'));
assert(read('functions/index.js').includes('這組碼屬於舊版外聘老師資料，已停止自動回寫。'));
