...
test('diagonal gesture locks to one axis, one page at a time',()=>{const h=setup();h.swipe(-250,-120);assert.equal(h.scroll.scrollLeft,350);assert.equal(h.scroll.scrollTop,0);h.swipe(-250,-120);assert.equal(h.scroll.scrollLeft,350);h.swipe(500,80);assert.equal(h.scroll.scrollLeft,0);h.swipe(-20,-900);assert.equal(h.scroll.scrollLeft,0);assert.equal(h.scroll.scrollTop,0);});
