...
test('obsolete waiting and input-stability search layers are completely removed', () => {
  ...
  for (const html of [portal, hub]) {
    ...
    assert.match(html, /operations-phase1\.css\?v=20260819-image-collector-session-recovery-v1/);
    assert.match(html, /operations-shopee-autofill-handoff-v1\.js\?v=20260813-shopee-autopublish-v17/);
    assert.match(html, /operations-phase1\.js\?v=20260819-image-collector-session-recovery-v1/);
  }
});
