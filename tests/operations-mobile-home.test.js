...
test('portal URL opens the operations application directly', () => {
  ...
  assert.match(portal, /operations-phase1\.js\?v=20260829-verified-nine-series-cover-import-v1/);
...
test('variant workflow searches first and requires each SKU own representative image', () => {
  const source = read('operations-phase1.js');
  const css = read('operations-phase1.css');
  assert.match(source, /class="ops-listing-mode-options"/);
  ...
  assert.match(source, />細項名稱<input/);
  assert.match(source, /valueName:'variantGroupPrimaryValue'/);
  assert.match(source, /\|\|productVariantSuggestion\(product\)/);
  assert.match(source, /variantParentAttributeValue=clean\(data\.get\('variantParentAttributeValue'\)\)\|\|\(variantParent\?productVariantSuggestion\(variantParent\)\|\|'原款':''\)/);
  assert.match(source, /variantAttributeValue=clean\(data\.get\('variantAttributeValue'\)\)\|\|productVariantSuggestion\(p\)/);
  assert.match(source, /requireProductVariantRepresentativeImages\(form\)/);
  assert.match(source, /請先替每一個細項選擇 1 張代表圖/);
  assert.match(css, /\.ops-listing-mode-options>label/);
  assert.match(css, /\.ops-listing-variant-item\.is-required-missing/);
  assert.match(css, /\.ops-listing-variant-key-fields/);
  assert.match(css, /\.ops-listing-variant-representative/);
  assert.match(source, /copyProductVariantReferenceImage/);
  assert.match(source, /persistProductVariantReferenceImages\(form,id,copied,copied\)/);
  assert.doesNotMatch(source, /shared-variant-/);
  assert.match(source, /上架圖片預覽/);
  assert.match(css, /\.ops-listing-single-image-controls\.is-hidden\{display:none\}/);
