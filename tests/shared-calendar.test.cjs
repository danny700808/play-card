...
test('attachments remain scoped and notifications are sent only after publish, once',async()=>{
 ...
 const r=await a.call('publish',{id:'work'});assert.equal(r.notification.sent,1);assert.equal(f.sent[0].to,'U'+'a'.repeat(32));assert.match(f.sent[0].message,/owner=1/);await a.call('publish',{id:'work'});assert.equal(f.sent.length,1);
});
