...
(async()=>{
 ...
 assert.equal(await page.locator('#studentRentalNav a').count(),5);
 ...
 assert.deepEqual(errors,[]);
 await page.locator('#unbindSelf').click();
 await page.waitForFunction(()=>calls.some(x=>x.action==='remove-self' && x.studentId==='b' && x.confirmed===true));
 assert.equal(await page.locator('#logoutBtn').count(),0);
 console.log('Student rental navigation, teacher isolation, selected-student return and confirmed self removal passed');
 await browser.close();
