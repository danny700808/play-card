...
assert(html.includes('teacher-course-portal-v8.css?v=20260913-attendance-visibility-v4'), '老師首頁樣式 cache key 過期');
assert(html.includes('teacher-course-portal-v8.js?v=20260915-teacher-mode-feedback-v1'), '老師首頁程式 cache key 過期');
...
assert(lessonQuickSource.includes("const canNormalAttendance = sameDay && status === 'scheduled'"), '老師不能在當天任何時間簽到當天課程');
...
assert(!/<button class="(?:primary|danger)"[^>]*(?:data-quick-attendance|data-quick-state="(?:leave|absent|cancel_change)"|data-quick-action="single_move")/.test(lessonQuickSource), '課堂快速操作不應使用特殊顏色');
assert(source.includes("timeZone: 'Asia/Taipei'"), '簽到日期未固定使用台北時區');
assert(!source.includes('這是「提早簽到」'), '簽到確認仍顯示提早簽到說明');
...

{
 ...
 assert.equal(ctx.nextWeekPage(y,0,-3),0);
...

{
 ...
 const ctx={weekViewport:viewport,changingWeek:false,weekMotionId:0,weekAnimating:false,animateWeekPosition(){},nearestWeekPage:()=>0,swipeWeekDirection:()=>0,weekGesture:null,weekSnapTimer:0,suppressWeekClickUntil:0,Date,global:{matchMedia:()=>({matches:true}),clearTimeout(){}},weekPageTargets:()=>[0,300],nextWeekPage:()=>300};
