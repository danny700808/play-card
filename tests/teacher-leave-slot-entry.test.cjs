...
function render(status='leave',planning=false,extra=[],past=false){
 ...
 const c={document:{getElementById:()=>node},weekStart:'2026-09-16',data:{events:[event,...extra],hours:{start:17,end:19}},planner:planning?{slots:[{date:'2026-09-16',startTime:'17:00'}]}:null,uniqueEvents:x=>x,addDays:(d,n)=>new Date(Date.parse(d+'T12:00:00Z')+n*86400000).toISOString().slice(0,10),overlapGroups:events=>events.map(e=>({startTime:e.startTime,endTime:e.endTime,events:[e]})),timeMinutes:t=>Number(t.slice(0,2))*60+Number(t.slice(3)),timeText:n=>String(Math.floor(n/60)).padStart(2,'0')+':'+String(n%60).padStart(2,'0'),escapeHtml:String,todayKey:()=> '2026-09-16',dayLabel:String,courseSlotIsPast:()=>past,requestAnimationFrame:()=>{},lessonCard:e=>'<button class="lesson leave" data-event="'+e.id+'">請假紀錄</button>',plannerDurationMinutes:()=>60,continuousTeacherGapMinutes:()=>120,clean:v=>String(v||'')};
