...
test('teacher profile is a stable standalone page, not a profile-and-contract wizard', () => {
  ...
  assert.match(page, /teacher-profile\.js\?v=20260919-taiwan-v2/);
  ...
  assert.match(runtime, /profileChangePending/);
  assert.match(runtime, /授課科目與程度已先同步套用/);
  assert.match(runtime, /主管退回/);
...
test('profile API stores a server draft without creating or navigating to a contract', () => {
  ...
  assert.match(save, /TEACHER_PROFILE_DRAFTS/);
  assert.match(save, /TEACHER_PROFILE_CHANGE_DRAFTS/);
  assert.match(save, /db\.collection\('profileChangeRequests'\)\.doc\(\)/);
  assert.match(save, /profileChangeRows\(baselineProfile, afterSnapshot\)/);
  assert.match(save, /subjectChangesAlreadyEffective/);
  assert.match(save, /profileReviewStatus:[\s\S]*initialStatus/);
  ...
  assert.match(save, /saveBatch\.set\(privateProfileRef, initialPrivatePatch/);
  ...
  assert.match(save, /profileAssignmentPatch/);
  assert.match(save, /source:\s*'teacher-self-declared-profile'/);
  assert.match(save, /saveBatch\.set\(assignmentRef, assignmentPatch/);
  assert.match(save, /delete submittedDraftRow\.revisionReason/);
