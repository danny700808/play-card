...
test('Today reveals the pair containing the current day including Sunday',async()=>{
 ...
 for(const [day,left] of [['2026-09-08',0],['2026-09-09',300],['2026-09-13',750]]){
  let click;const button={addEventListener:(type,fn)=>click=fn},viewport={scrollLeft:0,scrollWidth:1098,clientWidth:348};
  const context={document:{getElementById:id=>id==='todayWeek'?button:{style:{getPropertyValue:()=>150}}},changingWeek:false,weekStart:'2026-08-01',loading(){},todayKey:()=>day,load:async()=>{},updateWeekViewport(){},weekViewport:viewport,addDays:(d,n)=>new Date(Date.parse(d+'T12:00:00Z')+n*86400000).toISOString().slice(0,10)};
