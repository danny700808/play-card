...
test('horizontal edge swipes cross weeks and retain time position; vertical swipes do not',async()=>{
 ...
 listeners.touchstart({touches:[{clientX:300,clientY:300}]});await listeners.touchend({changedTouches:[{clientX:270,clientY:100}]});assert.equal(loads,2);assert([...buttons.values()].every(b=>!b.disabled));
});
